% about "categories and sheaves" (version ald)
...
\end{prop}
%
\pf To prove this, consider the commutative diagram
$$
\begin{tikzcd}
...
\end{prop} 
%
\pf The proof is essentially the same that that of Proposition 2.7.1 on p.~62 of the book. (See also \S\ref{c271b} p.~\pageref{c271b}.) Again, we give some more details about the proof of the fact that $i^\dagger(F)$ commutes with small filtrant inductive limits. Put $\widetilde F:=i^\dagger(F)$. 
...
%
\begin{s} 
P.~173, Proposition 8.2.12 and Theorem 8.2.14 \mv. 
\begin{nota}
If $\C$ and $\C'$ are categories admitting finite products, we denote be $\oo P(\C,\C')$ the category of those functors from $\C$ to $\C'$ which commute with finite products.
\end{nota}

Here is a slightly different way of stating Proposition 8.2.12:
%
...
$$ 
is an isomorphism, $\Phi^{-1}$ being given by Lemma 8.2.11 p.~172.
\end{prop}
%
\n{\em Proof.} The functor $\Phi$ being fully faithful by Proposition 8.2.12 p. 173, it suffices to prove that 
$$
\oo{Ob}(\Phi):\oo{Ob}(\oo P(\C,\Mod(\mathbb Z)))\to\oo{Ob}(\oo P(\C,\Set))
$$ 
is bijective. The map is surjective by Proposition 8.2.13 p. 173, and it is injective by Proposition~\ref{8.2.12}. q.e.d.
...
\end{thm}
%
\n{\em Proof.} Let $\C$ be our additive category, and let $X$ be in $\C$. Applying Proposition~\ref{8.2.12} to the functor $\Hom_\C(X,\ ):\C\to\Set$, we see that there is one and only one way to turn this functor into a functor from $\C$ to $\Mod(\mathbb Z)$. In particular, we get a structure of abelian group on each of the set $\Hom_\C(X,Y)$ for $Y\in\C$, and, for $g_i:Y\to Z$ ($i=1,2$), we have $(g_1+g_2)\circ f=g_1\circ f+g_2\circ f$. Let us consider now a morphism $h:W\to X$. As the morphism of functors $\circ h:\Hom_\C(X,\ )\to\Hom_\C(W,\ )$ in $\oo P(\C,\Set)$ can be viewed, in a unique way, as a morphism of functors in $\oo P(\C,\Mod(\mathbb Z))$, we have $(f_1+f_2)\circ h=f_1\circ h+f_2\circ h$ for $f_i:X\to Y$. q.e.d.
%
\end{s}
%
%%
