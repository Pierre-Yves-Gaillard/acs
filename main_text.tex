% about "categories and sheaves" (version apd)
...
$$ 
Corollary 2.4.6 p. 56 results immediately from the following remark:
%
\begin{rk}\label{r236}
...
%
\begin{proof}
It suffices to prove the first isomorphism. Write $p_i:X\times_YX\to X$ for the $i$-th projection, use the abbreviations 
$$
P:=X\times_YX,\quad p:=p_1-p_2:P\to X,
$$
and recall that $h:\Ker f\mono X$ is the canonical monomorphism. Let $Z$ be in $\C$, let $Z_p\subset\Hom_\C(X,Z)$ be the image of the injection 
$$
\Hom_\C(\Coim f,Z)\mono\Hom_\C(X,Z)
$$ 
induced by the natural epimorphism $X\epi\Coim f$, and let $Z_h\subset\Hom_\C(X,Z)$ be the image of the injection 
$$
\Hom_\C(\Coker h,Z)\mono\Hom_\C(X,Z)
$$ 
induced by the natural epimorphism $X\epi\Coker h$. It suffices to show $Z_p=Z_h$. Let $z$ be in $\Hom_\C(X,Z)$. The following conditions are equivalent: 
$$
z\in Z_p,
$$
$$
...
$$
$$
z\circ p_1=z\circ p_2,
$$
\begin{equation}\label{zp1}
w_1,w_2:W\parar X\ \implies\ f\circ w_1=f\circ w_2.
\end{equation}
...
w:W\to X\ \implies\ f\circ w=0.
\end{equation}
As \eqref{zp1} and \eqref{fw} are clearly equivalent, the proof is complete.
\end{proof}
