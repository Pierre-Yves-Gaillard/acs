% about "categories and sheaves" (version akl)
...
\end{prop}
%
\n{\em Proof.} By (i) there is a $g:B\to A$ satisfying $f\circ g=\id_B$, yielding $f\circ g\circ f=f$, and (ii) implies $g\circ f=\id_A$.
\end{s}
%
%%
%
...
%
\begin{s} 
P.~56, Lemma 2.4.7. Let us give a slightly different wording of the proof. Recall the statement: 
%
\begin{lem} 
...
%
\begin{s} 
P.~61, Proposition 2.6.4. Here is a minor variant. 
%
...
$$
as well as the two following remarks:
...
\begin{rk}
If $F$ is fully faithful, then there is an isomorphism $\widehat F(A)\circ F\xr\sim A$ functorial in $A\in\C^\wedge$. Indeed, we have 
$$
...
%%
%
\begin{s} P.~81, proof of Proposition 3.3.2. Here is a minor variation of the proof. Recall the statement: 
%
\begin{prop} 
%
...
%
\begin{s} 
P.~95, proof of Proposition 4.1.4 (i). Here are some more details: Let us denote vertical composition by $\circ$ and horizontal composition by $*$. Equality $(R*\eta*L)\circ(\e*P)=P$, that is $(R*\eta*L)\circ(\e*R*L)=R*L$, can be written as 
$$
...
%
\begin{s} 
P.~116, proof of Proposition 5.1.7 (i). Here is a very slightly different wording. Recall the statement: 
%
\begin{prop} 
...
$$ 
where $h$ is the Yoneda embedding. Proposition~\ref{cocop} p.~\pageref{cocop} implies that $M$ is filtrant. Let $M\to\C_A$ be the obvious functor. One easily checks that Condition (iii) of Proposition 3.2.2 p.~78 holds for the obvious functor $M\to\C_A$. Thus, the result follows from Proposition 3.2.2. 
\end{s}
%
%%
%
...
%
\begin{s} 
P.~136, proof of Proposition 6.1.18 (i). Here is a minor variant: If $\iota:\C\hookrightarrow\Ind(\C)$ is the inclusion functor and $(\p_i,\psi_i:\alpha_i\pa\beta_i)_{i\in I}$ is a small filtrant inductive system of parallel arrows in $\C$ whose limit in $\Ind(\C)$ is a pair of parallel arrows $f,g:A\pa B$ (that is, $A=\ic_i\ \alpha_i$, and so on...), then we have 
$$
...
Let $X$ and $Y$ be in $\C$. We define the addition on $\h_\C(X,Y)$ by letting $k:\C\to\C^\vee$ be the Yoneda embedding and observing that, in the notation ofProposition~\ref{8212}, $\h_\C(X,Y)$ is the set underlying the abelian group $\Phi(k(X))(Y)$. In particular, we have $g\circ(f_1+f_2)$ for $X\xr{f_i}Y\xr gZ$. It is easy to conclude from this that the above construction endows $\C$ with a structure of pre-additive category.
\end{s}
%
%%
%
...
%
\begin{s} 
P.~188. Some more details about the end of the proof of Proposition 8.4.7. Let us just rewrite in a slightly less concise way the part of the proof which starts with the sentence ``Define $Y:=Y_0\times_XG_i$'' at the fifth line of the last paragraph of the proof, and goes to the end of the proof. 
...
%%
%
\begin{s} P.~191, proof of Theorem 8.5.8 (iii). Here is a slight rewording. Recall the statement: 
%
...
%
\begin{s} 
P.~191, Step (a) of the proof of Theorem 8.5.8 (iv). Here is a slightly different wording. Recall the statement: 
%
\begin{lem}
%
...
%
\begin{s} 
P.~199, Lemma 8.7.4 (ii). This comment is about the claim that the natural functor $E:\cc D'_{\cc S}\to\C$ is an equivalence. I don't understand the proof of the faithfulness of $E$ given in the book. I think that it suffices, in view ofProposition 7.1.2 (i) p.~150 and Theorem 7.1.16 p.~155, to check that
%
