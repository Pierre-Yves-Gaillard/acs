% about "categories and sheaves" (version agu)
...
%
\begin{cor}
Let $(\cc A,T)$ be an abelian category with translation. Then the functor 
$$
H:\text K_c(\cc A)\to\cc A
$$ 
is cohomological. 
\end{cor}
% 
...
\n{\em Proof.} Let $X\to Y\to Z\to T(X)$ be a d.t. in $K_c(\cc A)$. It is isomorphic to $V\xr{\alpha(u)}\text{Mc}(u)\xr{\beta(u)}T(U)\to T(V)$ for some morphism $u:U\to V$. Since the sequence in $\cc A_c$: 
$$
0\to V\to\text{Mc}(u)\to T(U)\to0
$$ 
is exact, it follows from Theorem 12.2.4 that the sequence 
$$
H(V)\to H(\text{Mc}(u))\to H(T(U))
$$ 
is exact. q.e.d. 
