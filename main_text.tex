% about "categories and sheaves" (version aib)
...
\n$\bu$ P. 253, last sentence of Definition 10.3.1. Recall the setting: $F:\cc D\to\cc D'$ is a triangulated functor; $\cc{N,N'}$ are null systems in $\cc{D,D'}$; and $Q,Q'$ are the localization functors. The existence, for all $X$ in $\cc D$, of 
$$
\ic_{(X\to Y)\in\cc NQ'}Q'(F(Y))
$$ 
in $\cc{D'/N'}$ is a sufficient condition for $F$ to be universally right localizable, but it is not clear to me that it is necessary. The simplest would be to refer to Definition 7.3.1 p.~159. (Of course there is an analogous remark for left localization.)
