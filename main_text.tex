% about "categories and sheaves" (version ara)
...
\nn$\bu$ Definition of the morphism $A_1\times A_2\to A$. Let $Y_i$ be in $\C_i$ for $i=1,2$, and put 
$$
S(X_1,X_2):=\Hom_{\C_1}(Y_1,X_1)\times\Hom_{\C_2}(Y_2,X_2)
$$ 
for all $(X_1,X_2)$ in $\C_1\times\C_2$. We must define the map represented by the dashed arrows in the diagram
$$
\begin{tikzcd}
(A_1\times A_2)(Y_1,Y_2)\ar[equal]{d}\ar[dashed]{r}&A(Y_1,Y_2)\ar[equal]{d}\\ 
\displaystyle\co_{x_1,x_2}S(X_1,X_2)\ar[dashed]{r}&\displaystyle\co_xS(X_1,X_2)\\ 
S(X_1,X_2)\ar{u}{b(x_1,x_2)}\ar[equal]{r}&S(X_1,X_2),\ar{u}[swap]{c(x)}
\end{tikzcd}
$$ 
where $b(x_1,x_2)$ and $c(x)$ are the coprojections. Let $x_i\in A_i(X_i)$ be given for $i=1,2$ and let 
$$
d_i(z):\Hom_{\C_i}(X_i,Z_i)\to A_i(X_i)
$$
be the coprojections. The category $(\C_1\times\C_2)_A$ being filtrant, there is a 5-tuple $(Z_1,Z_2,z,f_1,f_2)$ with $Z_i$ in $\C_i$, $z$ in $A(Z_1,Z_2)$, and $f_i$ in $\Hom_{\C_i}(X_i,Y_i)$, such that $x_i=d_i(z)(f_i)$ for $i=1,2$. Put $x:=A(f_1,f_2)(z)$:
$$
\begin{tikzcd}
...
\end{tikzcd}
$$ 
One checks that the map $c(x)$ does not depend on the choices made above, and that this process defines a morphism from $A_1\times A_2$ to $A$. %, and that this morphism satisfies the wished-for conditions. 
...
\noindent$\bu$ the coprojections $a_k(i_k):\alpha_k(i_k)\to G(k)$ (see Definition~\ref{c} p.~\pageref{c}) induce a morphism of functors from $P$ to $G$. 
...
$$ 
\begin{tikzcd} 
\alpha_k(i_k)\ar[equal]{r}&P(k)\ar{r}{P(f)}\ar{d}[swap]{a_k(i_k)}&P(\ell)\ar{d}{a_\ell(i_\ell)}\ar[equal]{r}&\alpha_\ell(i_\ell)\\ 
