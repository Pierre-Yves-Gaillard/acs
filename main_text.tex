...
\n Pp 36-43, Section 2.1. Let $\alpha:I\to\C,\beta:I^\op\to\C$ be as in the book. Then the graphs (see Comment~\ref{graph} p. \pageref{graph}) of $\coli\alpha\in\C^\vee$ and $\lim\beta\in\C^\wedge$, and the condition that these functors are representable, depend only on the graphs of $\alpha$ and $\beta$.
...
\n P. 133, Proposition 6.1.9. ``There exists a unique functor ...'' should be ``There exists a functor ... Moreover, this functor is unique up to unique isomorphism.'' See Subsection~\ref{su271} p.~\pageref{su271}. 
...
\n P. 140, Corollary 6.3.2. For the uniqueness up to unique isomorphism, see Subsection~\ref{su271} p.~\pageref{su271}. 
...
The following crucial fact is implicit in the book. If $\C$ is a $\U$-category and $\mathcal V$ a universe containing $\U$, then $\C^\wedge_\U$ is a full subcategory of $\C^\wedge_\mathcal{V}$. 
...
\n$\bu$ P. 40, Lemma 2.1.11. It is easy to see that the converse holds. In other words, the lemma can be stated as follows: If $\C$ is a category, $\alpha:\C\to\C$ the identity functor, and $S$ an object of $\C$, then $S$ represents $\coli\alpha$ if and only if $S$ is terminal. 

Here is another wording of the proof of the implication: $S$ represents $\coli\alpha\ \implies\ S$ is terminal. 

Let $\Delta:\C\to\C^\C$ be the diagonal functor, let $i\in\h_{\C^\C}(\id_{\C^\C},\Delta(S))$ be such that, for all $X\in\C$, the morphism $i_X:X\to S$ is the coprojection, and let $T$ be in $\C$. By assumption the map 
$$
\h_\C(S,T)\to\h_{\C^\C}(\id_\C,\Delta(T)),\quad f\mapsto\Delta(f)\circ i 
$$ 
is bijective. Let $\theta\mapsto\theta'$ be the inverse bijection. We have $\Delta(\theta_S)\circ i=\theta$, which implies $\theta'=\theta_S$, and thus $f=f\circ i_S$ for all $f$ in $\h_\C(S,T)$; in particular $\id_S=i_S$. For $g$ in $\h_\C(X,S)$ we get $g=i_S\circ g=i_X$. q.e.d. 

%% 

\n$\bu$ P. 52, Formula (2.3.6). The following observation will give rise to another proof of Corollary 2.4.6 p. 56 (see the comment containing Display (\ref{241s}) p. \pageref{241s}). Recall the setting: $\C\xleftarrow{\,\beta}J\xrightarrow{\p}I$, and $\p^\dagger(\beta):I\to\C$ is given by 
$$
...
\n$\bu$ P. 52, Theorem 2.3.3 (i). In the book the proof of Theorem 2.3.3 (i) is broken up in three steps (a), (b), (c). Let us add a few details to the proof of Step (c). In (b), the authors define a map 
$$
\Psi_{\alpha,\beta}:
...
$$ 
\cn As observed in Remark \ref{r236} p. \pageref{r236}, both sets are in natural bijection with the quotient of 
$$
...
$$ 
and thus follows from (1.5.9) p. 29 in the book. Equality $(R*\eta*L)\circ(P*\e)=P$, that is $(R*\eta*L)\circ(R*L*\e)=R*L$, can be written as 
$$
R*\Big((\eta*L)\circ(L*\e)\Big)*L=R*L,
$$ 
and thus follows from (1.5.8) p. 29 in the book. 
...
\n$\bu$ P. 122, proof of Lemma 5.3.3. The difference with the wording of the book is almost nil. We have, for $X\in\C$ and $F\in\F^\wedge$, 
$$
...
\n$\bu$ P. 133, Proposition 6.1.9. ``There exists a unique functor ...'' should be ``There exists a functor ... Moreover, this functor is unique up to unique isomorphism.'' See Subsection~\ref{su271} p.~\pageref{su271}. (See also Section \ref{619} p. \pageref{619}.) 
...
\n$\bu$ P. 140, Corollary 6.3.2. For the uniqueness up to unique isomorphism, see Subsection~\ref{su271} p.~\pageref{su271}. One could mention explicitly the formula 
%
...
\n\emph{Proof}. (a) follows immediately from the Proposition. In view of (a), (b) follows from Proposition 3.2.2 p. 78 in the book. (c) follows from the Proposition and (b). QED
...
% 
\subsection{Part 1}\label{su271} 
%
Here is a less elegant but slightly more precise formulation: 
...
The proof is essentially the same that that of Proposition 2.7.1 on p. 62 of the book. (See also Subsection~\ref{su271} p.~\pageref{su271}) and (\ref{271b}) p. \pageref{271b}.) Again, we give some more details about the proof of the fact that $i^\dagger(F)$ commutes with small filtrant inductive limits. Put $\widetilde F:=i^\dagger(F)$. 
