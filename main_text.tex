% about "categories and sheaves" (version anh)
...
\end{comment}
%
\noindent $*$ P. 424, proof of Theorem 17.5.2 (iv). ``The functor $f^\dagger$ is left exact'' should be ``The functor $f^\dagger$ is exact''. %[Here we are {\em not} following the convention of \eqref{ttau} p.~\pageref{ttau}.]
...
\begin{comment}
%
For the sake of completeness, one could add the following statements: 
...
\end{comment}
%
P.~24, the Yoneda Lemma. Let $F:\C^{\op}\to\Set$ be a functor and $X$ an object of $\C$.
%
\begin{df}\label{ue} 
...
%%
%
\begin{s}\label{revol2} P. 407. Thanks to the convention in \eqref{ttau}, Formula (17.1.3) follows from (2.3.6) p.~52 of the book. For the sake of completeness, let us rewrite Formulas (17.1.3) and (17.1.4) (in the notation of \eqref{ttau}):
...
(We implicitly set $f^\dagger:=(f^\tau)^\dagger$ and $f^\ddagger:=(f^\tau)^\ddagger$.) 
%
\begin{comment}
...
\nn $*$ P. 406, first line of second display: $(\C_Y)^\wedge$ should be $\C_Y$ (twice). (See \S\ref{406} p.~\pageref{406}.)
...
%
\begin{s}\label{revol}
P. 405. It seems to me it would be more convenient to denote by $f^t$ the functor from $(\C_Y)^{\op}$ to $(\C_X)^{\op}$ (and {\em not} the functor from $\C_Y$ to $\C_X$) which defines $f$. To avoid confusion, we shall adopt here the following convention:
...
%
\begin{s}\label{406}
P. 406. Recall that, in the first line of second display, $(\C_Y)^\wedge$ should be $\C_Y$ (twice). In notation \eqref{ttau}, Formula \eqref{275} p.~\pageref{275} gives, for $B$ in $\C_Y^\wedge$ and $U$ in $\C_X$,
%
