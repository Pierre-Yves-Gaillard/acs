% about "categories and sheaves" (version arc)
...
\nn$\bu$ Definition of the morphism $A_1\times A_2\to A$. %Let $Y_i$ be in $\C_i$ for $i=1,2$, and put $$S(X_1,X_2):=\Hom_{\C_1}(Y_1,X_1)\times\Hom_{\C_2}(Y_2,X_2)$$ for all $(X_1,X_2)$ in $\C_1\times\C_2$. 
We must define the map represented by the dashed arrow in the diagram %$$\begin{tikzcd}(A_1\times A_2)(Y_1,Y_2)\ar[equal]{d}\ar[dashed]{r}&A(Y_1,Y_2)\ar[equal]{d}\\ \displaystyle\co_{x_1,x_2}S(X_1,X_2)\ar[dashed]{r}&\displaystyle\co_xS(X_1,X_2)\\ S(X_1,X_2)\ar{u}{b(x_1,x_2)}\ar[equal]{r}&S(X_1,X_2),\ar{u}[swap]{c(x)}\end{tikzcd}$$ where $b(x_1,x_2)$ and $c(x)$ are the coprojections. Let $x_i\in A_i(X_i)$ be given for $i=1,2$ and let 
$$
\begin{tikzcd}
%A_1\times A_2\ar[equal]{d}\ar[dashed]{r}&A\ar[equal]{d}\\ 
...
\end{tikzcd}
$$ 
%$$d_i(z):\Hom_{\C_i}(X_i,Z_i)\to A_i(X_i)$$ be the coprojections. 
The category $(\C_1\times\C_2)_A$ being filtrant, there is a 5-tuple 
$$
\zeta:=(Z_1,Z_2,z,f_1,f_2)
$$ 
with $Z_i$ in $\C_i$, $z$ in $A(Z_1,Z_2)$, and $f_i$ in $\Hom_{\C_i}(X_i,Y_i)$, such that $x_i=d_i(z)(f_i)$ for $i=1,2$. We choose such a 5-tuple $\zeta$ and put $x:=A(f_1,f_2)(z)$:
$$
\begin{tikzcd}
...
\end{tikzcd}
$$ 
One checks that $x$ does not depend on the choice of $\zeta$, and that this process defines a morphism from $A_1\times A_2$ to $A$. %, and that this morphism satisfies the wished-for conditions. 
This ends the proof of \eqref{6112b}.%, and the proof of Proposition 6.1.12 p.~134 of the book is complete.
%
%%%%
%
\bc
...
$$ 
%so that we have $$G(k_0)=\ic F((i_k),P)$$ for all $k_0$ in $K$.
...
$$ 
be the natural functor (see \eqref{slice} for the definition of $\C/G(a)$ and recall that $\ic\alpha_k=G(k)$); define 
$$ 
\varphi:N\{L,G_L,(\alpha_\ell)\}\to(\C/G(a))^{L/a} 
$$ 
(see \eqref{slice} for the definition of $L/a$) by 
$$
\varphi((i_\ell),P)\left(b\xr f a\right):=\left(P(b)\xr{u'(b)} G(b)\xr{G(f)}G(a)\right),
$$
where $u'(b)$ is defined in \eqref{u'}; let 
$$
