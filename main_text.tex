% about "categories and sheaves" (version aqt)
...
Going back to our business, let $A$ be in $\Ind(\C_1\times\C_2)$ and set $(A_1,A_2):=\theta(A)$. We shall define morphisms $A\to A_1\times A_2$ and $A_1\times A_2\to A$, and leave it to the reader to check that these morphisms are mutually inverse isomorphisms of functors. 
...
$$
be the coprojections. The categories $(\C_1\times\C_2)_A$ and $(\C_i)_{A_i}$ being filtrant, there is a 5-tuple $(Z_1,Z_2,z,f_1,f_2)$ with $Z_i$ in $\C_i$, $z$ in $A(Y_1,Y_2)$, and $f_i$ in $\Hom_{\C_i}(X_i,Y_i)$, such that $x_i=d_i(z)(f_i)$ for $i=1,2$. Put $x:=A(f_1,f_2)(z)$:
$$
\begin{tikzcd}
(X_1,X_2)\ar{d}{x}\ar{r}{(f_1,f_2)}&(Z_1,Z_2)\ar{d}{z}\\ 
A\ar[equal]{r}&A.
\end{tikzcd}
$$ 
One checks that the map $c(x)$ does not depend on the choices made above, that this process defines a morphism from $A_1\times A_2$ to $A$, and that this morphism satisfies the wished-for conditions.
%
%%%%
%
\bc
