% about "categories and sheaves" (version als)
...
%
\begin{proof}
Let $\Delta:\C\to\C^\C$ be the diagonal functor, let $i\in\h_{\C^\C}(\id_\C,\Delta(S))$ be such that, for all $X\in\C$, the morphism $i_X:X\to S$ is the coprojection (see Definition~\ref{c} p.~\pageref{c}), and let $T$ be in $\C$. By assumption the map 
$$
...
g=i_S\circ g=i_X,
$$
the first equality following from the fact that $\id_S=i_S$, and the second one from the definition of $\h_{\C^\C}(\id_\C,\Delta(S))$.
\end{proof}
\end{s}
%
%%
...
=f\circ z\circ w, 
$$ 
the first equality following from the functoriality of $\xi$, the second one from the definition of $\rho$, the third one from the equality $\rho(\xi)=\p(f)$, and the fourth one from the definition of $\p$.
\end{proof}
\end{s}
%
%%
...
%
\begin{s} 
P.~136, proof of Proposition 6.1.18 (i)\mv: If $\iota:\C\hookrightarrow\Ind(\C)$ is the inclusion functor and $(\p_i,\psi_i:\alpha_i\parar\beta_i)_{i\in I}$ is a small filtrant inductive system of parallel arrows in $\C$ whose limit in $\Ind(\C)$ is a pair of parallel arrows $f,g:A\parar B$ (that is, $A=\ic_i\ \alpha_i$, and so on...), then we have 
$$
...
\n(a) $\implies$ (b). Let $f:X'\to IF(A)$ be a morphism in $\Ind(\cc C')$ with $X'$ in $\cc C'$ and $A$ in $\Ind(\cc C)$, let $\beta_0:I\to\cc C$ be a functor with $I$ small and filtrant and $\ic\beta_0\simeq A$; in particular $\ic F\circ\beta_0\simeq IF(A)$. By Proposition 6.1.13 p.~134 there are functors $\alpha:J\to\cc C'$ and $\beta:J\to\cc C$, and a morphism of functors $\p:\alpha\to F\circ\beta$ such that 
...
%
\begin{proof}
Let $\psi:\M\to\C$ be defined by $\psi(M)=G\otimes_RM$, let $\C_0$ be the full subcategory of $\C$ whose objects are the direct sums of finitely many copies of $G$, and let $\M_0$ be the full subcategory of $\M$ whose objects are the direct sums of finitely many copies of $R$. Then $\p$ and $\psi$ quasi-induce quasi-inverse equivalences 
$$
\begin{tikzcd}
...
\n(*) for any diagram $Z\parar A\to B$ with $Z\in\diamondsuit$, there exists a local $\heartsuit$ $S\to Z$ such that the two compositions $S\to Z\parar A$ coincide, 
...
For each $(B\xr sU)\in(\cc{LI}_U)^{\op}$ put $\alpha(B\xr sU):=\h_{\C^\wedge}(B,A)$. Let $f_1,f_2:U\parar A$ be two morphisms such that the compositions $U\parar A\to A^a$ coincide. By definition of $A\to A^a$, we have (in the above notation) $i(\id_U)(f_1)=i(\id_U)(f_2)$. By the definition of $A^a(U)$, by the fact that $\cc{LI}_U$ is cofiltrant, and by Proposition 3.1.3 p.~73, there is a morphism 
$$
\p:(B\xr sU)\to(U\xr{\id_U}U)
$$ 
in $\cc{LI}_U$ such that $\alpha(\p)(f_1)=\alpha(\p)(f_2)$. This is easily seen to mean that the compositions $B\to U\parar A$ coincide, and we conclude by Lemma 16.2.3 (iii), (b) implies (a) p. 395. q.e.d.
\end{s}
%
%%
%
...
\n(b) This process defines a morphism of functors 
$$
...
$$ 
In the setting of Lemma~\ref{r52} we define $\Phi:I\to\Cat$ by $\Phi(i):=J_i$ and we consider the morphism of functors $\theta:\Phi\to\C$ such that $\theta_i:\Phi(i)=J_i\to\C$ is the composition of $\beta$ with the natural functor from $J_i$ to $J$. We assume \eqref{52}. Then $\co\theta$ is nothing but $\p^\dagger(\beta)$. In particular $\p^\dagger(\beta)$ is a functor. 
%
\subsection{Step (b)} %%%%%%%%%%%%%%%
%
...
$$ 
(\Coim f)(Z)\simeq\Ker\Big(\h_\C(X,Z)\parar\h_{\C^\wedge}\big(\hy(X)\times_{\hy(Y)}\hy(X),\hy(Z)\big)\Big), 
$$ 
$$ 
(\Ima f)(Z)\simeq\Ker\Big(\h_\C(Z,Y)\parar\h_{\C^\vee}\big(\ky(Z),\ky(Y)\sqcup_{\ky(X)}\ky(Y)\big)\Big). 
$$ 
This implies 
%
...
(Recall that the functor $\p:\C\to\F^\wedge$ is defined by $\p(X)(Y):=\Hom_\C(Y,X)$, and that, by definition, $\F$ is strictly generating if and only if $\p$ is fully faithful.)
%
\begin{center}*\end{center}
%
Proof. We may assume from the beginning that $\F$ is small. (Recall that the functor $\p:\C\to\F^\wedge$ is defined by $\p(X)(Y):=\Hom_\C(Y,X)$, and that, by definition, $\F$ is strictly generating if and only if $\p$ is fully faithful.)

\n Step 1. By Proposition 5.2.3 (i) p.~118, the functor $\p$ is conservative and faithful.

\n Step 2. By Proposition 1.2.12 p.~16, a morphism $f$ in $\C$ is an epimorphism as soon as $\p(f)$ is an epimorphism.

\n Step 3. Let us fix $X\in\C$, and let $\zeta:\C_X\to\C$ be the natural functor, so that an object of $\C_X$ consists of a morphism $z:\zeta(z)\to X$ in $\C$. Let $(z_i)_{i\in I}$ be a small filtrant inductive system in $\C_X$. Define the morphism 
...
\end{tikzcd}
$$ 
Then \eqref{step3} is an isomorphism.
...
\n$\bu$ the coprojections $c_k(i_k):\alpha_k(i_k)\to G(k)$ (see Definition~\ref{c} p.~\pageref{c}) induce a morphism of functors from $P$ to $G$. 
...
$$ 
for the projections and coprojections. We have $q_a\circ(f_1\times f_2)=f_a\circ p_a$, and we must show $q_b\circ (f_1\times f_2)\circ i_a=q_b\circ j_a\circ f_a$ for all $b$. This follows immediately from Corollary~\ref{823}.
\end{proof}
...
%
\begin{proof}
Apply Lemma~\ref{sl1} to the diagram 
$$
\begin{tikzcd}
...
$$
in $\A_c$. As $H(\Mc(\id_X))\simeq0$, $\p$ is a qis by Lemma~\ref{sl2}.
\end{proof}
%
\newpage
