% about "categories and sheaves" (version ahw)
...
\n$\bu$ P. 52, Theorem 2.3.3 (i). Let us observe:
%
\begin{rk}\label{233ib}
In the setting of Theorem 2.3.3 (i), assume that $\p$ induces a bijection $\Ob(J)\to\Ob(I)$ and that there is a functor $\beta':I\to\C$ such that $\beta'\circ\p\simeq\beta$. Then $\p^\dagger\beta$ exists and is canonically isomorphic to $\beta'$.
\end{rk}
...
If $F(s)$ is an isomorphism for all $s$ in $\cc S$, then $F$ is universally right localizable and the functors $R_{\cc S}F$ and $F_{\cc S}$ are canonically isomorphic. (See Remark~\ref{233ib} p.~\pageref{233ib}.)
