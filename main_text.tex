% about "categories and sheaves" (version aip)
...
%
\bc\label{cou}\footnote{\ ``\textbf{C} \ref{cou}'' stands for ``Comment \ref{cou}''.}\quad 
...
\bc P.~24, the Yoneda Lemma. Let $\C$ be a category, let $h:\C\to\C^\wedge$ be the Yoneda embedding, let $F$ be in $\C^\wedge$, let $A$ be in $\C$, and define 
$$
\begin{tikzcd} 
...
\bc P.~24, \pr\ 1.4.3 (Yoneda Lemma). Let $F:\C^{\op}\to\Set$ be a functor and $X$ an object of $\C$.  
%
\begin{df}\label{ue} 
...
\bc P.~25, Corollary 1.4.7. A statement slightly stronger than Corollary 1.4.7 of the book can be proved more naively:
% 
\bp
...
\bc P.~36, Definition 2.1.2. Let $\alpha:I^{\op}\to\C$ be a functor, let $\Delta:\C\to\C^{I^{\op}}$ be the diagonal functor, let $X$ be an object of $\C$, and let $u\in\h_{\C^{I^{\op}}}(\Delta(X),\alpha)$ be an $(\h_{\C^{I^{\op}}}(\Delta(-),\alpha),X)$-universal element (see Definition~\ref{ue}). 
%
\begin{df}\label{p}
...
\bc P.~39, Proposition 2.1.7. The following slightly stronger statement holds, statement independent of the Universes Axiom. Let $I, J, \C$ be categories and let 
$$
...
\bc P.~55, Corollary 2.4.4 (iii). Here is a slightly different proof. 
%
\bp\label{244}
...
\bc P.~56, proof of Corollary 2.4.6. We shall give two other proofs. Recall the statement: 
%
\bp 
In the setting 
% 
...
\bc P.~56, Lemma 2.4.7. Slightly different wording of the proof. Recall the statement: 
%
\bl 
...
\bc P.~58, proof of implication (vi) $\implies$ (i) of Proposition 2.5.2. 
%
\bp 
...
\bc P.~61, \pr\ 2.6.3 (i). Let $\C$ be a category and $A$ be in $\C^\wedge$. Consider the statements  
% 
...
\bc P.~61, Proposition 2.6.3. Here is a Lemma implicitly used in the proof of Proposition 2.6.3 (ii) p.~61 of the book: 
% 
\begin{lem} 
...
\bc P.~61, \pr\ 2.6.4. Here is a minor variant. 
%
\bp\label{264}
...
\bc P.~62, \pr\ 2.7.1. Consider the diagram 
%
...
\bc P.~63, Notation 2.7.2. The formula 
$$
...
\bc P.~72, proof of Lemma 3.1.2. Here is a minor variant of the proof of one of the implications. We assume that $\p:J\to I$ is a functor with $I$ filtrant and $J$ finite, and we want to prove 
$$
\lim\h_I(\p,i)\neq\varnothing
$$ 
for some $i$ in $I$. 
...
\bc P.~74, Theorem 3.1.6. Here is an immediate corollary: 
%
\begin{cor}\label{316}
...
\bc P.~74, Theorem 3.1.6. Here is a minor variant of part (i) of the proof of the implication (a)$\implies$(b).\smallskip 
...
\bc P.~75, Proposition 3.1.8 (i). In the proof of Proposition 3.3.15 p.~85, a slightly stronger result is needed (see Remark \ref{3315} p.~\pageref{3315}). We state and prove this stronger result. 
%
...
\bc P.~75. Here is a remark about the IPC Property: 
% 
\begin{rk}\label{ipc} 
...
\bc P.~78, \pr\ 3.2.2. It is easy to see that Condition (iii) is equivalent to: 
%
...
\bc P.~85, proof of Proposition 3.3.15. 
%
\begin{rk}\label{3315}  
% 
...
\bc P.~89, Proposition 3.4.5 (iii). The proof uses implicitly the following fact: 
...
\bc P.~94, proof of (a) $\implies$ (b) in Proposition 4.1.3 (ii). Here are more details. In the commutative diagram 
$$
\begin{tikzcd}
...
\bc P.~95, proof of Proposition 4.1.4 (i). Some more details: Let us denote vertical composition by $\circ$ and horizontal composition by $*$. Equality $(R*\eta*L)\circ(\e*P)=P$, that is $(R*\eta*L)\circ(\e*R*L)=R*L$, can be written as 
$$
...
\bc P.~116, proof of \pr\ 5.1.7 (i). Here is a very slightly different wording. Recall the statement: 
%
\bp 
...
\bc P.~133, Step (i) of the proof of Theorem 6.1.8. Here is a minor variant. Recall the statement: 
%
\bl 
...
\bc P.~133, Proposition 6.1.9. ``There exists a unique functor ...'' should be ``There exists a functor ... Moreover, this functor is unique up to unique isomorphism.'' (See Section \ref{619} p.~\pageref{619}.)
\ec 

%% 

\bc P.~136, proof of \pr\ 6.1.18 (i). Here is a minor variant: If $\iota:\C\hookrightarrow\Ind(\C)$ is the inclusion functor and $(\p_i,\psi_i:\alpha_i\pa\beta_i)_{i\in I}$ is a small filtrant inductive system of parallel arrows in $\C$ whose limit in $\Ind(\C)$ is a pair of parallel arrows $f,g:A\pa B$ (that is, $A=\ic_i\ \alpha_i$, and so on...), then we have 
$$
...
\bc P.~137, table. In view of Corollary 6.1.17 p.~136, one can add two lines to the table:\bigskip 
...
\bc P.~138, proof of Proposition 6.1.21. One can also argue as follows. Assume $\C$ admits finite projective limits. By Remark 2.6.5 p.~62 and Corollary 6.1.17 p.~136, all the inclusions represented in the diagram 
\[
\begin{tikzcd}
...
%
\bc\footnote{\ ``\textbf{C} \ref{d125}'' stands for ``Comment \ref{d125}''.}\quad 
%\footnote{Typographical detail: I'm contemplating replacing (for easier referencing) the ``$\bu$ P.~\dots'' with ``\textbf{C n.} P.~\dots'', where n is an automatically generated positive integer, and ``\textbf{C}'' stands for ``Comment''. I'm starting by testing this new system. So, for the time being, the reader is invited to consider that ``$\bu$ P.~\dots'' and ``\textbf{C n.} P.~\dots'' are synonymous.}
\label{d125}
%
...
\bc P. 90, Exercise 3.4 (i). (This exercise is used in the second sentence of p.~227). Recall the statement: 
%
...
\bc P.~147, Exercise 6.11. We prove the following slightly more general statement: 
% 
\begin{prop}
%
...
\bc P.~149, Definition 7.1.1. The set $\cc S$ is a subset of $\Ob(\Mor(\C))$ (see Notation~\ref{mor} p.~\pageref{mor}). The proof of the following lemma is obvious. 
%
\begin{lem}\label{711}
%
Let 
...
\bc P.~159, Definition 7.3.1 (ii). The following conditions on the right localization $(\C_{\cc S},Q)$ of $\C$ are equivalent: 
...
\bc P.~160, \pr\ 7.3.2. 
\begin{rk}\label{732}
If, in the setting of \pr\ 7.3.2, any $t$ in $\cc T$ is an isomorphism, then the right localization $(\C_{\cc S},Q)$ of $\C$ is universal in the sense of Definition~\ref{url}.
\end{rk}
\ec 

%%

\bc P. 169, Definition 8.2.1. The proofs of the Proposition and Lemma below are obvious. 
...
\bc P.~169, Lemma 8.2.3. Here is a statement contained in Lemma 8.2.3:
%
\begin{cor}\label{823}
...
\bc P.~199, Lemma 8.7.4 (ii). This comment is about the claim that the natural functor $E:\cc D'_{\cc S}\to\C$ is an equivalence. I don't understand the proof of the faithfulness of $E$ given in the book. I think that it suffices, in view of \pr\ 7.1.2 (i) p.~150 and Theorem 7.1.16 p.~155, to check that 
%
\be\label{l}
...
\bc P.~202, Exercise 8.4. Recall the statement: 
...
\bc P.~220, proof of Corollary 9.2.11: Use Lemma~\ref{922}.\ec 

%%

\bc P.~222, \pr\ 9.2.17, proof of the implication (ii) $\implies$ (i). I suspect that the argument of the book is better than the one given here, but, unfortunately, I don't understand it. It suffices to prove the following statement. 
...
\bc P. 227, Theorem 9.3.4. Firstly I think it would be better to state the result as follows:
...
\bc P. 232, Theorem 9.5.4, minor variant: 
\begin{rk}\label{954}
...
\bc\label{t955} P. 233, Theorem 9.5.5. I suggest two changes: 
...
\bc P. 235, Theorem 9.6.1. Here are a few details. 
%
\begin{df}\label{cb}
...
\bc P. 250, proof of Theorem 10.2.3 (iii). In view of Corollary~\ref{may} p.~\pageref{may}, it is not necessary to prove TR4.
\ec 
...
\bc P. 263, proof of Lemma 10.5.9. Recall the Yoneda isomorphisms 
$$
...
\begin{cm}\label{pg270} P. 270. Recall that $(\A,T)$ is an additive category with translation. Let 
\be\label{iliad}
...
\bc P. 337. I would state Theorem 13.4.1 as follows:
...
\bc P. 350, proof of Theorem 14.1.7. See Comment~\ref{t955} p.~\pageref{t955}.
\ec
...
\bc P. 395, Lemma 16.2.3 (iii). Consider the condition 
...
\bc P. 397, Notation 16.2.5 (ii). The fact that 
\be\label{1625}
...
\bc P. 398, proof of Lemma 16.2.8. Recall that $\C$ is a small category. Here is a variant of the two sentences ``Let us consider the set $\cc S$ of $(J,S,v,w)$ where $J$ is a subset of $I$, $v:C_J\to S$ is an epimorphism and $w:S\to A$ is a local isomorphism. By Proposition 5.2.9 and the result of Exercise 5.1, the set of quotients of any object of $\C^\wedge$ is small, and hence $\cc S$ is a small set.'' 
...
\bc P. 400, proof of Lemma 16.3.1. Here are more details: 
...
\bc P. 400, Step (ii) in the proof of Lemma 16.3.2. Here are a few more details: 
...
\bc P. 409, proof of (17.1.12). The isomorphism is first proved in the particular case $B\in\C_X$, and then in the general case $B\in\C_X^\wedge$. It seems to me that the argument given in the particular case also works in the general case. Here the way I understand this argument. 
...
\bc P. 410, Formula (17.1.15) follows from \eqref{17115} p.~\pageref{17115}.\ec 

%%

\bc P. 413, Proof of Lemma 17.2.5. Part~(i): see Proposition~\ref{p406} p.~\pageref{p406}. Part~(iii): see \eqref{1725} p.~\pageref{1725}.
\ec 

%%

\bc P. 414, Definition 17.7.8. Here is a variant. 
%
\begin{df}\label{1778}
...
\bc P. 418, proof of Lemma 17.4.2. Here is a slightly different wording: 
...
\bc P. 421. I don't understand the proof of Lemma 17.4.6 (i). So far I haven't seen this statement used in the sequel.
\ec 

%%

\bc P. 422, proof of Theorem 17.4.9 (iv). The exactness $(\cdot)^a$ (Theorem 17.4.7 (iv) p.~421) is an important ingredient.
\ec 

%%

\bc P. 424, proof of Theorem 17.5.2 (iv). Firstly, as already mentioned, there is a typo: ``The functor $f^\dagger$ is left exact'' should be ``The functor $f^\dagger$ is exact''. Secondly, the exactness of $f^\dagger$ follows from Proposition~\ref{407} p.~\pageref{407}.
\ec 

%%

\bc P. 426, proof of Proposition 17.6.7 (i). The isomorphism 
$$
