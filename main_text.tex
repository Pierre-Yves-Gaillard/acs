% about "categories and sheaves" (version alf)
...
$$ 
is bijective. The injectivity and the surjectivity follow respectively from Lemma 8.2.11 and Proposition 8.2.13. q.e.d.
...
\end{thm}
%
\n{\em Proof.} Let $\C$ be our additive category, and let 
$$
\begin{tikzcd}
X\ar[yshift=0.7ex]{r}{f_1}\ar[yshift=-0.7ex]{r}[swap]{f_2}&
Y\ar[yshift=0.7ex]{r}{g_1}\ar[yshift=-0.7ex]{r}[swap]{g_2}&Z
\end{tikzcd}
$$ 
be morphisms in $\C$. By Proposition~\ref{8.2.12} there is one and only one way to turn the functor $\Hom_\C(X,\ ):\C\to\Set$ into a functor $\Hom_\C(X,\ ):\C\to\Mod(\mathbb Z)$. In particular 
$$
\Hom_\C(X,g_1)=g_1\circ:\Hom_\C(X,Y)\to\Hom_\C(X,Z)
$$ 
is a morphism in $\Mod(\mathbb Z)$, and we have $g_1\circ(f_1+f_2)=g_1\circ f_1+g_1\circ f_2$. By Proposition~\ref{8.2.12} again, 
$$
\circ f_1:\Hom_\C(\ ,Z)\to\Hom_\C(\ ,Y),
$$ 
being a morphism in $\oo P(\C,\Mod(\mathbb Z))$, the map 
$$
\circ f_1:\Hom_\C(X,Z)\to\Hom_\C(X,Y)
$$ 
is a morphism in $\Mod(\mathbb Z)$, and we have $(g_1+g_2)\circ f_1=g_1\circ f_1+g_2\circ f_1$. q.e.d.
%
\end{s}
%
%%
