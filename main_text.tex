% about "categories and sheaves" (version agx)
...
\n$\bu$ P. 398, Lemma 16.2.8. In the course of the proof of Lemma 16.2.8 the following fact is stated and proved: 

If $\C$ is a small category and $A$ an object of $\C^\wedge$, then the set of quotients of $A$ is small. 

Here is another proof. For any set $S$ write $E_S$ for the set of equivalence relations on $S$. Let $S_A$ be the set of epimorphisms $A\epi B$ in $\C^\wedge$, let $T_A$ be the set of quotients of $A$, and put 
$$
U_A:=\prod_{X\in\C}E_{A(X)}. 
$$ 
Then the obvious map $S_A\to U_A$ induces an injection $T_A\mono U_A$. 
%
