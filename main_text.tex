% about "categories and sheaves" (version aid)
...
Let $\C$ be an abelian category, let $X,Y\in\oo D(\C)$. Assume that the functor 
$$
\Hom_\C^\bullet:\oo K(\C)\times\oo K(\C)^{\op}\to\oo D(\Mod(\bb Z))
$$ 
given by $(X',Y')\mapsto\oo{tot}_\pi\Hom_\C^{\bullet,\bullet}(X',Y')$ (see \S\ 11.7) is universally right localizable, and denote its right localization by $\oo R\Hom_\C$. Then\bigskip 
