...
%
\begin{lem}\label{711}
Let $\begin{tikzcd}\C\ar[yshift=.4ex]{r}{Q}&\C'\ar[yshift=-.4ex]{l}{R}\end{tikzcd}$ be functors such that $Q\circ R\simeq\id_{\C'}$, let $\cc S$ be a set of morphisms in $\C$, let $\theta:\id_\C\to R\circ Q$ satisfy $\theta_X\in\cc S$ for all $X$ in $\C$. Then $Q$ is a localization of $\C$ by $\cc S$. 
\end{lem}
...
\n$\bu$ P.~199, Lemma 8.7.4 (ii). This comment is about the claim that the natural functor $E:\cc D'_{\cc S}\to\C$ is an equivalence. I don't understand the proof of the faithfulness of $E$ given in the book. I think that it suffices, in view of Theorem 7.1.16 p.~155, to check that the functor $Q:\cc D'\to\C$ is a localization of $\cc D'$ by $\cc S$. To prove this, one can argue as follows. Recall that $\cc D'$ is a category whose objects are the morphisms in $\C$ (see the book for the precise definition of $\cc D'$), and that $Q$ maps $f$ to $\Coker f$. Then we can apply Lemma~\ref{711} p.~\pageref{711} with $R:\C\to\cc D'$ defined by $R(X):=(0\to X)$. 
% 
\end{document}
