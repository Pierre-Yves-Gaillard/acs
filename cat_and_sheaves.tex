...
$$
\begin{tikzcd}
\al(\pp(j))\ar{dd}\ar{r}{\id}&\al(\pp(j))\ar{d}\ar{r}{\id}&\al(\pp(j))\ar{d}\\ 
...
\ds\col_{\theta(\pp(j))\to\psi(k)}\al(\pp(j))\ar{d}\ar{r}&\ds\col_{\theta(i)\to\psi(k)}\al(i)\ar{d}\ar{r}&\ds\col_{\theta(i)\to\psi(k)}\al(i)\ar{d}\\ 
...
$$
\begin{tikzcd}
\al(\pp(j))\ar{d}&\al(\pp(j))\ar{l}[swap]{\id}\ar{dd}\\ 
\ds\col_{\theta(i)\to\psi(k)}\al(i)\ar{d}&\ar{d}\\ 
\ds\col_{\ell}\col_{\theta(i)\to\ell}\al(i)&\col\al\ar{l}{e}
\end{tikzcd}
$$ 
where the vertical arrows are coprojections, the squares above $a$ and $e$ are obtained by \qr{coco} p.~\pr{coco}, the squares above $b$ and $d$ result from the cofinality of $\psi$, and the squares above $c$ result from the cofinality of $\pp_k$. Now the proposition follows from \qr{coco}. 
\end{proof}
\end{s}
