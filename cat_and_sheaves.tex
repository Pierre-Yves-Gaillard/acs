...
\begin{s} 
P. 150, Definition 7.1.1. Condition (c) says that, in terms of \S\ref{s232} (c) p.~\pr{s232}, for all functor $G:\C_\SSS\to\A$ the functor $Q^\ddg Q_*G$ exists and is isomorphic to $G$ via the identity of $G\ci Q$. (Recall that $v*Q$ denotes the horizontal composition of $v$ and $Q$; see Definition~\ref{dil1} p.~\pr{dil1}.)
\end{s}
...
$$
\Hom_{\A^{\C'}}(F,G)\to\Hom_{\A^\C}(F\ci Q,G\ci Q),\quad v\mt v*Q
$$ 
