% about "categories and sheaves" (version avf)
...
%
\begin{proof}
We assume, as we may, that $I$ is nonempty and well-ordered. We shall achieve the following task by transfinite induction: 

\nn[Beginning of the description of the task.] Construct, for each $i$ in $I$, a commutative diagram 
$$
\begin{tikzcd}
X_j\ar{rr}{u_i}\ar{d}[swap]{f_i}&&Y_j\ar{d}{h_i}\\
X'\ar{r}[swap]{v_i}&Y'_{<i}\ar{r}[swap]{w_i}&Y'_i
\end{tikzcd}
$$
with $v_j,w_j\in\Mor(\C_0)$, and, for each $(i,j)$ in $I^2$ with $i>j$, construct a commutative diagram 
$$
\begin{tikzcd}
...
\end{equation}
%
\nn[End of the description of the task.]

Let $0$ be the least element of $I$, let us define $Y'_0,v_0$, and $h_0$ by (9.5.2) (ii):
$$
\begin{tikzcd}
...
\end{tikzcd}
$$
From this point, it is straightforward to verify that the required conditions are still satisfied. Our task has been achieved. 

In particular, $(Y'_i)_{i\in I}$ is an inductive system in $\C_0$. We denote its limit (which exists in $\C_0$ thanks to (9.5.2) (i)) by $Y'$, and we write $p_i$ for the coprojection $Y'_i\to Y'$. Putting
$$
g_i:=p_i\circ h_i,\quad u':=p_0\circ v_0,
$$
we get
$$
g_i\circ u_i=p_i\circ h_i\circ u_i=p_i\circ w_i\circ v_i\circ f_i=p_i\circ w_i\circ p_{i0}\circ v_0\circ f_i
$$
$$
=p_i\circ u_{i0}\circ v_0\circ f_i=p_0\circ v_0\circ f_i=u'\circ f_i.
$$
\end{proof}
%
%%%
