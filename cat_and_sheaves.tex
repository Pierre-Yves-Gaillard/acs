...
then the assumption that $\delta$ is an epimorphism and the equality $p_1\ci\delta=p_2\ci\delta$ proved in (i) imply $p_1=p_2$, and thus $g=h$.
\end{s}
...
\end{tikzcd}
$$ 
where the vertical arrows are coprojections, the squares above $a$ and $e$ result from the proof of \qr{coco} p.~\pr{coco}, the squares above $b$ and $d$ result from the cofinality of $\psi$, and the squares above $c$ result from the cofinality of $\pp_k$. In particular, the maps $a,b,c,d$ and $e$ are bijective. As the bijection $f:=e^{-1}\ci d\ci c\ci b\ci a$ satisfies $f\ci p_j=q_j$, it is the natural map from $\col\al\ci\pp$ to $\col\al$.
\end{proof}
\end{s}
