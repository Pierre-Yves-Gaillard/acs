% about "categories and sheaves" (version avr)
...
We recall the notion of {\em construction by transfinite induction} in the setting of Bourbaki's set theory, to which, as already mentioned, we adhere.

\begin{mt}[Construction by Transfinite Induction]\label{meta}
Let $T$ be a term, let $x$ be a letter, and, for each term $X$, write $T(X)$ for the term obtained by substituting $X$ for $x$ in $T$. Let $I$ be a well-ordered set, and, for each $i$ in $I$, write $I_{<i}$ for the set $\{j\in I\,|\,j<i\}$. Then there is a unique pair $(S,f)$ such that $S$ is a set, $f:I\to S$ is a surjection, and we have $f(i)=T(f(j)_{j<i})$ for all $i$ in $I$. 
\end{mt}
