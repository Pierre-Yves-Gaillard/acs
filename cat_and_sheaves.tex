% about "categories and sheaves" (version aub)
...
\nn$\bu$ an inductive system $(u_{ij}:Y'_j\to Y'_i)_{i>j}$ in $\C_0$,

\nn$\bu$ a family $(v_i:X'\to Y'_i)_i$ of morphisms in $\C_0$ such that $u_{ij}\circ v_j=v_i$ for all $i>j>k$, 
...
$$ 
in $\C_0$ and a family of morphisms 
$$
(v_j:X'\to Y'_j)_{j<i}
$$ 
in $\C_0$ such that $v_j\circ u_{jk}=v_j$ for all $k<j<i$. By (9.5.2) (i) (see \eqref{952ii} p.~\pageref{952ii}), the object 
$$
Y'_{<i}:=\co_{j<i}Y'_i
$$ 
exists in $\C_0$. Let $p_{ij}:Y_j'\to Y'_{<i}$ be the coprojection, and define $w_i$ and $h_i$ via Condition (9.5.2) (ii): 
$$
\begin{tikzcd}
...
\end{equation}
%
This will imply that $(u_{ij})$ and $(v_i)$ satisfy the required conditions. We prove \eqref{pij} as follows:

Let $i$ be a fixed element of $I$. The statement
$$
(S_i)\qquad p_{ij}\circ w_j\circ p_{jk}=p_{ik}\quad\forall\quad k<j<i
$$ 
implies $u_{jk}\circ u_{k\ell}=u_{j\ell}$ and $v_j\circ u_{jk}=v_k$ for all $\ell<k<j<i$. Using this fact, one easily proves 
$$
