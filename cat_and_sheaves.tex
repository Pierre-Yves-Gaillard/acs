% about "categories and sheaves" (version-0289)
...
\nn(a)$\then$(c): Put $u:=(\ee_{P(X)})^{-1}\circ P(\ee_X)$. It suffices to show $u=\id_{P(X)}$. As we have $\ee_{P(X)}\circ u=P(\ee_X)$ by definition of $u$, and $P(\ee_X)\circ\ee_X=\ee_{P(X)}\circ\ee_X$ by naturality of $\ee$, we get 
$$
...
\nn As $\ee_{P(X)}$ is a monomorphism, this implies $v=\id_{P(X)}$, as required.
\end{proof}
