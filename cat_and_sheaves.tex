...
%\begin{s} 
\begin{thm}\lb{233}
If $I$ and $J$ are big categories, if $\C$ is a $\U$-category, if $I\xl\pp J\xr\bt\C$ is a functor, and if 
\begin{equation}\lb{e233}
\col_{(\pp(j)\to i)\in J_i}\bt(j)
\end{equation}
exists in $\C$ for all $i$ in $I$, then $\pp^\dg\bt$ exists, and $(\pp^\dg\bt)(i)$ is isomorphic to \qr{e233}. Moreover, \qr{e233} exists universally in the sense of Definition~\ref{uil} p.~\pr{uil} if and only if $\pp^\dg\bt$ exists universally in the sense of Definition~\ref{232} p.~\pr{232}. 
\end{thm}
\begin{proof}
...
\nn\emph{(c)} if $F:\C\to\A$ is a functor turning the elements of $\SSS$ into isomorphisms, the there is a unique functor $G:\C_\SSS\to\A$ such that $G\ci Q=F$,

\nn\emph{(d)} if $G_1$ and $G_2$ are two functors from $\C_\SSS$ to $\A$, then the natural map 
$$
Hom_{\Fct(\C_\SSS,\A)}(G1,G2)\to\Hom_{\Fct(\C,\A)}(G_1\ci Q,G_2\ci Q)
$$
is bijective.
\end{df}
\begin{prop}\lb{p711}
...
\end{tikzcd} 
$$ 
commutes for all $f\in\Hom_{\C'}(X,Y)$. It suffices to check that this square commutes when $f=Q(s)^{-1}$ for $s$ in $\SSS\cap\Hom_\C(Y,X)$. 
\end{proof}
...
$R_\SSS F\iso F_\SSS$ (for the definition of $F_\SSS$, see condition (b) in Definition 7.1.1 p.~149 in the book), and for any functor $K:\A\to\A'$ the diagram below commutes up to isomorphism
$$
\begin{tikzcd}
...
\begin{s}
P. 163, Proposition 7.4.4. The fact that (i) implies (ii) follows from Theorem~\ref{233i} p.~\pr{233i}. To show that (ii) implies (i), note that, for all $X$ in $\C$, we have 
$$
\iota_\A(R_\SSS F(Q(X)))\iso R_\SSS(\iota_\A\ci F)(Q(X))\iso\col_{(X\to X')\in\SSS^X}\iota_\A(F(X')), 
$$
the isomorphisms following respectively from the assumption that $F$ is universally right localizable and from Theorem~\ref{233i} p.~\pr{233i}. (The argument is essentially the same as the one used in the proof of Theorem~\ref{233} p.~\pr{233}.)
\end{s}

%

\begin{s}\lb{744}
P. 163, Proposition 7.4.4. In view of \S\ref{742} p.~\pr{742}, the assumption that $\SSS^X$ is cofinally small for any $X$ in $\C$ is not necessary.
\end{s}
...
The second sentence in the above quote is justified by \S\ref{744} p.~\ref{744}. %Proposition 7.4.4 p.~163 of the book.
...
\begin{s}
P. 320, Parts (i) and (ii) of Proposition 13.1.5. Part (i): the argument showing that the cohomology functor $H:\oo K(\C)\to\oo C(\C)$ factors through $Q:K(\C)\to\oo D(\C)$ are implicit in \S\ref{300301}, \S\ref{300301b} and \qr{300301c}. Part (ii): the statement that, in the notation of the proposition, $f$ is an isomorphism if $H(f)$ is, also follows from the argument implicit in \S\ref{300301}, \S\ref{300301b} and \qr{300301c}. To be slightly more explicit, one considers the long exact sequence 
$$
\cdots\to H(X)\to H(Y)\to H(Z)\to H(TX)\to\cdots
$$ 
...
\end{tikzcd}
$$ 
\emph{be a commutative diagram in $\C$. If the left small square and the big square (or ``rectangle'') are cocartesian, then the right small square is cocartesian too.}
\end{s}
...
This is justified by \S\ref{744} p.~\ref{744}. %Proposition 7.4.4 p.~163 of the book %In fact, the existence of the indicated limit is a sufficient condition for the existence of the right derived functor, but it is not obvious \emph{a priori} (at least to me) that this condition is necessary. 
(see \S\ref{253} p.~\pr{253}). 
\end{s}

%\begin{s}
%\begin{uspb}The\index{unsolved problem} sentence just before Notation 13.3.2 p.~330 starts with the claim that $R^*F$ is triangulated. I don't understand why.\end{uspb}

%\nn\textbf{Warning.} In this text, we adopt the following terminology: If $F$ is universally right localizable in the sense of Definition 7.3.1 p.~159 of the book, \emph{and if its right localization is triangulated}, we denote it by $R^*F$, and we say that ``$R^*F$ exists''.
%\end{s}

%\begin{s}P. 330, proof Corollary 13.3.3. In view of \S\ref{330} above, it seems to me that the proof should be phrased slightly differently.\end{s}

%\begin{s}P. 331, proof of Proposition 13.3.5 (ii). In part ``(a) $\then$ (b) (2)'' of the proof, the exactness of $F(X)\to F(Z^0)\to F(Z^1)$ follows from the comment made in the book just before the statement of Proposition 13.3.5.%%can be verified as follows: If $W$ is an exact complex in $\oo K^+(\J)$, then we have $0\iso R^+F(0)\iso F(W)$, the first isomorphism resulting from the additivity of $R^+F$ (see \ref{1335i} p.~\pr{1335i}) and the second isomorphism resulting from (13.3.1) p.~330 of the book. Now Proposition 13.1.5 (ii) p.~320 of the book implies that $F(W)$ is exact.\end{s}
