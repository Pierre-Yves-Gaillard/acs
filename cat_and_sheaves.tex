% about "categories and sheaves" (version arr)
...
Let us denote the cokernel of any morphism $h:Y\to Z$ in any abelian category by $Z/\Ima h$. Recall that, by Proposition 8.3.18 p.~183, an additive functor between abelian categories $F:\C\to\C'$ is left exact if and only if
%
...
$$
are exact. We must show that $F(X')\to F(X)\to F(X'')$ is exact. We have
$$
\Ker(F(X)\to F(X''))\simeq\Ker(F(X)\to F(I_g))
$$
$$
\simeq\Ima(F(K_g)\to F(X))\simeq\Ima(F(X')\to F(X)),
$$ 
the isomorphisms following respectively from the exactness of 
$$
0\to F(I_g)\to F(X''),\quad F(K_g)\to F(X)\to F(I_g),\quad F(X')\to F(K_g)\to0.
$$
\end{proof}
%
%%%
