...
This gives rise to a diagram in $\C_1$ whose vertices are the $X_i$ with $i\in M^n$, and whose arrows are the $f_{i,p}$ with $i,i+e_p\in M^n$. 

Clearly, all the squares in this diagram commute, and, thus, the whole diagram commutes. In particular, all the paths from $X_{(m+1,\dots,m+1)}$ to $X_{(1,\dots,1)}$ coincide. (Such paths obviously exist.) So, we get a well-defined morphism in $\C_{n+1}$
$$
X_{(m+1,\dots,m+1)}=F_{m+1,1}\cdots F_{m+1,n}X\to F_{1,1}\cdots F_{1,n}X=X_{(1,\dots,1)}.
$$
When $X$ varies in $\C$, this family of morphisms in $\C$ defines the sought-for morphism of functors from $F_{m+1,1}\cdots F_{m+1,n}$ to $F_{1,1}\cdots F_{1,n}$. 
...
\begin{df}\label{dil} 
We call $*$ the {\em horizontal composition}. \index{horizontal composition} We call $\circ$ the {\em vertical composition}. \index{vertical composition} We call the equalities in Proposition~\ref{pil} the {\em Interchange Law}. \index{Interchange Law}
\end{df}
...
\begin{s} 
P.~95, proof of Proposition 4.1.4 (i) (additional details): In Notation~\ref{nil} p.~\pageref{nil}, the sentence of the book ``The two compositions 
$$
\begin{tikzcd}
...
\nn the third and fourth equalities resulting respectively from \eqref{159} p.~\pageref{159} and \eqref{158} p.~\pageref{158}, and the other equalities following from Proposition~\ref{pil} p.~\pageref{pil}.
\end{s}
