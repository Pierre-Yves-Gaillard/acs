...
If $\al:I\to\C$ is a functor, if $h:\C\to\C^\wg$ is the Yoneda embedding, if $\col h\ci\al$ exists in $\C^\wg$, if $X$ is in $\C$, if $f:X\to\col h\ci\al$ is an isomorphism in $\C^\wg$, if $q_i:\al(i)\to\col h\ci\al$ are the coprojections, then $X$ is a universal limit of $\al$, the coprojections $p_i:\al(i)\to\col\al$ being given by the formula $p_i:=f^{-1}\ci q_i$.
\end{prop}
\begin{proof}
It suffices to prove that $X$ is a universal limit of $\al$, the coprojections $p_i:\al(i)\to\col\al$ being given by the indicated formula. Let $Y$ be in $\C$ and $u_i:\al(i)\to Y$ a functorial family of morphisms indexed by $i\in I$. It suffices to show that there is a unique morphism $u:X\to Y$ in $\C$ such that $u\ci p_i=u_i$, that is $u\ci f^{-1}\ci q_i=u_i$, for all $i$. But by assumption there is a unique morphism $v:X\to Y$ in $\C^\wg$ such that $v\ci f^{-1}\ci q_i=u_i$ for all $i$, so that it suffices to take for $u$ the morphism $v$ viewed as a morphism in $\C$. %set $u:=v$. 
\end{proof}
...
\begin{proof}
To prove the proposition, we may assume that $I$ is small. Let $h:\C\to\C^\wg$ be the Yoneda embedding and $j$ a fixed object of $J$. Then the proposition follows immediately from Proposition~\ref{stupid} p.~\pr{stupid}. % previous version:
...
\begin{proof}
The first sentence and the ``only if'' part of the second sentence follow from Theorem~\ref{233i} p.~\pr{233i}. To prove the ``if'' part of the second sentence, we may assume that $J$ is small, and the proposition follows immediately from Proposition~\ref{stupid} p.~\pr{stupid}. 
% previous version:
% https://docs.google.com/document/d/1K004uKK3ECssS-Y1etYqzLcEilskE8Hlb0wVrElSqM0/edit
\end{proof}
%
