...
Recall that $\C$ is a category, that $\F$ is an essentially small full subcategory (Definition~\ref{myuscat} p.~\pr{myuscat}) of $\C$, that the functor $\pp:\C\to\F^\wg$ is defined by $\pp(X)(Y):=\Hom_\C(Y,X)$, and that, by definition, $\F$ is strictly generating if and only if $\pp$ is fully faithful.
...
\nn\emph{(ii)} small filtrant inductive limits are stable by base change (see Section~\ref{sbsarsbc} p.~\pr{sbsarsbc}), 

\nn\emph{(iii)} any epimorphism is strict. 

\nn Let $\F$ be an essentially small full subcategory of $\C$ (Definition~\ref{myuscat} p.~\pr{myuscat}) such that 
