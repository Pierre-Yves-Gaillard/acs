...
\begin{s}\lb{ldefat}
Let us spell out the statement  to \S\ref{rdefat}:
...
In \S\ref{patho} p.~\ref{patho} we shall see a way to prevent the kind of pathology displayed by the above example.
\end{s}
...
\begin{s}\lb{patho}
Claim: If $I$ and $J$ are big categories, if $\C$ is a $\U$-category, if $\al:I\tm J\to\C$ is a functor and if $\col_i\al(i,\ )$ exists universally in the sense of Definition~\ref{uil} p.~\pr{uil}, then $\col_i\al(i,j)$ exists universally for all $j$.

In \S\ref{c38} p.~\pr{c38} we saw that, without the adverb ``universally'', the claim is false.

To prove the claim, we may assume that $I$ is small. Let $h:\C\to\C^\wg$ be the Yoneda embedding and note the isomorphisms 
$$
\left(\col_ih(\al(i,\ ))\right)(j)\xr\sim\bigg(h\Big(\col_i\al(i,\ )\Big)\bigg)(j)\iso h\left(\col_i\al(i,j)\right).\square
$$ 
\end{s}
...
\begin{proof}
The first sentence and the ``only if'' part of the second sentence follow from Theorem~\ref{233i} p.~\pr{233i}. To prove the ``if'' part of the second sentence, we may assume that $J$ is small. Consider the diagram 
$$
\begin{tikzcd}
J\ar[dd,"\pp"']\ar[rr,"\bt"]&&\C\ar[dd,"h"]\\ \\ 
I\ar[rr,"\pp^\dg(h\ci\bt)"']\ar[rruu,"\pp^\dg\bt"]&&\C^\wg,
\end{tikzcd}
$$ 
where $h$ is the Yoneda embedding. We have 
$$
h(\pp^\dg(\bt)(i))\iso\pp^\dg(h\ci\bt)(i)\iso\col_{(\pp(j)\to i)\in J_i}h(\bt(j)),
$$ 
the isomorphisms following respectively from the assumption that $\pp^\dg\bt$ exists universally and from Theorem~\ref{233i} p.~\pr{233i}. (The argument is essentially the same as the one used in \S\ref{patho} p.~\pr{patho}.)
\end{proof}
%\end{s}
...
\end{equation} 
by $\mu_X:=\ld_X$ for all $X$ in $\C$. To do this, we must check that the $\ld_X$ commute (in the obvious sense of the term) with all morphisms of $\C_\SSS$, even if \emph{a priori} they only commute with the morphism of $\C$ (or, more precisely, with the images in $\C_\SSS$ of the morphism of $\C$). Any morphism of $\C_\SSS$ being of the form $Q(s)^{-1}\ci Q(f)$ with $s$ in $\SSS$ and $f$ a morphism in $\C$, it suffices to check that the $\ld_X$ commute with the $Q(s)^{-1}$, which is easy. This shows that the map \qr{7116c2} is well-defined, and it is straightforward to check that it is inverse to \qr{7116c1}. (This argument was implicit in the proof of Proposition~\ref{p711} p.~\pr{p711}.)
\end{s}
...
\end{tikzcd}
$$ 
The book attaches, to each functor $G:\C_\SSS\to\A$, a bijection 
$$
...
have the following equalities %(it is on purpose that I write ``equalities'' instead of ``isomorphisms'', %don't write ``isomorphisms'', but ``equalities''  I really mean ``equalities'', not ``isomorphisms'', see \S\ref{l711} p.~\pr{l711}) 
between functors: 
$$
