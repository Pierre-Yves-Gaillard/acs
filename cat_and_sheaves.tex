% about "categories and sheaves" (version awn)
...
\begin{s}\label{922}
P.~218, Definition 9.2.2. If $I$ admits inductive limits indexed by categories $J$ such that $\operatorname{card}(\Mor(J))<\pi$, then $I$ is $\pi$-filtrant.
...
\subsection{Theorem 9.3.4 (p. 227)}\label{s934}
...
\subsubsection{Statement of Theorem 9.3.4 p.~227}

\begin{thm}[Theorem 9.3.4 p.~227]\label{934a}
Assume Conditions {\em(i)--(v)} above. Then there is a regular infinite $\U$-cardinal $\pi$ such that, for any $X$ in $\C$, we have 
$$
X\in\C_\pi\ssi\card(X(G))<\pi.
$$ 
Moreover, $\pi$ can be chosen arbitrarily large (among the regular infinite $\U$-car\-di\-nals).
\end{thm}

\subsubsection{Lemma 9.3.1 (p. 224)}

We give a slightly more detailed rewriting of the second sentence in Step (a) of the proof of Lemma 9.3.1. This second sentence is ``Set $S:=\co_iY_i(G)\subset Y(G)$.'' Here is the rewriting:

Claim: the coprojection $p_i:Y_i\to Y$ is injective.

In the diagram 
$$
\begin{tikzcd}
\co_jY_j\ar{rr}{\sim}&&Y\\ 
{}&Y_i,\ar{ul}{p_i}\ar{ur}[swap]{q_i}
\end{tikzcd}
$$ 
let the horizontal arrow be the isomorphism of the first display of the proof of Lemma 9.3.1, and let $q_i$ be the canonical morphism. Then $q_i$ is a monomorphism by the observation made on p.~114 of the book, right after Definition 5.1.1. As the diagram commutes, $p_i$ is also a monomorphism, and Proposition 5.2.3 (ii) p. 118 of the book yields the claim. 

As $Y_i\to Y$ is a monomorphism, so is $Y_i(G)\to Y(G)$. As small filtrant inductive limits are exact, $S:=\co_iY_i(G)\to Y(G)$ is also a monomorphism.
...
\begin{equation}\label{cardipi}
\card(I)<\pi
\end{equation}
%
in the first display of the proof of Lemma 9.3.3 p.~226 of the book can be justified as follows:
...
To prove the proposition, let $A$ be a well-ordered set of cardinal $\alpha$, and, for each cardinal $\beta$ with $\beta<\alpha$, let $B$ be a well-ordered set of cardinal $\beta$. Then $B$ is isomorphic to $A_{<a}$ for a unique $a$ in $A$, and the map $\beta\mapsto a$ is injective.
\end{proof}

Now we prove \eqref{cardipi} as follows: Putting $\alpha:=\card(A)$ we have
$$
\card(I)=\sum_{\pi'<\pi_0}\ \binom{\alpha}{\pi'}\le\sum_{\pi'<\pi_0}\ \alpha^{\pi_0}<\pi,
$$ 
the last inequality following from Proposition~\ref{ord}, (c), and (a).
...
\end{thm}
%
Clearly Theorem~\ref{934} implies Theorem~\ref{934a}.
...
$$ 
Then we see that $G^{\coprod X(G)}\to X$ is an epimorphism by Proposition 5.2.3 (iv) p.~118, that $G^{\coprod A}\to X$ is an epimorphism for some $A$ in $I$ by Lemma 9.3.1 p.~224, and that $\card(X(G))<\pi$ by Lemma 9.3.3 p.~226.
...
$$ 
By Lemma \ref{933} and (c) we have 
$$
\card(I)\le\card(X(G))^{\pi_0}<\pi.
$$
Since $\card(G^{\coprod B}(G))<\pi$ for all $B$ in $I$ by (e), this implies \eqref{934b}, and Proposition~\ref{932} p.~\pageref{932} entails $X\in\C_\pi$.
\end{proof}
...
Theorem~\ref{qtot} clearly implies Theorem~\ref{zorn}. Lemma~\ref{945} below will shows that Theorem~\ref{942} follows also from Theorem~\ref{qtot}. Theorem~\ref{qtot} will be used in the book to prove Theorem 9.5.5 p.~133.
...
\begin{comment}
\begin{s} 
P. 350, proof of Theorem 14.1.7. See \S\ref{t955} p.~\pageref{t955} and Lemma~\ref{952i} p.~\pageref{952i}.
\end{s}
\end{comment}
%
%%
