% about "categories and sheaves" (version_0038)
...
%\noindent\href{https://drive.google.com/folderview?id=0BzbJLwZkWgt8RzlXdWNoWXNsdkk&usp=sharing}{https://drive.google.com/folderview?id=0BzbJLwZkWgt8RzlXdWNoWXNsdkk\&usp=sharing}

\nn\href{https://www.mediafire.com/folder/2kgfet0by7ifv/WEBSITE}{https://www.mediafire.com/folder/2kgfet0by7ifv/WEBSITE}

\nn\href{https://www.dropbox.com/s/vv2fqguf2got2zc/ks.pdf}{https://www.dropbox.com/s/vv2fqguf2got2zc/ks.pdf}

\nn\href{https://www.dropbox.com/s/xj9ve3z6gizobou/ks.txt}{https://www.dropbox.com/s/xj9ve3z6gizobou/ks.txt}

%The \TeX file of the very last version is available at \nn\href{http://goo.gl/jsrxbE}{http://goo.gl/jsrxbE}\nn\href{https://www.mediafire.com/folder/6ocfdrx41rjb7/LAST}{https://www.mediafire.com/folder/6ocfdrx41rjb7/LAST}
...
\subsection{Horizontal and Vertical Compositions (p.~19) 2}\label{hove2}

This Section and Section~\ref{hove1} p.~\pageref{hove1} serve exactly the same purpose. The reader should read at most one of them.
...
We shall define a morphism from $F_{m+1,1}\cdots F_{m+1,n}$ to $F_{1,1}\cdots F_{1,n}$.

Let $X$ be an object of $\C_{n+1}$, and, for $i\in M^n$, where $M:=\{1,2,\dots,m+1\}$, put 
$$
X_i:=F_{i_1,1}\cdots F_{i_n,n}X\in\C_1,
$$
for $1\le p\le n$ set 
$$
...
$$
f_{i,p}:X_{i+e_p}\to X_i
$$
in $\C_1$ by 
$$
f_{i,p}:=(F_{i_1,1}\cdots F_{i_{p-1},p-1})(\theta_{i_p,p}(F_{i_{p+1},p+1}\cdots F_{i_n,n}X)).
$$

This yields a diagram $\Delta$ in $\C_1$ whose vertices are the $X_i$ with $i\in M^n$, and whose arrows are the $f_{i,p}$ with $i,i+e_p\in M^n$. 

For instance, if $m=n=2$ the diagram is
...
Clearly $\Delta$ commutes. In particular, all the paths from $X_{(m+1,\dots,m+1)}$ to $X_{(1,\dots,1)}$ coincide. (Such paths obviously exist.) So, we get a well-defined morphism in $\C_{n+1}$
$$
X_{(m+1,\dots,m+1)}=F_{m+1,1}\cdots F_{m+1,n}X\to F_{1,1}\cdots F_{1,n}X=X_{(1,\dots,1)}.
$$
When $X$ varies in $\C_{n+1}$, this family of morphisms in $\C_1$ defines the sought-for morphism of functors from $F_{m+1,1}\cdots F_{m+1,n}$ to $F_{1,1}\cdots F_{1,n}$. 

\begin{nota}\label{nil2}
We denote this morphism of functors by
$$
\begin{pmatrix}
\theta_{1,1}&\cdots&\theta_{1,n}\\
...
and we put 
$$
\theta_{1,1}*\cdots*\theta_{1,n}:=(\theta_{1,1}\ \cdots\ \theta_{1,n})
$$
for $m=1$, and 
$$
\theta_{1,1}\circ\cdots\circ\theta_{m,1}:=
\begin{pmatrix}
\theta_{1,1}\\
\vdots\\ 
\theta_{m,1}
...
\begin{prop}\label{pil2}
The operations $*$ and $\circ$ are associative, and we have$$
(\theta_{1,1}*\cdots*\theta_{1,n})\circ\cdots\circ(\theta_{m,1}*\cdots*\theta_{m,n})
=\begin{pmatrix}
\theta_{1,1}*\cdots*\theta_{1,n}\\
\vdots\\ 
...
\begin{df}\label{dil2} 
We call $*$ the {\em horizontal composition}. \index{horizontal composition} We call $\circ$ the {\em vertical composition}. \index{vertical composition} We call the equalities in Proposition~\ref{pil2} the {\em Interchange Law}. \index{Interchange Law}
\end{df}
...
Using Notation~\ref{nhove} p.~\pageref{nhove}, %or Notation~\ref{nil2} p.~\pageref{nil2}, 
Equalities (1.5.8) and (1.5.9) become respectively 
...
If $\C$ is a category, $\alpha:\C\to\C$ is the identity functor, and $S$ is an object of $\C$ representing $\coli\alpha$, then $S$ is terminal. Conversely, if $S$ is terminal, then it is an inductive limit of the identity functor.
\end{lem}
%
\begin{proof}
...
The maps $\lambda\mapsto\lambda*\oo j_X$, where $*$ denotes horizontal composition (see Definition~\ref{dil1} p.~\pageref{dil1}, %or Definition~\ref{dil2} p.~\pageref{dil2}), 
induce a map $f:L\to R$. Conversely, to an element  
$$
...
\begin{s} P.~53, Corollary 2.3.4. (Another proof will be given in Section~\ref{2111} p.~\pageref{2111}.) Recall that we have functors $\C\xleftarrow\beta J\xrightarrow\varphi I$, where $I$ and $J$ are small. One can prove $\co\beta\simeq\co\varphi^\dagger\beta$, that is 
%
...
Put 
$$
Q_1:=\co_{(G(X)\to B)\in\C_B}\Hom_{\C'}(A,F(X)),\quad Q_2:=\co_{(A\to F(X))\in(\C^A)^{\op}}\Hom_{\C''}(G(X),B).
$$ 
...
\begin{s} 
P.~61, Proposition 2.6.4 (minor variant).

\begin{prop}\label{264}
Let $\alpha:I\to\C$ be a functor, let $X$ be an object of $\C$, let $\Delta(X)\in\C^I$ be the constant functor attached to $X$, and let $p:\alpha\to\Delta(X)$ be a morphism in $\C^I$. For all universe $\U$ such that $\C$ is a $\U$-category, let $h_\U:\C\to\C^\wedge_\U$ be the Yoneda embedding. Then the following conditions are equivalent:

{\em\nn(a)} for all universe $\U$ such that $\C$ is a $\U$-category and $I$ is $\U$-small we have 
$$
(h_\U(p_i)\to h_\U(X))_{i\in I}\simeq\co h_\U(\alpha),
$$
{\em\nn(b)} for all functor $F:\C\to\C'$ we have 
$$
(F(p_i)\to F(X))_{i\in I}\simeq\co F(\alpha).
$$
\end{prop}

\begin{proof}
It is clear that (b) implies (a). Let us assume (a), and let us prove (b). Let $F:\C\to\C'$ be as in (b), let $\U$ be a universe such that $\C$ and $\C'$ are $\U$-categories and $I$ is $\U$-small, and let $X'$ be in $\C'$. We know 
$$
A(X)\xrightarrow\sim\lim A(\alpha)\quad\forall\ A\in\C^\wedge_\U 
$$ 
and we want to prove 
$$
\Hom_{\C'}(F(X),X')\xrightarrow\sim\lim\Hom_{\C'}(F(\alpha),X'). 
$$ 
It suffices to set $A:=\Hom_{\C'}(F(\ ),X')$.
\end{proof}

\begin{comment}

\begin{rk}\label{rurl}
Let $\alpha:I\to\C$ be a functor, let $X$ be an object of $\C$, let $\Delta(X)\in\C^I$ be the constant functor attached to $X$, let $p:\alpha\to\Delta(X)$ be a morphism in $\C^I$, let $\U$ be a universe such that 
%
\begin{equation}\label{eurl}
\text{$\C$ is a $\U$-category and $I$ is $\U$-small,}
\end{equation}
% 
and let $h_\U:\C\to\C^\wedge_\U$ be the corresponding Yoneda embedding. I do not know the answer to the following question: Does the condition 
$$
(h_\U(p_i)\to h_\U(X))_{i\in I}\simeq\co h_\U(\alpha)
$$ 
depend on $\U$ subject to \eqref{eurl}? By Proposition~\ref{621} p.~\pageref{621}, the answer is ``no'' if $I$ is filtrant.% and $\C$ is a $\U$-category. 
\end{rk}

Note that the question can be stated without the condition that $\C$ is a $\U$-category:

\begin{q}% Jan. 3, 2014
Let $\U\subset\V$ be universes, let $\alpha:I\to\C$ be a functor such that $I$ is $\U$-small, let $X$ be an object of $\C$, and let $p:\alpha\to\Delta(X)$ be a morphism of functors, where $\Delta(X)$ is the constant functor attached to $X$. Assume that, for all contravariant functor $A$ from $\C$ to $\U\text-\Set$, the natural map $A(X)\to\lim A(\alpha)$ is bijective. Does this imply that, for all contravariant functor $B$ from $\C$ to $\V\text-\Set$, the natural map $B(X)\to\lim B(\alpha)$ is bijective.
\end{q}

%By Proposition~\ref{621} p.~\pageref{621}, the answer is ``yes'' if $I$ is filtrant and $\C$ is a $\U$-category. 
...
$$
(See (\ref{convnot}) p.~\pageref{convnot} for an explanation of the notation.) Let us rewrite the proof of the fact that the natural morphism $\coli\widetilde F(\alpha)\to\widetilde F\left(\coli\alpha\right)$ is an isomorphism. 
...
%
\begin{s} 
P.~80, proof of Lemma 3.2.8 (minor variant). As already pointed out, a ``$\displaystyle\colim$'' is missing in the last display. Recall the statement:
\begin{lem}
...
By Proposition 3.3.3 p.~82 of the book, it suffices to show that $A$ commutes with finite projective limits. Let $(X_i)$ be a finite inductive system in $\C$. We must check that the natural map $A(\co_i X_i)\to\lim_iA(X_i)$ is bijective. Consider the diagram below, in which we abbreviate $(Y\to A)\in\C_A$ by $Y$: 
$$
\begin{tikzcd}
\co_Y\Hom_\C(\co_iX_i,Y)\ar{r}{a}\ar{dd}[swap]{d}&\co_Y\lim_i\Hom_\C(X_i,Y)\ar{d}{b}\\ 
{}&\lim_i\co_Y\Hom_\C(X_i,Y)\ar{d}{c}\\ 
A(\co_iX_i)\ar{r}[swap]{e}&\lim_iA(X_i).
\end{tikzcd}
$$ 
...
\begin{s} 
P.~95, proof of Proposition 4.1.4 (i) (additional details): In Notation~\ref{nhove} p.~\pageref{nhove} and Notation~\ref{nmat} p.~\pageref{nmat}, %or Notation~\ref{nil2} p.~\pageref{nil2}, 
the sentence of the book ``The two compositions 
$$
\begin{tikzcd}
...
Let us try to express the condition ``$f$ is a strict epimorphism'' in a more elementary form. We start with the sequence 
$$
\begin{tikzcd}
\ky(X)\ar[two heads]{r}{\lambda}&\Coim f\ar{r}{\mu}&\ky(Y).
\end{tikzcd}
$$ 
Our condition becomes ``$\mu$ is an isomorphism''. Evaluating the above sequence on $Z\in\C$ yields
$$
\begin{tikzcd}
\Hom_\C(X,Z)&(\Coim f)(Z)\ar[tail]{l}[swap]{\lambda_Z}\ar[equal]{d}&\Hom_\C(Y,Z)\ar{l}[swap]{\mu_Z}\\ 
{}&\Hom_{\C^\wedge}(\Ima\hy(f),\hy(Z)),
\end{tikzcd}
$$ 
and the condition becomes ``$\mu_Z$ is an isomorphism for all $Z$''. We need to describe the subset $\Ima\lambda_Z$ of $\Hom_\C(X,Z)$ in elementary terms. Let $x$ be in $\Hom_\C(X,Z)$. Then $x$ is in $\Ima\lambda_Z$ if and only if the diagram of solid arrows 
$$
\begin{tikzcd}
\hy(X)\ar[two heads]{r}\ar{rd}[swap]{\hy(x)}&\Ima\hy(X)\ar[dashed]{d}\\ 
{}&\hy(Z).
\end{tikzcd}
$$ 
can be completed to a commutative diagram as indicated. This is the case if and only if, for all $W$ in $\C$, the diagram of solid arrows 
$$
\begin{tikzcd}
\Hom_\C(W,X)\ar[two heads]{r}\ar{rd}[swap]{x\circ}&f\circ\Hom_\C(W,X)\ar[dashed]{d}\\ 
{}&\Hom_\C(W,Z).
\end{tikzcd}
$$ 
can be completed to a commutative diagram as indicated.

\nn We see that the dashed arrow in the above diagram exists if and only if 
%
\begin{equation}\label{sz}
\left[W\parar X\xr fY\right]\then\left[W\parar X\xr xZ\right].
\end{equation}
%
Let $S_Z$ be set of all those $x$ in $\Hom_\C(X,Z)$ which satisfy \eqref{sz} for all $W$ in $\C$. Then we have $\Ima\lambda_Z=S_Z$ and 

\begin{prop}\label{elem}
The morphism $f$ is a strict epimorphism if and only if the map $\Hom_\C(Y,Z)\to S_Z$ given by $y\mapsto y\circ f$ is bijective for all $Z$ in $\C$.
\end{prop}

In the book, the above condition is taken as the definition of a strict epimorphism.
...
\begin{s}
Proposition 6.2.1 p. 138. Let $\alpha:I\to\C$ be a functor, and let $Z$ be an object of $\C$. Consider the following condition:

There is an $i_0$ in $I$ and a morphism $\tau:Z\to\alpha(i_0)$ in $\C$ such that for all morphism $s_0:i_0\to i$ there is a morphism $g:\alpha(i)\to Z$ and a morphism $t:i\to j$ satisfying 
$$
g\circ\alpha(s)\circ\tau=\id_Z,\quad\alpha(t\circ s)\circ\tau\circ g=\alpha(t).
$$
%
\begin{nota}\label{sic}
Given two categories $I$ and $\C$, we write $S(I,\C)$ for the set of those couples $(\alpha,Z)$ such that $\alpha$ is a functor from $I$ to $\C$, $Z$ is an object of $\C$, and the above condition holds.
\end{nota}

Let $I$ and $\C$ be two categories, let $\U$ be a universe such that 
%
\begin{equation}\label{icz}
\text{$I$ is $\U$-small and $\C$ is a $\U$-category,}
\end{equation}
%
and let $T(I,\C,\U)$ be the set of those couples $(\alpha,Z)$ such that $\alpha$ is a functor from $I$ to $\C$, $Z$ is an object of $\C$, and $``\co"\alpha$ is representable by $Z$.
%
\begin{prop}[Proposition 6.2.1 p. 138]\label{621}
For $I,\C$ and $\U$ as above, we have $S(I,\C)=T(I,\C,\U)$. In particular $T(I,\C,\U)$ does not depend on $\U$ subject to \eqref{icz}.
\end{prop}
\end{s}

See also Proposition~\ref{264} p.~\pageref{264}.
...
%\begin{s}Definition 7.3.1 p.~159. It might be worth displaying the formula $$(R_{\cc S}F)(Q(X))\simeq\co_{(X\to Y)\in\cc S^X}F(Y).$$ Define $\beta^X:\cc S^X\to\A$ by $\beta^X(X\to Y):=F(Y)$. One can say that $F$ is {\em universally right localizable} at $Q(X)$ if we have $$\Big(\beta^X,(R_{\cc S}F)(Q(X))\Big)\in S(\cc S^X,\A)$$ (see Notation~\ref{sic} p.~\pageref{sic}, and see Proposition~\ref{621} p.~\pageref{621} for the signification of this condition), and that $F$ is {\em universally right localizable} if it is universally right localizable at $Q(X)$ for any $X$ in $\C$.\end{s}

\begin{s}\label{durl} 
P.~159, Definition 7.3.1. Recall the definition: 

Let $\C$ be a $\U$-small category (see \S~\ref{loc} p.~\pageref{loc}), let $\cc S$ be a right multiplicative system, and let $Q:\C\to\C_{\cc S}$ be the right the localization of $\C$ by $\cc S$. A functor $F:\C\to\A$ is said to be {\em right localizable} if $Q^\dagger F$ exists, in which case we say that $Q^\dagger F$ a {\em right localization} of $F$, and denote this functor by $R_{\cc S}F$. It might be worth displaying the formula 
$$
(R_{\cc S}F)(Q(X))\simeq\co_{(X\to Y)\in\cc S^X}F(Y).
$$ 

Define $\beta^X:\cc S^X\to\A$ by $\beta^X(X\to Y):=F(Y)$. One can say that $F$ is {\em universally right localizable} \index{universally right localizable} at $Q(X)$ if we have 
$$
\Big(\beta^X,(R_{\cc S}F)(Q(X))\Big)\in S(\cc S^X,\A)
$$ 
(see Notation~\ref{sic} p.~\pageref{sic} for the definition of $S(\cc S^X,\A)$, and see Proposition~\ref{621} p.~\pageref{621} for the signification of this condition), and that $F$ is {\em universally right localizable} if it is universally right localizable at $Q(X)$ for any $X$ in $\C$. By Proposition~\ref{264} p.~\pageref{264} and Proposition~\ref{621} p.~\pageref{621}, $F$ is universally right localizable if and only if for any big category $\A'$ and any functor $K:\A\to\A'$, the functor $K\circ F$ is localizable and $R_{\cc S}(K\circ F)\xr\sim K\circ R_{\cc S}F$.

%Moreover, $F$ is said to be {\em universally right localizable} if for any functor $K:\A\to\A'$, the functor $K\circ F$ is localizable and $R_{\cc S}(K\circ F)\xr\sim K\circ R_{\cc S}F$. 

%It seems more prudent to assume that $\C$ is a $\U$-small category, and that $\A$ and $\A'$ are big categories (see Remark~\ref{rurl} p.~\pageref{rurl}). For the sake of emphasis, let us state formally the definition used here:

%\begin{df}\label{durl}Let $\C$ be a $\U$-category, let $\cc S$ be a right multiplicative system, and let $\A$ be a big category. A functor $F:\C\to\A$ is {\em universally right localizable} \index{universally right localizable} if for any big category $\A'$ and any functor $K:\A\to\A'$, the functor $K\circ F$ is localizable and $R_{\cc S}(K\circ F)\xr\sim K\circ R_{\cc S}F$.\end{df}

%By Lemma 7.1.21 p.~157, Theorem 2.3.3 p.~52, and Proposition 2.6.4 p.~61 (see Proposition~\ref{264} p.~\pageref{264} and Proposition~\ref{264} p.~\pageref{264}), $F$ is universally right localizable if and only if for all $X$ in $\C$ $$\icolim_{(X\to Y)\in\SSS^X}\ F(Y)$$ is represented by some object of $\A$. 
...
$$
\co_{i\in I}\lim_{j\in J}\alpha(i,j)\to\lim_{j\in J}\co_{i\in I}\alpha(i,j)
$$ 
is bijective.
...
{\em(b1)} for any $A\subset\Ob(I)$ such that $\card(A)<\pi$ there is a $j$ in $J$ such that for any $a$ in $A$ there is a morphism $a\to j$ in $I$,

{\em(b2)} for any $i$ and $j$ in $I$ and for any $B\subset\Hom_I(i,j)$ such that $\card(B)<\pi$ there is a morphism $j\to k$ in $I$ such that the composition $i\xr sj\to k$ does not depend on $s\in B$.
...
\begin{s}
Here are more details about Step~(b) of the proof of Lemma 9.2.5 p.~218: Let $i_1,i_2\in I$ and let $B\subset\Hom_I(i_1,i_2)$ with $\card(B)<\pi$. There exist $j_1\in J$ and a morphism $i_1\to\pp(j_1)$. As $I$ is filtrant, there are morphisms $i_2\to i_3\leftarrow\pp(j_1)$. As $\pp$ is cofinal, there is a morphism $i_3\to\pp(j_2)$. As $J$ is filtrant, there are morphisms $j_1\to j_3\leftarrow j_2$. Let $s\in B$. By Proposition 3.2.2, (i) $\then$ (iii) (b), p.~78 of the book, there is a morphism $j_3\to j(s)$ making the diagram 
$$
\begin{tikzcd}
i_1\ar[equal]{d}\ar{r}{s}&i_2\ar{r}&i_3\ar{r}&\pp(j_2)\ar{rr}{\pp(j_2\to j_3)}&&\pp(j_3)\ar[equal]{d}\ar{rr}{\pp(j_3\to j(s))}&&\pp(j(s))\ar[equal]{d}
\\ 
%
i_1\ar{rrr}&&&\pp(j_1)\ar{rr}[swap]{\pp(j_1\to j_3)}&&\pp(j_3)\ar{rr}[swap]{\pp(j_3\to j(s))}&&\pp(j(s))
\end{tikzcd}
$$
commute. Putting $t(s):=(j_1\to j_3\to j(s))$, we get a commutative diagram 
$$
\begin{tikzcd}
i_1\ar{d}\ar{r}{s}&i_2\ar{d}\\ 
\pp(j_1)\ar{r}[swap]{\pp(t(s))}&\pp(j(s)).
\end{tikzcd}
$$ 
\end{s}
...
$$ 
In particular the family $(y_j)$ belongs to $\lim A(\psi)$. Our assumption about $\C$ implies that $\co\psi$ exists in $\C$. Let $p_j:\psi(j)\to\co\psi$ be the coprojection. By surjectivity of (9.2.2) p.~222 of the book, there is an $x:\co\psi\to A$ such that $x\circ p_j=y_j$ for all $j$. Letting 
$$
...
%\begin{thm}[Theorem 9.3.4 p.~227]\label{934a}Assume Conditions {\em(i)--(v)} above. Then there is a regular infinite $\U$-cardinal $\pi$ such that, for any $X$ in $\C$, we have $$X\in\C_\pi\ssi\card(X(G))<\pi.$$ Moreover, $\pi$ can be chosen arbitrarily large (among the regular infinite $\U$-car\-di\-nals).\end{thm}
...
\begin{lem}\label{ppi}
Let $\pi$ be an infinite cardinal, and $\alpha:I\to\C$ a functor admitting an inductive limit $X$ in $\C$. Assume that the coprojections $p_i:\alpha(i)\to X$ are monomorphisms, and consider the conditions below:
...
\nn{\em(d)} $\alpha(s):\alpha(i)\to\alpha(j)$ is an isomorphism for all morphism $s:i\to j$ in $I$.
...
We give a slightly more detailed writing of the second sentence in Step (a) of the proof of Lemma 9.3.1. This second sentence is ``Set $S:=\co_iY_i(G)\subset Y(G)$.'' Here is the rewriting: 
%Claim: the coprojection $p_i:Y_i\to Y$ is a monomorphism.
%
%In the diagram $$\begin{tikzcd}\co_jY_j\ar{rr}{\sim}&&Y\\ {}&Y_i,\ar{ul}{p_i}\ar{ur}[swap]{q_i}\end{tikzcd}$$ 
%let the horizontal arrow be the isomorphism of the first display of the proof of Lemma 9.3.1, and let $q_i$ be the canonical morphism. Then $q_i$ is a monomorphism by the observation made on p.~114 of the book, right after Definition 5.1.1. As the diagram commutes, $p_i$ is also a monomorphism, and the claimed is proved.% Proposition 5.2.3 (ii) p. 118 of the book yields the claim. 
The coprojection $Y_i\to Y$ being a monomorphism, so is $Y_i(G)\to Y(G)$. As small filtrant inductive limits are exact in $\Set$ (Proposition 3.3.7 (iv) p. 83 of the book), $S:=\co_iY_i(G)\to Y(G)$ is also a monomorphism.

\subsubsection{Proposition 9.3.2 (p. 224)}

%We state Proposition 9.3.2 p.~224 of the book, and make a few comments about its proof.

\begin{prop}[Proposition 9.3.2 p.~224]\label{932}
If $\pi$ is an infinite regular cardinal, if $X$ is in $\C$, and if 
$$
\card(X(G))<\pi,\quad\card(G^{\coprod X(G)}(G))<\pi,
$$ 
then $X$ is in $\C_\pi$.
\end{prop}
...
$$
\lambda:\co_{i\in I}\Hom_\C(A,X_i)\to\Hom_\C(A,X).
$$

Let us show that $\lambda$ is injective. (Here we shall only use $\card(A(G))<\pi$.)

Let $f,g:A\parar X_{i_0}$ be such that the two composition $A\parar X_{i_0}\to X$ coincide. For each $s:i_0\to i$, set $N_s:=\Ker(A\parar X_i)$. As $\co_{s\in I^{i_0}}$ is exact in $\C$ by assumption, we get, in view of Corollary 3.2.3 (i) p.~79 of the book, 
$$
\co_{s\in I^{i_0}}N_s\simeq\Ker\left(A\parar\co_{s\in I^{i_0}}X_i\right)\simeq\Ker(A\parar X)\simeq A.
$$ 
As $\card(A(G))<\pi$, we may apply Lemma 9.3.1 p.~224 and conclude that there is an $s:i_0\to i$ such that $N_s\to A$ is an epimorphism. Hence, the two compositions $A\parar X_{i_0}\to X_{i_1}$ coincide. This proves that $\lambda$ is injective. 
...
\nn Step 4: (a) holds. For each $i$ in $I$ define $Y_i:=A\times_XX_i$. As $\co_i$ is exact in $\C$, we have $\co_iY_i\simeq A$. As $\card(A(G))<\pi$, Lemma 9.3.1 p.~224 implies that there is an $i_0$ in $I$ such that $B:=Y_{i_0}\to A$ is an epimorphism.
...
$$
\mu:\co_{i\in I}\Hom_\C(P,X_i)\to\Hom_\C(P,X)
$$ 
...
\end{tikzcd}
$$ 
%As the category $I^{i_0}$ is filtrant and the natural functor $I^{i_0}\to I$ is cofinal by Corollary 3.2.3 (i) p.~79 of the book, a
As $\mu$ is injective, and as the compositions $P\parar C\xr xX_{i_0}\xr{p_{i_0}}X$ are equal, there is, by Step~2, a morphism $s:i_0\to i$ such that the compositions $P\parar C\xr xX_{i_0}\xr{X_s} X_i$ are equal. The exactness of $P\parar C\xr aA$ implies the existence of a morphism $g:A\to X_i$ such that $X_s\circ x=g\circ a$. As indicated at the end of Step~2, we only need to verify the equality $p_i\circ g=f$.
...
We have 
$$
f\circ a=p_{i_0}\circ x=p_i\circ X_s\circ x=p_i\circ g\circ a,
$$ 
the first equality following from (b), the second equality being obvious, and the third equality following from the definition of $a$. As $a$ is an epimorphism, this forces $f=p_i\circ g$, and the proof is complete.
\end{proof}
...
for $A$ in $I$, and 
$$
\co_{A\in I}G^{\coprod A}\xr\sim G^{\coprod X(G)}\to X.
$$ 
Then we see that $G^{\coprod X(G)}\to X$ is an epimorphism by Proposition 5.2.3 (iv) p.~118, that $G^{\coprod A}\to X$ is an epimorphism for some $A$ in $I$ by Lemma 9.3.1 p.~224, and that $\card(X(G))<\pi$ by Lemma 9.3.3 p.~226 (which is Lemma~\ref{933} above).
...
To verify this inequality, we argue as in the proof of Lemma 9.3.3 p. 226 of the book (which is Lemma~\ref{933} above). (Conditions (b), (c), and (e) referred to below are stated in Section~\ref{tirg} p.~\pageref{tirg}.)
...
$$
G^{\coprod X(G)}\simeq\co_{B\in I}G^{\coprod B}.
$$ 
...
\begin{df}
Let $I$ be a well-ordered nonempty small set and $\alpha:I\to\C$ a functor. An {\em upper bound} for $\alpha$ is a family of morphisms $(a_i:\alpha(i)\to X)_{i\in I}$. If $\C$ has the property that any such functor admits some upper bound, we say that $\C$ {\em admits small well-ordered upper bounds}.
\end{df}
...
Theorem~\ref{qtot} clearly implies Theorem~\ref{zorn}. Lemma~\ref{945} below will show that Theorem~\ref{942} follows also from Theorem~\ref{qtot}. Theorem~\ref{qtot} will be used in the book to prove Theorem 9.5.5 p.~133.
...
Let $\C$ be as in the statement. We assume (as we may) that $\C$ is small. Let us choose an $\C$-special well-ordered set $I$, and let us define an inductive system $(X_i)_{i\in I}$ by transfinite induction as follows: For the least element $0$ of $I$ we choose an arbitrary object $X_0$ of $\C$. Let $i>0$ and assume that $X_j$ and $u_{jk}:X_k\to X_j$ have been constructed for $k\le j<i$. 
...
Then $(X_i)_{i\in I}$ is indeed an inductive system in $\C$. Let $(b_i:X_i\to X)_{i\in I}$ be some upper bound, and let $i_0$ in $I$ be such that $b_{i_0}:X_{i_0}\to X$ is an epimorphism. Let $u:X\to Y$ be a morphism. By Property $P(u_{i_0+1,i_0})$ applied to $u\circ b_{i_0+1}:X_{i_0+1}\to Y$, we find a morphism $w:Y\to X_{i_0+1}$ such that $w\circ u\circ b_{i_0+1}\circ u_{i_0+1,i_0}=u_{i_0+1,i_0}$. Putting $v:=b_{i_0+1}\circ w:Y\to X$, we get 
$$
...
If $\C$ is a small nonempty category admitting small filtrant inductive limits, if $\pi$ is an infinite regular cardinal such that $\card(\Mor(\C))<\pi$, if $I$ is a $\pi$-filtrant small category, and if $(X_i)_{i\in I}$ is an inductive system in $\C$ indexed by $I$, then there is an $i_0$ in $I$ such that the coprojection $X_{i_0}\to\co_iX_i$ is an epimorphism. 
\end{lem}

\begin{proof}
Set $X:=\co_iX_i$ and let $a_i:X_i\to X$ be the coprojection. For any $Y$ in $\C$ let 
$$
b^Y_i:\Hom_\C(Y,X_i)\to\co_j\Hom_\C(Y,X_j)
$$ 
be the coprojection, let $F(Y)$ be the image of the natural map 
$$
\co_j\Hom_\C(Y,X_j)\to\Hom_\C(Y,X),
$$ 
and define $\pp^Y$ by the commutative diagram 
$$
\begin{tikzcd}
\co_j\Hom_\C(Y,X_j)\ar[two heads]{r}{\pp^Y}&F(Y)\ar[hook]{r}&\Hom_\C(Y,X)\\ 
\Hom_\C(Y,X_i).\ar{u}{b^Y_i}\ar{ur}[swap]{\pp^Y_i:=a_i\circ}
\end{tikzcd}
$$ 

Claim: There is an $i_0$ in $I$ such that $a_{i_0}\circ:\Hom_\C(Y,X_{i_0})\to F(Y)$ is surjective for all $Y$ in $\C$.
...
For any $i$ in $I$ there is a morphism $h_i:X_i\to X_{i_0}$ such that $a_{i_0}\circ h_i=a_i$. Indeed, this follows from the commutativity of 
$$
\begin{tikzcd}
\Hom_\C(X_i,X_i)\ar{d}[swap]{b_i^{X_i}}\ar{rr}{\pp_i^{X_i}:=a_i\circ}&&F(X_i)\ar[equal]{d}\\ 
\co_j\Hom_\C(X_i,X_j)\ar[two heads]{rr}{\pp^{X_i}}&&F(X_i)\ar[equal]{d}\\ 
\Hom_\C(X_i,X_{i_0})\ar{u}{b^{X_i}_{i_0}}\ar[two heads]{rr}[swap]{\pp^{X_i}_{i_0}:=a_{i_0}\circ}&&F(X_i)
\end{tikzcd}
$$
and the equality 
$$
a_i=\pp^{X_i}_i(\id_{X_i}).
$$
...
%
\begin{proof}
We assume, as we may, that $I$ is nonempty, well-ordered, and admits a maximum $m$. We shall complete the following Task $(T_i)$ by transfinite induction on $i\in I$ (see Theorem~\ref{meta} p.~\pageref{meta}): 
...
\end{tikzcd}
$$
with $v_j,w_j\in\Mor(\C_0)$, %for each $(i,j)$ in $I^2$ with $i>j$, construct a commutative diagram $$\begin{tikzcd}X'\ar{r}{v_i}\ar[equal]{d}&Y'_{<i}\ar{r}{w_i}&Y'_i\\ X'\ar{r}[swap]{v_j}&Y'_{<j}\ar{r}[swap]{w_j}&Y'_j,\ar{ul}[swap]{p_{ij}}\end{tikzcd}$$with $p_{ij}\in\Mor(\C_0)$, 
and for each $(i,j,k)$ in $I^3$ with $i\ge j>k$, construct a commutative diagram 
$$
\begin{tikzcd}
...
Let $0$ be the least element of $I$. To handle Task $(T_0)$, let us define $Y'_0,v_0$, and $h_0$ by (9.5.2) (ii):
$$
\begin{tikzcd}
...
Let $i$ in $I$ satisfy $i>0$, and let us tackle Task $(T_i)$. We assume (as we may) that Task $(T_j)$ has already been completed for $j<i$, {\em i.e.} that the $Y'_{<j},Y'_j,h_j,v_j,w_j$ have already been constructed for $j<i$, that the $p_{jk}$ have already been constructed for $k<j<i$, and that all these morphisms satisfy the required conditions. It suffices to define $Y'_{<i},Y'_i,v_i,w_i,$ and $p_{ij}$ for $j<i$, in such a way that the required conditions are still satisfied. For $k<j<i$ we define $u_{jk}:Y'_k\to Y'_j$ by
$$
u_{jk}:=w_j\circ p_{jk}.
$$
By \eqref{pijk} we have $u_{jk}\circ u_{k\ell}=u_{j\ell}$ for all $\ell<k<j<i$. In particular, $(Y'_j)_{j<i}$ is an inductive system in $\C_0$. We denote its limit (which exists in $\C_0$ thanks to (9.5.2) (i)) by $Y'_{<i}$, and we write $p_{ij}$ for the coprojection $Y'_j\to Y'_{<i}$. We also put
$$
v_i:=p_{i0}\circ v_0,
$$
and we define
...
\end{tikzcd}
$$
From this point, it is straightforward to verify that the required conditions are still satisfied. 
...
$$
g_i:=u_{mi}\circ h_i,\quad u':=w_m\circ v_m,
$$
we get
$$
g_i\circ u_i=u_{mi}\circ h_i\circ u_i=u_{mi}\circ w_i\circ v_i\circ f_i
$$
$$
=w_m\circ p_{mi}\circ w_i\circ v_i\circ f_i=w_m\circ v_m\circ f_i=u'\circ f_i.
$$
\end{proof}
%
%%%
...
Let $\C$ be a category. Assume that small filtrant inductive limits exist in $\C$ and are exact. Let $\alpha:I\to\C$ be a functor such that $I$ is small and filtrant, and $\alpha(s):\alpha(i)\to\alpha(j)$ is a monomorphism for all morphism $s:i\to j$ in $I$. Then the coprojection $p_i:\alpha(i)\to\co\alpha$ is a monomorphism.
\end{lem}
%
\begin{proof}
...
$$
\begin{tikzcd}
\co\Delta(\alpha(i))\ar{d}[swap]{\sim}\ar[tail]{rr}{\co\theta}&&\co(\alpha\circ\oo j)\ar{d}{\sim}\\ 
\alpha(i)\ar{rr}[swap]{p_i}&&\co\alpha.
\end{tikzcd}
$$
\end{proof}
\end{s}
...
\begin{lem}[Theorem 12.2.4 p.~301]\label{sl2}
...
\nn[\S\ref{399} p.~\pageref{399} about the beginning of Section 16.3 on p~399 will be inserted here.]
...
[Section~\ref{s1853} p.~\pageref{s1853} and \S\ref{1867} p.~\pageref{1867} will be inserted here.]
...
\begin{s}\label{loc}
P. 153, Definition 7.1.11. The set $\Hom_{\C^r_{\cc S}}(X,Y)$ is defined by 
$$
\Hom_{\C^r_{\cc S}}(X,Y):=\co_{(Y\to Y')\in\cc S^Y}\Hom_\C(X,Y').%=\co\Hom_\C(X,\alpha^Y).
$$ 
I find this formulation puzzling because we are taking the inductive limit of a functor form the category $\cc S^Y$, which is not $\U$-small in general, to the category of $\U$-sets. Moreover, it is indicated in Remark 7.1.14 p.~154 of the book, that this inductive limit in not a $\U$-set in general. My understanding is that we assume implicitly that the inductive limit is taken in the category of $\V$-sets for a universe $\V$ containing $\U$ and making $\C$ a $\V$-small category. But in this case would it not be simpler to assume from the beginning that $\C$ is $\U$-small? 

For the sake of emphasis, let us state formally the definition used here:

\begin{df}[localization of a category]\label{dloc}
Let $\C$ be a $\U$-small category, and let $\cc S$ be a right multiplicative system. The {\em right localization} \index{localization of a category} of $\C$ by $\cc S$ is the $\U$-small category $\C^r_{\cc S}$ mentioned after Lemma 7.1.13 on p.~154 of the book.
\end{df}
\end{s}
...
\begin{s}\label{durl2}
P. 159, Definition of universally right localizable functor (Definition 7.3.1 (ii)): see \S~\ref{durl} p.~\pageref{durl}. %Here is how I understand the definition (see Remark~\ref{rurl} p.~\pageref{rurl}): $\C$ is a $\U$-small category, $\A$ and $\A'$ are big categories.
\end{s}
...
%\begin{s}\label{ltf}P. 253, Definition of right localizable functor with respect to $(\cc N,\cc N')$ (Definition 10.3.1). I think it is simpler to define the notion of universally right localizable functor as in \S\ref{durl2} p.~\pageref{durl2} (see Remark~\ref{rurl} p.~\pageref{rurl}).\end{s}
