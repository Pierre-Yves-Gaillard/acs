...
\end{prop}
\begin{proof}
For the duration of the proof, $\Sh(X)$ will denote the category of ``\emph{Verdier}-sheaves''. By Theorem 3.4 in SGA4.II, the inclusion $\Sh(X)\subset\PSh(X)$ admits an exact left adjoint $L$. We will also use tacitly the fact that $\Sh(X)$ admits finite limits and colimits (Theorem 4.1 in SGA4.II).

\nn Claim~1: If $A\to U$, with $U$ in $\C_X$, is a monomorphism and a local isomorphism in $\PSh(X)$, then $LA\to LU$ is an isomorphism in $\Sh(X)$.
...
\nn Claim~2: If $A\to B$ is a monomorphism and a local isomorphism in $\PSh(X)$, then $LA\to LB$ is an isomorphism in $\Sh(X)$.

\nn Proof of Claim~2: Let $f:A\to B$ be a monomorphism and a local isomorphism in $\PSh(X)$. By Proposition~4.2 in SGA4.II and Proposition~\ref{34i} p.~\pr{34i} above, it suffices to show that $Lf$ is an epimorphism in $\Sh(X)$. Let $x,y:B\rightrightarrows F$ be two morphisms in $\PSh(X)$ with $F$ in $\Sh(X)$ and $x\ne y$. It suffices to prove $x\ci f\ne y\ci f$. %[In this proof we write $\alpha\beta$ for $\alpha\circ\beta$.]
As we have 
$$
...
\end{tikzcd}
$$ 
in $\PSh(X)$, where the square is cartesian. As $f_U$ is a monomorphism by Lemma~\ref{8311} (a) (i) p.~\pageref{8311} and a local isomorphism by Lemma 16.2.4 p.~395 of the book, we have $x\ci u\ci f_U\ne y\ci u\ci f_U$ by Claim~1. This implies $x\ci f\ne y\ci f$, as desired.
...
\Ima(LA\to LB)\to LB
$$ 
is an isomorphism by exactness of $L$, and that $LA\to LB$ is an epimorphism by Proposition 5.1.2 (iv) p.~114 of the book. This proves Claim~3. 
