...
\begin{s}\lb{abar}
P. 160, Proposition 7.3.2. Let $A:\C_\SSS\to\I_\T$ be a quasi-inverse to $\iota_Q$, and let $A_o:\Ob(\C)\to\Ob(\I)$ be the map induced by $A$. Note that the object $A_o(X)\in\I$ does \emph{not} depend functorially on $X\in\C$, but $F(A_o(X))\in\A$ does, and is equal to $RF(X)$.
\end{s}
...
Let $\T$ be the set of qis in $\widetilde\PP$, let $A:\oo D(\C)\to\widetilde\PP_\T$ be a quasi-inverse to the obvious equivalence $\widetilde\PP_\T\to\oo D(\C)$, and let $A_o:\Ob(\oo K(\C))\to\Ob(\widetilde\PP)$ be the map induced by $A$. Similarly, let $B:\oo D(\C')\to\oo K_{\text{hi}}(\C')$ be a quasi-inverse to the obvious equivalence $\oo K_{\text{hi}}(\C')\to\oo D(\C')$, and let $B_o:\Ob(\oo K(\C'))\to\Ob(\oo K_{\text{hi}}(\C'))$ be the map induced by $B$. Let $X$ be in $\oo K(\C)$ and $Y$ be in $\oo K(\C')$. We have 
$$
\RHom_{\C'}(LG(X),Y)\iso\Hom_{\C'}^\bu(G(A_o(X)),B_o(Y))
$$ 
because $\widetilde\PP$ is $\oo K(G)$-projective and $(\oo K_{\text{hi}}(\C'),\oo K(\C')^{\op})$ is $\Hom_{\C'}$-injective by Corollary 14.3.2 p.~356 of the book. Similarly we have 
$$
\Hom_\C^\bu(A_o(X),F(B_o(Y)))\iso\RHom_\C(X,RF(Y))
$$ 
because $\oo K_{\text{hi}}(\C')$ is $\oo K(F)$-injective and $(\oo K(\C),\widetilde\PP^{\op})$ is $\Hom_\C$-injective by Assumption~\qr{upb}. Finally we have the adjunction 
$$
