...
\begin{s}\lb{fuqu1}
P. 319. Sentence ``One shall be aware that the category $\oo D_c(\A)$ may be a big category'' after Definition 13.1.1. Using Tag~07JS of the Stacks Project it is easy to get an example of an abelian category $\C$ such that $\oo D(\C)$ is a big category. (The example given in the Stacks Project is due to Peter Freyd.) Here is the link:
...
\begin{s}\lb{fuqu2}
P. 337, Theorem 13.4.1. Here is a corollary: In the setting of the theorem, $\Hom_{\oo D(\C)}(X,Y)$ is equipotent to a $\U$-set. We conclude firstly that, if $\RHom_\C$ exists, then $\oo D(\C)$ is equivalent to a $\U$-category, and, secondly, that $\RHom_\C$ does \emph{not} exist when $\C$ is the abelian category $\C$ mentioned in \S\ref{fuqu1} p.~\pr{fuqu1}. %(see \S\ref{fuqu1} p.~\pr{fuqu1}). Using Tag~07JS of the Stacks Project it is easy to get an example of an abelian $\U$-category
\end{s}
