...
\sbs{Definition of a category (p.~11)}
...
\end{df}
Let $\U$ be a universe such that $\Hom_\C(X,Y)\in\U$ for all $X,Y$; and let $F$ be a finite subset of $\Ob(\C)$. Then we can assume (in a sense that we leave it to the reader to make precise) that the union 
$$
\bigcup_{X,Y\in F}\Hom_\C(X,Y)
$$ 
is disjoint.

This enable us (I think) to abbreviate $\ci_{ZYX}$ by $\ci$.

If we abbreviated $\ci_{ZYX}$ by $\ci$ without the above precaution, we may end up with an inequality of the form $g\ci f\ne g\ci f$, as suggested by the diagram 
$$
\begin{tikzcd}
&Y\ar[dr,"g"]\\
Z\ar[ur,"f"]\ar[dr,"f"']&&W\\ 
...
Another option (which would be simpler in my humble opinion) would be to impose, in the definition of a category, the condition that the Hom-sets are disjoint, and, for each category $\C$, to choose a universe $\U$ such that the (automatically disjoint) union of the Hom-sets is an element of $\U$.
...
\begin{comment}
%\sbs{Definition of a sheaf (page 414)}%\sbs{Brief comments}
\begin{s}
Definition of a sheaf (page 414). 
...
\sbs{Definition of a sheaf (page 414)}
...
the beginning of Verdier's Exposé, up to Proposition~4.2. Here is a rewriting of Dylan Wilson's argument:
\begin{prop}
A ``Verdier sheaf'' is a ``KS-sheaf''.
\end{prop}
\begin{proof}
For the duration of the proof, $\Sh(X)$ will denote the category of ``\emph{Verdier}-sheaves''. By Theorem 3.4 in SGA4.II, the inclusion $\Sh(X)\subset\PSh(X)$ admits an exact left adjoint $\mathbf a$. We will also use tacitly the fact that $\Sh(X)$ admits finite limits and colimits (Theorem 4.1 in SGA4.II).
...
\nn Proof of Claim~2: Let $f:A\to B$ be a monomorphism and a local isomorphism in $\PSh(X)$. By Proposition~4.2 in SGA4.II and (the dual of) Proposition~\ref{34i} p.~\pr{34i} above, it suffices to show that $\mathbf af$ is an epimorphism in $\Sh(X)$. Let $x,y:B\rightrightarrows F$ be two morphisms in $\PSh(X)$ with $F$ in $\Sh(X)$ and $x\ne y$. It suffices to prove $x\ci f\ne y\ci f$. 
As we have 
$$
