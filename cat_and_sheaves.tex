% about "categories and sheaves" (version_0069)
...
It seems clear to me that GT1, GT2, GT3 and GT4' are equivalent to LE1-LE4, that is, the datum of a Grothendieck topology in this nonstandard sense is equivalent to the datum of a set of morphisms in $\C^\wedge$ satisfying LE1-LE4.
...
Let $\C$ be a category with a single object, and let $M$ be the corresponding monoid of endomorphisms. For each $f$ in $M$ and each sieve $S$, write $f^*(S)$ for the fibered product of $S$ with the unique object of $\C$ along $f$. Assume that $M$ is freely generated by a single endomorphism $a$. For $i\in\bb N$ put 
$$
S_i:=\{a^i,a^{i+1},\dots\}.
$$ 
Then $S_0=M,S_1,S_2,\dots$ is a strictly decreasing sequence of sieves, and any nonempty sieve occurs in this sequence. Putting $S_i:=S_0$ for $i\in\bb Z,i<0$, we get 
$$
a^{i*}(S_j)=S_{j-i}
$$ 
for all $i,j\in\bb N$. In particular $s'^*(S_2)\in\{S_0,S_1\}$ for all $s'\in S_1$, and it is easy to see that $\{S_0,S_1\}$ satisfies GT1, GT2, GT3 and GT4', but not GT4.
\end{s}
