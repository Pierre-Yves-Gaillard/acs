...
\end{tikzcd}
$$ 
\begin{rk}\lb{ufreerepr}
In view of Definition~\ref{myucat} p.~\pr{myucat}, the Yoneda Lemma (Theorem~\ref{yol} p.~\pr{yol}) and Convention~\ref{term} p.~\pr{term} we have:

Let $\U$ and $\V$ be universes, let $\C$ be a $\U$-category and a $\V$-category, let $A$ be in $\C^\wg_\U$ and $B$ in $\C^\wg_\V$ and assume $A(X)=B(X)$ for all object $X$ in $\C$, and $A(f)=B(f)$ for all $f\in\Hom_\C(X,Y)$ for all $X,Y\in\C$. Then $A$ is representable if and only if $B$ is. Let $X$ be in $\C$. Then $X$ is a representative of $A$ if and only if $X$ is a representative of $B$. Let $x$ be in $A(X)=B(X)$. Then $x$ is a representation of $A$ if and only if $x$ is a representation of $B$. %Let $\U$ and $\V$ be universes, let $\C$ be a $\U$-category and a $\V$-category (Definition~\ref{myucat} p.~\pr{myucat}), let $A$ be in $\C^\wg_\U$, assume $A(X)\in\V$ for all $X$ in $\C$, define $A_\V\in\C^\wg_\V$ by $A_\V(X):=A(X)$ for all $X$ and $A_\V(f):=A(f)$ for all morphism $f$ in $\C$, let $X$ be in $\C$, let $x$ be in $A(X)$, let $x_\U:X\to A$ be the morphism in $\C^\wg_\U$ attached to $x$ and $x_\V:X\to A_\V$ the morphism in $\C^\wg_\V$ attached to $x$. Then $x_\V$ is an isomorphism if and only if $x_\U$ is.
...
In Proposition~\ref{patho} p.~\ref{patho} we shall see a way to prevent the kind of pathology displayed by the above example.
\end{s}
...
\begin{prop}%\lb{basic}
If $\al:I^{\op}\to\C$ is a functor defined on a small category (Definition~\ref{myuscat} p.~\pr{myuscat}), if $X$ is in $\C$, if $p:\DT X\to\al$ is a morphism in $\Fct(I^{\op},\C)$, and if $h:\C\to\C^\wg$ is the Yoneda embedding, then the following conditions are equivalent:
...
\nn\emph{(d)} if \emph{(b)} holds for some universe $\U$ such that $I$ is $\U$-small and $\C$ is a $\U$-category (Definitions \ref{myucat} p.~\pr{myucat} and \ref{myuscat} p.~\pr{myuscat}), then it holds for any such universe; the same applies to \emph{(c)}.
\end{prop}
...
Statement (d) is obvious (see Remark~\ref{ufreepdra2} p.~\pr{ufreepdra2}).
\end{proof}

The proof of the following proposition is similar to the previous one and is left to the reader as an easy exercise.

\begin{prop}\lb{basic*}
If $\al:I\to\C$ is a functor defined on a small category (Definition~\ref{myuscat} p.~\pr{myuscat}), if $X$ is in $\C$, if $p:\al\to\DT X$ is a morphism in $\C^I$, and if $k:\C\to\C^\vee$ is the Yoneda embedding, then the following conditions are equivalent:
...
\nn\emph{(e)} if \emph{(b)} holds for some universe $\U$ such that $I$ is $\U$-small and $\C$ is a $\U$-category (Definitions \ref{myucat} p.~\pr{myucat} and \ref{myuscat} p.~\pr{myuscat}), then it holds for any such universe; the same applies to \emph{(c)} and \emph{(d)}.
\end{prop}
...
\begin{prop}\lb{copr}
Let $\al:I\to\C$ be an arbitrary functor. For any universe $\U$ such that $\C$ is a $\U$-category (Definition~\ref{myucat} p.~\pr{myucat}), let $h_\U:\C\to\C^\wg_\U$ be the Yoneda embedding. Let $\U$ be a universe such that $I$ is $\U$-small (Definition~\ref{myuscat} p.~\pr{myuscat}) and $\C$ is a $\U$-category and define $A_\U\in\C^\wg_\U$ by $A_\U(X):=\col\Hom_\C(X,\al)$, where $\col\Hom_\C(X,\al)$ is defined as in Convention~\ref{thei} p.~\pr{thei}, so that we have $\col h_\U\ci\al\iso X$ in $\C^\wg_\U$. Let $\V$ be a universe satisfying the same conditions as $\U$. Then $A_\U$ is representable if and only if $A_\V$ is. Let $X$ be in $\C$. Then $X$ represents $A_\U$ if and only if $X$ represents $A_\V$. Assume that such is the case. Then $\col\al$ exists in $\C$ and is isomorphic to $X$, and, for any functor $F:\C\to\C'$ the natural morphism $\col F\ci\al\to F(X)$ is an isomorphism.
\end{prop}
\begin{proof}
Let us show that the natural morphism $\col F\ci\al\to F(X)$ is an isomorphism, the other claims following from Remark~\ref{ufreerepr} p.~\pr{ufreerepr}. Let $\U$ be a universe such that $I$ is $\U$-small and $\C$ and $\C'$ are $\U$-categories, let $X'$ be in $\C'$ and define $A\in\C^\wg_\U$ by $A:=\Hom_{\C'}(F(\ ),X')$. By Proposition~\ref{basic*} p.~\pr{basic*}, it suffices to show that the morphism 
$$
...
\begin{comment}
% Version before 161218

\begin{prop}\lb{copr}
Let $I$ and $\C$ be arbitrary categories, let $\al:I\to\C$ be a functor, let $X$ be an object of $\C$ and let $p:\al\to\DT X$ be a morphism in $\C^I$. For any universe $\U$ such that $\C$ is a $\U$-category (Definition~\ref{myucat} p.~\pr{myucat}), let $h_\U:\C\to\C^\wg_\U$ be the Yoneda embedding, and, for any category $\C'$ and any functor $F:\C\to\C'$, define the morphism 
$$
F(p):F\ci\al\to\DT F(X)
$$
in $\C'^I$ by $F(p)_i:=F(p_i)$ for all $i$ in $I$. Write $(1F)$ for the condition ``$F(p)$ is a coprojection'' (see Definition~\ref{ilim} p.~\pr{ilim}), and write $(2F)$ for the condition ``the morphism 
$$
\Hom_{\C'}(F(p),X'):\DT \Hom_{\C'}(F(X),X')\to\Hom_{\C'}(F\ci\al,X')
$$
in $\Fct(I^{\op},\Set_\U)$ is a projection (see Definition~\ref{plim} p.~\pr{plim}) for any $X'$ in $\C'$ and for some (and thus, by Proposition~\ref{basic*} p.~\pr{basic*}, for any) %\footnote{See Lemma~\ref{u2}.} 
universe $\U$ such that $I$ is $\U$-small and $\C'$ is a $\U$-category (Definitions \ref{myucat} p.~\pr{myucat} and \ref{myuscat} p.~\pr{myuscat})''. Note that $(1F)$ and $(2F)$ are equivalent by Proposition~\ref{basic*} p.~\pr{basic*}. Then we have

\nn\emph{(a)} If the equivalent conditions $(1h_\U)$ and $(2h_\U)$ hold for some universe $\U$ such that $I$ is $\U$-small and $\C$ is a $\U$-category, they hold for any such universe.

\nn\emph{(b)} The conditions \emph{(b1)} and \emph{(b2)} below are equivalent:

\nn\emph{(b1)} The equivalent conditions $(1h_\U)$ and $(2h_\U)$ hold for some (and thus for any) universe $\U$ such that $I$ is $\U$-small and $\C$ is a $\U$-category. 

\nn\emph{(b2)} The equivalent conditions $(1F)$ and $(2F)$ hold for all for all category $\C'$ and all functor $F:\C\to\C'$.
\end{prop}

\begin{proof}
%(a) Let $\U$ be a universe such that $I$ is $\U$-small and $\C$ is a $\U$-category. For any $Y$ in $\C$ define $\col\Hom_\C(Y,\al)$ as in Convention~\ref{thei} p.~\pr{thei}, and note that the condition that the natural map $\col\Hom_\C(Y,\al)\to\Hom_\C(Y,X)$ is bijective doesn't depend on the choice of $\U$ submitted to the indicated requirements.

%(a) Let $\U$ and $\V$ be universes such that $I$ is $\U$-small and $\V$-small, and $\C$ is a $\U$-category and a $\V$-category. For any $Y$ in $\C$ define $\col\Hom_\C(Y,\al)$ as in Convention~\ref{thei} p.~\pr{thei}. Then the following conditions are equivalent:

(a) Let $\U$ and $\V$ be universes such that $I$ is $\U$-small and $\V$-small, and $\C$ is a $\U$-category and a $\V$-category. For any $Y$ in $\C$ define $\col\Hom_\C(Y,\al)$ as in Convention~\ref{thei} p.~\pr{thei}. Then $\col\Hom_\C(Y,\al)$ is a colimit of 
$$
\Hom_\C(Y,\al):I\to\Set_\U
$$ 
and of 
$$
\Hom_\C(Y,\al):I\to\Set_\V,
$$ 
and, by Proposition~\ref{evcol=colev} p.~\pr{evcol=colev} and Proposition~\ref{basic*} p.~\pr{basic*} the following conditions are equivalent:

\nn$\bu\ (1h_\U)$,

\nn$\bu$ the natural morphism $\col h_\U\ci\al\to h_\U(X)$ in $\C^\wg_\U$ is an isomorphism,

\nn$\bu$ the natural map $\col\Hom_\C(Y,\al)\to\Hom_\C(Y,X)$ is bijective for any $Y$ in $\C$,

\nn$\bu$ $(1h_\V)$.

The point is that neither the set $\col\Hom_\C(Y,\al)$ not the above map $$\col\Hom_\C(Y,\al)\to\Hom_\C(Y,X)$$ (which can be regarded as a morphism in $\Set_\U$ or in $\Set_\V$) depend on $\U$ or $\V$.

\nn(b) Condition (b2) implies trivially Condition (b1). Let us prove that (b1) implies (b2). Let $F:\C\to\C'$ be as in (b2) and let $\U$ be a universe such that $I$ is $\U$-small and $\C$ and $\C'$ are $\U$-categories. It suffices to prove $(2F)$. Let $X'$ be in $\C'$ and define $A\in\C^\wg_\U$ by $A:=\Hom_{\C'}(F(\ ),X')$. We get the commutative diagram 
$$
\begin{tikzcd}
\DT A(X)\ar[d,"\sim"']\ar[rrr,"A(p)"]&&&A\ci\al\ar[d,"\sim"]\\ 
\DT\Hom_{\C^\wg_\U}(X,A)\ar{rrr}[swap]{\Hom_{\C^\wg_\U}(h_\U(p),A)}&&&\Hom_{\C^\wg_\U}(\al,A)
\end{tikzcd}
$$ 
in $\Fct(I^{\op},\Set_\V)$, where $\V$ is a universe such that $\C^\wg_\U$ is a $\V$-category, the vertical isomorphisms being given by the Yoneda Lemma. The bottom horizontal arrow being a projection by assumption, the top horizontal arrow is also a projection. This proves $(2F)$, and thus the proposition.
\end{proof}
\end{comment}
% previous version:
...
Let $\al:I\to\C$ be a functor defined on a small category (Definition~\ref{myuscat} p.~\pr{myuscat}), let $X$ be an object of $\C$, and let $p:\al\to\DT X$ be a morphism in $\C^I$ (see Notation~\ref{diag} p.~\pr{diag}). If, in the notation of Proposition~\ref{copr}, the functor $A_\U$ is representable for some $\U$ such that $I$ is $\U$-small and $\C$ is a $\U$-category, we say that $(X,p)$ is a {\em universal} inductive limit\index{universal inductive limit} of $\al$, and that $\col\al$ exists {\em universally}\index{universal existence of a limit} in $\C$.
\end{df}
...
If $\al:I\to\C$ is a functor defined on a small category (Definition~\ref{myuscat} p.~\pr{myuscat}), if $h:\C\to\C^\wg$ is the Yoneda embedding and if $X$ is in $\C$, we denote by $$H[I,\al,\C,X]$$ the condition 

\centerline{$X$ is an inductive limit of $h\ci\al$,}

\nn and by $$C[I,\al,\C,X]$$ the condition 

\centerline{$X$ is an inductive limit of $\al$.}%$C[I,\al,\C,X]$ the condition ``$X$ is an inductive limit of $\al$''. 

\nn By Proposition~\ref{copr} p.~\pr{copr} the conjunction of $H[I,\al,\C,X]$ and $C[I,\al,\C,X]$ implies that $X$ is a \emph{universal} inductive limit of $\al$. The proof of the following proposition is obvious.

\begin{prop}\lb{stupid}
We have $H[I,\al,\C,X]\then C[I,\al,\C,X]$.
%If $\al:I\to\C$ is a functor, if $h:\C\to\C^\wg$ is the Yoneda embedding and if $X\in\C$ is an inductive limit of $h\ci\al$, then $X$ is an inductive limit of $\al$, and thus a universal inductive limit of $\al$.
\end{prop}

(The letters $H$ and $C$ stand for \emph{hypothesis} and \emph{conclusion}.)
%\begin{prop}If $\al:I\to\C$ is a functor, if $h:\C\to\C^\wg$ is the Yoneda embedding and if $X\in\C$ is an inductive limit of $h\ci\al$, then $X$ is an inductive limit of $\al$, and thus a universal inductive limit of $\al$.\end{prop}
% previous version:\begin{prop}If $\al:I\to\C$ is a functor, if $h:\C\to\C^\wg$ is the Yoneda embedding, if $\col h\ci\al$ exists in $\C^\wg$, if $X$ is in $\C$, if $f:X\to\col h\ci\al$ is an isomorphism in $\C^\wg$, if $q_i:\al(i)\to\col h\ci\al$ are the coprojections, then $X$ is a universal limit of $\al$, the coprojections $p_i:\al(i)\to\col\al$ being given by the formula $p_i:=f^{-1}\ci q_i$.\end{prop}\begin{proof}It suffices to prove that $X$ is an inductive limit of $\al$, the coprojections $p_i:\al(i)\to\col\al$ being given by the indicated formula. Let $Y$ be in $\C$ and $u_i:\al(i)\to Y$ a functorial family of morphisms indexed by $i\in I$. It suffices to show that there is a unique morphism $u:X\to Y$ in $\C$ such that $u\ci p_i=u_i$, that is $u\ci f^{-1}\ci q_i=u_i$, for all $i$. But by assumption there is a unique morphism $v:X\to Y$ in $\C^\wg$ such that $v\ci f^{-1}\ci q_i=u_i$ for all $i$, so that it suffices to take for $u$ the morphism $v$ viewed as a morphism in $\C$.\end{proof}

Let us insist on the following point:
\begin{prop}\lb{insist1}
If $\al:I\to\C$ is an arbitrary functor and if there is a universe $\U$ such that $I$ is $\U$-small, $\C$ is a $\U$-category (Definitions \ref{myucat} p.~\pr{myucat} and \ref{myuscat} p.~\pr{myuscat}) and $\col h_\U\ci\al$ exists in $\C^\wg_\U$ and is representable (here $h_\U:\C\to\C^\wg_\U$ is the Yoneda embedding), then $\col\al$ exists universally (Definition~\ref{uil} p.~\pr{uil}).
\end{prop}
\begin{proof}
This follows from Proposition~\ref{copr}.
\end{proof}
...
If the above equivalent conditions hold, we say that $\pp^\dg\bt$ {\em exists} (this is compatible with \S\ref{s232} (a) p.~\pr{s232}). If, in addition, $\pp^\dg(F\ci\bt)$ exists and the natural morphism $\pp^\dg(F\ci\bt)\to F\ci\pp^\dg(\bt)$ is an isomorphism for all functor $F:\C\to\C'$, we say that $\pp^\dg\bt$ exists {\em universally}.
\end{df}  
\end{s}
...
\end{thm}
\begin{proof}
This follows from Theorem~\ref{233i} p.~\pr{233i} and Proposition~\ref{copr} p.~\pr{copr}.
% previous version:
% https://docs.google.com/document/d/1K004uKK3ECssS-Y1etYqzLcEilskE8Hlb0wVrElSqM0/edit
\end{proof}
% previous version:
...
\begin{lem}\lb{533}
Let $\C$ be a category which admits small inductive limits, let $\F$ be a small full subcategory (Definition~\ref{myuscat} p.~\pr{myuscat}) of $\C$, let $F$ be in $\F^\wg$, set 
$$
...
\end{lem}
\begin{proof}
This follows from Proposition~\ref{copr} p.~\pr{copr}.
...
%\begin{s}\lb{744}P. 163. Proposition 7.4.4 holds even without the assumption that $\SSS^X$ is cofinally small for any $X$ in $\C$. This follows from Proposition~\ref{insist2} p.~\pr{insist2}. %Theorem~\ref{stupidkan} p.~\pr{stupidkan} (see also \S\ref{742} p.~\pr{742}). %The fact that (i) implies (ii) follows from Theorem~\ref{233i} p.~\pr{233i}. To show that (ii) implies (i), note that, for all $X$ in $\C$, we have $$\iota_\A(R_\SSS F(Q(X)))\iso R_\SSS(\iota_\A\ci F)(Q(X))\iso\col_{(X\to X')\in\SSS^X}\iota_\A(F(X')),$$ the isomorphisms following respectively from the assumption that $F$ is universally right localizable and from Theorem~\ref{233i} p.~\pr{233i}. (The argument is essentially the same as the one used in the proof of Theorem~\ref{stupidkan} p.~\pr{stupidkan}.)\end{s}

%\begin{s}P. 163, Proposition 7.4.4. In view of \S\ref{742} p.~\pr{742}, the assumption that $\SSS^X$ is cofinally small for any $X$ in $\C$ is not necessary.\end{s}
...
%\begin{s}
%P. 163, proof of Proposition 7.4.4, implication (i)$\then$(ii). Here are some of the tasks implicitly left to the reader: 
...
\nn\emph{Let $\C$ be a small (Definition~\ref{myuscat} p.~\pr{myuscat}) pre-additive category and $\C'$ an abelian category. Then the category $\A$ of additive functors from $\C$ to $\C'$ is a fully abelian subcategory of the category $\F$ of all functors from $\C$ to $\C'$. If $\C'$ admits inductive or projective limits indexed by a small category $I$, then so do $\A$ and $\F$, and the inclusion functor $\A\to\F$ commutes with such limits; if in addition these limits are exact in $\F$, they are exact sn $\A$.}
\end{s}
...
$$ 
holds. This inclusion follows from Propositions 3.3.3 p.~82 (see Proposition~\ref{333} p.~\pr{333}), 6.1.7 p.~132, 8.2.13 p.~173 (see \S\ref{8213} p.~\pr{8213}) 8.2.15 p.~173 in the book.
\end{s}
...
\nn(iii) I haven't been able to solve this part of the exercise.
...
Let $\C$ be a $\U$-category (Definition~\ref{myucat} p.~\pr{myucat}), let $\F\subset\C_0$ be subcategories of $\C$ such that $\F$ is essentially small (see Remark~\ref{954} below), let $\pi$ be an infinite cardinal such that $X$ is in $\C_\pi$ for any $X\to Z$ in $\F$, and assume 
...
\end{proof}
\begin{lem}\lb{idt}
If $(\DD,T)$ $(\DD,T,\Delta)$ and $(\DD',T',\Delta')$ are two triangulated categories, if $F:\DD\to\DD'$ and $G:\DD'\to\DD$ are two quasi-inverse equivalences, and if $F$ is triangulated, then so is $G$.
\end{lem}
\begin{proof}
...
%\begin{s}\lb{1026} P.~251, Proposition 10.2.6. Being unable to prove that the localization of $F$ is triangulated in Proposition 10.3.3 p.~253 of the book, I make the extra assumption that $\I$ is \emph{saturated}. By \S\ref{dtdn}, it is then clear that the obvious functor $\I/(\N\cap\I)\to\DD/\N$ is triangulated, and that any quasi-inverse (if it exists) is also triangulated. \end{s}

%

%\begin{s} P.~253, Definition 10.3.1. One of the key ingredients justifying the second sentence is Display~\qr{(7.3.7)} p.~\pr{(7.3.7)}.\end{s}
...
It is implicitly assumed that the underlying universe $\U$ has been chosen so that $\DD$ is $\U$-small (Definition~\ref{myuscat} p.~\pr{myuscat}). The second sentence in the above quote is justified by Theorem~\ref{stupidkan} p.~\pr{stupidkan}. %Theorem~\ref{stupidkan} p.~\pr{stupidkan}. %\S\ref{744} p.~\ref{744}. %Proposition 7.4.4 p.~163 of the book.

%In fact, the existence of the indicated limit is a sufficient condition for the existence of the corresponding localization, but it is not obvious \emph{a priori} (at least to me) that this condition is necessary. In Definition 7.3.1 p.~159 of the book, the right localization $RF$ of $F$ is said to exist universally if $Q^\dg(Q'\ci F)$ exists universally in the sense of Definition~\ref{232} p.~\ref{232}, in which case we set $RF:=Q^\dg(Q'\ci F)$.
\end{s}

%

%\begin{s} 
%P.~253, Proposition 10.3.3. The fact that the localization of $F$ is triangulated follows from Exercise 8.4 p.~202 (see Section~\ref{84} p.~\pr{84}) and %%\S\ref{1026}, and 
%the diagram on p.~254 of the book). %%(Recall that I have made the extra assumption that $\I$ is saturated.)\end{s}
...
\begin{s} 
Proof of Theorem 10.4.1 p.~257. The fact that ``the canonical functor $\T_{X_1}\tm \T_{X_2}\to\T_{X_i}$ (i=1,2) is cofinal'' follows from Lemma~\ref{proj} p.~\ref{proj}.
...
\begin{s}
Proof of Lemma 10.5.5 p. 261. We adhere to the notation of the lemma. Let $F$ be in $\SSS^{\wg,prod}$. Define $X\in\SSS$ by $X:=\bigoplus_{C\in\SSS_0}\bigoplus_{i\in\ F(C)}C$, and for $C$ in $\SSS_0$ and $i$ in $F(C)$ let $p_{Ci}:C\to X$ be the coprojection. Define $u:X\to F$, that is $u\in F(X)\iso\prod_C\prod_iF(C)$, by setting $u_{Ci}:=i$. If $D$ is in $\SSS_0$ and $j$ in $F(D)$, then the map $u(D):X(D)\to F(D)$ satisfies $u(D)(p_{Dj})=j$. In particular $u(D)$ is surjective, and $u$ is an epimorphism.
\end{s}
...
$$
\begin{tikzcd}
\bigoplus\pp(Z_i)\ar[d,"\sim"]\ar[r]&\bigoplus\pp(Y_i)\ar[d,"\sim"]\ar[r]&\bigoplus \widetilde\pp(X_i)\ar[d]\ar[r]&0\\ 
\pp(\bigoplus Z_i)\ar[r]&\pp(\bigoplus Y_i)\ar[r]&\widetilde\pp(\bigoplus X_i)\ar[r]&0.
\end{tikzcd}
$$ 
\end{s}

%\begin{s}\begin{uspb} P. 263. I don't understand the proof of Lemma 10.5.9\index{unsolved problem}. I don't even understand how the morphism $X_0\to H$ in $\DD^\wg$ is constructed.\end{uspb}\end{s}
...
Vanishing of $Z_n\to X_n\to H$: Let $0\to F\to X_n\to H$ be exact in $\DD^{\wg,add}$, set 
$$
Z_n:=\bigoplus_{C\in\SSS_0}\ \bigoplus_{f\in F(C)}C,
$$ 
and define the epimorphism $g:Z_n\epi F$ in $\DD^{\wg,add}$ as in \S\ref{1056} p.~\pr{1056}. The vanishing of $Z_n\to X_n\to H$ is then clear.
...
\begin{s}\lb{brown}
P.~264, proof of Lemma 10.5.11. It is not hard to prove (using \S\ref{ccaf} p.~\pr{ccaf} and \S\ref{1057ii} p.~\pr{1057ii} in particular) that small filtrant inductive limits exist and are exact in $\SSS^{\wg,prod}$. 
\end{s}
...
\begin{s}
P. 326. The first isomorphism in the third display follows from the fact that $d_Y^p:Y^p\to Y^{p+1}$ factors as $Y^{p-1}\epi\Coker d_Y^{p-1}\to Z^p\mono Y^{p+1}$. (See Proposition 5.1.7 (ii) p.~116 of the book and \S\ref{517ii} p.~\pr{517ii}.)
\end{s}
...
It is implicitly assumed that the underlying universe $\U$ has been chosen so that $\C$ is $\U$-small (Definition~\ref{myuscat} p.~\pr{myuscat}). This is justified by Theorem~\ref{stupidkan} p.~\pr{stupidkan}. %Theorem~\ref{stupidkan} p.~\pr{stupidkan}.
\end{s}
...
We assume that (C) holds, and we must show that $\J$ is $F$-injective. Let $Y'\mono X$ be a monomorphism in $\C$ with $Y'$ in $\J$, let $X\mono Y$ be a monomorphism in $\C$ with $Y$ in $\J$, and let $0\to Y'\to Y\to Y''\to0$ be an exact sequence in $\C$. The (C) implies that $Y''$ is in $\J$ and that $0\to F(X')\to F(X)\to F(X'')\to0$ is exact. Now Theorem 13.8.7 entails that $\J$ is $F$-injective.
\end{s}
