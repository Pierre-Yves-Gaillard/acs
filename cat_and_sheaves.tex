...
\begin{proof}
The exactness at $\Ker u_2$ and $\Coker u_2$ is straightforward. Assume $u_1$ is an epimorphism (the case when $u_3$ is a monomorphism being dual). It suffices to show that the morphism $g_0$ defined by the commutative square 
$$
\begin{tikzcd}
...
\nn$\bu$ Exactness at $\Coker d_Y^{p-1}$: This is an immediate consequence of the following lemma:
\begin{lem}
If 
$$
\begin{tikzcd}
...
\nn$\bu$ Exactness at $Z^p$: This is an immediate consequence of the following lemma:
\begin{lem}
If 
$$
\begin{tikzcd}
...
\begin{s}
P.~332, proof of Lemma 13.3.10. See Lemma~\ref{738b} p.~\pr{738b} below.
\end{s}
...
Here is a partial solution. Let $\C$ be an abelian category. We shall need the following statement:
...
We prefer to prove the dual statement:
