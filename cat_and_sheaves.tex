...
Now let $k$ be a field, set $A:=k[x_1,x_2,\dots]$, where $x_1,x_2,\dots$ are indeterminates, set $\wt\C:=\Mod(A)$, let $\C$ be the category of finite dimensional $A$-modules, let $\C'$ be the category of finite dimensional $k$-vector spaces, let $\mathfrak m$ be the ideal generated by the $x_i$, and set $U:=V:=A/\mathfrak m$ and $n:=1$. It is easy to see that we get a contradiction. We can thus set $X:=(A/\mathfrak m)[0]$ and conclude that the objects $RF(X),R^+F(X),R^-F(X)$ and $R^{\text b}F(X)$ do \emph{not} exist, as required.
\end{s}
