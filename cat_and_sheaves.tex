...
Let us denote by $I$ the set of finite subsets of $\Ob(\F_X)$, ordered by inclusion. Regarding $I$ as a category, it is small (Definition~\ref{myuscat} p.~\pr{myuscat}) and filtrant. For any $A$ in $I$ set $Z_A:=\bigsqcup_{a\in A}\zeta(a)$, where $\zeta:\C_X\to\C$ is the forgetful functor.
...
$$ 
is bijective by the Yoneda Lemma. As $\pp$ is faithful by assumption, the natural map 
$$
...
\end{tikzcd}
$$ 
quasi-commutes. The functors $\ld$ and $\ld'$ are fully faithful by Section~\ref{gcsbc} p.~\pr{gcsbc} and Theorem 5.3.6 p.~124 of the book. %Theorem~\ref{536} p.~\pr{536} above. 
As $\widehat\pp_{{}_0}$ is an equivalence (a quasi-inverse being $\widehat\psi_{{}_0}$, see Proposition 2.7.1 p.~62 in the book), the proof is complete.
\end{proof}
\end{s}
