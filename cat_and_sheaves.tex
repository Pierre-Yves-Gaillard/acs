% about "categories and sheaves" (version 0008)
...
%
\begin{s} 
P.~74, Theorem 3.1.6, Part (i) of the proof of implication (a)$\then$(b)\mv. 
%
\begin{lem} 
...
In the above setting, there are natural morphisms $$\ky(X)\to\Coim f\to\ky(Y)$$ in $\C^\vee$, and $\ky(X)\to\Coim f$ is an epimorphism. There are also a natural morphisms $$\hy(X)\to\Ima f\to\hy(Y)$$ in $\C^\wedge$, and $\Ima f\to\hy(Y)$ is a monomorphism.
\end{prop} 
...
\begin{lem}
A strict epimorphism is an epimorphism. Also, $f$ is a strict epimorphism if and only if the natural map $\Hom_\C(Y,Z)\to(\Coim f)(Z)$ is bijective for all $Z$ in $\C$. 
\end{lem} 
...
\nn(c) $\ssi$ (e): Write $P$ for $X\times_YX$. Recall that (c) says that $P\parar X\to Y$ is exact. Let $Z$ be an object of $\C$, and recall that (e) is the condition in Proposition~\ref{elem} p.~\pageref{elem}, so that we must show that $P\parar X\to Y$ is exact if and only if, for all $Z$ in $\C$, the map $u\mapsto u\circ f$ is a bijection $\Hom_\C(Y,Z)\to S_Z$.

Let $T_Z$ be the set of those morphisms $v:X\to Z$ such that $[P\parar X\xr vZ]$ (see Convention \ref{bra} p.~\pageref{bra}). It is enough to show $S_Z=T_Z$.

Let $u$ be in $S_Z$. As $[P\parar X\xr fY]$ by definition of $P$, we have $[P\parar X\xr uZ]$ by definition of $S_Z$, so that $u$ is in $T_Z$. Let $v$ be in $T_Z$, and let $v_1,v_2:W\parar X$ satisfy $[W\parar X\to Y]$. It suffices to prove $v\circ v_1=v\circ v_2$. The commutative diagram  
$$
\begin{tikzcd}
...
$$
(Note that the equivalence (c) $\ssi$ (e) also follows from Proposition~\ref{coimim} p.~\pageref{coimim} and Proposition~\ref{elem} p.~\pageref{elem}.)
\end{s}
...
We recall the notion of {\em construction by transfinite induction}. %For any element $i$ of any ordered set $I$ we freely use the self-explanatory pieces of notation $I_{<i}$ and $I_{\le i}$. 
