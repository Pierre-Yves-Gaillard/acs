% about "categories and sheaves" (version asp)
...
%
\begin{proof}
The implication $\then$ is clear. To prove $\Longleftarrow$, let 
$$
...
%
\begin{proof}
The implication $\Longleftarrow$ is clear. To prove $\then$, let 
$$
...
\begin{comment}
%
% Remember: You're working on this! Don't delete!!!
%
\begin{s} 
P. 409, proof of (17.1.12). The isomorphism is first proved in the particular case $B\in\C_X$, and then in the general case $B\in\C_X^\wedge$. It seems to me that the argument given in the particular case also works in the general case. Here is the way I understand this argument. Recall the setting: Let $X$ be a presite, let $A$ be in $\C_X^\wedge$, let $G$ be in $\PSh(A,\A)$, and let $B$ be in $\C_X^\wedge$. We must show 
%
\begin{equation}
\oo j_{A\to X}^\ddagger(G)(B)\simeq G(B\times A).
\end{equation}
% 
Recall also the following. If $f:X\to Y$ is a morphism of presites, if $\A$ is a category $\A$ admitting small inductive and projective limits, if $G$ is in $\PSh(Y,\A)$ and if $U$ is in $\C_X$, then we have
$$
f^\ddagger(G)(U)=\lim_{(U\to f^\tau(V))\in((\C_Y)^{\op})^U}G(V),
$$
where $U\to f^\tau(V)$ is a morphism in $(\C_X)^{\op}$ (corresponding to a morphism $f^t(V)\to U$ in $\C_X$). (See \eqref{1714} p.~\pageref{1714}.) It suffices to prove 
$$
\lim_{V\xr{(b,a)}B\times A}G(V\xr a A)\xr\sim G(B\times A\to A),
$$
that is, it suffices to prove that we have an isomorphism 
$$
\ic_{V\xr{(b,a)}B\times A}(V\xr a A)\xr\sim(B\times A\to A)
$$ 
