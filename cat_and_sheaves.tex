% about "categories and sheaves" (version-0233)
...
% new
\DeclareMathOperator*{\colim}{colim}%\DeclareMathOperator*{\coli}{colim}
...
\DeclareMathOperator{\Qis}{Qis}%\DeclareMathOperator{\Set}{Set}
...
\tableofcontents\newpage%\vskip2em\hrule%\bigskip%\vskip1em

\nn The purpose of this text is to make a few comments about the book%\bigskip 

\textbf{Categories and Sheaves} by Kashiwara and Schapira, Springer 2006,%\bigskip 

\nn referred to as ``the book'' henceforth.%\bigskip 
...
\nn\href{http://goo.gl/df2Xw}{http://goo.gl/df2Xw} 
...
\nn$*$ P.~64, proof of Proposition 2.7.5: $(\C')^\wedge$ should be $\C^\wedge$.

\nn$*$ P.~65, Exercise 2.7 (i): ``$\cdot\times_ZY:\Set_Z\to\Set_Z$'' should be ``$\cdot\times_ZY:\Set_Z\to\Set_Y$''. (See \S\ref{bc} p.~\pageref{bc}.) 

\nn$*$ P.~74, last four lines: $\alpha$ should be replaced with $\pp$.

\nn$*$ P.~80, last display: a ``$\ds\ilim$'' is missing.

\nn$*$ P.~83, Statement of Proposition 3.3.7 (iv) and (v) p.~83: $k$ might be replaced with $R$. 
...
\nn$*$ P.~180, Lemma 8.3.11 (b) (i): $\Coker f\xr\sim\Coker f'$ should be $\Coker f'\xr\sim\Coker f$. Proof of Lemma 8.3.11: The notation $\Hom$ for $\Hom_\C$ occurs eight times.
...
\nn$*$ P.~227. The second sentence uses Exercise~3.4 (i) p. 90 (see Proposition~\ref{34i} p.~\pageref{34i}).
...
\nn$*$ P.~245, first display: The subscript $\cc D$ is missing (three times) in $\Hom_{\cc D}$.
...
\nn$*$ P.~254. The functor $RF$ of Notation 10.3.4 p.~254 coincides with the functor $R_{\cc NQ}F$ of Definition 7.3.1 p.~159 of the book.
...
\nn(ii) small filtrant inductive limits are stable by base change (see Definition 2.2.6 p.~47 of the book), 
...
\end{tikzcd}
$$
and Proposition 10.1.15 p.~246 of the book implies that $(g,h)$ is an isomorphism.\bigskip 
...
(A_{S'}\to U)\in\cc E.
\end{equation}%$A_{S'}\to U$ is in $\cc E$. 
...
(B\to A_S)\in\cc E.
\end{equation}%$B\to A_S$ is in $\cc E$. 
%
...
\end{tikzcd}
$$ 
being cartesian, and $V\to U$ being in $S$ by the claim, \eqref{cv} holds by assumption, and implies successively \eqref{bas}, \eqref{as'}, and the lemma. 
\end{proof}
