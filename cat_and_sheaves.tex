...
In fact, the existence of the indicated limit is a sufficient condition for the existence of the corresponding localization, but it is not obvious \emph{a priori} (at least to me) that this condition is necessary.
\end{s}
...
the diagram on p.~254 of the book). %(Recall that I have made the extra assumption that $\I$ is saturated.)
\end{s}
...
Let $0\to X\xr f Y\xr g Z\to0$ be an exact sequence in $\A_c$ and define $\pp:\Mc(f)\to Z$ by $\pp:=(0,g)$. Then $\pp$ is a morphism in $\A_c$, this morphism is a qis, and it satisfies $\pp\ci\al(f)=g$. In particular, there are natural morphisms $H(T^nZ)\to H(T^{n+1}X)$ such that the sequence 
$$
...
\begin{s}
P.~334, proof of Lemma 13.3.12. Assuming $RF$ is triangulated (see Comment~\ref{rf} p.~\pr{rf}), the existence of an exact sequence $R^jF(X)\to R^jF(X'')\to R^{j+1}F(X')$ attached to a given exact sequence $0\to X'\to X\to X''\to0$ in $\C$ follows from Proposition~\ref{sl4} p.~\pr{sl4}.
\end{s}
