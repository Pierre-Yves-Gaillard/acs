% about "categories and sheaves" (version_0034)
...
\end{equation}
% 
and let $h_\U:\C\to\C^\wedge_\U$ be the corresponding Yoneda embedding. I do not know if the condition 
$$
(h_\U(p_i)\to h_\U(X))_{i\in I}\simeq\co h_\U(\alpha),
$$ 
depends on $\U$ subject to \eqref{eurl}.
\end{rk}
...
By Proposition 6.2.1 p.~138 of the book, the answer is ``yes'' if $I$ is filtrant and $\C$ is a $\U$-category. 
...
Let $\C$ be a category, let $\SSS$ a right multiplicative system, and let $F:\C\to\A$ be a functor such that $F(s)$ is an isomorphism for all $s$ in $\SSS$. Then $F$ is universally right localizable (see Definition~\ref{durl} p.~\pageref{durl}), $R_{\SSS}F\simeq F_{\SSS}$, and for any functor $K:\A\to\A'$ the diagram below commutes up to isomorphism
$$
\begin{tikzcd}
...
\begin{s}\label{durl2}
P. 159, Definition of universally right localizable functor (Definition 7.3.1 (ii)): see Definition~\ref{durl} p.~\pageref{durl}. %Here is how I understand the definition (see Remark~\ref{rurl} p.~\pageref{rurl}): $\C$ is a $\U$-small category, $\A$ and $\A'$ are big categories.
\end{s}
