...
\end{tikzcd}
$$
(See \S\ref{durl} p.~\pr{durl} below.)
...
\nn$\bu$ The isomorphism \qr{263a} can be decoded as follows: Consider the functor 
$$
...
\nn$\bu$ The isomorphism \qr{263b} can be decoded as follows: Let $Y$ be in $\C$, consider the functor 
$$
...
$$
f(Y\to A):=\xi_{Y\to A}(\id_Y).
$$

\nn$\bu$ The isomorphism \qr{263c} can be decoded as follows: Let $B$ be in $\C^\wg$, consider the functor 
$$
...
$$ 
for all $X\to A$ in $\C_A$ and all $A\to B$ in $B(A)$, and let $S$ be in $\Set$. The claim is that the map 
