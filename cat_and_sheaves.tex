% about "categories and sheaves" (version_0133)
...
\begin{s}\label{2211}
P. 50, Corollary 2.2.11 implies: 

Let $Z$ be an object of a category $\C$. If $\C$ admits finite projective limits, so does $\C_Z$, and $\C_Z\to\C$ commutes with such limits.
\end{s}
...
\begin{s}\label{333}
P. 82, Proposition 3.3.3 p.~82 of the book and \S\ref{2211} p.~\pageref{2211} imply: 

Let $X$ be an object of a category $\C$. If $\C$ admits finite projective limits, so does $\C_X$, and $\C_X\to\C$ is left exact.
\end{s}
...
\begin{s}%\label{399}
P. 399, beginning of Section 16.3. The category $\cc{LI}$ is a left multiplicative system by Lemma 16.2.6 p.~397 of the book. The category $\cc{LI}_A$ is co-cofinally small by Lemma 16.2.8 p.~398 of the book. 
...
\begin{s}\label{1725b}
P. 413, Proof of Lemma 17.2.5. Part~(ii): see Proposition~\ref{p406} p.~\pageref{p406}. Part~(iii): see \eqref{1725} p.~\pageref{1725}.
\end{s}
...
If $f:X\to Y$ is a weakly exact morphism of presites, then $\fthat:\C_Y^\wedge\to\C_X^\wedge$ is left exact. 
...
\begin{s}%\label{}
P. 419, proof of Lemma 17.4.4, Step (ii). More details: The morphism $\ee_b(F^b)(A):F^b(A)\to F^{bb}(A)$ is obtained as the composition 
$$
...
\begin{s}
P. 420, proof of Lemma 17.4.4, Step (ii). We draw a few diagrams with the hope of helping the reader visualize the argument. 
...
$$
\begin{tikzcd}
\cc E\ar[xshift=-.7ex]{d}[swap]{p_D}\\ 
\cc{LI}_D\ar[xshift=.7ex]{u}[swap]{R}
\end{tikzcd}
$$ 
are defined as follows: Recall that 
$$
D:=B``\coprod_A"C.
$$
The functor $p_D$ maps the object 
$$
\begin{tikzcd}
B'\ar{d}&A'\ar{l}\ar{r}\ar{d}&C'\ar{d}\\ 
B&A\ar{r}\ar{l}&C
\end{tikzcd}
$$ 
of $\cc E$ to the object 
$$
B'``\coprod_{A'}"C'\longrightarrow B``\coprod_A"C=D
$$ 
of $\cc{LI}_D$, whereas the functor $R$ maps the object $D''\to D$ of $\cc{LI}_D$ to the object 
$$
\begin{tikzcd}
B\times_DD''\ar{d}&A\times_DD''\ar{l}\ar{r}\ar{d}&C\times_DD''\ar{d}\\ 
B&A\ar{r}\ar{l}&C
\end{tikzcd}
$$ 
of $\cc E$. 
...
$$
$$
\overset{(\text c)}{\simeq}\col_{(B_i\to A_i)_{i\in I}\in K}\ F\left(``\coprod_i"B_i\right)
