% about "categories and sheaves" (version-0255)
...
\begin{lem}[Lemma 8.3.11 p.~180]\label{8311}%(stated as Lemma~\ref{8311} p.~\pageref{8311})
We have:
...
By Proposition 2.1.10 p. 40 of the book (see \S\ref{coll} p.~\pageref{coll}), it suffices to check that the functor $G:\A\to\C^\wedge$ defined by 
$$
G(X')(X):=\Hom_{\A}(F(X),X').
$$ 
...
\eqref{prepa4c} follows from the fact that small inductive limits in $\Set$ are stable by base change (see \S~\ref{coll} p.~\pageref{coll}). 
\end{proof}
...
\begin{comment}
\begin{s}\label{bc}
P.~65, Exercise 2.7 (i). As already indicated, ``$\cdot\times_ZY:\Set_Z\to\Set_Z$'' should be ``$\cdot\times_ZY:\Set_Z\to\Set_Y$''.
...
\begin{s}\label{coll}
P.~83, Proposition 3.3.6. For the reader's convenience we collect five statements closely related to (and including) Proposition 3.3.6. 

Statement 1: Proposition 2.1.10 p.~40 of the book (see Proposition~\ref{2.1.10} p.~\pageref{2.1.10}):
...
Statement 2: Definition 2.2.6 p.~47 of the book: stability by base change of certain inductive limits. 

Statement 3: Exercise 2.7 p.~65 of the book (see \S\ref{27i} p.~\pageref{27i}): 
...
Statement 4: Proposition 3.3.3 p.~82:
...
Statement 5: Proposition 3.3.6 p.~83:
...
By Proposition~\ref{333} it suffices to show that $A$ commutes with finite projective limits. Let $(X_i)$ be a finite inductive system in $\C$. We must check that the natural map $A(\colim_i X_i)\to\lim_iA(X_i)$ is bijective. Consider the diagram below, in which we abbreviate $(Y\to A)\in\C_A$ by $Y$: 
$$
\begin{tikzcd}
...
It remains to prove Steps 3, 3', 4, 5, 6. We refer to the book for Step 3. The proof of Step 3' is the same. (It is easy to see that small inductive limits in $\F^\wedge$ are stable by base change --- see Definition 2.2.6 p.~47 of the book and \S~\ref{coll} p.~\pageref{coll}.) 
...
$$ 
As $\theta$ is a monomorphism, Proposition~\ref{333} p.~\pageref{333} and Proposition~\ref{34i} p.~\pageref{34i} imply that $\colim\theta$ is also a monomorphism. Then the conclusion follows from the commutativity of the diagram 
$$
\begin{tikzcd}
...
Moreover, a morphism $u:A\to U$ in $(\oo{Op}_X)^\wedge$ with $U$ in $\oo{Op}_X$ is a local epimorphism if and only if for all $x$ in $U$ there is a $V$ in $\oo{Op}_X$ such that $A(V)\ne\varnothing$. 
\end{s}
...
%\begin{s}P.~397, proof of Proposition 16.2.2 (vi): see \S\ref{16111} p.~\pageref{16111}.\end{s}
...
Let us show that if a local isomorphism $u:A\to B$ in $\C_Y^\wedge$ is either a monomorphism or an epimorphism, then $\fthat(u)$ is a local isomorphism in $\C_X^\wedge$. 
...
%\begin{s}P.~417, Displays (17.4.2) and (17.4.2): see Proposition~\ref{fpa} p.~\pageref{fpa}.\end{s}
