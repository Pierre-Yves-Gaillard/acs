...
\nn\emph{Let $\C$ and $\C'$ be pre-additive categories, let $\C'^\C$ be the category of functors from $\C$ to $\C'$ and let $\al:I\to\C'^\C$ be a functor such that $\al(i)$ is additive for all $i$ in $I$, and the colimit $\col\al$ exists in $\C'^\C$. Then $\col\al$ is additive. There is a similar statement for limits.}
\end{s}
...
\begin{s}\lb{1057ii} 
[Tag 1]\index{Tag 1}\footnote{I use tags to refer to a part of this text in an other text (private or public). Tags will never be renumbered.}. P.~261, Lemma 10.5.7 (ii). The proof %(see above) 
