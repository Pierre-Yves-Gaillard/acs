...
\begin{s}
P.~16. We compute the endomorphisms of the \emph{covariant} power set functor\index{covariant power set functor}\index{power set functor} $P:\Set\to\Set$. Let $\varepsilon:P\to P$ be defined by $\varepsilon_A(Z)=\varnothing$ for all set $A$ and all $Z\subset A$.
...
\noindent Set $\mathbf 1:=\{0\},\mathbf 2:=\{0,1\}$ and let $\theta$ be an endomorphism of $P$.

\noindent(b) We obviously have $\theta_\varnothing=\id_{P(\varnothing)}$.

\noindent(c) Using (b) and applying $\theta$ to $\varnothing\to\mathbf 1$ we see that $\theta_{\mathbf 1}(\varnothing)=\varnothing$. 

\noindent $\bullet$ Case 1: $\theta_{\mathbf 1}(\mathbf 1)=\varnothing$. 

\noindent We claim $\theta=\varepsilon$ for all set $A$ and all $Z\subset A$, and prove the claim by applying $\theta$ to $A\to\mathbf 1$.

\noindent $\bullet$ Case 2: $\theta_{\mathbf 1}(\mathbf 1)=\mathbf 1$. 

\noindent We claim 

\noindent(d) $\theta=\id_P$. 

\noindent This will imply (a).

\noindent(e) We have $\theta_{\mathbf 2}=\id_{P(\mathbf 2)}$. [Left to the reader.]

\noindent(f) Let $A$ be a set with at least three elements. It suffices to show $\theta_A=\id_{P(A)}$.

\noindent(g) If $Z$ and $Y$ are two distinct subsets of $A$, then there is an $f:A\to\mathbf 2$ such that $P(f)(Z)\ne P(f)(Y)$. 

\noindent Proof: We can assume that there is a $y$ in $Y\setminus Z$. Letting $f$ be the characteristic function of $Y\setminus Z$, we get $1\in P(f)(Y)\setminus P(f)(Z)$.

\noindent(h) Let $Z$ be a subset of $A$. Applying $\theta$ to all the maps from $A$ to $\mathbf 2$ and using (g), we see that $\theta_A(Z)=Z$. This proves (f), and thus (d), and thus (a).
\end{s}
...
\noindent Section 5 of the appendix. Then $\kappa=\aleph_\kappa$. See\medskip 

\centerline{\href{https://mathoverflow.net/a/117809/461}{https://mathoverflow.net/a/117809/461}}

\noindent Hence the set of cardinals less than $\kappa$ coincides with the set
$$
...
\noindent\emph{Part 3}. %If $F$ is separated, $\ell(F)$ is a monomorphism because a colimit filtrant of monomorphisms is a monomorphism.
...
\noindent Claim 1: For all morphism $V\to LF$, where $V$ be an object of $\C$, the sieve $F\times_{LF}V$ is a covering sieve of $V$.
%$F$ is a covering sieve of $LF$. 

\noindent Proof of Claim 1: %Let $V$ be an object of $\C$ and $V\to LF$ a morphism. 
...
\noindent Proof of Claim 2: Let $W$ be an object of $\C$ and $W\to S$ a morphism. There is a commutative diagram 
$$
\begin{tikzcd}
F\ar[r,hookrightarrow,"\ell(F)"]&LF\\ 
...
\noindent\emph{Part 4}. Clear.%\end{enumerate}
\end{proof}
