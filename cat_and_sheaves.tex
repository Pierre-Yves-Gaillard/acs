...
If $\al:I^{\op}\to\C$ is a functor and $\DD:\C\to\C^{I^{\op}}$ is the diagonal functor, then a {\em projective limit of}\index{projective limit} $\al$ {\em in} $\C$ is a pair 
$$
...
\nn$\si$: Let $\DD:\C\to\C^\C$ be the diagonal functor, let $p\in\Hom_{\C^\C}(\id_\C,\DD(T))$ be a $(\Hom_{\C^\C}(\id_\C,\DD(\ )),T)$\--co-universal element  (see Definition~\ref{ue2} p.~\pr{ue2}). Recall that this means that, for each object $X$ of $\C$, the map 
$$
...
%
\begin{prop}
If $\DD:\Set\to\Set^I$ is the diagonal functor, then there is a canonical bijection
$$
...
$$ 
and let $p:\al\to\DD(A)$ be the tautological morphism in $(\C^\wg)^{\C_A}$ defined by 
%
...
\begin{proof}
If $I$ and $\C$ are categories, we write $\DD$ for the diagonal functor from $\C$ to $\C^I$. Let $p\in\Hom_{\A^I}(\al,\DD(A))$ be the coprojection. For any $C$ in $\A$ the map 
$$
f_C:\Hom_\A(A,C)\to\Hom_{\A^I}(\al,\DD(C)),\quad u\mt\DD(u)\ci p
$$ 
is bijective. Let $b=(b:B\to A)$ be an object of $\A_A$. We have the inclusions 
$$
\Hom_{\A_A}(a,b)\subset\Hom_\A(A,B),\quad
%
\Hom_{(\A_A)^I}(\al',\DD(b))\subset\Hom_{\A^I}(\al,\DD(B)).
$$ 
Let $u$ be in $\Hom_\A(A,B)$. It suffices to check 
$$
f_B(u)\in\Hom_{(\A_A)^I}(\al',\DD(b))\ssi u\in\Hom_{\A_A}(a,b),
$$ 
that is 
$$
\DD(b)\ci\DD(u)\ci p=p\ssi\DD(b\ci u)=\id_{\DD(A)},
$$ 
that is 
$$
\DD(b\ci u)\ci p=p\ssi\DD(b\ci u)=\id_{\DD(A)},
$$ 
that is 
$$
...
Let $\al:I\to\C$ be a functor, let $X$ be an object of $\C$, and let $p:\al\to\DD(X)$ be a morphism in $\C^I$ (where $\DD(X)$ is constant with value $X$). For all universe $\U$ such that $\C$ is a $\U$-category, we write $h_\U$ for the Yoneda embedding from $\C$ to $\C^\wg_\U$.
...
$$ 
Then $(X,f)\iso\col\bt$, the coprojection $q:\bt\to\DD(X,f)$ being given by $q_i:=p_i$ for all $i$ in $I$.
\end{lem}
\begin{proof}
Recall that $\DD$ denotes the appropriate diagonal functor. Let 
$$
...
$$ 
($\DD$ being the diagonal functor) by 
$$
