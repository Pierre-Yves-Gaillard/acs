...
Let $(\Gamma_U)_{U\in\C}$ be a subfamily of the family $(\Sigma_U)_{U\in\C}$ satisfying GT1\--GT4, let $\U$ be a universe such that $\C$ is $\U$-small (Definition~\ref{myuscat} p.~\pr{myuscat}), and let 
...
(C\to V)\in\E.
\end{equation} 
The morphism $V\to U$ being in $S$ by the first sentence of Lemma~\ref{prepagt4}, the sieve $S'\tm_UV$ is in $\Gamma_V$ by \qr{sgu}, and $A_{S'\tm_UV}\to V$ is in $\E$ by definition of $\Gamma_V$. We have 
$$
...
%Soit $C$ un site dont la catégorie sous-jacente est une $\U$-catégorie. Un préfaisceau $F$ à valeurs dans $\U$-Ens est dit séparé (resp. est un faisceau) si pour tout crible $R$ couvrant $X$, objet de $C$, l'application: $$\Hom_{C^\wedge}(X,F)\to\Hom_{C^\wedge}(R,F)$$ est une injection (resp. une bijection). 
\end{df} 

See \href{http://www.normalesup.org/~forgogozo/SGA4/02/02.pdf}{http://www.normalesup.org/$\sim$forgogozo/SGA4/02/02.pdf}%It is not obvious to me that these definitions are equivalent, and that the first one is invariant under change of universes.

Let us assume $\A=\Set$.

In \href{https://mathoverflow.net/a/283271/461}{https://mathoverflow.net/a/283271/461} Dylan Wilson\index{Wilson, Dylan} proved as follows that%\bigskip\centerline{\emph{a ``Verdier sheaf'' is a ``KS-sheaf''.}}
%
\begin{prop}
...
%
\begin{proof}
For the duration of the proof, $\Sh(X)$ will denote the category of ``\emph{Verdier}-sheaves''. By Theorem 3.4 in SGA4.II, the inclusion $\Sh(X)\subset\PSh(X)$ admits an exact left adjoint $L$. 

\nn Claim~1: If $A\to U$, with $U$ in $\C_X$, is a monomorphism and a local isomorphism, then $LA\to LU$ is an isomorphism.

\nn Proof of Claim~1: Let $F$ be in $\Sh(X)$. We must show that 
$$
\Hom_{\Sh(X)}(LU,F)\to\Hom_{\Sh(X)}(LA,F)
$$ 
is bijective, that is, that 
$$
\Hom_{\PSh(X)}(U,F)\to\Hom_{\PSh(X)}(A,F)
$$ 
is bijective, which is clear.

\nn Claim~2: If $A\to B$ is a monomorphism and a local isomorphism, then $LA\to LB$ is an isomorphism.

\nn Proof of Claim~2: Let $f:A\to B$ be a monomorphism and a local isomorphism. It suffices to show that $Lf$ is an epimorphism. Let $x,y:B\rightrightarrows F$ be two morphisms in $\PSh(X)$ with $F$ in $\Sh(X)$ and $x\ne y$. It suffices to prove $x\ci f\ne y\ci f$. %[In this proof we write $\alpha\beta$ for $\alpha\circ\beta$.] 
As we have 
$$
\colim_{U\to B}U\xr\sim B
$$ 
(the colimit being taken in $\PSh(X)$), there is a morphism $u:U\to B$ such that $x\ci u\ne y\ci u$. Consider the commutative diagram 
$$
\begin{tikzcd}
...
\end{tikzcd}
$$ 
in $\PSh(X)$, where the square is cartesian. As $f_U$ is a monomorphism by Lemma~\ref{8311} (a) (i) \pageref{8311} and a local isomorphism by Lemma 16.2.4 p.~395 of the book, we have $x\ci u\ci f_U\ne y\ci u\ci f_U$ by Claim~1. This implies $x\ci f\ne y\ci f$, as desired.

\nn Claim~3: $L$ maps local epimorphisms to epimorphisms.

\nn Proof of Claim~3: Let $A\to B$ be a local epimorphism. Then we see that 
$$
\Ima(A\to B)\to B
$$ 
...
\Ima(LA\to LB)\to LB
$$ 
is an isomorphism by exactness of $L$, and that $LA\to LB$ is an epimorphism. This proves Claim~3. 

\nn Claim~4: $L$ maps local isomorphisms to isomorphisms.

\nn Proof of Claim~4: Claim~4 follows from Claim~3 and the exactness of $L$.

Let us show that a ``Verdier sheaf'' is a ``KS-sheaf''. Let $F$ be a ``Verdier sheaf'' and let $A\to U$ be a local isomorphism in $(\C_X)^\wedge=\PSh(X)$ such that $U\in\C_X$. It suffices to prove that $FU\to FA$, that is 
$$
\Hom_{\PSh(X)}(U,F)\to\Hom_{\PSh(X)}(A,F),
$$ 
that is 
...
\Hom_{\Sh(X)}(LU,F)\to\Hom_{\Sh(X)}(LA,F),
$$ 
is an isomorphism. But this follows from Claim~4.
\end{proof}
\end{s}
...
\begin{comment}
\begin{muspb}
%\footnote{Here is problem which, I hope, will turn out to be very easy, even if I'm unable to solve it. I'm using the overdramatic expression of ``major unsolved problem'' mainly as a note to myself, to make sure I don't forget it.}. 
About\index{Major Unsolved Problem} Definition 17.3.1(ii) p.~414 of the book, stated as  Definition~\ref{sheaf} above: I'm unable to answer the following two closely related questions:

(a) Let $X$ be a site, let $\U\subset\V$ be universes, let $F\in\PSh(X,\U\textbf{-set})$ be a $\U\textbf{-set}$-valued presheaf and let $F'\in\PSh(X,\V\textbf{-set})$ be the corresponding $\V\textbf{-set}$-valued presheaf. Is the implication ``$F$ is a sheaf $\implies F'$ is a sheaf'' true?

(b) Are Definitions \ref{sheaf} and \ref{faisceau} above equivalent?
\end{muspb}
\end{comment}
