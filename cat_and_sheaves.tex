...
$$
in $\Fct(I^{\op},\Set_\U)$ is a projection (see Definition~\ref{plim} p.~\pr{plim}) for any $X'$ in $\C'$ and for some (and thus for any) universe $\U$ such that $\C'$ is a $\U$-category''. Note that $(1F)$ and $(2F)$ are equivalent by Propositions~\ref{aioc} and \ref{aic}. Then the conditions \emph{(a)} and \emph{(b)} below are equivalent:

\nn\emph{(a)} The equivalent conditions %We have 
$(1h_\U)$ and $(2h_\U)$ hold for %all 
some (and thus for any) universe $\U$ such that $I$ is $\U$-small and $\C$ is a $\U$-category. %and $I$ is $\U$-small.

\nn\emph{(b)} The equivalent conditions $(1F)$ and $(2F)$ hold for all for all category $\C'$ and all functor $F:\C\to\C'$.
\end{prop}

\begin{proof}
Condition (b) implies trivially Condition (a). Let us prove that (a) implies (b). Let $F:\C\to\C'$ be as in (b) and let $\U$ be a universe such that $I$ is $\U$-small and $\C$ and $\C'$ are $\U$-categories. %$I,\C$ and $\C'$ are %$\U$-categories. %and $I$ is %$\U$-small. 
It suffices to prove $(2F)$. Let $X'$ be in $\C'$ and define $A\in\C^\wg_\U$ by $A:=\Hom_{\C'}(F(\ ),X')$. We get the commutative diagram 
$$
\begin{tikzcd}
