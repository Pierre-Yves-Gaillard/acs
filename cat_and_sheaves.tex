...
\Hom_{\Fct(\C,\A)}(F,G)\simeq\lim_{X\in\C}\Hom_{\Fct(\C^X,\A)}(F\circ\oo j^X,G\circ\oo j^X).
\end{equation}
...
$$
\Hom_{\Fct(\C^X,\A)}(F\circ\oo j^X,G\circ\oo j^X)\to\Hom_{\Fct(\C^Y,\A)}(F\circ\oo j^Y,G\circ\oo j^Y)
$$ 
by sending $\theta:F\circ\oo j^X\to G\circ\oo j^X$ to the horizontal composition $\theta*(\circ f)$ (see Definition~\ref{dil1} p.~\pageref{dil1}): 
$$
\begin{tikzcd}
%
\A&{}&\C^X\ar{ll}{}[near start,swap]{G\circ\oo j^X}&{}&\C^Y\ar{ll}{}[near start,swap]{\circ f}\\ 
%
\A&\ar{u}{\theta}&\C^X\ar{ll}{}[near start]{F\circ\oo j^X}&\ar{u}{\circ f}&\C^Y.\ar{ll}{}[near start]{\circ f}
%
\end{tikzcd}
$$
...
The commutative diagram \eqref{d447} also induces the commutative diagram 
$$
\Delta:=\left\{
\begin{tikzcd}
