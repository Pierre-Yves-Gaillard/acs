...
$$ 
\Hom_\B\left(F\left(\col\al\right),Y\right)\iso\Hom_\C\left(G\left(\col\al\right),Z\right)\xr\sim\Hom_\C\left(\col G\ci\al,Z\right)
$$
$$
\xr\sim\lim\ \Hom_\C(G(\al),Z)\iso\lim\ \Hom_\B(F(\al),Y)\xleftarrow\sim\Hom_\B(\col F\ci\al,Y),
$$ 
...
\begin{prop}\lb{b}
If $\al:I\to\C$ is a functor defined on a small category, if $X$ is an object of $\C$, if $p:\al\to\DT X$ is a morphism in $\C^I$ %(see Notation~\ref{diag} p.~\pr{diag}), 
and if $h:\C\to\C^\wg$ be the Yoneda embedding, then the following conditions are equivalent:
...
Here is the classic example of a non-universal inductive limit. Letting $\al$ be the unique functor from the empty category to $\Set$, we get $\col\al=\varnothing$. Writing $h:\Set\to\Set^\wg$ for the Yoneda embedding yields $\col h\ci\al=\DT\varnothing$. But we have, on the one hand $(\DT\varnothing)(\varnothing)=\varnothing$, and on the other hand $(h(\varnothing))(\varnothing)\not\iso\varnothing$, implying 
$$
\col(h\ci\al)\not\iso h(\col\al).
$$ 
...
To prove \qr{coco} we define morphisms 
$$
\begin{tikzcd} 
...
%We shall give two other proofs. 
Recall the statement: 
...
\col_{(G(X)\to X'')\in\C_B}\Hom_{\C'}(X',F(X))\iso\col_{(X'\to F(X))\in(\C^A)^{\op}}\Hom_{\C''}(G(X),X''). 
\end{equation} 
%Here $$a\in\Hom_{\C'}(A,F(X)),\quad b\in\Hom_{\C''}(G(X),B). $$ 
\end{prop}
...
$$
\begin{tikzcd}
\ds\col_{(G(X)\to X'')\in\C_B}\Hom_{\C'}(X',F(X))\ar[r,yshift=0.7ex,dashed,"f"]&\ds\col_{(X'\to F(X))\in(\C^A)^{\op}}\Hom_{\C''}(G(X),X'')\ar[l,yshift=-0.7ex,dashed,"g"]\\ 
\Hom_{\C'}(X',F(X))\ar[u,"p(G(X)\to X'')"]&\Hom_{\C''}(G(X),X'')\ar[u,"q(X'\to F(X))"'],
\end{tikzcd}
$$ 
where the vertical arrows are the coprojections. We leave it to the reader to check firstly that there are maps $f$ and $g$ as in the above diagram satisfying %and the maps $f$ and $g$ are defined by 
$$
...
%\left.
%\begin{aligned}
%\col_{(F(Y)\to X')\in\C_{X'}}\Hom_\C(X,Y)&\iso\col_{(X\to Y)\in\C^X}\Hom_{\C'}(F(Y),X')\quad\\ % [use \quad as spacer between equation and right brace]
%&\iso\Hom_{\C'}(F(X),X')
%\end{aligned}
%\right\}
%\end{equation}
...
\begin{align}
\col_{(F(Y)\to X')\in\C_{X'}}\Hom_\C(X,Y)&\iso\col_{(X\to Y)\in\C^X}\Hom_{\C'}(F(Y),X')\lb{fyx'1}\\
&\iso\Hom_{\C'}(F(X),X').\lb{fyx'2}
\end{align}
\end{subequations}
Indeed, we obtain \qr{fyx'1} by replacing the setting (\ref{241s}) p.~\pr{241s} with 
$$
X\in\C\xl{\id_\C}\C\xr{F}\C'\ni X' 
$$ 
in the isomorphism (\ref{241}) p.~\pr{241}, and we prove \qr{fyx'2} by noting that the identity of $X$ is an initial object of $\C^X$. We have bijections 
$$ 
...
\paragraph{Second proof of \qr{coco} p.~\pr{coco}}%\nn\textbf{Second proof of \qr{coco} p.~\pr{coco}}

\nn In the above setting, let us assume that 
$$
K=J,\quad\psi=\id_J,
$$ 
that $\al:I\to\C$ is a functor, and that $\C$ admits small inductive limits. We must prove 
$$
...
\paragraph{Third proof of \qr{coco} p.~\pr{coco}}%\nn\textbf{Third proof of \qr{coco} p.~\pr{coco}}
...
$$
\begin{tikzcd}
\al(i)\ar{dd}{a_{iju}}\ar{r}{\id}&\al(i)\ar{d}{b_{iju}}\ar{r}{\id}&\al(i)\ar{d}{d_{i\psi(j)u}}\ar{r}{\id}&\al(i)\ar{dd}{f_i}\\ 
&\ds\col_{i,u}\al(i)\ar{d}{c_j}\ar{r}&\ds\col_{i,u}\al(i)\ar{d}{e_{\psi(j)}}\\ 
\col\bt\ar{r}[swap]{z}&\ds\col_j\col_{i,u}\al(i)\ar{r}\ar{r}[swap]{y}&\ds\col_k\col_{i,v}\al(i)\ar{r}\ar{r}[swap]{x}&\col\al.
\end{tikzcd}
$$ 
The third column involves the coprojections 
$$
\al(i)\xr{d_{ikv}}\col_{(i,v)\in I_k}\al(i)\xr{e_k}\col_{k\in K}\col_{(i,v)\in I_k}\al(i).
$$ 
The diagram being commutative, $x\ci y\ci z$ is the natural map $\col\bt\to\col\al$. Moreover $z$ is bijective by the proof of Proposition~\ref{coco2}, $y$ is bijective because $\psi$ is cofinal and $x$ is bijective by the proof of \qr{coco} p.~\pr{coco}.
