...
Let $\C$ be a category, let $h:\C\to\C^\wg$ be the Yoneda embedding, let $F$ be in $\C^\wg$, let $X$ be in $\C$, and define 
$$
\begin{tikzcd} 
F(X)\ar[yshift=0.7ex]{r}{\pp}&\Hom_{\C^\wg}(h(X),F)\ar[yshift=-0.7ex]{l}{\psi}
\end{tikzcd}
$$
by 
\begin{equation}\lb{yo}
\pp(u)_Y(f):=F(f)(u),\quad\psi(\theta):=\theta_X(\id_X)
\end{equation}
for 
$$
u\in F(X),\quad Y\in\C,\quad f\in\Hom_\C(Y,X),\quad\theta\in\Hom_{\C^\wg}(h(X),F):
$$ 
$$
f\in\Hom_\C(Y,X)\xr{\pp(u)_Y}F(Y)\xl{F(f)}F(X)\ni u.
$$
Then $\pp$ and $\psi$ are mutually inverse bijections. In the particular case where $F$ is equal to $h(Z)$ for some $Z$ in $\C$, we get 
$$
\pp(u)=h(u)\in\Hom_{\C^\wg}(h(X),h(Z)).
$$
This shows that $h$ is fully faithful.

Let $k:\C\to\C^\vee$ be the Yoneda embedding, let $F$ be in $\C^\vee$, let $X$ be in $\C$, and define 
$$
\begin{tikzcd} 
F(X)\ar[yshift=0.7ex]{r}{\pp}&\Hom_{\C^\vee}(F,k(X))=\Hom_{\Set^\C}(k(X),F)\ar[yshift=-0.7ex]{l}{\psi}
\end{tikzcd}
$$
by \qr{yo} for 
$$
u\in F(X),\quad Y\in\C,\quad f\in\Hom_\C(X,Y),\quad\theta\in\Hom_{\Set^\C}(k(X),F):
$$ 
$$
f\in\Hom_\C(X,Y)\xr{\pp(u)_Y}F(Y)\xl{F(f)}F(X)\ni u.
$$
Then $\pp$ and $\psi$ are mutually inverse bijections. In the particular case where $F$ is equal to $k(Z)$ for some $Z$ in $\C$, we get 
$$
\pp(u)=k(u)\in\Hom_{\C^\vee}(k(Z),k(X)).
$$
...
$$
\psi(\pp(u))=\pp(u)_X(\id_X)=F(\id_X)(u)=u
$$ 
and
$$
\pp(\psi(\theta))_Y(f)=F(f)(\psi(\theta))=F(f)(\theta_X(\id_X))=\theta_Y(f),
$$ 
the last equality following from the commutativity of the square 
$$
\begin{tikzcd}
h(X)(Y)\ar{r}{\theta_Y}&F(Y)\\ 
h(X)(X)\ar{u}{h(X)(f)}\ar{r}[swap]{\theta_X}&F(X),\ar{u}[swap]{F(f)}
\end{tikzcd}
$$ 
which is equal to the square 
$$
\begin{tikzcd}
\Hom_\C(Y,X)\ar{r}{\theta_Y}&F(Y)\\ 
\Hom_\C(X,X)\ar{u}{\ci f}\ar{r}[swap]{\theta_X}&F(X).\ar{u}[swap]{F(f)}
\end{tikzcd}
$$
\end{proof}
...
The Yoneda Lemma (Theorem~\ref{yol} p.~\pr{yol}) says that $(F,X)$-universal elements are in functorial bijection with isomorphisms $\hy_\C(X)\xr\sim F$, such an isomorphism being called a {\em representation of} $F$ \emph{by} $X$. More precisely, the Yoneda Lemma says that a morphism $u:\hy_\C(X)\to F$ in $\C^\wg$ is universal if and only if any morphism $\hy_\C(Y)\to F$ factors uniquely through $u$:
$$
\begin{tikzcd}
\hy_\C(Y)\ar[rr]\ar[dr,dashed]&&F\\ 
&\hy_\C(X).\ar[ru,"u"']
\end{tikzcd}
$$ 
(This unique factorization condition implies that $u$ is an isomorphism.) In particular, if the pairs $(X,u)$ and $(X',u')$ both represent $F$, then there is a unique morphism $g:X\to X'$ such that $F(g)(u')=u$; moreover $g$ is an isomorphism.
...
The Yoneda Lemma (Theorem~\ref{yol} p.~\pr{yol}) says that $(F,X)$-co-universal elements are in functorial bijection with isomorphisms $F\xr\sim\ky_\C(X)$, such an isomorphism being called a \emph{co-representation of} $F$ \emph{by} $X$. More precisely, the Yoneda Lemma says that a morphism $u:F\to\ky_\C(X)$ in $\C^\vee$ is co-universal if and only if any morphism $F\to\ky_\C(Y)$ factors uniquely through $u$:
$$
\begin{tikzcd}
F\ar[rr]\ar[dr,"u"']&&\ky_\C(Y)\\ 
&\ky_\C(X).\ar[ru,dashed]
\end{tikzcd}
$$ 
(This unique factorization condition implies that $u$ is an isomorphism.) In particular, if the pairs $(X,u)$ and $(X',u')$ both co-represent $F$, then there is a unique morphism $g:X\to X'$ such that $F(g)(u)=u'$; moreover $g$ is an isomorphism.
...
\nn(a) Let $L:\C\to\C'$ be a functor and $X'$ an object of $\C'$. Let us assume we are given an object of $\C$ which we denote by $R(X')$. (The reason for this seemingly silly notation will be given soon!) We say that ``the value of the right adjoint $R$ to $L$ at $X'$ is defined and isomorphic to $R(X')$'', or, abusing the terminology, that ``$R(X')$ exists'' if there is a morphism $\eta_{X'}:L(R(X'))\to X'$ such that the pair $(R(X'),\eta_{X'})$ represents the functor $\Hom_{\C'}(L(\ ),X'):\C^{\op}\to\Set$ in the sense of Definition~\ref{ue} p.~\pr{ue}. This means that, for all $X\in\C$, the map 
$$
...
\end{tikzcd}
$$ 
The object $R(X')$ does not necessarily exist, but it is unique up to unique isomorphism.

\nn(b) Let $T$ be a terminal object of $\Set$. Then a functor $F:\C^{\op}\to\Set$ is representable if and only if the right adjoint of $F^{\op}:\C\to\Set^{\op}$ is defined at $T$.

Let us spell out the statement dual to (a):

\nn(c) Let $R:\C'\to\C$ be a functor and $X$ an object of $\C'$. Let us assume we are given an object of $\C'$ which we denote by $L(X)$. We say that ``the value of the left adjoint $L$ to $R$ at $X$ is defined and isomorphic to $L(X)$'', or, abusing the terminology, that ``$L(X)$ exists'' if there is a morphism $\ee_X:X\to R(L(X))$ such that the pair $(L(X),\ee_X)$ co-represents the functor $\Hom_\C(X,R(\ )):\C'\to\Set$ in the sense of Definition~\ref{ue2} p.~\pr{ue2}. This means that, for all $X'\in\C'$, the map 
$$
...
\end{tikzcd}
$$ 
The object $L(X)$ does not necessarily exist, but it is unique up to unique isomorphism.
...
The definition of projective and inductive limits is a particular case of the situation considered in Section~\ref{defat} p.~\pr{defat}:

Let $\al:I^{\op}\to\C$ be a functor. In short, a projective limit of $\al$ in $\C$ is an object $X$ of $\C$ such that ``the value of the right adjoint $\lim$ to $\DT:\C\to\C^{I^{\op}}$ is defined at $\al$ and isomorphic to $X$'' in the sense of Section~\ref{defat} p.~\pr{defat}. For the reader's convenience we spell out this condition.
...
Let $\al:I\to\C$ be a functor. In short, an inductive limit of $\al$ in $\C$ is an object $X$ of $\C$ such that ``the value of the left adjoint $\col$ to $\DT:\C\to\C^I$ is defined at $\al$ and isomorphic to $X$'' in the sense of Section~\ref{defat} p.~\pr{defat}. For the reader's convenience we spell out this condition.
...
If $\al:I\to\Set$ is a functor defined on a small category and $X$ is in $\Set$, then the map 
$$
\Hom_\Set(\col\al,X)\to\lim\Hom_\Set(\al,X),\quad f\mt(f\ci p_i)_{i\in I}
$$ 
...
$$ 
More precisely, given $\theta$ as in the diagram
$$
\begin{tikzcd}
...
\end{tikzcd}
$$ 
we can define $f$ by %For $S$ in $\Set$, the inverse of $$\Hom_\Set(S,\Hom_{\C^I}(\al,\DT Y))\to\Hom_{\Set^{I^{\op}}}(\DT S,\Hom_\C(\al,Y)),\quad f\mt p\ci\DT f$$ is the map $\xi\mt f$ defined by the formula 
$f(s)_i:=\theta_i(s)$.
\end{s}
...
We have a functor $\al:I\to\C^J$, we assume that for each $j$ in $J$ the functor $\al(\ )(j)$ admits an inductive limit $(X_j,p_j)$: 
\begin{equation}\lb{ajpj}
\begin{tikzcd}
\al(\ )(j)\ar[r,"p_j"]\ar[dr]&\DT X_j\ar[d]&X\ar[d,dashed]\\ 
&\DT Y&Y.
\end{tikzcd}
\end{equation}
...
\begin{prop}\lb{216p}
In the above setting $(\bt,q)$ is an inductive limit of $\al$. 
\end{prop}
...
\begin{s} 
P.~39, Proposition 2.1.7. Let $\al:I\tm J\to\C$ be a bifunctor, and let $(X_i)_{i\in I}$ be a family of objects of $\C$. Assume that, for all $i$ in $I$ the functor $\al(i,\ )$ admits an inductive limit of the form $(X_i,p_i)$: 
$$
\begin{tikzcd}
...
\end{tikzcd}
$$ 
By arguing as in \S\ref{216} p.~\pr{216} we see that there is a natural functor $\bt:I\to\C$ such that $\bt(i)=X_i$ for all $i$. Let $(X,q)$ be an inductive limit of $\bt$: 
...
We claim that $(X,r)$ is an inductive limit of $\al$, where $r:\al\to\DT X$ is the obvious morphism of functors. 
...
\nn\emph{(a)} $(X,p)$ is an projective limit of $\al$ in $\C$,

\nn\emph{(b)} $(h(X),h(p))$ is an projective limit of $h\ci\al$ in $\C^\wg$, where $h(p):\DT h(X)\to h\ci\al$ is the morphism in $(\C^\wg)^{I^{\op}}$ induced by $p$,

\nn\emph{(c)} $(\Hom_\C(Y,X),\Hom_\C(Y,p))$ is a projective limit of $\Hom_\C(Y,\al)$ in $\Set$, where $\Hom_\C(Y,p):\DT\Hom_\C(Y,X)\to\Hom_\C(Y,\al)$ is the morphism in $\Set^{I^{\op}}$ induced by $p$.
\end{prop}
...
\nn$\si$: Let $p:\id_\C\to\DT T$ be the coprojection (this is a morphism in $\C^\C$). Let $X$ be in $\C$. For all morphism of functors $\theta:\id_\C\to\DT X$ there is a unique morphism $f:T\to X$ satisfying $\DT f\ci p=\theta$. %Let us denote this $f$ by $f^\theta$: 
$$
\begin{tikzcd}
...
$$ 
is bijective, or, more naively, that for all $w:\pp_*\al\to\pp_*\gamma$ there is a unique $v:\al\to\gamma$ such that $u\ci(v*\pp)=w$: 
$$
\begin{tikzcd}
...
$$X:=h(\C_A)^\ddg\Big(F\ci j(\C_A)\Big)\Big(\ld(B\to A)\Big),$$
...
\begin{align*}
%
X&\iso h(\C_A)^\ddg(F\ci j(\C_A))\left(\ld\left(\left(\col_{(U\to B)\in\C_B}h(\C)(U)\right)\to A\right)\right)&\text{by \qr{263a}}\\
%
&\iso h(\C_A)^\ddg(F\ci j(\C_A))\left(\ld\left(\col_{(U\to B)\in\C_B}\Big(h(\C)(U)\to A\Big)\right)\right)&\text{by Lemma~\ref{cc}}\\
%
&\iso\col_{(U\to B)\in\C_B}\Bigg(h(\C_A)^\ddg\Big(F\ci j(\C_A)\Big)\bigg(\ld\Big(h(\C)(U)\to A\Big)\bigg)\Bigg)&\text{by \qr{leq} \& Prop.~\ref{271}}\\
%
&\iso\col_{(U\to B)\in\C_B}\Bigg(h(\C_A)^\ddg\Big(F\ci j(\C_A)\Big)\bigg(\ld\Big(h(\C)_A(U\to A)\Big)\bigg)\Bigg)\\
%
&\iso\col_{(U\to B)\in\C_B}\Bigg(h(\C_A)^\ddg\Big(F\ci j(\C_A)\Big)\bigg(h(\C_A)(U\to A)\bigg)\Bigg)&\text{by \qr{lcom}}\\
%
&\iso\col_{(U\to B)\in\C_B}\Big(\big(F\ci j(\C_A)\big)(U\to A)\Big)&\text{by Prop.~\ref{271}}\\
%
&\iso\col_{(U\to B)\in\C_B}F(U)\\
%
&\iso\col_{(U\to B)\in\C_B}\Bigg(h(\C)^\ddg(F)\Big(h(\C)(U)\Big)\Bigg)&\text{by Prop.~\ref{271}}\\
%
&\iso h(\C)^\ddg(F)\left(\col_{(U\to B)\in\C_B}h(\C)(U)\right)&\text{by Prop.~\ref{271}}\\
%
&\iso\Bigg(h(\C)^\ddg(F)\ci j\Big((\C^\wg)_A\Big)\Bigg)\left(\left(\col_{(U\to B)\in\C_B}h(\C)(U)\right)\to A\right)\\
%
&\iso\Bigg(h(\C)^\ddg(F)\ci j\Big((\C^\wg)_A\Big)\Bigg)\left(B\to A\right)&\text{by \qr{263a}.}
%
\end{align*}
\end{proof}
