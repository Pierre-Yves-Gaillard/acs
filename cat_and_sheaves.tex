% about "categories and sheaves" (version avd)
...
%
\begin{proof}
We assume, as we may, that $I$ is nonempty and well-ordered. For all $i$ in $I$ we consider the following statement, which we call $(S_i)$: 

\nn[Beginning of Statement $(S_i)$.] 

\nn(a) For each $j\le i$ There is a commutative diagram
$$
\begin{tikzcd}
...
\end{tikzcd}
$$ 
with $v_j,w_j\in\Mor(\C_0)$. 

\nn(b) For each $k<j\le i$ there is a commutative diagram
$$
\begin{tikzcd}
X'\ar{r}{v_j}\ar[equal]{d}&Y'_{<j}\ar{r}{w_j}&Y'_i\\ 
X'\ar{r}[swap]{v_k}&Y'_{<k}\ar{r}[swap]{w_k}&Y'_k\ar{ul}[swap]{p_{jk}}
\end{tikzcd}
$$ 
with $p_{jk}\in\Mor(\C_0)$. 

\nn(c) We have
%
\begin{equation}\label{pijk}
p_{jk}\circ w_k\circ p_{k\ell}=p_{j\ell}\quad\forall\quad\ell<k<j\le i.
\end{equation}
% 
[End of Statement $(S_i)$.]

Let $0$ be the least element of $I$ and let us prove $(S_0)$. We define $v_0$ and $h_0$ by (9.5.2) (ii): 
$$
\begin{tikzcd}
...
\end{tikzcd}
$$ 
and we define $w_0$ by the commutative diagram 
$$
\begin{tikzcd}
...
Now assume that $i>0$ and that $(S_j)$ holds for all $j<i$. For $k<j<i$ we define $
...
\end{tikzcd}
$$ 
From this point, the proof of $(S_i)$ is straightforward. 

The fact that $(S_i)$ holds for all $i$ implies that $(Y'_i)_{i\in I}$ is an inductive system in $\C_0$. We denote its limit (which exists in $\C_0$ thanks to (9.5.2) (i)) by $Y'$, and we write $p_i$ for the coprojection $Y'_i\to Y'$. Putting 
$$
