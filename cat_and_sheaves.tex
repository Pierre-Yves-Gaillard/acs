% about "categories and sheaves" (version_0062)
...
\end{align*}
%
%the first equality following from (b), the second equality being obvious, and the third equality following from the definition of $a$. 
...
This follows immediately from Corollary 14.1.12 (i). %(See also \S~\ref{1811} p.~\pageref{1811}.)
\end{s}
...
[\S\ref{revol} p.~\pageref{revol} and \S\ref{421} p.~\pageref{421} will be inserted here.]
...
Let $X$ be a site, let $k$ be a commutative ring, let $R$ be a $k$-algebra, let $M$ and $N$ be $R$-modules, let $\U\subset\V$ be universes such that $X,k,R,M$ and $N$ are $\U$-small, and let $p$ be a nonnegative integer. We use the superscripts $\U$ and $\V$ to indicate the dependence on $\U$ or $\V$. In particular, we have the constant sheaves of $k$-modules $R_X^\U,M_X^\U,N_X^\U,R_X^\V,M_X^\V,N_X^\V$ over $X$. 

%[If an entity $\Xi$ depends on $\U$ or $\V$, we denote it by $\Xi^\U$ or $\Xi^\V$.]
...
%To each pair $(X,k)$, where $X$ is a site and $k$ a commutative ring, is attached a $k$-algebra $R:=\Hom_{\D(k_X)}(k_X,k_X)$ well-defined up to isomorphism. 

%Indeed, we can define $R$ by choosing a universe $\U$ such that $R\in\U$ and $\C_X$ is $\U$-small, and \S\ref{14112} p.~\pageref{14112} shows that, up to isomorphism, $R$ does not depend on the choice of $\U$.
\end{s}
