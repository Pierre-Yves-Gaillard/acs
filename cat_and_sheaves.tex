...
$$ 
where $(Y\to Y')\in\SSS$ and $X\to Y'$ are attached to the identity of $X$. We leave it to the reader to check that there is a (unique) map $g$ as in \qr{cfg} such that 
$$
g(r)=b[Y\to Y'](X\to Y')
$$ 
...
\al(X\to X')\big(a[X',Y\to Y'\,](X'\to Y')\big)=a[X,Y\to Y'\,](X\to X'\to Y'),
$$
where $\al$ is the functor defined in \qr{al}. But the above equality holds by the definition of $\al(X\to X')$. This proves the claim. We get
$$
