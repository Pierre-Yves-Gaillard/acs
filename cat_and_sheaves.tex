% about "categories and sheaves" (version avo)
...
Theorem~\ref{qtot} clearly implies Theorem~\ref{zorn}. Lemma~\ref{945} below will shows that Theorem~\ref{942} follows also from Theorem~\ref{qtot}. 
...
%
\begin{proof}
We assume, as we may, that $I$ is nonempty, well-ordered, and admits a maximum $m$. We shall complete the following Task $(T_i)$ by transfinite induction on $i\in I$: 

\nn[Beginning of the description of Task $(T_i)$.] Construct, for each $i$ in $I$, a commutative diagram 
$$
\begin{tikzcd}
X_i\ar{rr}{u_i}\ar{d}[swap]{f_i}&&Y_i\ar{d}{h_i}\\
X'\ar{r}[swap]{v_i}&Y'_{<i}\ar{r}[swap]{w_i}&Y'_i,
\end{tikzcd}
$$
with $v_i,w_i\in\Mor(\C_0)$, for each $(i,j)$ in $I^2$ with $i>j$, construct a commutative diagram 
$$
\begin{tikzcd}
X'\ar{r}{v_i}\ar[equal]{d}&Y'_{<i}\ar{r}{w_i}&Y'_i\\
X'\ar{r}[swap]{v_j}&Y'_{<j}\ar{r}[swap]{w_j}&Y'_j,\ar{ul}[swap]{p_{ij}}
\end{tikzcd}
$$
with $p_{ij}\in\Mor(\C_0)$, and for each $(i,j,k)$ in $I^3$ with $i>j>k$, construct a commutative diagram 
$$
\begin{tikzcd}
...
p_{ij}\circ w_j\circ p_{jk}=p_{ik}\quad\forall\quad k<j\le i.
\end{equation}
