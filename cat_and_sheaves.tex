% about "categories and sheaves" (version-0375)
% old version:
...
\begin{s}
P.~43, end of Section~2.1. One could add the following observation, which will be used in \S\ref{17115ps} p.~\pageref{17115ps}: 

If $\A$ and $\C$ are categories, and, if, for each $X$ in $\C$, we denote by $\oo j^X$ the forgetful functor from $\C^X$ to $\C$, then we have
%
\begin{equation}\label{17115mod}
\Hom_{\oo{Fct}(\C,\A)}(F,G)\simeq\lim_{X\in\C}\Hom_{\oo{Fct}(\C^X,\A)}(F\circ\oo j^X,G\circ\oo j^X).
\end{equation}

The following point is implicit in the definition of the right hand side of \eqref{17115mod}: To a morphism $f:X\to Y$ in $\C$ we attach a map 
$$
\Phi(f):\Hom_{\oo{Fct}(\C^Y,\A)}(F\circ\oo j^Y,G\circ\oo j^Y)\to\Hom_{\oo{Fct}(\C^X,\A)}(F\circ\oo j^X,G\circ\oo j^X)
$$ 
by sending $\theta:F\circ\oo j^Y\to G\circ\oo j^Y$ to the horizontal composition $\theta*(\circ f)$ (see Definition~\ref{dil1} p.~\pageref{dil1}): 
$$
\begin{tikzcd}
%
\A&{}&\C^Y\ar{ll}{}[near start,swap]{G\circ\oo j^Y}&{}&\C^X\ar{ll}{}[near start,swap]{\circ f}\\ 
%
\A&\ar{u}{\theta}&\C^Y\ar{ll}{}[near start]{F\circ\oo j^Y}&\ar{u}{\circ f}&\C^X.\ar{ll}{}[near start]{\circ f}
%
\end{tikzcd}
$$ 

Let us spell out the proof of \eqref{17115mod}: 

Let $L$ and $R$ be the left and right-hand sides of \eqref{17115mod}. 

The maps $\ell\mapsto\ell*\oo j^X$, where $*$ denotes horizontal composition (see Definition~\ref{dil1} p.~\pageref{dil1}), induce a map $f:L\to R$:
$$
\begin{tikzcd}
%
\A&{}&\C\ar{ll}{}[near start,swap]{G}&{}&\C^X\ar{ll}{}[near start,swap]{\oo j^X}\\ 
%
\A&\ar{u}{\ell}&\C\ar{ll}{}[near start]{F}&\ar{u}{\oo j^X}&\C^X.\ar{ll}{}[near start]{\oo j^X}
%
\end{tikzcd}
$$  
Concretely, to a family 
$$
\ell=\left(F(X)\xr{\ell_X}G(X)\right)_X\in L
$$
we attach the family 
$$
r:=\left(F(Y)\xr{\ell_Y}G(Y)\right)_{X,y:X\to Y}\in R.
$$

Conversely, to a family 
%
\begin{equation}\label{r_X}
r=(r_X)_X=\Big(\big(r_{X,y:X\to Y}:F(Y)\to G(Y)\big)_{y:X\to Y}\Big)_X\in R
\end{equation}
% 
we attach the family
%
\begin{equation}\label{rid_X}
\ell:=\big(r_{\id_X:X\to X}:F(X)\to G(X)\big)_X.
\end{equation}
% 
We easily check that the assignment $r\mapsto\ell$ defines a map $g:R\to L$, and that $f$ and $g$ are mutually inverse bijections. The key point is that a family 
$$
\Big(\big(r_{X,y:X\to Y}:F(Y)\to G(Y)\big)_{y:X\to Y}\Big)_X
$$ 
as above belongs to $R$ if and only if, for all diagram 
$$
X\xr fY\xr zZ
$$ 
in $\C$, we have  
%
\begin{equation}\label{Y,z:Yto Z}
r_{Y,z:Y\to Z}\circ F(z)=G(z)\circ r_{X,f:X\to Y}.
\end{equation}
%
...
\begin{s}\label{737}
P.~161. We paste Display (7.3.7), which appears in Proposition 7.3.3 (iii) on p.~161 of the book:
%
\begin{equation}\label{(7.3.7)}
(R_{\cc S}F)(Q(X))\simeq\colim_{(X\to Y)\in\cc S^X}F(Y).
\end{equation}
%
(See \eqref{rsfq} p.~\pageref{rsfq}.) 
Let $\C$ be a $\U$-category (Definition~\ref{myucat} p.~\pageref{myucat}), and let $\V$ be a universe such that $\U\in\V$ and $\C$ is a $\U$-small category (Definition~\ref{myucat} p.~\pageref{myucat}). Writing $\A$ for the category of $\V$-sets, Proposition 7.3.3 (iii) of the book implies the following:
...
In this section we adhere to Convention 11.7.1 of the book, according to which, paradoxically, $Y$ is considered as the first variable and $X$ is as the second variable in the expression $\Hom_\C(X,Y)$.
...
\nn More precisely, the diagram 
$$
\begin{tikzcd}
R_{\cc S}\big(\Hom_\C(X,\ \ )\big)(Y)\ar[leftrightarrow]{dr}\ar{r}&R_{{\cc S}\times{\cc S}^{\op}}\Hom_\C(X,Y)\ar[leftrightarrow]{d}&R_{{\cc S}^{\op}}\big(\Hom_\C(\ \ ,Y)\big)(X)\ar{l}\ar[leftrightarrow]{dl}\\ 
&\Hom_{\C_{\cc S}}(X,Y),
\end{tikzcd}
$$
where the horizontal arrows are the natural morphisms, and the other arrows are the above isomorphisms, commutes.
...
\begin{s}
P.~337. (See \S\ref{1341} p. \pageref{1341} and \S\ref{q337} p.~\pageref{q337}.) In view of Section~\ref{745} p.~\pageref{745} and Theorem 13.4.1 p.~337 of the book, the functors 
...
\nn give rise to the following commutative diagram of functorial isomorphisms:
$$
\begin{tikzcd}
R^0(\Hom_\C^\bu(X,\ \ ))(Y)\ar[leftrightarrow]{d}\ar{r}&
R^0\Hom_\C^\bu(X,Y)\ar[leftrightarrow]{d}&
R^0(\Hom_\C^\bu(\ \ ,Y))(X)\ar{l}\ar[leftrightarrow]{d}\\ 
%
R(\Hom_{\oo K(\C)}(X,\ \ ))(Y)\ar[leftrightarrow]{dr}\ar{r}&
R\Hom_{\oo K(\C)}(X,Y)\ar[leftrightarrow]{d}&
R(\Hom_{\oo K(\C)}(\ \ ,Y))(X)\ar{l}\ar[leftrightarrow]{dl}\\ 
%
&\Hom_{\oo D(\C)}(X,Y),
\end{tikzcd}
$$
where the horizontal arrows are the natural morphisms and the other arrows are the natural isomorphisms, and where 
$$
R(\Hom_{\oo K(\C)}(X,\ \ ))(Y),\quad R\Hom_{\oo K(\C)}(X,Y),\quad R(\Hom_{\oo K(\C)}(\ \ ,Y))(X)
$$ 
are defined by Notation 10.3.8 p.~255 of the book. This implies 
...
\Hom_{\oo{PSh}(X,\A)}(F,G)\simeq\lim_{U\in\C_X}\cc Hom_{\oo{PSh}(X,\A)}(F,G)(U).
$$
This follows from \eqref{17115mod} p.~\pageref{17115mod}. 
\end{s} 
