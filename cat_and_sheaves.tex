...
\col\bt\iso\col_i\ \col_{j,u}\bt(j),
\end{equation} 
where $(j,u)$ runs over $J_i$, with $u:\pp(j)\to i$. 

\nn\textbf{First proof of \qr{coco}}

\nn We shall define maps 
$$
\begin{tikzcd} 
\col\bt\ar[yshift=0.7ex]{r}{f}&\ds\col_i\ \col_{j,u}\bt(j),\ar[yshift=-0.7ex]{l}{g}
\end{tikzcd}
$$ 
and leave the verification that $f$ and $g$ are inverse isomorphisms to the reader. We have coprojections 
$$
\bt(j)\xr{p_j}\col\bt,\quad\bt(j)\xr{q_{iju}}\col_{j,u}\bt(j)\xr{r_i}\col_i\ \col_{j,u}.
$$ 
The definition of $f$ is obvious. To define $g$, consider the diagram
\begin{equation}\lb{biju}
\begin{tikzcd}
\bt(j)\ar{d}[swap]{q_{iju}}\ar{r}{\id}&\bt(j)\ar{d}{p_j}\\ 
\ds\col_{i,u}\bt(j)\ar{d}[swap]{r_i}\ar{r}{g_i}&\col\bt\ar{d}{\id}\\ 
\ds\col_i\col_{j,u}\bt(j)\ar{r}[swap]{g}&\col\bt.
\end{tikzcd}
\end{equation} 
We let $i$ be in $I$ and define $g_i$ by the condition that the top square of \qr{biju} commutes for all $(j,u)$. Then we define $g$ by the condition that the bottom square of \qr{biju} commutes for all $i$. q.e.d.
...
This section is divided into two parts. In Part 1 we give a proof of Proposition 3.4.3 (i) which is slightly different from the one in the book. In Part 2 we give two proofs of \qr{coco} p.~\pr{coco}. 
...
Let  
$$
\al(i,j,u)\xr{p_{iju}}\col\al,\quad\al(i,j,u)\xr{q_{iju}}\col_{i,u}\al(i,j,u)\xr{r_j}\col\al
$$ 
...
In \S\ref{2111} p.~\pr{2111} we gave a first proof of \qr{coco} p.~\pr{coco}. Here are two other proofs.
