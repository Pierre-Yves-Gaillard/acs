% about "categories and sheaves" (version-0202)
...
$$ 
be the natural functor (see \eqref{slice} p.~\pageref{slice} for the definition of $\C/G(a)$ and recall that $\ic\alpha_k=G(k)$); put 
$$
...
$$ 
(see \eqref{slice} p.~\pageref{slice} for the definition of $L/a$) by 
$$
\varphi((i_\ell),Q)\left(b\xr f a\right):=\left(Q(b)\xr{u'(b)} G(b)\xr{G(f)}G(a)\right),
$$
where $u'(b)$ is defined in \eqref{u':} p.~\pageref{u':}; and let 
$$
