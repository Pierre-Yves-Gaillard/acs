% about "categories and sheaves" (version-0321)
% old version:
...
\end{equation}
% 
be morphisms in an abelian category $\C$ (we do not assume $g\circ f=0$). The commutative diagram 
$$
\begin{tikzcd}
...
\begin{proof} 
Our first claim is that the diagram of solid arrows 
$$
\begin{tikzcd}
...
\end{tikzcd}
$$ 
whose rows are exact sequences, can be completed as indicated. Indeed, the existence of $b$ follows from the vanishing of $g\circ f\circ a$. This proves the first claim. To prove the lemma, it is enough to check that $c\circ b$ factors through $\Ima g$, or, equivalently, that $c\circ b=0$. As $d$ is an epimorphism, the vanishing of $c\circ b$ is equivalent to the vanishing of $c\circ b\circ d$. But we have $c\circ b\circ d=c\circ g\circ f=0\circ f=0$. 
\end{proof} 
...
$$
\Ker\big(F(X)\to F(X'')\big)\simeq\Ker\big(F(X)\to F(I)\big)\simeq\Ima\big(F(X')\to F(X)\big),
$$ 
...
$$
\Ker\big(F(X)\to F(X'')\big)\simeq\Ker\big(F(X)\to F(I_g)\big)\simeq\Ima\big(F(K_g)\to F(X)\big),$$ 
the isomorphisms following respectively from the exactness of 
$$
0\to F(I_g)\to F(X'')\quad\text{and}\quad F(K_g)\to F(X)\to F(I_g),
$$ 
it suffices to observe that, in view of the exactness of $F(X')\to F(K_g)\to0$, the isomorphism
$$
\Ima\big(F(K_g)\to F(X)\big)\simeq\Ima\big(F(X')\to F(X)\big)
$$
results from Lemma~\ref{817b1} p.~\pageref{817b1} with $F(X')\to F(K_g)\to F(X)$ instead of \eqref{e817} p.~\pageref{e817}. 
\end{proof} 
...
It only remains, in order to prove Proposition~\ref{932}, to check that $\lambda$ is surjective. Let $f:A\to X$ be a morphism. Claim~\ref{pig=f} below will imply the surjectivity of $\lambda$, and thus the truth of Proposition~\ref{932}. 
...
\nn$*$ Step 5: (a) implies (b). Assuming (a), put $S:=\Ima(B(G)\to A(G))\subset A(G)$ and $C:=G^{\sqcup S}$, so that we have maps $B(G)\to S\to A(G)$. By Step~1 we have $\card(C(G))<\pi$. The vertical arrows of the commutative diagram 
...
\nn$*$ Step 6: (b) implies Claim~\ref{pig=f} p.~\pageref{pig=f}, and thus Proposition~\ref{932} p.~\pageref{932}. Assuming (b), put $P:=C\times_AC$. Epimorphisms in $\C$ being strict, the sequence $P\parar C\xr aA$ is exact. On the other hand, we have $P(G)\le\card(C(G))^2<\pi$. By Step~2 the natural map 
$$
...
\end{tikzcd}
$$ 
The exactness of $P\parar C\xr aA$ implies the existence of a morphism $g:A\to X_i$ such that 
%
...
As already indicated, Claim~\ref{pig=f} implies Proposition~\ref{932}.
\end{proof} 
...
\begin{df}[Definition 17.7.8, Grothendieck topology]\label{1778} 
