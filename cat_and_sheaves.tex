...
$$ 
and thus $f\ci g\neq f\ci h$. Hence, $f$ is a monomorphism. %\begin{uspb}\index{unsolved problem}I don't know if the natural morphism $f:\bb N'\to\bb N$ is an epimorphism.\end{uspb} 

Claim: $f$ is \emph{not} an epimorphism. 

Proof 1: Use Theorem~\ref{si} p.~\pr{si} below. 
...
\end{tikzcd}
$$ 
satisfy $p_n\ci f_X=p_n\ci g_X$, there is an $m>n$ such that the diagram 
$$
\begin{tikzcd}
...
\begin{lem}\lb{ci}
If $\C$ is a category admitting finite inductive and projective limits, and if $I$ is a small filtrant category, then the functor $ \ic:\C^I\to\Ind(\C)$ is exact. 
\end{lem} 

\begin{proof} 
Note that $\C^I$ admits finite inductive and projective limits. Let $h:\C\to\C^\wg$ be the Yoneda embedding and $\iota:\C\to\Ind(\C)$ the natural embedding. If $\al$ is in $\C^I$, then $\ic\al$ can be defined as $\col h\ci\al$ or as $\col\iota\ci\al$ (Theorem 6.1.8 p.~132 in the book). Let $J$ be a finite category. Let $\al:I\tm J^{\op}\to\C$ be a functor. We claim 
$$
\ic_i\lim_j\al(i,j)\xr\sim\col_i h\left(\lim_j\al(i,j)\right). 
$$ 
Clearly $\lim_j$ commutes with $h$. As $\lim_j$ commutes also with $\col_i$ as far as $\C^\wg$-valued functors are concerned, $\ic_i$ commutes with $\lim_j$, and the claim is proved. Let $\al:I\tm J\to\C$ be a functor. We claim 
$$
\ic_i\col_j\al(i,j)\xr\sim\col_i\iota\left(\col_j\al(i,j)\right). 
$$ 
The functor $\iota$, being right exact (Corollary 6.1.6 p.~132 in the book), commutes with $\col_j$. As $\col_j$ commutes with $\col_i$ for obvious reasons, it commutes with $\ic_i$, and the claim is proved.
\end{proof}
...
\begin{prop}[Proposition 6.1.16 (i) p.~136] 
...
Let $I,J$ and $\C$ be categories. Assume that $I$ is small and filtrant, and that $J$ is finite. 

\begin{lem}%1
If $\C$ admitting finite projective limits, and if $\al:I\tm J^{\op}\to\C$ is a functor, then we have a natural isomorphism 
$$
\lim_j\ic_i\al(i,j)\iso\ic_i\lim_j\al(i,j)
$$ 
in $\Ind(\C)$. 
\end{lem}

\begin{proof} 
This follows from Corollary 6.1.17 (i) p.~136 in the book. 
\end{proof}

\begin{lem}%2
If $\C$ admitting finite inductive limits, and if $\al:I\tm J\to\C$ is a functor, then we have a natural isomorphism 
$$
\col_j{}^{\Ind(\C)}\ic_i\al(i,j)\iso\ic_i\col_j\al(i,j) 
$$ 
in $\Ind(\C)$, where the notation $\ds\col_j{}^{\Ind(\C)}$ indicates that the colimit is taken in $\Ind(\C)$ (not in $\C ^\wg$).
\end{lem} 

\begin{proof} 
This follows from the proof of Proposition 6.1.18 p.~136 in the book. 
\end{proof} 

For any morphism $f$ in any category admitting finite inductive and projective limits, we write $[f]$ for the induced morphism $\Coim f\to\Ima f$. 

\begin{proof}[Proof of Theorem~\ref{si}] 
Let $f:A\to B$ be a morphism in $\Ind(\C)$. It suffices to show that $[f]$ is an isomorphism. In view of Corollary 6.1.15 p.~135 in the book, we can assume that there is a small filtrant category $I$, a pair of functors $\al,\bt:I\parar\C$ and a morphism of functors $\pp:\al\to\bt$ such that 
$$
A=\ic\al,\quad B=\ic\bt,\quad f=\ic\pp.
$$ 
The above two lemmas imply $[\ic\pp]\iso\ds\ic_i[\pp_i]$. As $[\pp_i]$ is an isomorphism for all $i$, we see that $[f]=[\ic\pp]$ is an isomorphism.
\end{proof}
...
\begin{proof}[Proof of Theorem~\ref{si}] 
Let $f:A\to B$ be a morphism in $\Ind(\C)$. By Corollary 6.1.14 p.~135 in the book, there is a small filtrant category $I$ and a morphism $\pp$ in $\C^I$ such that $\ic\pp\iso f$. Clearly $\C^I$ is strict, and the theorem follows now from the above lemma. 
\end{proof}
...
\nn$\star\ \pp_X$ is injective. Let $u,v\in\Hom_\C(X,Z)$ satisfy $\pp\ci u=\pp\ci v$, that is $$q_{i_0}\ci\tau\ci u=q_{i_0}\ci\tau\ci v.$$ As $I$ is filtrant and as $\Hom_\C(X,A)=\col\Hom_\C(X,\al)$, there is a morphism $s:i_0\to i$ such that $\al(s)\ci\tau\ci u=\al(s)\ci\tau\ci v$. We have $q_i\ci\al(s)\ci\tau\ci u=q_{i_0}\ci\tau\ci u=\pp\ci u$, and, similarly, $q_i\ci\al(s)\ci\tau\ci v=\pp\ci v$, yielding 
$$
