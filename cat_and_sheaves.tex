% about "categories and sheaves" (version avs)
...
\begin{comment}
\begin{s}
Proposition 9.3.2 p.~224. Recall the statement:
%
...
\end{prop}
%
Here are more details about the proof: 
...
%
We state the proposition and make a few comments about its proof.
...
Conditions (i)--(v) hold; $\pi_0$ and $\pi_1$ are as above; and $\pi$ is the successor of $2^{\pi_1}$.
\end{cond}
%
...
$$
Since $\card(G^{\coprod B}(G))<\pi$ for all $B$ in $I$ by (e), this implies \eqref{934b}, and Proposition~\ref{932} entails $X\in\C_\pi$.
\end{proof}
%
%%%
%
...
We recall the notion of {\em construction by transfinite induction}. We recall it first in the setting of Bourbaki's set theory, to which, as already mentioned, we adhere. Then we shall phrase this concept in a way which does not involve explicitly Bourbaki's set theory (see Metatheorem~\ref{meta2} below.)

\begin{mt}[Construction by Transfinite Induction]\label{meta}
Let $T$ be a term, let $x$ be a letter, for each term $X$, write $T(X)$ for the term obtained by substituting $X$ for $x$ in $T$, and let $I$ be a well-ordered set. Then there is a unique pair $(S,f)$ such that $S$ is a set, $f:I\to S$ is a surjection, and we have $f(i)=T(f(j)_{j<i})$ for all $i$ in $I$. 
\end{mt}

\begin{proof}
Uniqueness: Assume that $(S,f)$ and $(U,g)$ have the indicated properties. It suffices to prove $f(i)=g(i)$ for all $i$ in $I$. Suppose this is false, and let $i$ be the least element for such that $f(i)\neq g(i)$. We have 
$$
f(i)=T(f(j)_{j<i})=T(g(j)_{j<i})=g(i),
$$ 
a contradiction. 

Existence: We may assume that $I$ is nonempty. Let $i$ be in $I$. We first prove that the metatheorem holds with $I_{\le i}$ (obvious notation) instead of $I$, {\em i.e.}, we prove that there is a pair $(S_i,f_i)$ such that $S_i$ is a set, $f_i:I_{\le i}\to S_i$ is a surjection, and we have $f_i(j)=T(f_i(k)_{k<j})$ for all $j\le i$. This is clear if $i=0$, where $0$ is the minimum of $I$. So, suppose $i>0$. We assume, as we may, that the statement holds for $I_{\le j}$ when $j<i$. For $j<i$ set $f_i(j):=f_j(j)$, put $f_i(i):=T(f_i(j)_{j<i})$, and set $S_i:=\{\,f_i(j)|\,j\le i\}$. As we have $f_j(\ell)=f_k(\ell)$ for all $\ell\le k<j<i$ by the uniqueness part, the pair $(S_i,f_i)$ satisfies the required conditions. Observing that $i$ is an arbitrary element of $I$, we can set $S:=\{\,f_i(i)|\,i\in I\}$ and $f(i):=f_i(i)$. It follows again from the uniqueness part that the pair $(S,f)$ satisfies the required conditions. 
\end{proof}

We now give, as promised, the second wording of the notion of construction by transfinite induction (the proof is the same).

\begin{mt}[Construction by Transfinite Induction]\label{meta2} 
Let $\U$ be a universe, let $F:\U\to\U$ be a map, and let $I$ be a well-ordered $\U$-set. Then there is a unique pair $(S,f)$ such that $S$ is a set, $f:I\to S$ is a surjection, and we have $f(i)=F(f(j)_{j<i})$ for all $i$ in $I$.  
\end{mt}
...
%
\begin{proof}
We assume, as we may, that $I$ is nonempty, well-ordered, and admits a maximum $m$. We shall complete the following Task $(T_i)$ by transfinite induction on $i\in I$ (see Metatheorem~\ref{meta} p.~\pageref{meta}): 
...
p_{ij}\circ w_j\circ p_{jk}=p_{ik}\quad\forall\quad i\ge j>k.
\end{equation}
...
In the above setting, for any $X$ in $\C_0$ there is a $\Mor(\C_0)$-injective object $Y$ of $\C$, and morphism $f:X\to Y$ in $\C_0$.
\end{thm}
%
\begin{rk}\label{954}
