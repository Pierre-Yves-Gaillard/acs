...
\begin{s} 
Proposition 8.2.13 p.~173. Here is a slightly different wording: 
\begin{prop}
Let $\C$ be an additive category, let $F:\C\to\Set$ be a functor commuting with finite products, and for each $X$ in $\C$ denote the composition 
$$
F(X)\tm F(X)\xleftarrow\sim F(X\oplus X)\xr{F(\sigma_X)}F(X)
$$ 
by $+_X$. Then we have 
