% about "categories and sheaves" (version 0009)
...
We shall define vertical composition \index{vertical composition} of morphisms of functors, denoted by $\circ$, and horizontal composition \index{horizontal composition} of morphisms of functors, denoted by $*$, and prove 
%
...
$$ 
(See (\ref{convnot}) p.~\pageref{convnot} for an explanation of the notation.) By Corollary~\ref{c233i} p.~\pageref{c233i}, both sets are in natural bijection with the quotient of 
$$
...
$$ 
for all $Z$ in $\C$. Moreover, we regard $(\Coim f)(Z)$ as a subset of $\Hom_\C(X,Z)$, and $(\Ima f)(Z)$ as a subset of $\Hom_\C(Z,Y)$.
\end{df} 
...
%We give a definition of strict epimorphism which, although slightly different, will turn out to be equivalent to that of the book:
...
By Proposition \ref{epimono} p.~\pageref{epimono} this condition does not depend on the universe $\U$ making $\C$ a $\U$-category. The following lemma is obvious:

\begin{lem}\label{strepi}
A strict epimorphism is an epimorphism. Also, $f$ is a strict epimorphism if and only if, for all $Z$ in $\C$, the map $\circ f:\Hom_\C(Y,Z)\to\Hom_\C(X,Z)$ induces a bijection $\Hom_\C(Y,Z)\xr\sim(\Coim f)(Z)$.
\end{lem} 
...
Let $T_Z$ be the set of those morphisms $v:X\to Z$ such that $[P\parar X\xr vZ]$ (see Convention \ref{bra} p.~\pageref{bra}). It is enough to show $(\Coim f)(Z)=T_Z$.

Let $u$ be in $(\Coim f)(Z)$. As $[P\parar X\xr fY]$ by definition of $P$, we have $[P\parar X\xr uZ]$ by Proposition~\ref{epimono} p.~\pageref{epimono}, so that $u$ is in $T_Z$. Let $v$ be in $T_Z$, and let $v_1,v_2:W\parar X$ satisfy $[W\parar X\to Y]$. It suffices to prove $v\circ v_1=v\circ v_2$. The commutative diagram  
$$
\begin{tikzcd}
...
\begin{s}
P. 118, proof of Prop. 5.2.3 (v). We add a few details to the last four lines of the proof.

Put $\pp:=\pp_G$. Let $f_i:Y_i\mono X$ $(i=1,2)$ be two monomorphisms such that $\pp(f_1)$ and $\pp(f_2)$ have same image. We have a cartesian square
$$
\begin{tikzcd}
...
\end{tikzcd}
$$ 
the morphism $\pp(Y_1\times_XY_2)\to\pp(Y_1)\times_{\pp(X)}\pp(Y_2)$ being an isomorphism by left-exactness of $\pp$. As $\pp(f_1)$ and $\pp(f_2)$ have same image, $q_1$ and $q_2$ are bijective. The diagram being commutative, $\pp(p_1)$ and $\pp(p_2)$ are also bijective. As $\pp$ is conservative,  $p_1$ and $p_2$ are indeed isomorphisms.
\end{s}
...
\end{tikzcd}
$$ 
quasi-commutes. As $\lambda$ and $\lambda'$ are fully faithful by Theorem 5.3.6 p.~124 (stated at the beginning of Section~\ref{536} p.~\pageref{536}) and Remark 1.4.13 p.~27, and $\widehat\varphi_{{}_0}$ is an equivalence (a quasi-inverse being $\widehat\psi_{{}_0}$), the proof is complete.
\end{proof}
\end{s}
%
%%
...
%
\subsection{Definition of a Triangulated Category (p.~243)\index{triangulated category}}
%
