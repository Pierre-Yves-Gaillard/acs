% about "categories and sheaves" (version_0040)
...
\begin{prop}
Let $\alpha:I\to\C$ be a functor, and let $\Gamma(\U)$ be the condition ``$\U$ is a universe, $I$ is $\U$-small, and $\C$ is a $\U$-category''. If $\C$ is a $\U$-category, we write $h_\U$ for the Yoneda embedding from $\C$ to $\C^\wedge_\U$. Let $X$ be an object of $\C$ and $F:\C\to\C'$ a functor. Then we have:
...
\begin{df}\label{uil}
If, in the above setting, the assumptions in {\em(c)} above hold, we say that $X$ is a {\em universal} inductive limit \index{universal inductive limit} of $\alpha$.
\end{df}
\end{s}
