% about "categories and sheaves" (version_0072)
...
\begin{s}
P. 391. I am unable to derive GT4 from LE1-LE4, but it seems to me the only fact really used in the sequel of the book is that GT1-GT4 imply LE1-LE4. Here are some details about this implication, starting with a few reminders:

We assume that the hom-sets of $\C$ are disjoint. Let $M$ be the set of morphisms of $\C$, and for each $U$ in $\C$ let $M_U\subset M$ be the set of morphisms whose target is $U$. A subset $S$ of $M_U$ is a {\em sieve} over $U$ if it is a right ideal of $M$, in the sense that $S$ contains all the morphisms of the form $s\circ f$ with $s$ in $S$. If $S$ is a sieve over $U$ and $f:V\to U$ is a morphism, we put
$$
S\times_UV:=\{W\to V|(W\to V\to U)\in S\}.
$$
One easily checks that this is a sieve over $V$. 

To a sieve $S$ over $U$ we attach a subobject $A_S$ of $U$ in $\C^\wedge$ by the formula
$$
...
Let $\Sigma_U$ be the set of sieves over $U$, and let $\Sigma$ be the set of all sieves in $\mathcal C$. Let $\Gamma$ be a subset of $\Sigma$, put $\Gamma_U:=\Gamma\cap\Sigma_U$, and consider the following conditions:

\nn GT1: for all $U$ in $\C$ we have: $M_U\in\Gamma$,
...
$$
S\in\Sigma_U,\ S'\in\Gamma_U,\ S\times_UV\in\Gamma\ \forall\ (V\to U)\in S'\implies S\in\Gamma_U.
$$
...
Let $\Gamma$ be a subset of $M$ satisfying GT1-GT4, and let $\cc E$ be the set of those morphisms $A\to B$ in $\C^\wedge$ such that, for any morphism $U\to B$ in $\C^\wedge$ with $U$ in $\C$, the sieve $S_{A\times_BU}$ is in $\Gamma_U$. Let us check that $\cc E$ satisfies LE1-LE4.

\nn LE1 follows immediately from GT1.

\nn LE2: Let $A\to B\to C$ be a diagram in $\C^\wedge$, and assume that the two arrows are in $\cc E$. Consider the commutative diagram of solid arrows with cartesian squares 
$$
\begin{tikzcd}
...
\end{tikzcd}
$$
in $\C^\wedge$ (with $U$ in $\C$). We have $S_E\in\Gamma_U$ (because $B\to C$ is in $\cc E$) and we must prove that $S_D$ is in $\Gamma_U$. Let $V\to U$ be in $S_E$, and let us complete the commutative diagram with cartesian squares as indicated. By GT4 it suffices to check that $S_F$ is in $\Gamma_V$. But this follows from the assumption that $A\to B$ is in $\cc E$ (together with a transitivity property of cartesian squares which has already been tacitly used).

\nn LE3 follows immediately from GT2.

\nn LE4 results easily form the following observation: In the setting $A\to U\leftarrow V$ (obvious notation), we have the magic formula 
$$
...
[\S\ref{revol} p.~\pageref{revol} %and \S\ref{421} p.~\pageref{421} 
...
[Section~\ref{s1853} p.~\pageref{s1853}, \S\ref{1811} p.~\pageref{1811}, and \S\ref{1867} p.~\pageref{1867} will be inserted here.]
...
Let $X$ be a site, let $k$ be a commutative ring, let $R$ be a $k$-algebra, let $M$ and $N$ be $R$-modules, let $\U$ and $\V$ be universes such that $X,k,R,M$ and $N$ are $\U$-small and $\V$-small, and let $p$ be a nonnegative integer. We use the superscripts $\U$ and $\V$ to indicate the dependence on $\U$ or $\V$. In particular, we have the constant sheaves (see \S\ref{1811} p.~\pageref{1811}) of $k$-modules $R_X^\U,M_X^\U,N_X^\U,R_X^\V,M_X^\V,N_X^\V$ over $X$. 

\begin{thm}[Change of Universes]\index{change of universes}\label{cou}
There is a canonical isomorphism of $k$-modules 
$$
\Ext^p_{R_X^\U}(M_X^\U,N_X^\U)\simeq\Ext^p_{R_X^\V}(M_X^\V,N_X^\V).
$$
\end{thm}

\begin{proof}
We may assume $\U\subset\V$. By the first display on p. 421 of the book, we have 
$$
M_X^\U(U)\simeq\co_{A\in\cc{LI}^\U_U}M\simeq M^{\oplus\pi_0(\cc{LI}^\U_U)},
$$ 
and similarly with $\V$ instead of $\U$. On the one hand, there is an obvious functor from $\cc{LI}^\U_U$ to $\cc{LI}^\V_U$. On the other hand, if $s:A\to U$ is in $\cc{LI}^\V_U$, then, by Lemma 16.2.3 (i) p.~395 of the book, $\Ima s\to U$ is in $\cc{LI}^\U_U$. This shows that we can identify the sets $\pi_0(\cc{LI}^\U_U)$ and $\pi_0(\cc{LI}^\V_U)$. As a result, we can, with a slight abuse of notation, write 
$$
M_X:=M_X^\U=M_X^\V,
$$ 
and similarly with $R$ or $N$ instead of $M$. Note that $\Mod^\U(R_X)$ is a Grothendieck $\U$-category, that $\Mod^\V(R_X)$ is a Grothendieck $\V$-category, and that $\Mod^\U(R_X)$ is a fully abelian subcategory of $\Mod^\V(R_X)$. Hence, the conclusion follows from \S\ref{14112} p.~\pageref{14112}.
\end{proof}
\end{s}
