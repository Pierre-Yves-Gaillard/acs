% about "categories and sheaves" (version aty)
...
%
\begin{s}
P. 231, proof of Lemma 9.5.3. Here are more details:

Let us represent Condition (9.5.2) (ii) by the diagram 
$$
\begin{tikzcd}
...
\nn$\bu$ an inductive system $(u_{ij}:Y'_j\to Y'_i)_{j<i}$ in $\C_0$,

\nn$\bu$ an inductive system $(v_i:X'\to Y'_i)_i$ in $(\C_0)^{X'}$, 
...
\end{tikzcd}
$$ 
Let $i>0$. By (9.5.2) (i), $Y'_{<i}:=\co_{j<i}Y'_i$ exists in $\C_0$. Let $p_{ij}:Y_j'\to Y'_{<i}$ be the coprojection. In particular we have a morphism $X'\xr{v_0}Y_0'\xr{p_{i0}}Y'_{<i}$ in $\C_0$, and we can define $w_i$ and $h_i$ via Condition (9.5.2) (ii): 
$$
\begin{tikzcd}
X_i\ar{d}[swap]{p_{i0}\circ v_0\circ f_i}\ar{r}{u_i}&Y_i\ar[dashed]{d}{h_i}\\ 
Y'_{<i}\ar[dashed]{r}[swap]{w_i}&Y_i'.
\end{tikzcd}
$$ 
Put $v_i:=w_i\circ p_{i0}\circ v_0$ for all $i$, and $u_{ij}:=w_i\circ p_{ij}$ for all $j<i$. One checks that $(u_{ij})$ and $(v_i)$ are indeed inductive systems. Set $Y':=\co_iY_i'$, let $p_i:Y_i'\to Y'$ be the coprojection, and put $g_i:=p_i\circ h_i$, $u':=p_1\circ w_1\circ p_{10}\circ v_0$. Finally we verify that we have indeed $g_i\circ u_i=u'\circ f_i$.
\end{s}
%
%%%
%
