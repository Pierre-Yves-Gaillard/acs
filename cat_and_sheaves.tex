% about "categories and sheaves" (version atw)
...
\nn We can assume that $I$ is nonempty and well-ordered. We shall construct by transfinite induction (letting $i$ and $j$ run over $I$):
...
\nn Let $i=0$ be the least element of $I$. We define $v_0$ and $h_0$ via Condition (9.5.2) (ii) (see \eqref{952ii}): 
$$
\begin{tikzcd}
...
%%%
%
\subsection{Theorems 9.5.4 and 9.5.5 (pp 232-234)}
%
...
\nn(9.5.2) (ii) For any $X,Y,X'\in\C_0$, any $u:X\to Y$ in $\Mor(\C_0)$, and any $f:X\to X'$ in $\Mor(\C)$, there exists a commutative diagram 
$$
\begin{tikzcd}
X\ar{d}[swap]{f}\ar{r}{u}&Y\ar{d}{g}\\ 
X'\ar{r}[swap]{u'}&Y'
\end{tikzcd}
$$ 
with $u'$ in $\Mor(\C_0)$ and any $g$ in $\Mor(\C)$. 
...
Line 6: Condition (9.5.2) (i) follows from 
%
