% about "categories and sheaves" (version-0373)
% old version:
...
The maps $\lambda\mapsto\lambda*\oo j^X$, where $*$ denotes horizontal composition (see Definition~\ref{dil1} p.~\pageref{dil1}), induce a map $f:L\to R$. Concretely, to a family 
$$
\left(F(X)\xr{\ell_X}G(X)\right)_X\in L
$$
we attach the family 
$$
\left(F(Y)\xr{\ell_Y}G(Y)\right)_{X,y:X\to Y}\in R.
$$
