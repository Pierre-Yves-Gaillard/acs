...
\lambda_{RLX,LX}\big(\mu_{X,LX}(\id_{LX})\big)\overset{\text{(b)}}{=}
\lambda_{RLX,LX}(\ee_X)\overset{\text{(c)}}{=}
...
$$\overset{(\text b)}\simeq h(\C_A)^\ddagger(F\circ j(\C_A))\left(\lambda\left(\icolim_{(U\to B)\in\C_B}(U\to A)\right)\right)$$

$$\overset{(\text c)}\simeq\colim_{(U\to B)\in\C_B}h(\C_A)^\ddagger(F\circ j(\C_A))(U\to A)$$

$$\overset{(\text d)}\simeq\colim_{(U\to B)\in\C_B}F(U),$$

\nn where (a) follows from \eqref{263a} p.~\pageref{263a}; (b) follows from Lemma~\ref{2113}; (c) follows from Proposition~\ref{271} and \eqref{leq}, \eqref{lcom}; and (d) follows from Proposition~\ref{271}. A similar computation shows 
$$
