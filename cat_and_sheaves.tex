...
\nn\href{https://goo.gl/ORIE5W}{https://goo.gl/ORIE5W}
...
The notation of the book will be freely used. We will sometimes write $\B^\A$ for $\Fct(\A,\B)$, $\alpha_i$ for $\alpha(i)$, $fg$ for $f\ci g$, and some parenthesis might be omitted. We write $\bigsqcup$\index{$\bigsqcup$} instead of $\coprod$\index{$\coprod$} for the coproduct\index{coproduct}. 
...
\nn$*$ P.~53, Part (i) (c) of the proof of Theorem 2.3.3 (Line 2): ``$\beta\in\Fct(J,\A)$'' should be ``$\beta\in\Fct(J,\C)$''.
...
\nn$*$ P.~133, proof of Proposition 6.1.8, Line 2: ``It is enough to show that $A$ belongs to $\Ind(\C)$''. More generally: Let $I\xr{\alpha}\C\xr{F}\C'$ be functors. Assume that $F$ is fully faithful, and that there is an $X$ in $\C$ such that $F(X)\iso\col F(\alpha)$. Then $X\iso\col\alpha$. The proof is obvious.
...
$$
``\ilim"(IF\ci\alpha)\xr\sim IF(``\ilim"\alpha)
$$
instead of 
$$
IF(``\ilim"\alpha)\xr\sim``\ilim"(IF\ci\alpha). 
$$ 
\end{s}
...
\nn$*$ P.~153, statement of Lemma 7.1.12. The readability might be slightly improved by changing $s:X\to X'\in\mc S$ to $(s:X\to X')\in\mc S$. Same for Line 4 of the proof of Lemma 7.1.21 p.~157.

\nn$*$ P.~156, first line of the first display and first line after the first display: $\C_{\cc S}$ should be $\C_{\cc S}^r$.

\nn$*$ P.~160, second line after the diagram: ``commutative'' should be ``commutative up to isomorphism''. Line 7 when counting from the bottom to the top: $F(s)$ should be $Q_{\mc S}(s)$.
...
\nn$*$ P.~229, proof of 9.4.3 (i): it might be better to write ``containing $\mc S$ strictly'' (or ``properly''), instead of just ``containing $\mc S$''. 
...
\nn$*$ P.~409, line 2: $\lambda\ci(\oo h_X^t)_A\iso\oo h_A$ should be $\lambda\ci(\oo h_X^t)_A\iso\oo h_A^t$. 
...
\nn$*$ P.~439, Proposition 18.2.3 (ii). Here is a slightly stronger statement: If $\cc{R,S,T}$ are $k_X$-algebras, if $F$ is a $(\cc T\otimes_{k_X}\R^{\op})$-module, if $G$ is an $(\R\otimes_{k_X}\cc S)$-module, and if $H$ is an $(\cc S\otimes_{k_X}\cc T)$-module, then there are isomorphisms 
$$
\Hom_{\cc S\otimes_{k_X}\cc T}(F\otimes_\R G,H)\iso
\Hom_{\R\otimes_{k_X}\cc S}(G,\HOM_{\cc T}(F,H)), 
$$ 
$$
\HOM_{\cc S\otimes_{k_X}\cc T}(F\otimes_\R G,H)\iso
\HOM_{\R\otimes_{k_X}\cc S}(G,\HOM_{\cc T}(F,H)), 
...
(viii) $U,V\in\U\then U\times V\in\U$,
...
$$
U\times V=\bigcup_{u\in U}\ \bigcup_{v\in V}\ \{(u,v)\}\in\U.\text{ q.e.d.} 
$$ 

Assume $U,V\in\U$, and let $V^U$ be the set of all maps from $U$ to $V$. As $V^U\in\mc P(U\times V)$, Statements (viii), (iv), and (ii) give
...
$\hfill\ds\{(a,b)\in\Hom_\C(X,U)\times\Hom_\C(Y,V)\ | \ g\ci a=b\ci f\},$\bigskip
...
Let $F:\C\to\Set$ be a functor and $X$ an object of $\C$. An $(F,X)$\--{\em co-universal element}\index{co-universal element} is an element $u$ of $F(X)$ such that, for all $Y$ in $\C$, the map $\Hom_\C(X,Y)\to F(Y),\ f\mapsto F(f)(u)$ is bijective. 
\end{df}
...
$$
\lambda_{X,X'}:\Hom_\C(X,RX')\to\Hom_{\C'}(LX,X')
$$ 
for $X$ in $\C$ and $X'$ in $\C'$. The diagram (1.5.6) can be written as 
\begin{equation}\lb{156}
\begin{tikzcd}
\Hom_{\C'}(X',Y')\ar{r}{R}\ar{dr}[swap]{\ci\lambda_{RX',X'}(\id_{RX'})}&\Hom_\C(RX',RY'),\ar{d}{\lambda_{RX',Y'}}\\ 
&\Hom_{\C'}(LRX',Y').
\end{tikzcd}
\end{equation} 
...
$$
\begin{tikzcd}
\Hom_\C(RX',RX')\ar{r}{\lambda_{RX',X'}}\ar{d}[swap]{R(f)\ci}&\Hom_{\C'}(LRX',X'),\ar{d}{f\ci}\\ 
\Hom_\C(RX',RY')\ar{r}[swap]{\lambda_{RX',Y'}}&\Hom_{\C'}(LRX',Y')
\end{tikzcd}
$$ 
commutes for $f$ in $\Hom_{\C'}(X',Y')$, we get in particular
$$
f\ci\lambda_{RX',X'}(\id_{RX'})=\lambda_{RX',Y'}(R(f)\ci\id_{RX'})=\lambda_{RX',Y'}(R(f)).
$$ 
...
$$
\lambda_{RLX,LX}(\id_{RLX})\ci L(\ee_X)=\id_{LX},
$$ 
and we have 
$$
\id_{LX}\overset{\text{(a)}}{=}
\lambda_{X,LX}\big(\mu_{X,LX}(\id_{LX})\big)\overset{\text{(b)}}{=}
\lambda_{X,LX}(\ee_X)\overset{\text{(c)}}{=}
\big(\lambda_{X,LX}\ci(\ci\ee_X)\big)(\id_{RLX})
$$
$$
\overset{\text{(d)}}{=}\Big(\big(\ci L(\ee_X)\big)\ci\lambda_{RLX,LX}\Big)(\id_{RLX})\overset{\text{(e)}}{=}\lambda_{RLX,LX}(\id_{RLX})\ci L(\ee_X),
$$ 
...
$$
\begin{tikzcd}
\Hom_\C(RLX,RLX)\ar{d}[swap]{\ci\ee_X}\ar{rr}{\lambda_{RLX,LX}}&&\Hom_{\C'}(LRLX,LX)\ar{d}{\ci L(\ee_X)}\\
\Hom_\C(X,RLX)\ar{rr}[swap]{\lambda_{X,LX}}&&\Hom_{\C'}(LX,LX),
\end{tikzcd}
$$ 
...
$$
such that $p$ is a $(\Hom_{\C^{I^{\op}}}(\Delta(\ ),\alpha),X)$\--universal element (see Definition~\ref{ue} p.~\pr{ue}). For each $i$ in $I$ the morphism $p_i:X\to\alpha(i)$ is called the $i$-{\em projection}\index{projection} of $X$. (We almost always write $X$ for $(X,p)$, the mental picture being that $p$ is a structure $X$ is equipped with.)
\end{df}

Recall that the condition that $p$ is a $(\Hom_{\C^{I^{\op}}}(\Delta(\ ),\alpha),X)$\--universal element means that for each $Y$ in $\C$ the map 
$$
\Hom_\C(Y,X)\to\Hom_{\C^{I^{\op}}}(\Delta(Y),\alpha),\quad f\mapsto p\ci\Delta(f)
$$ 
...
$$
such that $p$ is a $(\Hom_{\C^I}(\alpha,\Delta(\ )),X)$\--co-universal element (see Definition~\ref{ue2} p.~\pr{ue2}). For each $i$ in $I$ the morphism $p_i:\alpha(i)\to X$ is called the $i$-{\em coprojection}\index{coprojection} of $X$. (We almost always write $X$ for $(X,p)$, the mental picture being that $p$ is a structure $X$ is equipped with.)
\end{df}
...
\Hom_\C(X,Y)\to\Hom_{\C^I}(\alpha,\Delta(Y)),\quad f\mapsto\Delta(f)\ci p
\end{equation}
is bijective. Here is a picture:
$$
...
\begin{s}\lb{c38}
P.~38, Proposition 2.1.6. Here is an example of a functor $\alpha:I\to\C^J$ such that $\col\alpha$ exists in $\C^J$ but there is a $j$ in $J$ such that $\col\ (\rho_j\ci\alpha)$ does not exist in $\C$. (Recall that $\rho_j:\C^J\to\C$ is the evaluation at $j\in J$.) This example is taken from Section 3.3 of the book \textbf{Basic Concepts of Enriched Category Theory} of G.M. Kelly\index{Kelly}:%\medskip 
%
...
%
\begin{proof}
We have for any functor $\alpha:I\to\A$ any object $Y$ in $\B$ 
$$ 
\Hom_\B\left(F\left(\col\alpha\right),Y\right)\iso
\Hom_\C\left(G\left(\col\alpha\right),Z\right)
\xr\sim
\Hom_\C\left(\col G(\alpha),Z\right)
$$
$$
\xr\sim\lim \ \Hom_\C(G(\alpha),Z)\iso\lim \ \Hom_\B(F(\alpha),Y)\iso\Hom_\B(\col F(\alpha),Y),
$$ 
...
\nn$\si$: Let $\Delta:\C\to\C^\C$ be the diagonal functor, let $p\in\Hom_{\C^\C}(\id_\C,\Delta(T))$ be a $(\Hom_{\C^\C}(\id_\C,\Delta(\ )),T)$\--co-universal element  (see Definition~\ref{ue2} p.~\pr{ue2}). Recall that this means that, for each object $X$ of $\C$, the map 
$$
\pp_X:\Hom_\C(T,X)\to\Hom_{\C^\C}(\id_\C,\Delta(X)),\quad f\mapsto\Delta(f)\ci p 
$$ 
...
For any $\theta$ in $\Hom_{\C^\C}(\id_\C,\Delta(T))$ and any $Y$ in $\C$ we have $\theta_T\ci p_Y=\theta_Y$, and thus $\Delta(\theta_T)\ci p=\theta$, which implies the claim. 

In other words, we have 
$$
\begin{tikzcd} 
\Hom_\C(T,T)\ar[yshift=0.7ex]{rr}{\pp_T=\Delta(?)\ci p}&&\Hom_{\C^\C}(\id_\C,\Delta(T)).\ar[yshift=-0.7ex]{ll}{\pp_T^{-1}=?_T}
\end{tikzcd}
$$ 
This implies 
$$
\pp_T(\id_T)=\Delta(\id_T)\ci p=\id_{\Delta(T)}\ci p=p=\pp_T(\pp_T^{-1}(p))=\pp_T(p_T),
$$ 
and thus 
%
...
f=p_T\ci f=p_X,
$$
the first equality following from \qr{it=pt}, and the second one from the definition of $\Hom_{\C^\C}(\id_\C,\Delta(T))$.
\end{proof}
...
\begin{s}
Lemma 2.1.15 p.~42. Here is a complement which will be used in \S\ref{17115ps} p.~\pr{17115ps}. Let $I$ be a small category with disjoint hom-sets, let $\alpha,\beta:I\to\C$ be two functors, and, for each $i$ in $I$, let $U^i:I^i\to I$ be the forgetful functor. 

\begin{thm}\lb{fgd}
The natural map 
\begin{equation}\lb{hficab}
\Hom_{\Fct(I,\C)}(\alpha,\beta)\to\lim_{i\in I}\Hom_{\Fct(I^i,\C)}(\alpha\ci U^i,\beta\ci U^i)
\end{equation} 
is bijective.
\end{thm}
...
View 
$$
\lim_{i\in I}\Hom_{\Fct(I^i,\C)}(\alpha\ci U^i,\beta\ci U^i)
$$ 
as a subset $S$ of 
$$
...
$$
\Hom_{\Fct(I^i,\C)}(\alpha\ci U^i,\beta\ci U^i),
$$ 
...
\nn This implies that we have $x_{i,i\to j}=x_{j,\id_j:j\to j}$ for all $i\to j$, and that the $x_{j,\id_j:j\to j}$ define a morphism from $\alpha$ to $\beta$. We get a map 
\begin{equation}\lb{liifii}
\lim_{i\in I}\Hom_{\Fct(I^i,\C)}(\alpha\ci U^i,\beta\ci U^i)\to\Hom_{\Fct(I,\C)}(\alpha,\beta),
\end{equation} 
...
\end{tikzcd}
$$
is cartesian, then $K\iso\Ker(f,g)$. (As usual, $\Delta$ is the diagonal morphism.)
\end{s}
...
\begin{s} 
P.~51, Definition 2.3.2 (minor variant). We assume that no underlying universe has been given. Let $I\xl\pp J\xr\beta\C$ be functors, let $\beta$ be in $\C^J$, and let $\pp^\dg\beta$ be in $\C^I$. The following conditions are equivalent:

\nn(a) $\pp^\dg\beta$ represents $\Hom_{\C^J}(\beta,\pp_*(\ ))\in(\C^I)^\vee_\U$ for \emph{some} universe $\U$ such that $\C^J$ is a $\U$-category,

\nn(b) $\pp^\dg\beta$ represents $\Hom_{\C^J}(\beta,\pp_*(\ ))\in(\C^I)^\vee_\U$ for \emph{any} universe $\U$ such that $\C^J$ is a $\U$-category. 

\begin{df}[Definition 2.3.2 p. 51]\lb{232} 
If the above equivalent conditions hold, we say that $\pp^\dg\beta$ {\em exists}. If $\pp^\dg(F\ci\beta)$ exists and is isomorphic to $F\ci\pp^\dg(\beta)$ for all functor $F:\C\to\C'$, we say that $\pp^\dg\beta$ exists {\em universally}.
\end{df}
\end{s}
...
Let $I\xl\pp J\xr\beta\C$ be functors. Assume that 
$$
\col_{(\pp(j)\to i)\in J_i}\beta(j)
$$ 
exists in $\C$ for all $i$ in $I$. Then $\pp^\dg(\beta)$ exists and we have 
%
\begin{equation}\lb{236}
\pp^\dg(\beta)(i)\iso\col_{(\pp(j)\to i)\in J_i}\beta(j)
\end{equation} 
%
...
We define $\pp^\dg(\beta)$ by \qr{236}. The purpose of Step~(a) is to show that $\pp^\dg(\beta)$ is indeed a functor. Here is a variant of the argument of the book. The proof of the following lemma is obvious: 
...
We want to prove that $\pp^\dg$ defined by \qr{236} p.~\pr{236} is a functor. In the setting of Lemma~\ref{r52} we set $K:=\C$, we choose $\U$ so that $I$ and $k=\C$ are in $\Cat$, we define $\Phi:I\to\Cat$ by $\Phi(i):=J_i$ and we consider the morphism of functors $\theta:\Phi\to\C$ such that $\theta_i:\Phi(i)=J_i\to\C$ is the composition of $\beta$ with the forgetful functor from $J_i$ to $J$. We assume \qr{52}. Then $\col\theta$ is nothing but $\pp^\dg(\beta)$. In particular $\pp^\dg(\beta)$ is a functor by Lemma~\ref{r52}. q.e.d.
...
\Hom_{\C^I}(\pp^\dg(\beta),\alpha)\iso\Hom_{\C^J}(\beta,\pp_*(\alpha)) 
\end{equation} 
...
\Hom_{\C^I}\big(\pp^\dg(\beta),\alpha\big)\xr\sim\lim\gamma.
\end{equation} 
...
%
\Psi_{\alpha,\beta}:
\Hom_{\C^I}\big(\pp^\dg(\alpha),\beta\big)\to
...
$$
f:=\Psi_{\alpha,\pp^\dg(\alpha)}:
\Hom_{\C^I}\big(\pp^\dg(\alpha),\pp^\dg(\alpha)\big)\to
\Hom_{\C^J}\big(\alpha,\pp_*(\pp^\dg(\alpha))\big).
$$ 
We claim that 
\begin{equation}\lb{fip}
f(\id_{\pp^\dg(\alpha)})\text{ is an isomorphism.}
\end{equation}
To prove this, we define 
$$
u:\pp_*(\pp^\dg(\alpha))\to\alpha
$$
as follows. Since 
$$ 
\Big(\pp_*\big(\pp^\dg(\alpha)\big)\Big)(j):=\pp^\dg(\alpha)(\pp(j)):=\col_{\pp(j')\to\pp(j)}\alpha(j'),
$$
we must define 
$$
u\Big(\pp(j')\to\pp(j)\Big):\alpha(j')\to\alpha(j),
$$
that is, we must attach, to each morphism $\pp(j')\to\pp(j)$, a morphism $\alpha(j')\to\alpha(j)$. As $\pp$ is fully faithful by assumption, there is an obvious way to do it. We leave it to the reader to verify that $f(\id_{\pp_*(\pp^\dg(\alpha))})$ and $u$ are mutually inverse isomorphisms. This proves \qr{fip}. The fact that $\pp^\dg$ is fully faithful follows now from Proposition 1.5.6 (ii) p.~29 of the book.
...
\begin{cor}\lb{c233i}
If, in the setting of Theorem \ref{233i}, we have $\C=\Set$ (and $I$ and $J$ are small), then $\pp^\dg(\beta)(i)$ is (in natural bijection with) the quotient of 
$$
...
$$
p_j(x,u'\ci\pp(s))\sim p_{j'}(\beta(s)(x),u'),
$$ 
...
$$
\bigsqcup_{(\pp(j)\to i)\in J_i}\beta(j)=\bigsqcup_{j\in J}\ \bigsqcup_{u\in\Hom_I(\pp(j),i)}\ \beta(j)\iso\bigsqcup_{j\in J}\beta(j)\times\Hom_I(\pp(j),i),
$$ 
...
\begin{s} P.~53, Corollary 2.3.4. (Another proof will be given in Section~\ref{2111} p.~\pr{2111}.) Recall that we have functors $\C\xl\beta J\xr\pp I$, where $I$ and $J$ are small. One can prove $\col\beta\iso\col\pp^\dg\beta$, that is 
%
\begin{equation}\lb{coco}
\col_j\beta(j)\iso\col_i\ \col_{j,u}\beta(j),
\end{equation} 
%
...
$$ 
\beta(j)\xr{p_{i,j,u}}\col_{j,u}\beta(j)\xr{q_i}\col_i\ \col_{j,u}\beta(j)
$$ 
be the coprojections. We easily check that the compositions 
$$
\beta(j)\xr{p_{\pp(j),j,\id_{\pp(j)}}}\col_{j,u}\beta(j)\xr{q_{\pp(j)}}\col_i\ \col_{j,u}\beta(j)
$$ 
...
\C\xl\beta K\xr\psi J\xr\pp I
\end{equation} 
be a diagram of functors. Assume that $I,J$ and $K$ are small, and that $\C$ admits small projective limits. Then Theorem 2.3.3 (ii) p.~52 of the book implies that the functors $\pp^\ddg(\psi^\ddg(\beta))$ and $(\pp\ci\psi)^\ddg(\beta)$ from $I$ to $\C$ exist and are naturally isomorphic. 
\end{s}
...
$$ 
let $J$ be the full subcategory of $I$ whose single object is $R$, and let $\C\xl\beta J\xr\pp I$ be the inclusion functors. We identify $\Hom_R(R,M)$ to $M$ whenever convenient. 

We claim that the functor $\pp^\dg(\beta):I\to\C$ satisfies 
%
...
$$ 
and let $p_x:R\to M'$ be the coprojections. As Theorem 2.3.3 (i) p.~52 of the book implies $M'\iso\pp^\ddg(\beta)(M)$, it suffices to prove $M'\iso M$. We define a family of linear maps $\Phi_x:R\to M$ by setting $\Phi_x:=x$, and leave it to the reader to check that the $\Phi_x$ induce a linear map $\Phi:M'\to M$. We define the set theoretic map $\Psi:M\to M'$ by putting $\Psi(x):=p_x(1)$, and leave it to the reader to verify that $\Phi$ and $\Psi$ are mutually inverse bijections. This proves \qr{pdb1}. 

We claim that the functor $\pp^\ddg(\beta):I\to\C$ satisfies 
%
...
$$ 
and let $p_x:M'\to R^{\op}$ be the projections. As Theorem 2.3.3 (ii) p.~52 of the book implies $M'\iso\pp^\ddg(\beta)(M)$, it suffices to prove $M'\iso M^*$. We define a family of linear maps $\Phi_x:M^*\to R^{\op}$ by setting $\Phi_x(f):=f(x)$, and leave it to the reader to check that the $\Phi_x$ induce a linear map $\Phi:M^*\to M'$. We define the linear map $\Psi:M'\to M^*$ by putting $\Psi((\lambda_x))(y):=\lambda_y$, and leave it to the reader to verify that $\Phi$ and $\Psi$ are mutually inverse linear bijections. This proves \qr{pddb2}. 
...
$$
\col\Delta(S)\iso(\Ob(I)\times S)/\!\!\approx\ ,
$$
...
$$
\iso\bigsqcup_{X\in\C}\Hom_{\C'}(A,F(X))\times\Hom_{\C''}(G(X),B)
$$
...
$$
\Hom_{\Set}\left(\col k(i_0),X\right)\iso\Hom_{{\Set}^I}(k(i_0),\Delta(X))\iso X,
$$
the first bijection being a particular case of \qr{cue} p.~\pr{cue}, and the second one following from the Yoneda Lemma (Theorem~\ref{yol} p.~\pr{yol}).
\end{proof}
\end{s}
...
Clearly, \qr{263b} implies \qr{263a} and \qr{263c}, and the proof of \qr{263b} is straightforward. (See \S\ref{c38} p.~\pr{c38} for the relationship between \qr{263a}, \qr{263b}, and \qr{263c}.) 

Isomorphism \qr{263a} can be decoded as follows: Consider the functor 
$$
\alpha:\C_A\to\C^\wg,\quad(X\to A)\mapsto X,
$$ 
and let $p:\alpha\to\Delta(A)$ be the tautological morphism in $(\C^\wg)^{\C_A}$ defined by 
%
...
$$
\Hom_{\C^\wg}(A,B)\to\Hom_{\Fct(\C_A,\C^\wg)}(\alpha,\Delta(B)),\quad\theta\mapsto\Delta(\theta)\ci p
$$ 
is bijective. 
...
$$
\Hom_{\Set}(A(Y),S)\to\Hom_{\Fct(\C_A,\Set)}(\beta,\Delta(S)),\quad f\mapsto\Delta(f)\ci q
$$ 
is bijective. 

Isomorphism \qr{263c} can be decoded as follows: Let $B$ be in $\C^\wg$, consider the functor 
$$
\gamma:(\C_A)^{\op}\to\Set,\quad(X\to A)\mapsto B(X),
$$ 
and let $r:\Delta(B(A))\to\gamma$ be the morphism in $(\Set)^{(\C_A)^{\op}}$ defined by 
$$
r_{X\to A}(A\to B):=(X\to A\to B)
$$ 
for all $X\to A$ in $\C_A$ and all $A\to B$ in $B(A)$. The claim is that the map 
$$
\Hom_{\Set}(S,B(A))\to\Hom_{\Fct((\C_A)^{\op},\Set)}(\Delta(S),\gamma),\quad g\mapsto r\ci\Delta(g)
$$ 
is bijective. 
...
\begin{proof}
If $I$ and $\C$ are categories, we write $\Delta$ for the diagonal functor from $\C$ to $\C^I$. Let $p\in\Hom_{\A^I}(\alpha,\Delta(A))$ be the coprojection. For any $C$ in $\A$ the map 
$$
f_C:\Hom_\A(A,C)\to\Hom_{\A^I}(\alpha,\Delta(C)),\quad u\mapsto\Delta(u)\ci p
$$ 
is bijective. Let $b=(b:B\to A)$ be an object of $\A_A$. We have the inclusions 
$$
\Hom_{\A_A}(a,b)\subset\Hom_\A(A,B),\quad
%
\Hom_{(\A_A)^I}(\alpha',\Delta(b))\subset\Hom_{\A^I}(\alpha,\Delta(B)).
$$ 
Let $u$ be in $\Hom_\A(A,B)$. It suffices to check 
$$
f_B(u)\in\Hom_{(\A_A)^I}(\alpha',\Delta(b))\ssi u\in\Hom_{\A_A}(a,b),
$$ 
that is 
$$
\Delta(b)\ci\Delta(u)\ci p=p\ssi\Delta(b\ci u)=\id_{\Delta(A)},
$$ 
that is 
$$
\Delta(b\ci u)\ci p=p\ssi\Delta(b\ci u)=\id_{\Delta(A)},
$$ 
that is 
$$
f_A(b\ci u)=f_A(\id_A)\ssi b\ci u=\id_A.
$$ 
But this follows from the injectivity of $f_A$.
\end{proof}
\end{s}
...
Let $\alpha:I\to\C$ be a functor, let $X$ be an object of $\C$, and let $p:\alpha\to\Delta(X)$ be a morphism in $\C^I$ (where $\Delta(X)$ is constant with value $X$). For all universe $\U$ such that $\C$ is a $\U$-category, we write $h_\U$ for the Yoneda embedding from $\C$ to $\C^\wg_\U$.

\begin{df}[universal inductive limit]\lb{uil}
If the natural map $\col\Hom_\C(Y,\alpha)\to\Hom_\C(Y,X)$ is bijective for all $Y$ in $\C$, we say that $X$ is a {\em universal} inductive limit\index{universal inductive limit} of $\alpha$, and that $\col\alpha$ exists {\em universally}\index{universal existence of a limit} in $\C$.
\end{df}

\begin{prop}[Proposition 2.6.4 p. 61]\lb{puil}
...
\nn{\em(b)} $\col h_\U\ci\alpha\iso h_\U(X)$ for \emph{some} universe $\U$ such that $I$ is $\U$-small and $\C$ is a $\U$-category,

\nn{\em(c)} $\col h_\U\ci\alpha\iso h_\U(X)$ for \emph{any} universe $\U$ such that $I$ is $\U$-small and $\C$ is a $\U$-category, 

\nn{\em(d)} $\col F\ci\alpha\iso F(X)$ for \emph{any} functor $F:\C\to\C'$. 
\end{prop}
...
$$
\Hom_{\C'}(F(X),X')\iso\Hom_{\C^\wg_\U}\big(h_\U(X),\Hom_{\C'}(F(\ ),X')\big)
$$
$$
\iso\lim\Hom_{\C^\wg_\U}\big(h_\U\ci\alpha,\Hom_{\C'}(F(\ ),X')\big)\iso\lim\Hom_{\C'}(F\ci\alpha,X')
$$ 
$$
\iso\Hom_{\C'}(\col F\ci\alpha,X').
$$
\end{proof}
\end{s}
...
\begin{s} 
Proposition~\ref{puil} yields the following minor variant of Theorem 2.3.3 p.~52 (i) of the book. Let $I\xl\pp J\xr\beta\C$ be functors and let $\beta$ be in $\C^J$. 
...
\end{equation}
%
exists in $\C$ for all $i$ in $I$, then $\pp^\dg\beta$ exists, and $(\pp^\dg\beta)(i)$ is isomorphic to \qr{e233}. If, in addition, \qr{e233} exists universally in the sense of Definition~\ref{uil} p.~\pr{uil}, then $\pp^\dg\beta$ exists universally in the sense of Definition~\ref{232} p.~\pr{232}. 
\end{thm}
\end{s}
...
$$
\begin{tikzcd}
\C\ar{r}{\hy_\C}\ar{dr}[swap]{F}&\C^\wg\ar{d}{\widetilde F}&I\ar{l}[swap]{\alpha}\\
&\A,
\end{tikzcd}
$$
...
$$ 
for all $A$ in $\C^\wg$. Let us rewrite the proof of the fact that the natural morphism $\col\widetilde F(\alpha)\to\widetilde F\left(\col\alpha\right)$ is an isomorphism. 
...
$$
\A^\C\to\A,\quad F\mapsto(\oo h_\C^\dg F)(A)\quad\text{and}\quad
\C^\wg\to\A,\quad A\mapsto(\oo h_\C^\dg F)(A)
$$ 
...
$$
\lambda:(\C^\wg)_A\to(\C_A)^\wg
$$ 
such that 
%
\begin{equation}\lb{lambda}
\lambda(B\to A)(U\to A)=\Hom_{(\C^\wg)_A}(U\to A,B\to A)
\end{equation} 
%
for all $(B\to A)$ in $(\C^\wg)_A$ and all $(U\to A)$ in $\C_A$. Moreover 
\begin{equation}\lb{leq}
\text{$\lambda$ is an equivalence,}
\end{equation} 
and we have 
\begin{equation}\lb{lcom}
\lambda\ci h(\C)_A\iso h(\C_A),
\end{equation}
that is, the diagram
$$
\begin{tikzcd}
\C_A\ar{r}{h(\C)_A}\ar{rd}[swap]{h(\C)_A}&(\C^\wg)_A\ar{d}{\lambda}\\ 
&(\C_A)^\wg
\end{tikzcd}
$$ 
...
\begin{lem}\lb{cc}
Let $I\xr\alpha\C\xr F\C'$ be functors, let $X$ be in $\C$ and $X'$ in $\C'$, let $f:F(X)\to X'$ be a morphism, assume $X\iso\col\alpha$, let $p:\alpha\to\Delta X$ be the coprojection, define $\beta:I\to\C_{X'}$ by $\beta(i):=(\alpha(i),f\ci F(p_i))$:
$$
F(\alpha(i))\xr{F(p_i)}F(X)\xr fX'.
$$ 
Then $(X,f)\iso\col\beta$, the coprojection $q:\beta\to\Delta(X,f)$ being given by $q_i:=p_i$ for all $i$ in $I$.
\end{lem}
\begin{proof}
Recall that $\Delta$ denotes the appropriate diagonal functor. Let 
$$
(Y,g)\in\C_{X'},\quad h\in\Hom_{\Fct(I,\C_{X'})}(\beta,\Delta(Y,g));
$$ 
let $h'$ in $\Hom_{\Fct(I,\C)}(\beta,\Delta Y)$ be defined by the condition $h'_i=h_i\in\Hom_\C(\alpha(i),Y)$ for all $i$ in $I$; and let $k$ be in $\Hom_\C(X,Y)$. Then we have 
$$
\Delta k\ci q=h\ssi\Delta k\ci p=h'.
$$
\end{proof}
...
h(\C_A)^\ddg(F\ci j(\C_A))\ci\lambda\iso h(\C)^\ddg(F)\ci j((\C^\wg)_A)
\end{equation}
...
{}\\ 
(\C_A)^\wg\ar{uurr}[swap]{h(\C_A)^\ddg(F\ci j(\C_A))}&&(\C^\wg)_A\ar{ll}{\lambda}\ar{rr}[swap]{j((\C^\wg)_A)}&&\C^\wg.\ar{uull}[swap]{h(\C)^\ddg(F)}
\end{tikzcd}
\end{equation}
%\begin{align*}h(\C_A)^\ddg(F\ci j(\C_A))(\lambda(B\to A))&\iso\lim_{((B\to A)\to(U\to A))\in((\C^\wg)_A)^{B\to A}}F(U)\\ \\ &\iso\lim_{(B\to U)\in\C^B}F(U)\\ \\ &\iso h(\C)^\ddg(F)(B)\\ \\ &\iso h(\C)^\ddg(F)\Big(j\big((\C^\wg)_A\big)(B\to A)\Big).\end{align*} 

For $U$ in $\C$ and $U\to A$ in $\C_A$, we have 

$$h(\C_A)^\ddg(F\ci j(\C_A))(\lambda(U\to A))\iso F(U)$$
...
$$h(\C_A)^\ddg(F\ci j(\C_A))(\lambda(B\to A))$$

$$\overset{(\text a)}\iso h(\C_A)^\ddg(F\ci j(\C_A))\left(\lambda\left(\left(\icolim_{(U\to B)\in\C_B}U\right)\to A\right)\right)$$

$$\overset{(\text b)}\iso h(\C_A)^\ddg(F\ci j(\C_A))\left(\lambda\left(\col_{(U\to B)\in\C_B}(U\to A)\right)\right)$$
...
(\C^\wg)_A\ar{l}[swap]{\lambda}\\ 
\A\ar[equal]{r}&\A\ar[equal]{r}&\A\ar[equal]{r}&\A.
\end{tikzcd}
$$ 
(See \qr{lambda} p.~\pr{lambda} for the definition of $\lambda$.) Let $B$ be in $\C^\wg$. We claim 
\begin{equation}\lb{prepa3}
h^\dg(j^\dg(G))(B)\iso(h_A)^\dg(G)(\lambda(B\times A\to A)).
\end{equation} 
...
$$
$$
\iso\col_{((V\to A)\to(U\times A\to A))\in(\C_A)_{U\times A\to A}}G(V\to A)\iso(h_A)^\dg(G)(\lambda(U\times A\to A)),
$$
that is: 
%
\begin{equation}\lb{prepa4}
j^\dg(G)(U)\iso(h_A)^\dg(G)(\lambda(U\times A\to A)).
\end{equation} 
%
For $B$ in $\C^\wg$ we get 

$$h^\dg(j^\dg(G))(B)\iso\col_{(U\to B)\in\C_B}j^\dg(G)(U)\overset{(\text a)}\iso\col_{(U\to B)\in\C_B}(h_A)^\dg(G)(\lambda(U\times A\to A))$$

$$\overset{(\text b)}\iso(h_A)^\dg(G)\left(\lambda\left(\col_{(U\to B)\in\C_B}(U\times A\to A)\right)\right)$$

$$\overset{(\text c)}\iso(h_A)^\dg(G)\left(\lambda\left(\left(\col_{(U\to B)\in\C_B}(U\times A)\right)\to A\right)\right)$$

$$\overset{(\text d)}\iso(h_A)^\dg(G)\left(\lambda\left(\left(\icolim_{(U\to B)\in\C_B}U\right)\times A\to A\right)\right)\overset{(\text e)}\iso(h_A)^\dg(G)(\lambda(B\times A\to A)),$$ 
...
\begin{rk}\lb{cof}
Recall that $F:\C\to\C'$ is a functor of small categories. Let $A'$ be in $\cc C'^\wg$, and let $\cc C_{A'\ci F}\xr\pp\cc C'_{A'}\xr\psi\C'^\wg$ be the natural functors. By \qr{263a} p.~\pr{263a} the natural morphism $\col(\psi\ci\pp)\to\col\psi$ induces a morphism $f:\widehat F(A'\ci F)\to A'$ functorial in $A'$. Moreover, if $\pp$ is cofinal, then $f$ is an isomorphism. (This remark will be used to prove Proposition~\ref{myprop1} p.~\pr{myprop1}.)
\end{rk}
...
The functors 
$$
\A^{\C^{\op}}\to\A,\quad F\mapsto(h^{\op})^\ddg(F)(A)\quad\text{and}\quad
\C^\wg\to\A,\quad A\mapsto(h^{\op})^\ddg(F)(A)
$$ 
...
$$
X_{ij}:=\alpha(i,j),\quad Y_i:=\lim_jX_{ij},\quad Y:=\col_iY_i,
$$
$$
...
%
\begin{cor}\lb{316}
Let $I$ be a (not necessarily small) filtrant $\U$-category, $J$ a finite category, and $\alpha:I\times J^{\op}\to\Set$ a functor such that $\col_i\alpha(i,j)$ exists in $\Set$ for all $j$. Then $\col_i\lim_j\alpha(i,j)$ exists in $\Set$, and the natural map 
$$
\col_i\lim_j\alpha(i,j)\to
\lim_j\col_i\alpha(i,j)
$$ 
is bijective. 
\end{cor}
%
...
\col\ \alpha\ci\pp\ \iso\ 
%
\col_{\ell\in L}\ \col_{\theta(\pp(j))\to\ell}\ \alpha(\pp(j))\ \iso\ 
%
\col_{k\in K}\ \col_{\theta(\pp(j))\to\psi(k)}\ \alpha(\pp(j))
$$
$$
\ \iso\ \col_{k\in K}\ \col_{\theta(i)\to\psi(k)}\ \alpha(i)\ \iso\ 
%
\col_{\ell\in L}\ \col_{\theta(i)\to\ell}\ \alpha(i)\ \iso\ 
%
\col\ \alpha.
$$
...
$$
\col\alpha\iso\col_{J\in\cc J}\col\alpha_J.
$$
\end{lem}
\begin{proof}
Put
...
\nn$\bu$ the morphisms$f_i:=p_{\{i\}}\ci p_{i,\{i\}}:\alpha_i\to B$ induce a morphism $f:A\to B$, 
...
Consider functors $I\xr\alpha\C\xr F\C'$, and assume that $I$ is finite, that $F$ is right exact, and that $\col\alpha$ exists in $\C$. Then $\col(F\ci\alpha)$ exists in $\C'$, and the natural morphism $\col(F\ci\alpha)\to\col\alpha$ is an isomorphism. 
\end{prop} 

\begin{proof}
Let $X'$ be in $\C'$. It suffices to show that the natural map 
$$
\Hom_{\C'}(F(\col\alpha),X')\to\lim\Hom_{\C'}(F(\alpha),X')
$$ 
...
We have bijections 
$$ 
\Hom_{\C'}(F(\col\alpha),X')\iso\col_{(F(Y)\to X')\in\C_{X'}}\Hom_\C(\col\alpha,Y)
$$
$$
\xr\sim\col_{(F(Y)\to X')\in\C_{X'}}\lim\Hom_\C(\alpha,Y)\xr\sim\lim\col_{(F(Y)\to X')\in\C_{X'}}\Hom_\C(\alpha,Y)
$$
$$
\iso\lim\Hom_{\C'}(F(\alpha),X'). 
$$ 
...
\begin{s}\lb{3315}
P.~85, proof of Proposition 3.3.15. To prove that $\A\to\C$ is cofinal, one can apply Proposition~\ref{318i} p.~\pr{318i} with $J=\A,I=\C,L=\C',K=\cc S$. 
\end{s}
...
``For any category $\C$ and any functor $\alpha:M[I\to K\rightarrow J]\to\C$ we have $\col\alpha\iso\col_{j\in J}\col_{i\in I_{\psi(j)}}\alpha(i,j,\pp(i)\to\psi(j))$.'' 
...
$$
j\mapsto\col_{(i,u)\in I_{\psi(j)}}\alpha(i,j,u) 
$$ 
...
\col\alpha\iso\col_j\ \col_{i,u}\alpha(i,j,u),
\end{equation} 
%
...
$$
\col_i\alpha(i)\iso\col_j\ \col_{i,u}\alpha(i). 
$$ 
(Recall: $u:\pp(i)\to j$.) In view of \qr{coco2}, it suffices to prove that we have 
$$
\col_{i,j,u}\alpha(i)\iso\col_i\alpha(i),
$$ 
...
$$ 
\Coker(U\times_VU\parar U)\iso\IM g\iso\Ker(V\parar V\sqcup_UV),
$$ 
...
\begin{split}
\IM\hy(f)\iso\Coker(\hy(X)\times_{\hy(Y)}\hy(X)\parar\hy(X)),\\ \\ 
...
By \qr{IM2} we have 
$$ 
(\Coim f)(Z)\iso\Ker\Big(\Hom_\C(X,Z)\parar\Hom_{\C^\wg}\big(\hy(X)\times_{\hy(Y)}\hy(X),\hy(Z)\big)\Big), 
$$ 
$$ 
...
If $P:=X\times_YX$ exists in $\C$, then $\Coim f$ is naturally isomorphic to $\Coker(P\parar X)\in\C^\vee$. If $S:=Y\sqcup_XY$ exists in $\C$, then $\Ima f$ is naturally isomorphic to $\Ker(Y\parar S)\in\C^\wg$. 
\end{prop} 
...
$f\mapsto\Ima\hy(f)$ and $\Ima$ are functors from $\Mor(\C)$ to $\C^\wg$, 
...
\nn(c)$\ssi$(e): Write $P$ for $X\times_YX$. Recall that (c) says that $P\parar X\to Y$ is exact. By Proposition~\ref{epimono} p.~\pr{epimono} Condition~(e) is equivalent to the condition in Lemma~\ref{strepi} p.~\pr{strepi}. Thus, it suffices to show that, letting $Z$ be an object of $\C$, we have in the notation of Convention~\ref{bra} p.~\pr{bra} 
$$
...
Let $S$ be a set of objects of a category $\C$ and $\cc S$ the corresponding subcategory. We say that $S$ is a {\em system of generators}\index{generator}\index{system of generators} if the functor $\pp:\C\to\cc S^\wg$, $X\mapsto\Hom_\C(\ ,X)$ is conservative.
\end{df}
\end{s}
...
We have 
$$
\Hom_\C(\Coim z,Y)\iso\Hom_\C\big(\Coker(Z\times_XZ\rightrightarrows Z),Y\big)
$$
$$
\iso\Ker\big(\Hom_\C(Z,Y)\rightrightarrows\Hom_\C(Z\times_XZ,Y)\big),
$$ 
and also 
$$
...
$$
\Hom_\C(Z\times_XZ,Y)\to\Hom_{\F^\wg}\big(\pp(Z\times_XZ),\pp(Y)\big)
$$
$$
\iso\Hom_{\F^\wg}\big(\pp(Z)\times_{\pp(X)}\pp(Z),\pp(Y)\big).
$$ 
...
$$
$$
\iso\Ker\Big(\Hom_{\F^\wg}\big(\pp(Z),\pp(Y)\big)\rightrightarrows\Hom_{\F^\wg}\big(\pp(Z)\times_{\pp(X)}\pp(Z),\pp(Y)\big)\Big)
$$
$$
\iso\Hom_{\F^\wg}\Big(\Coker\big(\pp(Z)\times_{\pp(X)}\pp(Z)\rightrightarrows\pp(Z)\big),\pp(Y)\Big)
$$
$$
...
\begin{lem} 
If $\alpha:I\to\Ind(\C)$ is a functor, if $I$ is small and filtrant, and if $A=\ic\alpha$ is in $\C^\wg$, then $\C_A$ is filtrant. 
\end{lem} 

\begin{proof}
Let $M$ be the category attached by Definition 3.4.1 p. 87 of the book to the functors 
$$
\C\xr h\C^\wg\xl{\iota\ci\alpha}I,
$$ 
...
Let $c$ be given by the morphism $X\to A$ in $\C^\wg$, let $m$ be given by the morphism $Y\to\alpha(i)$ in $\Ind(\C)$, and let $\sigma$ and $\sigma'$ be given by the morphisms $s,s':X\parar Y$ making the diagram below commute:
$$
\begin{tikzcd}
...
As $p_i(X)(y\ci s)$ equals $p_i(X)(y\ci s')$ in $A(X)\iso\col\Hom_\C(X,\alpha)$ and $I$ is filtrant, there is a morphism $t:i\to j$ in $I$ such that $\alpha(t)\ci y\ci s=\alpha(t)\ci y\ci s'$, and we can set $Z:=\alpha(j)$ and $z:=\id_{\alpha(j)}$. The situation is depicted by the commutative diagram
$$
\begin{tikzcd}
...
We have 
$$
\mu(A_1,A_2)(X_1,X_2)\iso\col_{y_1,y_2}\Big(\Hom_{\C_1}(X_1,Y_1)\times\Hom_{\C_2}(X_2,Y_2)\Big)
$$
$$
\overset{\text{(a)}}{\iso}\col_{y_1}\Hom_{\C_1}(X_1,Y_1)\times\col_{y_2}\Hom_{\C_2}(X_2,Y_2)
\overset{\text{(b)}}{\iso}A_1(X_1)\times A_2(X_2),
$$ 
...
$$
\mu(A_1,A_2)(X_1,X_2)\iso A_1(X_1)\times A_2(X_2).
$$
...
\theta\ci\mu\iso\id_{\Ind(\C_1)\times\Ind(\C_2)}.
\end{equation}
%
...
\begin{proof}
Let $i_0$ be in $I$. We must check that $(I\times J)^{i_0}$ is connected. We have $(I\times J)^{i_0}\iso I^{i_0}\times J$, and it is easy to see that a product of connected categories is connected. 
\end{proof}
...
\mu\ci\theta\iso\id_{\Ind(\C_1\times\C_2)}.
\end{equation}
...
$$
\begin{tikzcd}
A\iso\ds\ic_y(Y_1,Y_2)\ar{r}&\ds\ic_{x_1,x_2}(X_1,X_2)\iso A_1\times A_2\\ 
...
$$
\begin{tikzcd}
A_1\times A_2\iso\ds\ic_{x_1,x_2}(X_1,X_2)\ar[dashed]{r}&\ds\ic_x(X_1,X_2)\iso A\\ 
(X_1,X_2)\ar{u}{(x_1,x_2)}\ar[equal]{r}&(X_1,X_2)\ar{u}[swap]{x}
\end{tikzcd}
$$ 
(see \qr{263a} p.~\pr{263a} and \qr{pxa} p.~\pr{pxa}). Let $x_i$ in $A_i(X_i)$ be given for $i=1,2$. The category $(\C_1\times\C_2)_A$ being filtrant, there is, by Proposition 3.1.3 p.~73 of the book, a 5-tuple 
$$
...
\end{tikzcd}
$$ 
One checks that $x$ does not depend on the choice of $\zeta$, and that this process defines a morphism from $A_1\times A_2$ to $A$. This ends the proofs of Isomorphism~\qr{6112b} p.~\pr{6112b} and Proposition~\ref{p6112} p.~\pr{p6112}. 
\end{proof} 
...
1&finite inductive limits&$\ci$&$\times$\\ \hline
2&finite coproducts&$\ci$&$\times$\\ \hline
3&small filtrant inductive limits&$\times$&$\ci$\\ \hline
...
$$
M:=M\left[N'\xr{\pp}\big(\C/G(a)\big)^{L/a}\xl{\ \Delta\ci\widetilde{\alpha_a}}I_a\right].
$$ 
...
$$ 
N\iso M\left[N'\xr{\pp}(\C/G(a))^{L/a}\xl{\ \Delta\ci\widetilde{\alpha_a}}I_a\right]. 
$$
...
\Delta\ci\widetilde{\alpha_a}\text{ is cofinal.}
\end{equation} 
% 
...
$$
(\exists\ Y\in\C)\left(\exists\ \mu\in\prod_{i\in I}\Hom_\C(\alpha(i),Y)\right)\Big(\forall\ (s:i\to j)\in S\Big)\Big(\mu_{j}\ci\alpha(s)=\mu_i\Big) 
$$ 
...
Let $F:\cc C\to\cc C'$ be a fully faithful functor, let $A'$ be in $\Ind(\cc C')$, and let $S$ be the set of objects $A$ of $\Ind(\cc C)$ such that $IF(A)\iso A'$. Then the following conditions are equivalent: 

\nn{\em(a)} $S\neq\varnothing$, 

\nn{\em(b)} all morphism $X'\to A'$ in $\Ind(\cc C')$ with $X'$ in $\cc C'$ factorizes through $F(X)$ for some $X$ in $\cc C$, 

\nn{\em(c)} the natural functor $\cc C_{A'\ci F}\to\cc C'_{A'}$ is cofinal, 
...
\nn(a)$\then$(b). Let $f:X'\to IF(A)$ be a morphism in $\Ind(\cc C')$ with $X'$ in $\cc C'$ and $A$ in $\Ind(\cc C)$, let $\beta_0:I\to\cc C$ be a functor with $I$ small and filtrant and $\ic\beta_0\iso A$; in particular $\ic(F\ci\beta_0)\iso IF(A)$. By Proposition 6.1.13 p.~134 of the book there are functors $\alpha:J\to\cc C'$ and $\beta:J\to\cc C$, and there is a morphism of functors $\pp:\alpha\to F\ci\beta$ such that 

$J$ is small and filtrant, 

$\alpha$ is constant equal to $X'$, 

$\ic(F\ci\beta)\iso IF(A)$, 
...
\begin{s} 
P.~149, Definition 7.1.1. The set $\cc S$ is a subset of $\Ob(\Mor(\C))$ (see Notation~\ref{mor} p.~\pr{mor}). The proof of the following lemma (which will be used to prove \qr{l} p.~\pr{l}) is obvious. Neither Definition 7.1.1 nor the lemma below requires the Axiom of Universes.
...
$$ 
be functors such that $Q\ci R\iso\id_{\C'}$, let $\cc S$ be a subset of $\Ob(\Mor(\C))$ such that $Q(s)$ is an isomorphism for all $s$ in $\cc S$, let 
$$
\theta:\id_\C\to R\ci Q
$$ 
...
\begin{s} 
P.~154. Below the statement of Lemma 7.1.13 it is written: ``The verification is left to the reader. // Hence, we get a big category ...''. One might add between the two sentences something like: We also leave it to the reader to define the identity $\id_X$ of $X$ viewed as an object of $\C^r_{\mc S}$, and to check the equalities $f\ci\id_X=f$, $\id_X\ci g=g$ for $f$ in $\Hom_{\C^r_{\mc S}}(X,Y)$ and $g$ in $\Hom_{\C^r_{\mc S}}(Y,X)$.
\end{s}

%

\begin{s} 
P.~155. In the text between Lemma 7.1.15 and Theorem 7.1.16, one might add the following observation. The inverse of $(s:X\to X')\in\mc S$ is given by 
$$
...
$$
\Hom_{\C_{\cc S}^r}(X,Y)\iso\col_{(X'\to X)\in\cc S_X}\Hom_{\C_{\cc S}^r}(X,Y)\iso\col_{(X'\to X)\in\cc S_X}\Hom_{\C_{\cc S}^r}(X',Y)
$$

$$
\iso
\col_{(X'\to X)\in\cc S_X}\ \col_{(Y\to Y')\in\cc S^Y}\Hom_{\C_{\cc S}^r}(X',Y')\iso
\col_{(Y\to Y')\in\cc S^Y}\ \col_{(X'\to X)\in\cc S_X}\Hom_{\C_{\cc S}^r}(X',Y')
$$

$$
\iso
\col_{(Y\to Y')\in\cc S^Y}\Hom_{\C_{\cc S}^\ell}(X,Y')\iso
\col_{(Y\to Y')\in\cc S^Y}\Hom_{\C_{\cc S}^\ell}(X,Y)\iso
...
Let $\C$ be a $\U$-small category, let $\cc S$ be a right multiplicative system, and let $Q:\C\to\C_{\cc S}$ be the right the localization of $\C$ by $\cc S$. A functor $F:\C\to\A$ is said to be {\em right localizable} if $Q^\dg F$ exists, in which case we say that $Q^\dg F$ a {\em right localization} of $F$, and denote this functor by $R_{\cc S}F$. It might be worth displaying the formula 
$$
(R_{\cc S}F)(Q(X))\iso\col_{(Q(Y)\to Q(X))\in\C_{Q(X)}}F(Y).
$$
...
$$
R_{\cc S}F\iso F\ci R_{\cc S}\id_\C.
$$
...
(R_{\cc S}F)(Q(X))\iso\col_{(X\to Y)\in\cc S^X}F(Y).
\end{equation}
%
...
$$
R_\SSS(\iota_\A\ci F)(Q(X))\iso IF(\alpha_\SSS(Q(X)))
$$ 
in $\Ind(\C)$. Recall the following facts: 

\nn$\bu$ Proposition 7.4.1 p.~162 of the book implies
$$
A:=\alpha_\SSS(Q(X))=\col_{(X',x')\in\SSS^X}\iota_\C(X')\in\Ind(\C).
$$ 
...
Let $\cc S$ be a left and right multiplicative system in $\C$, and let $X$ and $Y$ be two objects of $\C$. \S~\ref{737} p.~\pr{737} implies that the functors 
$$
R_{\cc S}\Hom_\C(X,\ \ ),\quad R_{{\cc S}^{\op}}\Hom_\C(\ \ ,Y),\quad R_{{\cc S}\times{\cc S}^{\op}}\Hom_\C
$$
exist and satisfy 
$$
\Hom_{\C_{\cc S}}(X,Y)\iso R_{\cc S}\big(\Hom_\C(X,\ \ )\big)(Y)
$$

$$
\iso R_{{\cc S}^{\op}}\big(\Hom_\C(\ \ ,Y)\big)(X)\iso R_{{\cc S}\times{\cc S}^{\op}}\Hom_\C(X,Y).
$$
...
\begin{lem}\lb{payl}
Let $\C$ and $\C'$ be pre-additive categories, let $\A$ be the category of additive functors from $\C$ to $\C'$, and let $\alpha:I\to\A$ be a functor such that $\col(\alpha(X))$ exists in $\C'$ for all $X$ in $\C$. Then $\col\alpha$ exists in $\A$ and satisfies 
$$
(\col\alpha)(X)\iso\col(\alpha(X))
$$ 
...
$$
i_1\ci p_1+i_2\ci p_2=\id_{X_1\times X_2}.
$$
\end{cor}
...
$$ 
for the projections and coprojections. We have $q_a\ci(f_1\times f_2)=f_a\ci p_a$ for all $a$, and we must show $q_b\ci (f_1\times f_2)\ci i_a=q_b\ci j_a\ci f_a$ for all $a,b$. This follows immediately from Corollary~\ref{823}.
\end{proof}
...
\end{tikzcd}
$$ 
commutative. By Lemma \ref{8311} (a) (i) p.~\pr{8311}, $c$ is a monomorphism, and, by Part (b) (ii) of the same lemma, $\lambda$ is also a monomorphism. As $Z$ is injective, there is a morphism $d:G_i\to Z$ satisfying $d\ci c=g_0\ci b$: 
$$
\begin{tikzcd}
...
\end{tikzcd}
$$ 
As $\lambda$ is an isomorphism by maximality of $(Y_0,g_0,l_0)$, we can set $a_0:=\lambda^{-1}\ci a_1$, and we get 
$$
l_0\ci a_0=l_0\ci\lambda^{-1}\ci a_1=l_1\ci\lambda\ci\lambda^{-1}\ci a_1=l_1\ci a_1=\pp.
$$ 
q.e.d.
...
\end{tikzcd}
$$ 
We can assume that $\C_0$ and $\M_0$ are small (in the sense of Definition~\ref{myuscat} p.~\pr{myuscat}). If $\lambda:\C\to(\C_0)^\wg$ and $\lambda':\M\to(\M_0)^\wg$ are the obvious functors, then the diagram 
$$
\begin{tikzcd}
...
\end{tikzcd}
$$ 
quasi-commutes. The functors $\lambda$ and $\lambda'$ are fully faithful by \S\ref{gcsbc} p.~\pr{gcsbc} and Theorem 5.3.6 p.~124 (stated above as Theorem~\ref{536} p.~\pr{536}). As $\widehat\pp_{{}_0}$ is an equivalence (a quasi-inverse being $\widehat\psi_{{}_0}$), the proof is complete.
\end{proof}
\end{s}
...
It is easy to equip $\C_{\cc S}$ with a pre-additive structure making $Q$ additive. Then the result follows from Corollary~\ref{823b} p.~\pr{823b}. 
...
$$
\begin{tikzcd}
{}&X\times_ZX\ar{r}{a}&X\ar{d}[swap]{f}\ar{r}{d}&\Coim(g\ci f)\ar{r}\ar[dashed]{ddl}{b}&0\\ 
{}&{}&Y\ar{d}[swap]{g}\\
...
the natural map 
$$
\col_{i\in I}\lim_{j\in J}\alpha(i,j)\to\lim_{j\in J}\col_{i\in I}\alpha(i,j)
$$ 
is bijective.
...
If $\C$ admits small $\pi$-filtrant inductive limits, if $J$ is a category satisfying $\card(\Mor(J))<\pi$, if $\beta:J\to\C_\pi$ is a functor, and if $\col\beta$ exists in $\C$, then it belongs to $\C_\pi$. 
\end{prop}
...
$$
\begin{tikzcd}
\col_i\Hom_\C(\col_j\beta(j),\alpha(i))\ar{r}{a}\ar{d}{b}[swap]{\sim}&\Hom_\C(\col_j\beta(j),\col_i\alpha(i))\ar{dd}{e}[swap]{\sim}\\ 
\col_i\lim_j\Hom_\C(\beta(j),\alpha(i))\ar{d}{c}[swap]{\sim}\\ 
\lim_j\col_i\Hom_\C(\beta(j),\alpha(i))\ar{r}{d}[swap]{\sim}&\lim_j\Hom_\C(\beta(j),\col_i\alpha(i)).
\end{tikzcd}
$$ 
...
Assume that Conditions (9.3.1) p.~223 of the book (see \S\ref{931} p.~\pr{931}) hold, let $\pi$ be an infinite regular cardinal, let $I$ be a $\pi$-filtrant small category, let $\alpha:I\to\C$ be a functor, and let $\col\alpha\to Y$ be an epimorphism in $\C$. Assume either $\card(Y(G))<\pi$ or $Y\in\C_\pi$. Then there is an $i_0$ in $I$ such that the obvious morphism $\alpha(i_0)\to Y$ is an epimorphism.
\end{lem}
...
The map 
$$
\lambda_A:\col_{i\in I}\Hom_\C(A,X_i)\to\Hom_\C(A,X).
$$ 
is bijective. 
\end{claim}
...
\nn$\bu$ Step 4: (a) holds. For each $i$ in $I$ define $Y_i:=A\times_XX_i$. As $\col_i$ is exact in $\C$, we have $\col_iY_i\iso A$. As $\card(A(G))<\pi$, Lemma 9.3.1 p.~224 of the book (stated above as Lemma~\ref{l331} p.~\pr{l331}) implies that there is an $i_0$ in $I$ such that $B:=Y_{i_0}\to A$ is an epimorphism.
...
$$
\lambda_P:\col_{i\in I}\Hom_\C(P,X_i)\to\Hom_\C(P,X)
$$ 
...
%
\begin{lem}\lb{952i}
Let $\C$ be a category. Assume that small filtrant inductive limits exist in $\C$ and are exact. Let $\alpha:I\to\C$ be a functor such that $I$ is small and filtrant, and $\alpha(s):\alpha(i)\to\alpha(j)$ is a monomorphism for all morphism $s:i\to j$ in $I$. Then the coprojection $p_i:\alpha(i)\to\col\alpha$ is a monomorphism.
\end{lem}
%
\begin{proof}
...
$$
\theta\in\Hom_{\C^{I^i}}(\Delta(\alpha(i)),\alpha\ci\pp)
$$ 
...
$$
\begin{tikzcd}
\col\Delta(\alpha(i))\ar{d}[swap]{\sim}\ar[tail]{rr}{\col\theta}&&\col(\alpha\ci\pp)\ar{d}{\sim}\\ 
\alpha(i)\ar{rr}[swap]{p_i}&&\col\alpha.
\end{tikzcd}
$$
\end{proof}
\end{s}
...
$$
\Hom_{\cc S^{\wg,\text{prod}}}(\pp(X),H_0)\iso H(X)\iso\Hom_{\cc D^\wg}(X,H)
$$ 
for $X$ in $\cc S$.
...
X\ar[swap]{rr}{g\ci f}&&Z\ar[swap]{rr}{\alpha(g\ci f)}&&TX\oplus Z\ar[swap]{rr}{\beta(g\ci f)}&&TX.
\end{tikzcd}
$$ 
...
X\ar{r}{g\ci f}\ar{d}[swap]{f}&Z\ar{r}{\alpha(g\ci f)}\ar[equal]{d}&TX\oplus Z\ar{r}{\beta(g\ci f)}\ar{d}{v}&TX\ar{d}{T(f)}\\ 
Y\ar{r}{g}\ar{d}[swap]{\alpha(f)}&Z\ar{r}{\alpha(g)}\ar{d}[swap]{\alpha(g\ci f)}&TY\oplus Z\ar{r}{\beta(g)}\ar[equal]{d}&TY\ar{d}{T(\alpha(f))}\\ 
...
$$
F_\pi:\oo{C}(\C)\times\oo{C}(\C')^{\op}\to\oo{C}(\C'').
$$ 
...
\begin{s} 
P.~350, last paragraph. In view of the comments made about Corollary~\ref{938} p.~\pr{938} and Theorem~\ref{961} p.~\pr{961}, one could replace ``there exists an essentially small full subcategory $\cc S$ of $\A_c$ such that \dots'' with ``there exists an infinite cardinal $\pi$ such that $(\A_c)_\pi$ is essentially small and satisfies \dots'', and replace $\cc S$ with $(\A_c)_\pi$ in (14.1.4) p.~351 of the book.
\end{s}
...
\nn LE4: a morphism $A\to B$ in $\C^\wg$ is in $\E$ if and only if, for any morphism $U\to B$ in $\C^\wg$ with $U$ in $\C$, the projection $A\times_BU\to U$ is in $\E$.

As proved in the book, $\E$ contains all epimorphisms. 

For the reader's convenience we paste the proof of the above statement:

Assume that $u:A\to B$ is an epimorphism in $\C^\wg$. If $w:U\to B$ is a morphism with $U$ in $\C$, there exists $v:U\to A$ such that $w=u\ci v$ (see Proposition 2.1.6 p.~38, Proposition 3.3.3 p.~82 and Exercise 5.1 p.~128 in the book). Hence, $\id_U:U\to U$ factors as $U\to A\times_BU\to U$. Therefore $A\times_BU\to U$ is a local epimorphism by LE1 and LE3, and this implies that $A\to B$ is a local epimorphism by LE4. q.e.d.
...
\end{equation}
%
be the set of those morphisms $A\to B$ in $\C^\wg$ such that, for any morphism $U\to B$ in $\C^\wg$ with $U$ in $\C$, the sieve $S_{A\times_BU\to U}$ is in $\Gamma_U$. 
...
S_{A\times_UV\to V}=S_{A\to U}\times_UV.
\end{equation} 
...
S_{A\times_UV\to V}\in\Gamma_V\quad\forall\quad V\to U,
\end{equation}
\begin{equation}\lb{28d}
S_{A\to U}\times_UV\in\Gamma_V\quad\forall\quad V\to U,
\end{equation}
\begin{equation}\lb{28e}
S_{A\to U}\in\Gamma_U.
\end{equation} 
We have \qr{28b} $\ssi$ \qr{28c} by definition of $\E$, and \qr{28c} $\ssi$ \qr{28d} by Remark~\ref{1613i}, \qr{28d} $\then$ \qr{28e} by GT1 and GT4, and \qr{28e} $\then$ \qr{28d} by GT3.
\end{proof}
...
\nn LE4 follows immediately from Remark~\ref{1613i}. %Let $A\to B$ be a morphism in $\C^\wg$. We must check: 
%\begin{center}
%$A\to B$ is in $\E$\\ $\iff$\\ for any morphism $U\to B$ in $\C^\wg$ with $U$ in $\C$, the projection $A\times_BU\to U$ is in $\E$.
%\end{center}
...
\end{equation} 
% 
The morphism $V\to U$ being in $S$ by Lemma~\ref{prepagt4}, the sieve $S'\times_UV$ is in $\Gamma_V$ by \qr{sgu}, and $A_{S'\times_UV}\to V$ is in $\E$ by definition of $\Gamma_V$. We have 
$$
\E\ni(A_{S'\times_UV}\to V)\iso(A_{S'}\times_UV\to V)\iso(C\to V).
$$ 
...
$$
\alpha:(\cc{LI}_U)^{\op}\to\Set,\quad(B\xr sU)\mapsto\Hom_{\C^\wg}(B,A)
$$ 
satisfies $\col\alpha\iso A^a(U)$. Let $i(s):\alpha(s)\to A^a(U)$ be the coprojection, and let $f_1,f_2:U\parar A$ be two morphisms such that the compositions $U\parar A\to A^a$ coincide. By definition of the natural morphism $A\to A^a$, we have 
$$
...
Proof of the claim: Let $V\to B$ be a morphism in $\C_Y^\wg$ with $V$ in $\C_Y$. Then $u_V: A\times_BV\to V$ is either a monomorphism or an epimorphism by Proposition~\ref{sbcs} p.~\pr{sbcs} and Proposition~\ref{34i} p.~\pr{34i}. Let us show that $\fthat(u_V)$ is a local isomorphism. 
...
$$
\col\alpha\xr f\col\alpha\ci\mu_u^{\op}\ci\lambda_u^{\op}\xr g\col\alpha\ci\mu_u^{\op}\xr h\col\alpha.
$$
We must show that $g\ci f$ is an isomorphism. The equality $h\ci g\ci f=\id_{\col\alpha}$ is easily checked. Being a right adjoint, $\mu_u^{\op}$ is left exact, hence cofinal by Lemma 3.3.10 p.~84 of the book, and $h$ is an isomorphism. q.e.d.
\end{s}  
...
\overset{(\text d)}{\iso}\col_{x\in\E}(F(q_B(x))\times_{F(q_A(x))}F(q_C(x)))
$$
$$
\overset{(\text e)}{\iso}\left(\col_{x\in\E}F(q_B(x))\right)\times_{\col_{x\in\E}F(q_A(x))}\left(\col_{x\in\E}F(q_C(x))\right)
$$ 
$$
\overset{(\text f)}{\iso}\left(\col_{y\in\cc{LI}_B}F(j_B(y))\right)\times_{\col_{y\in\cc{LI}_A}F(j_A(y))}\left(\col_{y\in\cc{LI}_C}F(j_C(y))\right)
$$ 
$$
\overset{(\text g)}{\iso}F^b(B)\times_{F^b(A)}F^b(C).
$$ 
...
\fthat(V\times B)\iso f^t(V)\times\fthat(B)
\end{equation} 
follows from Proposition~\ref{p406} p.~\pr{p406}, and we have 
\begin{align*} 
%
\oo j_{B\to Y}^\ddg\big(f_{B*}(G)(V)\big)&\iso f_{B*}(G)(V\times B\to B)&\text{by (17.1.12) p. 409}\\ \\ 
%
&\iso G\big(\fthat(V\times B)\to\fthat(B)\big)&\text{by (17.1.6) p. 408}\\ \\ 
%
&\iso G\big(f^t(V)\times A\to A\big)&\text{by \qr{e406}}, 
% 
\end{align*} 
as well as 
\begin{align*} 
...
%
&\iso G\big(f^t(V)\times A\to A\big)&\text{by (17.1.12) p. 409}. 
% 
\end{align*}  
\end{s} 
...
$$
\oo j_{A\to X}^\ddg(\oo j_{A\to X*}(G)(U))\iso\oo j_{A\to X*}(G)(U\times A\to A)
$$ 
...
As $V\times A\to V$ is a local isomorphism in $\C_X^\wg$ by \S\ref{1768}, and $V\to V\times A\to V$ is the identity of $V$, Lemma 16.2.4 (vii) p.~396 of the book implies that $V\to V\times A$ is a local isomorphism in $\C_X^\wg$, and thus, by \S\ref{1761b} p.~\pr{1761b}, that \qr{1768ii} is a local isomorphism in $\C_A^\wg$. 
\end{s} 
...
(A\to B)\to(A\to B)\times_\omega(A\to B)\iso(A\times_BA\to B)
\end{equation} 
...
Definition of $\psi$: Let $x$ be in $F(U)$. For each morphism $f:V\to U$ in $\C_X$ we define $\psi(x)(f):\R(V)\to F(V)$ by $\psi(x)(f)(\lambda):=\lambda\,F(f)(x)$.
\end{s}
...
\HOM_{\R_2^{\op}}({}_1M_2,\HOM_{\R_3^{\op}}({}_2M_3,{}_4M_3)).
$$ 
More generally, if $\cc{R,S,T}$ are $\cc O_X$-algebras, if $F$ is a $(\cc T\otimes_{\cc O_X}\R^{\op})$-module, if $G$ is an $(\R\otimes_{\cc O_X}\cc S)$-module, and if $H$ is an $(\cc S\otimes_{\cc O_X}\cc T)$-module, then we have 
$$ 
\Hom_{\cc S\otimes_{\cc O_X}\cc T}(F\otimes_\R G,H)\iso
\Hom_{\R\otimes_{\cc O_X}\cc S}(G,\HOM_{\cc T}(F,H)), 
$$ 
...
$$ 
As $(\lambda_{ij})_i$ is in $R^{\oplus I}$ for all $j$, the set of those $i$ in $I$ for which there is a $j$ such that $\lambda_{ij}\neq0$ is finite, and we can arrange the notation so that this set is contained in $\{1,\dots,\ell\}$ with $\ell\ge n$, and we get \qr{lij1}. As $(\lambda_{ij})_i$ is in $C$ for all $j$, we also have \qr{lij2}. 
\end{proof} 
...
\right.
$$
the top square being cartesian. Then $\pp_2$ is a monomorphism by Proposition 17.6.6 p.~425 and Notation 17.6.13 p.~428 of the book (recall that $M'\to M$ is a monomorphism by assumption). This implies $g_2\ci k_2=0$. Hence $\Delta$ is a commutative diagram of \emph{complexes}. The condition that the top square is cartesian is equivalent to the exactness of 
$$ 
\Sigma:=\Big(0\to N\to F'|_U\oplus F''|_U\to F|_U\Big).
$$  
The sequence $\Sigma\otimes_{\R|_U}P|_U$ being exact thanks to the assumption in Lemma~\ref{l1853} p.~\pr{l1853}, we see that the commutative diagram of complexes $\Delta\otimes_{\R|_U}P|_U$ has a cartesian top square, and that, by left exactness of $\Gamma(U;-)$ (see \S\ref{a438} p.~\pr{a438}), the commutative diagram of complexes $\Gamma(U;\Delta\otimes_{\R|_U}P|_U)$, that is (see Notation 17.6.13 p.~428 of the book), 
$$
\begin{tikzcd}
...
(N\otimes_{\R|_U}P|_U)(U)\iso(F'\otimes_\R P)(U)\times_{(F\otimes_\R P)(U)}(F''\otimes_\R P)(U).
\end{equation}
% 
Note that \qr{p1uf1t} implies 
$$
x:=(t,u)\in(F'\otimes_\R P)(U)\times_{(F\otimes_\R P)(U)}(F''\otimes_\R P)(U).
$$ 
