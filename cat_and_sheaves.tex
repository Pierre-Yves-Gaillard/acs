% about "categories and sheaves" (version aud)
...
%
\begin{proof}
We can assume that $I$ is nonempty and well-ordered. We shall construct by transfinite induction (letting $i$ and $j$ run over $I$):

\nn$\bu$ an inductive system $(u_{ij}:Y'_j\to Y'_i)_{i>j}$ in $\C_0$, {\em i.e.}
%
\begin{equation}\label{uij}
(U_{ijk})\qquad u_{ij}\circ u_{jk}=u_{ik}
\end{equation}
%
for all $i>j>k$.

\nn$\bu$ a family $(v_i:X'\to Y'_i)_i$ of morphisms in $\C_0$ such that 
%
\begin{equation}\label{vi}
(V_{ij})\qquad u_{ij}\circ v_j=v_i
\end{equation}
%
for all $i>j$.

\nn$\bu$ a family $(h_i:Y_i\to Y'_i)_i$ of morphisms in $\C$.

\nn If $i=0$ is the least element of $I$, we define $v_0$ and $h_0$ via Condition (9.5.2) (ii)  (see \eqref{952ii} p.~\pageref{952ii}): 
$$
\begin{tikzcd}
X_0\ar{d}[swap]{f_0}\ar{r}{u_0}&Y_0\ar[dashed]{d}{h_0}\\ 
X'\ar[dashed]{r}[swap]{v_0}&Y_0'.
\end{tikzcd}
$$ 

Assume that $i$ is a fixed element of $I$ satisfying $i>0$, and that we have an inductive system 
$$
(u_{jk}:Y'_j\to Y'_i)_{k<j<i}
$$ 
in $\C_0$ and a family 
$$
(v_j:X'\to Y'_j)_{j<i}
$$ 
of morphisms in $\C_0$ which satisfy respectively $(U_{jk\ell})$ and $(V_{jk})$ (see \eqref{uij} and \eqref{vi}) for all $\ell<k<j<i$. By (9.5.2) (i) (see beginning of this subsection), the object 
$$
Y'_{<i}:=\co_{j<i}Y'_i
$$ 
exists in $\C_0$. Let $p_{ij}:Y_j'\to Y'_{<i}$ be the coprojection, and define $w_i$ and $h_i$ via Condition (9.5.2) (ii) (see \eqref{952ii} p.~\pageref{952ii}): 
$$
\begin{tikzcd}
X_i\ar{d}[swap]{f_i}\ar{r}{u_i}&Y_i\ar[dashed]{ddd}{h_i}\\ 
X'\ar{d}[swap]{v_0}&{}\\ 
Y'_0\ar{d}[swap]{p_{i0}}&{}\\ 
Y'_{<i}\ar[dashed]{r}[swap]{w_i}&Y_i'.
\end{tikzcd}
$$ 
Put $v_i:=w_i\circ p_{i0}\circ v_0$ for all $i$, and $u_{ij}:=w_i\circ p_{ij}$ for all $j<i$.
%
\begin{comment}
$$
\begin{tikzcd}
X_i\ar{d}[swap]{f_i}\ar{rrr}{u_i}&&&Y_i\ar{d}{h_i}\\ 
X'\ar[equal]{d}\ar{r}[swap]{v_0}&Y_0'\ar{r}[swap]{p_{i0}}&Y'_{<i}\ar{r}[swap]{w_i}&Y_i'\ar[equal]{d}\\ 
X'\ar{rrr}[swap]{v_i}&&&Y_i',
\end{tikzcd}
$$ 
$$
\begin{tikzcd}
Y_j'\ar[equal]{d}\ar{r}{p_{ij}}&Y'_{<i}\ar{r}{w_i}&Y_i'\ar[equal]{d}\\ 
Y_j'\ar{rr}[swap]{u_{ij}}&&Y_i'.
\end{tikzcd}
$$ 
\end{comment}
%
...
\end{equation}
%
This will imply that the families $(u_{ij})$ and $(v_i)$ satisfy respectively $(U_{ijk})$ and $(V_{ij})$ (see \eqref{uij} and \eqref{vi}) for all $k<j<i$. We prove \eqref{pij} as follows:

The statement
$$
(S_i)\qquad p_{ij}\circ w_j\circ p_{jk}=p_{ik}\quad\forall\quad k<j<i
$$ 
implies $(U_{kj\ell})$ and $(V_{kj})$ for all $\ell<k<j<i$. Using this fact, one easily proves 
$$
(S_j)\ \forall\ j<i\quad\then\quad(S_i).
$$ 
This implies that $(S_i)$ holds for all $i$ in $I$.

Set $Y':=\co_iY_i'$, let $p_i:Y_i'\to Y'$ be the coprojection, put 
$$
