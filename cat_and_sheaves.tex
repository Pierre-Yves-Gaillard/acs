...
$$
(X,p)\in\Ob(\C)\tm\Hom_{\C^{I^{\op}}}(\Delta(X),\al)
$$
...
$$
(X,p)\in\Ob(\C)\tm\Hom_{\C^I}(\al,\Delta(X))
$$
...
\begin{lem}\lb{2112}
If $I$ and $\C$ are categories, if $X$ is in $\C$, if $\Delta X:I\to\C$ is the constant functor with value $X$, and if $I$ is connected, then 

\nn\emph{(a)} $(X,\id_{\Delta X})$ is an inductive limit of $\Delta X$ in $\C$ in the sense of Definition~\ref{ilim} p.~\pr{ilim},

\nn\emph{(b)} for all $Y$ in $\C$ the natural map $\Delta':\Hom_\C(X,Y)\to\Hom_{\C^I}(\Delta X,\Delta Y)$ is bijective, and, for all $i$ in $I$, the map $\theta\mapsto\theta_i$ is the inverse of $\Delta'$,

\nn\emph{(c)} we have $\theta_i=\theta_j$ for all $\theta:\Delta X\to\Delta Y$ with $Y$ in $\C$ and $i,j$ in $I$.
\end{lem}
...
$$
\bigsqcup_{j\in J}\ \beta(j)\tm\Hom_I(\pp(j),i) 
$$ 
...
$$
\bigsqcup_{(\pp(j)\to i)\in J_i}\beta(j)=\bigsqcup_{j\in J}\ \bigsqcup_{u\in\Hom_I(\pp(j),i)}\ \beta(j)\iso\bigsqcup_{j\in J}\beta(j)\tm\Hom_I(\pp(j),i),
$$ 
...
$$ 
and let $p_x:M'\to R^{\op}$ be the projections. As Theorem 2.3.3 (ii) p.~52 of the book implies $M'\iso\pp^\ddg(\beta)(M)$, it suffices to prove $M'\iso M^*$. We define a family of linear maps $\Phi_x:M^*\to R^{\op}$ by setting $\Phi_x(f):=f(x)$, and leave it to the reader to check that the $\Phi_x$ induce a linear map $\Phi:M^*\to M'$. We define the linear map $\Psi:M'\to M^*$ by putting $\Psi((\ld_x))(y):=\ld_y$, and leave it to the reader to verify that $\Phi$ and $\Psi$ are mutually inverse linear bijections. This proves \qr{pddb2}. 
...
$$
\col\Delta(S)\iso(\Ob(I)\tm S)/\!\!\approx\ ,
$$
...
$$
\C_Z\ni(X\to Z)\mapsto(X\tm_ZY\to Y)\in\C_Y
$$ 
...
\begin{proof}
It suffices to verify Conditions (a) and (b) of Proposition 3.2.2 (iii) p.~78 of the book. Condition (b) is clear. To check Condition (a), let $\al$ be in $\C^I$. We must show that there is pair $(X,\ld)$, where $X$ is in $\C$ and $\ld$ is a morphism of functors from $\al$ to $\Delta(X)$. Let $S$ be a set of morphisms in $I$. It is easy to prove 
$$
...
$$
R_{\cc S}\Hom_\C(X,\ \ ),\quad R_{{\cc S}^{\op}}\Hom_\C(\ \ ,Y),\quad R_{{\cc S}\tm{\cc S}^{\op}}\Hom_\C
$$
exist and satisfy 
$$
...
$$
\iso R_{{\cc S}^{\op}}\big(\Hom_\C(\ \ ,Y)\big)(X)\iso R_{{\cc S}\tm{\cc S}^{\op}}\Hom_\C(X,Y).
$$
...
\begin{tikzcd}
R_{\cc S}\oo H_\C(X,\ \ )(Y)\ar[leftrightarrow]{dr}\ar{r}&R_{{\cc S}\tm{\cc S}^{\op}}\oo H_\C(X,Y)\ar[leftrightarrow]{d}&R_{{\cc S}^{\op}}\oo H_\C(\ \ ,Y)(X)\ar{l}\ar[leftrightarrow]{dl}\\ 
&\oo H_{\C_{\cc S}}(X,Y),
\end{tikzcd}
\end{equation}
%
...
$$
\begin{tikzcd}
B'\ar{d}[swap]{\beta}&B'\tm_BA\ar{d}&A'\ar{l}[swap]{(u',\al)}\ar{r}{(v',\al)}\ar{d}{\al}&C'\tm_CA\ar{d}&C'\ar{d}{\gamma}\\ 
