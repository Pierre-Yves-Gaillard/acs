...
$$
such that $p$ is a $(\Hom_{\C^{I^{\op}}}(\DT(\ ),\al),X)$\--universal element (see Definition~\ref{ue} p.~\pr{ue}). For each $i$ in $I$ the morphism $p_i:X\to\al(i)$ is called the $i$-{\em projection}\index{projection} of $X$. If $\bt:I^{\op}\to\C$ is a functor, we often write $X\iso\lim\bt$ to indicate the fact that $(X,p)$ is a projective limit of $\bt$ in $\C$. (This huge abuse of notation seems unavoidable.) We also often say that $X$ (instead of $(X,p)$) is a projective limit of $\bt$ in $\C$. 
\end{df}
...
Even if it is easy, we give a formal proof of the fact that \emph{the} projective limit of $\al$ satisfies the expected universal property:

Let $\theta$ in \qr{yfy} be given. As often it is better to start by verifying the \emph{uniqueness} of $f$. We must have $p_i(f(y))=\theta_i(y)$ for all $i$ in $I$ and all $y$ in $Y$. This implies $f(y)=(\theta_i(y))_{i\in I}$, as already observed. It is straightforward to check that the map $f$ defined by the above equality does the job.
...
%If $\al:I\to\Set$ is a functor defined on a small category and $X$ is in $\Set$, then the map $$\Hom_\Set(\col\al,X)\to\lim\Hom_\Set(\al,X),\quad f\mt(f\ci p_i)_{i\in I}$$ is well-defined and bijective.

Again we give a formal proof of the fact that \emph{the} inductive limit of $\al:I\to\Set$ has the expected universal property:

Given $\theta$ in \qr{cue} let us prove the uniqueness of $f$. Any $x$ in $X$ is of the form $p_i(t)$ for some $i$ in $I$ and $t$ in $\al(i)$, and we must have $f(x)=\theta_i(t)$. This proves the uniqueness. To verify the existence, we must assume $p_i(t)=p_j(u)$ (obvious notation), and derive $\theta_i(t)=\theta_j(u)$. We may assume that there is a morphism $s:i\to j$, and the verification is straightforward.

Let $(X,p)$ be an inductive limit of $\al:I\to\C$: 
...
\end{tikzcd}
\end{equation} 
and let $Z$ be an object of $\C$. We claim that $(\Hom_\C(X,Z),\ci p)$ is a projective limit of $\Hom_\C(\al,Z)$: 
