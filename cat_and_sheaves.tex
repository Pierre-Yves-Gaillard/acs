...
\nn Step 3. Let $A$ be in $\F^\wg$, and let $(B_i\to A)_{i\in I}$ be a small filtrant inductive system in $(\F^\wg)_A$. We claim that the natural morphism
\begin{equation}\lb{colcoim}
\col_i\Coim(B_i\to A)\to\Coim\left(\col_iB_i\to A\right)
\end{equation}
is an isomorphism.

\nn Step 4. We claim that there is, for all $z:Z\to X$ in $\F_X$ and all $Y$ in $\C$, a natural isomorphism 
$$
