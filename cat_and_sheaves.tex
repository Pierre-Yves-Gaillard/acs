...
\begin{s}\lb{patho}
Claim: If $\al:I\tm J\to\C$ is a functor defined on a product of small categories and if $\col_i\al(i,\ )$ exists universally in the sense of Definition~\ref{uil} p.~\pr{uil}, then $\col_i\al(i,j)$ exists universally for all $j$.

In \S\ref{c38} p.~\pr{c38} we saw that, without the adverb ``universally'', the claim is false.

To prove the claim, let $h:\C\to\C^\wg$ be the Yoneda embedding and note the isomorphisms 
$$
...
\begin{s}
\begin{thm}\lb{233}
Let $I$ and $J$ be small categories and let $I\xl\pp J\xr\bt\C$ be functors. If 
...
\begin{proof}
The first sentence and the ``only if'' part of the second sentence follow from Theorem~\ref{233i} p.~\pr{233i}. To prove the ``if'' part of the second sentence, consider the diagram 
$$
\begin{tikzcd}
