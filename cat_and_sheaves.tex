% about "categories and sheaves" (version auz)
...
%
\begin{proof}
We can assume that $I$ is well-ordered. We shall construct by transfinite induction a family $(D_{ij})_{i\ge j}$ of commutative diagrams 
$$
\begin{tikzcd}
{}&Y'_j\ar{dd}{u_{ij}}&Y_j\ar{l}[swap]{h_j}\\ 
X'\ar{ru}{v_j}\ar{rd}[swap]{v_i}\\ 
{}&Y'_i&Y_i\ar{l}{h_i}
\end{tikzcd}
$$ 
such that $u_{ii}=\id_{Y'_i}$ and $u_{ij}\circ u_{jk}=u_{ik}$ for all $i\ge j\ge k$. In particular $(Y'_i)_{i\in I}$ will be an inductive system. We assume (as we may) that the set $I$ is nonempty, we let $0$ be its least element, and we define $v_0$ and $h_0$ by (9.5.2) (ii): 
$$
\begin{tikzcd}
...
\end{tikzcd}
$$ 
Now assume that $i>0$ and that the $(D_{jk})$ have been constructed for $k\le j<i$. In particular $(Y'_j)_{j<i}$ is an inductive system. We set $Y'_{<i}:=\co_{j<i}Y'_i$, and define $v_i,h_i$, and $u_{ij}$ for $j<i$ by the commutative diagram 
$$
\begin{tikzcd}
X_i\ar{rr}{u_i}\ar{d}[swap]{f_i}&&Y_i\ar[dashed]{d}{h_i}\\
X'\ar{r}\ar[equal]{dd}&Y'_{<i}\ar[dashed]{r}&Y'_i\ar[equal]{dd}\\ 
{}&Y'_j\ar{u}\ar{ur}[swap]{u_{ij}}\\ 
X'\ar{rr}[swap]{v_i}&&Y'_i,
\end{tikzcd}
$$ 
where $h_i$ and $Y'_{<i}\to Y'_i$ are furnished by (9.5.2) (ii), and $Y'_j\to Y'_{<i}$ is the coprojection. The construction of $(D_{ij})$ for all $i\ge j$ is now complete. 

Put $Y':=\co Y'_i$, let $p_i:Y'_i\to Y'$ be the coprojection, and set $g_i:=p_i\circ h_i$, $u':=p_0\circ v_0$. We have 
$$
g_i\circ u_i=p_i\circ h_i\circ u_i=p_i\circ v_i\circ f_i=p_i\circ u_{i0}\circ v_0\circ f_i=p_0\circ v_0\circ f_i=u'\circ f_i.
$$ 
\end{proof}
%
%%%
