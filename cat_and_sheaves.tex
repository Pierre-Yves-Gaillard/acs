...
\begin{comment}
\begin{s}
P. 330, Definition 13.3.1. The purpose of this \S\ is to given an example of a pair $(F,X)$ where $F:\C\to\C'$ is an additive functor of abelian categories and $X$ is an object of $\oo K(\C)$ such that none of the objects $RF(X),R^+F(X),R^-F(X),R^{\text b}F(X)$ exists. %In particular:\begin{center}\fbox{Not all additive functors of abelian categories are derivable.\index{non derivable}}\end{center}
This example is essentially due to Yonatan Harpaz\index{Harpaz}: see 

\nn http://mathoverflow.net/a/265142/461

\nn For another example, see \S\ref{fuqu2} p.~\pr{fuqu2} below. 
...
$$ 
This implies that $\oo{Ext}^n_\C(U,V)$ is in $\C'$, as desired, and the same holds if we replace $RF$ with $R^+F$, $R^-F$ or $R^{\text b}F$. (See the proof of Theorem 13.4.1 p.~337 of the book.) 
...
\begin{s}\lb{fuqu3}
P. 337, Theorem 13.4.1. Here is a corollary. Consider the following ``pathological conditions''\index{pathological conditions} on an abelian $\U$-category $\C$: 
