...
\end{prop}
\begin{lem}\lb{l711}
If $\C$ is a category and $\SSS$ is a set of morphisms in $\C$, then there is a category $\C'$ and a functor $Q:\C\to\C'$ such that any functor $\C\to\A$ turning the morphisms of $\SSS$ into isomorphisms factors uniquely through $Q$. Moreover this category is unique us to unique isomorphism.
...
\end{equation} 
for all $G:\C_\SSS\to\A$ and all $\mu:RF\to G$ (see Definition~\ref{dil1} p.~\pr{dil1} and \S~\ref{durl} p.~\pr{durl}). We can assume that we have the following equalities (it is on purpose that I write ``equalities'' instead of ``isomorphisms'', %don't write ``isomorphisms'', but ``equalities''  I really mean ``equalities'', not ``isomorphisms'', 
see \S\ref{l711} p.~\pr{l711}) between functors: 
$$
