...
Let $F:\C^{\op}\to\Set$ be a functor and $X$ an object of $\C$. An $(F,X)$-\emph{universal element}\index{universal element} is an element $u$ of $F(X)$ such that, for all $Y$ in $\C$, the map $\Hom_\C(Y,X)\to F(Y),\ f\mt F(f)(u)$ is bijective. We sometimes say that $X$ \emph{represents}\index{represents} $F$ via $u$ if $u$ is an $(F,X)$-universal element.
\end{df}

The Yoneda Lemma says that $(F,X)$-universal elements are in functorial bijection with isomorphisms $\hy_\C(X)\xr\sim F$, such an isomorphism being called a {\em representation of} $F$ \emph{by} $X$.

\begin{df}[co-universal element]\lb{ue2} 
Let $F:\C\to\Set$ be a functor and $X$ an object of $\C$. An $(F,X)$-\emph{co-universal element}\index{co-universal element} is an element $u$ of $F(X)$ such that, for all $Y$ in $\C$, the map $\Hom_\C(X,Y)\to F(Y),\ f\mt F(f)(u)$ is bijective. We sometimes say that $X$ \emph{co-represents}\index{co-represents} $F$ via $u$ if $u$ is an $(F,X)$-co-universal element.
\end{df}

The Yoneda Lemma says that $(F,X)$-co-universal elements are in functorial bijection with isomorphisms $F\xr\sim\ky_\C(X)$, such an isomorphism being called a \emph{co-representation of} $F$ \emph{by} $X$.
...
\nn(a) We say that $\pp^\dg\bt$ exists if there is a $u:\bt\to\pp_*\pp^\dg\bt$ such that $\pp^\dg\bt$ co-represents the functor $\Hom_{\C^J}(\bt,\pp_*(\ )):\C^I\to\Set$ via $u$ in the sense of Definition~\ref{ue2} p.~\pr{ue2}. This means that, for all $\al:I\to\C$, the map 
$$
\Hom_{\C^I}(\pp^\dg\bt,\al)\to\Hom_{\C^J}(\bt,\pp_*\al),\quad v\mt(v*\pp)\ci u
$$ 
is bijective. (See \S\ref{713} p.~\pr{713} below.)

\nn(b) We say that $\pp^\ddg\bt$ exists if there is a $u:\pp_*\pp^\ddg\bt\to\bt$ such that $\pp^\ddg\bt$ represents the functor $\Hom_{\C^J}(\pp_*(\ ),\bt):(\C^I)^{\op}\to\Set$ via $u$ in the sense of Definition~\ref{ue} p.~\pr{ue}. This means that, for all $\al:I\to\C$, the map 
$$
\Hom_{\C^I}(\al,\pp^\ddg\bt)\to\Hom_{\C^J}(\pp_*\al,\bt),\quad v\mt u\ci(v*\pp)
$$ 
is bijective.

\nn(c) Let $\gamma:I\to\C$ and let $u$ be an endomorphism of the functor $\pp_*\gamma:J\to\C$. The phrase ``$\pp^\ddg\pp_*\gamma$ exists and is isomorphic to $\gamma$ via $u$'' means that, for all $\al:I\to\C$, the map 
$$
\Hom_{\C^I}(\al,\gamma)\to\Hom_{\C^J}(\pp_*\al,\pp_*\gamma),\quad v\mt u\ci(v*\pp)
$$ 
is bijective. (See \S\ref{731} p.~\pr{731} below.)

In particular, the phrase ``$\pp^\ddg\pp_*\gamma$ exists and is isomorphic to $\gamma$ via the identity of $\pp_*\gamma:J\to\C$'' means that, for all $\al:I\to\C$, the map 
$$
...
\begin{s}\lb{731} 
P. 159, Lemma 7.3.1 (i). The claim is that, in terms of \S\ref{s232} (a) p.~\pr{s232}, there is some $\tau:F\to R_\SSS F\ci Q$ such that $Q^\dg F$ co-represents the functor 
$$
...
It might be worth displaying the formula 
$$
