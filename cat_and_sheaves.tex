...
Let $\C$ be a category, let $h:\C\to\C^\wg$ be the Yoneda embedding, let $A$ be in $\C^\wg$, let $X$ be in $\C$, and define 
...
Let $k:\C\to\C^\vee$ be the Yoneda embedding, let $A$ be in $\C^\vee$, let $X$ be in $\C$, and define 
...
$$%\begin{equation}\lb{fpor}
\begin{tikzcd}
Y\ar[rr]\ar[dr,dashed]&&A\\ 
&X.\ar[ru,"x"']
\end{tikzcd}
$$%\end{equation}
%We leave the precise formulation of the dual statement to the reader, and only display the diagram dual to \qr{fpor}: 
A morphism $x:A\to X$ in $\C^\vee$ with $X$ in $\C$ is a representation of $A$ if and only if any morphism $A\to Y$ with $Y$ in $\C$ factors uniquely through $x$:
$$
\begin{tikzcd}
A\ar[rr]\ar[dr,"x"']&&Y\\ 
&X.\ar[ru,dashed]
...
%\begin{df}[universal element]\lb{ue} Let $A:\C^{\op}\to\Set$ be a functor and $X$ an object of $\C$. An $(A,X)$-\emph{universal element}\index{universal element} is an element $u$ of $A(X)$ such that, for all $Y$ in $\C$, the map $\Hom_\C(Y,X)\to A(Y),\ f\mt A(f)(u)$ is bijective. In such a setting, we say that $X$ \emph{represents}\index{represents} $A$, or, when we want to be more precise, that the pair $(X,u)$ \emph{represents} $A$. \end{df}

%\begin{df}[co-universal element]\lb{ue2} Let $A:\C\to\Set$ be a functor and $X$ an object of $\C$. An $(A,X)$-\emph{co-universal element}\index{co-universal element} is an element $u$ of $A(X)$ such that, for all $Y$ in $\C$, the map $\Hom_\C(X,Y)\to A(Y),\ f\mt A(f)(u)$ is bijective. In such a setting, we say that $X$ \emph{co-represents}\index{co-represents} $A$, or, when we want to be more precise, that the pair $(X,u)$ \emph{co-represents} $A$. \end{df}
...
Let us spell out the statement dual to \S\ref{sdefat}:
...
Let $\al:I\to\C$ be a functor. If the value at $\al$ of the left adjoint $\col$ to $\DT:\C\to\C^I$ exists, we denote it by $\col\al$ and call it {\em the inductive limit}\index{inductive limit} of the $\al$ (see Section~\ref{defat} p.~\pr{defat}). Moreover, we say that the co-unit $p:\al\to\DT\col\al$ of the adjunction is \textbf{the} \emph{coprojection}\index{coprojection}. More generally we say that $q:\al\to\DT X$ (with $X$ in $\C$) is \textbf{a} \emph{coprojection} if the corresponding morphism 
$$
\Hom_{\C^I}(\al,\DT(\ ))\to X
$$ 
in $\C^\vee$ (see the Yoneda Lemma, stated on p.~\pr{yol} as Theorem~\ref{yol}) is an isomorphism.
\end{df}
...
Let $\al:I\to\C$ be a functor, $X$ an object of $\C$ and $p:\al\to\DT X$ a coprojection in the sense of Definition~\ref{ilim} p.~\pr{ilim}: 
...
\end{tikzcd}
\end{equation} 
and let $Z$ be an object of $\C$. We claim that 
$$
...
\begin{tikzcd}
S\ar[d,dashed,"g"']&\DT S\ar[d,"\DT g"']\ar[r,"\mu"]&\Hom_\C(\al,Z)\\ 
\Hom_\C(X,Z)&\DT\Hom_\C(X,Z).\ar[ru,"\ci p"']
\end{tikzcd}
\end{equation} 
...
$$ 
More precisely, we claim that the morphism 
$$
p:\DT\Hom_{\C^I}(\al,\DT Y)\to\Hom_\C(\al,Y)
$$ 
given by the obvious maps 
$$
p_i:\Hom_{\C^I}(\al,\DT Y)\to\Hom_\C(\al(i),Y).
$$ 
is a projection in the sense of Definition~\ref{plim} p.~\pr{plim}.

Indeed, given $\theta$ as in the diagram
$$
\begin{tikzcd}
S\ar[d,dashed,"f"']&\DT S\ar[d,"\DT f"']\ar[r,"\theta"]&\Hom_\C(\al,Y)\\ 
\Hom_{\C^I}(\al,\DT Y)&\DT\Hom_{\C^I}(\al,\DT Y),\ar[ru,"p"']
\end{tikzcd}
$$ 
we can define $f$ by $f(s)_i:=\theta_i(s)$.
\end{s}
...
\begin{s}\lb{216}
P.~38, Proposition 2.1.6. Here are more details. Recall the setting: 

We have a functor $\al:I\to\C^J$, we assume that for each $j$ in $J$ the functor $\al(\ )(j):I\to\C$ admits a coprojection $p_j:\al(\ )(j)\to\DT X_j$ in the sense of Definition~\ref{ilim} p.~\pr{ilim}: 
...
\begin{s} 
P.~39, Proposition 2.1.7. Let $\al:I\tm J\to\C$ be a bifunctor, and let $(X_i)_{i\in I}$ be a family of objects of $\C$. Assume that for any $i$ in $I$ there is some morphism $p_i:\al(i,\ )\to\DT X_i$ which is a coprojection in the sense of Definition~\ref{ilim} p.~\pr{ilim}: 
$$
\begin{tikzcd}
...
\nn\emph{(c)} the morphism $\Hom_\C(Y,p):\DT\Hom_\C(Y,X)\to\Hom_\C(Y,\al)$ in $\Set^{I^{\op}}$ induced by $p$ is a projection, 
\end{prop}
...
\begin{proof}
Conditions (b) and (c) are equivalent by Proposition~\ref{216p} p.~\pr{216p}. We shall prove that (a) and (c) are equivalent. Let us summarize (a) and (c) by the following self-explanatory commutative diagrams:
\begin{equation}\lb{zfz}
\begin{tikzcd}
Z\ar[d,dashed,"f"']&\DT Z\ar[d,"\DT f"']\ar[r,"\ld"]&\al\\ 
X&\DT X,\ar[ru,"p"']
\end{tikzcd}
\end{equation} 
...
\begin{tikzcd}
S\ar[d,dashed,"g"']&\DT S\ar[d,"\DT g"']\ar[r,"\mu"]&\Hom_\C(Y,\al)\\ 
\Hom_\C(Y,X)&\DT\Hom_\C(Y,X).\ar{ru}[swap]{\Hom_\C(Y,p)}
\end{tikzcd}
\end{equation} 

To prove (c)$\then$(a), we suppose $Z$ and $\ld$ given in \qr{zfz}, and in \qr{sgs} we let $S$ be a singleton, we set $Y:=Z$, and we define $\mu$ by the formula $\mu_i(s):=\ld_i$. The end of the proof of (a) is easy. 

To prove (a)$\then$(c), we suppose $S,Y$ and $\mu$ given in \qr{sgs}, and we let $s$ be in $S$. We must define $g(s):Y\to X$. We set $Z:=Y$ in \qr{zfz}. We must define $\ld:\DT Y\to\al$. Letting $i$ be in $I$, it suffices to define $\ld_i:Y\to\al(i)$. To do this we set $\ld_i:=\mu_i(s)$. Again we leave the rest of the proof to the reader.
\end{proof}
...
\nn\emph{(b)} the morphism $k(p):k\ci\al\to\DT k(X)$ in $(\C^\vee)^I$ induced by $p$ is a coprojection,% $(k(X),k(p))$ is an inductive limit of $k\ci\al$ in $\C^\vee$, where $k(p):k\ci\al\to\DT k(X)$ is the morphism in $(\C^\vee)^I$ induced by $p$,

\nn\emph{(c)} the morphism $k(p):\DT k(X)\to k\ci\al^{\op}$ in $(\Set^\C)^{I^{\op}}$ induced by $p$ is a projection in the sense of Definition~\ref{plim} p.~\pr{plim},% $(k(X),k(p))$ is a projective limit of $k\ci\al$ in $\Set^\C$, where $k(p):\DT k(X)\to k\ci\al$ is the morphism in $(\Set^\C)^{I^{\op}}$ induced by $p$,

\nn\emph{(d)} the morphism $\Hom_\C(p,Y):\DT\Hom_\C(X,Y)\to\Hom_\C(\al,Y)$ in $\Set^{I^{\op}}$ is a projection.% $(\Hom_\C(X,Y),\Hom_\C(p,Y))$ is a projective limit of $\Hom_\C(\al,Y)$ in $\Set$, where $\Hom_\C(p,Y):\DT\Hom_\C(X,Y)\to\Hom_\C(\al,Y)$ is the morphism in $\Set^I$ induced by $p$.
\end{prop}
...
Let $\C\os G\longleftarrow\A\os F\longrightarrow\B$ be functors. Assume that $\A$ admits inductive limits indexed by $I$, that $G$ commutes with such limits, and that for each $Y$ in $\B$ there is a $Z$ in $\C$ and an isomorphism 
$$
...
We give two proofs. The first proof is short and written in a traditional way. The second proof is much longer. Is the first proof rigorous enough? It is interesting (I think) to meditate a little bit on this question.

\begin{proof}[First proof] 
We have for any functor $\al:I\to\A$ and any object $Y$ in $\B$ 
$$ 
\Hom_\B\left(F\left(\col\al\right),Y\right)\iso\Hom_\C\left(G\left(\col\al\right),Z\right)\xr\sim\Hom_\C\left(\col G(\al),Z\right)
$$
$$
\xr\sim\lim\ \Hom_\C(G(\al),Z)\iso\lim\ \Hom_\B(F(\al),Y)\xleftarrow\sim\Hom_\B(\col F(\al),Y),
$$ 
and the conclusion follows from Proposition~\ref{yp} p.~\pr{yp}.
\end{proof}

\begin{proof}[Second proof] 
Recall that the notion of coprojection is defined in Definition~\ref{ilim} p.~\pr{ilim}. Let $\theta$ be the isomorphism $\Hom_\B(F(\ ),Y)\xr\sim\Hom_\C(G(\ ),Z)$, let $p:\al\to\DT\col\al$ be the coprojection, and let $Y$ be in $\B$. Consider the following commutative diagrams similar to \qr{cue} p.~\pr{cue}:
...
\end{tikzcd}
$$ 
where $q$ and $r$ are the coprojections. Note that $g$ is an isomorphism by assumption. Our goal is to prove that $f$ is an isomorphism too. It suffices to show that 
$$
\Hom_\B(f,Y):\Hom_\B(F(\col\al),Y)\to\Hom_\B(F(\col\al),Y)
$$ 
...
\DT\Hom_\B(F(\col\al),Y)\ar{rrr}{\Hom_\B(q,Y)}&&&\Hom_\B(F\ci\al,Y).\ar[u,"\sim","\theta_\al"']
\end{tikzcd}
$$ 
The last three horizontal arrows a projections in the sense of Definition~\ref{plim} p.~\pr{plim}. %, and we want to prove that the top horizontal arrow is also a projection. 
The bottom horizontal arrow being a projection, there is a unique morphism 
$$
h:\Hom_\C(\col G\ci\al,Y)\to\Hom_\B(F(\col\al),Y)
$$ 
making the diagram 
...
$$
k:\Hom_\B(F(\col\al),Y)\to\Hom_\B(F(\col\al),Y)
$$ 
by $k:=h\ci\Hom_\C(g,Z)\ci\theta_{\col\al}$. It is enough to check that we have $\Hom_\B(f,Y)=k$. As $\Hom_\C(G(p),Z)$ is a projection, this equality follows from the commutativity of \qr{faq} and \qr{dbf}.
%Define $f:\col F\ci\al\to F(\col\al)$ and $g:\col G\ci\al\to G(\col\al)$
%We have for any functor $\al:I\to\A$ and any object $Y$ in $\B$ $$ \Hom_\B\left(F\left(\col\al\right),Y\right)\iso\Hom_\C\left(G\left(\col\al\right),Z\right)\xr\sim\Hom_\C\left(\col G(\al),Z\right)$$ $$\xr\sim\lim\ \Hom_\C(G(\al),Z)\iso\lim\ \Hom_\B(F(\al),Y)\xleftarrow\sim\Hom_\B(\col F(\al),Y),$$ and the conclusion follows from Proposition~\ref{yp} p.~\pr{yp}.
\end{proof}
\end{s}
...
\begin{prop}\lb{b}
If $\al:I\to\C$ is a functor defined on a small category, if $X$ is an object of $\C$, if $p:\al\to\DT X$ is a morphism in $\C^I$ (see Notation~\ref{diag} p.~\pr{diag}), and if $h:\C\to\C^\wg$ be the Yoneda embedding, then the following conditions are equivalent:

\nn\emph{(a)} the obvious morphism $h(p):h\ci\al\to\DT h(X)$ is a coprojection in the sense of Definition~\ref{ilim} p.~\pr{ilim},%$(h(X),h(p))$ is an inductive limit of $h\ci\al$ in $\C^\wg$,

\nn\emph{(b)} the obvious morphism $\Hom_\C(Y,p):\Hom_\C(Y,\al)\to\DT\Hom_\C(Y,X)$ is a coprojection,%in the sense of Definition~\ref{ilim} p.~\pr{ilim},%$(\Hom_\C(Y,X),\Hom_\C(Y,p))$ is an inductive limit of $\Hom_\C(Y,\al)$ in $\Set$ for any $Y$ in $\C$,

\nn\emph{(c)} for any functor $F:\C\to\C'$ the obvious morphism $F(p):F\ci\al\to\DT F(X)$ is a coprojection,%$(F(X),F(p))$ is an inductive limit of $F\ci\al$ in $\C'$ for any functor $F:\C\to\C'$,

\nn\emph{(d)} for any functor $F:\C\to\C'$ the obvious morphism 
$$
\Hom_{\C'}(F(p),X'):\DT \Hom_{\C'}(F(X),X')\to\Hom_{\C'}(F\ci\al,X')
$$ 
is a projection in the sense of Definition~\ref{plim} p.~\pr{plim}.% $(\Hom_{\C'}(F(X),X'),\Hom_{\C'}(F(p),X'))$ is a projective limit of $\Hom_{\C'}(F\ci\al,X')$ in $\Set$ for any functor $F:\C\to\C'$ and any $X'$ in $\C'$.
\end{prop}

\begin{proof}
Propositions~\ref{aioc} and \ref{aic} imply (a) $\ssi$ (b), (c) $\ssi$ (d) and (c) $\then$ (a). To prove (a) $\then$ (d), observe the isomorphisms
$$
...
$$ 
Here $\lim\Hom_{\C'}(F\ci\al,X')$ is \emph{the} projective limit of $\Hom_{\C'}(F\ci\al,X')$ in the sense of Convention~\ref{the} p.~\pr{the}. In particular we have natural projections 
$$
...
Here is the classic example of a non-universal inductive limit. Letting $\al$ be the unique functor from the empty category to $\Set$, we get $\col\al\iso\varnothing$. Writing $h:\Set\to\Set^\wg$ for the Yoneda embedding yields $\col h\ci\al\iso\DT\varnothing$. But we have, on the one hand $(\DT\varnothing)(\varnothing)=\varnothing$, and on the other hand $(h(\varnothing))(\varnothing)\not\iso\varnothing$, implying 
$$
\col(h\ci\al)\not\iso h(\col\al).
$$ 
...
\nn$\si$: Let $p:\id_\C\to\DT T$ be a coprojection (see Definition~\ref{ilim} p.~\pr{ilim}). Let $X$ be in $\C$. For all morphism of functors $\theta:\id_\C\to\DT X$ there is a unique morphism $f:T\to X$ satisfying $\DT f\ci p=\theta$. %Let us denote this $f$ by $f^\theta$: 
$$
\begin{tikzcd}
...
\begin{proof}
This follows from Lemma~\ref{l2111} above and Convention~\ref{term} p.~\pr{term}.% Proposition 1.4.10 p.~26 of the book.
\end{proof}
\end{s}
...
\Hom_{\Fct(I,\C)}(\al,\bt)\to\lim_{i\in I^{\op}}\Hom_{\Fct(I^i,\C)}(\al\ci U^i,\bt\ci U^i)
\end{equation} 
is bijective.
\end{thm}
...
View 
$$
\lim_{i\in I^{\op}}\Hom_{\Fct(I^i,\C)}(\al\ci U^i,\bt\ci U^i)
$$ 
as a subset $S$ of 
$$
...
\lim_{i\in I^{\op}}\Hom_{\Fct(I^i,\C)}(\al\ci U^i,\bt\ci U^i)\to\Hom_{\Fct(I,\C)}(\al,\bt),
\end{equation} 
...
\begin{s}\lb{s232} 
P.~51, Definition 2.3.2. Recall that we have functors $I\xl\pp J\xr\bt\C$. We spell out Definition 2.3.2 using the terminology of Section~\ref{defat} p.~\pr{defat}. Let $\bt$ be in $\C^J$, and let $\pp^\dg\bt$ and $\pp^\ddg\bt$ be in $\C^I$. Recall that we denote horizontal composition of morphisms of functors by $*$ (see Definition~\ref{dil1} p.~\pr{dil1}).

%Items (a), (b) and (c) below are particular cases of the situation considered in Section~\ref{defat} p.~\pr{defat}:

\nn(a) We say that ``$\pp^\dg\bt$ exists'' if there is a co-unit $\ee_\bt:\bt\to\pp_*\pp^\dg\bt$, %such that the pair $(\pp^\dg\bt,\ee_\bt)$ co-represents the functor $\Hom_{\C^J}(\bt,\pp_*(\ )):\C^I\to\Set$ in the sense of Definition~\ref{ue2} p.~\pr{ue2}. This means 
that is, for all $\al:I\to\C$ %the map $$\Hom_{\C^I}(\pp^\dg\bt,\al)\to\Hom_{\C^J}(\bt,\pp_*\al),\quad v\mt(v*\pp)\ci \ee_\bt$$ is bijective, or, more naively, that for
and all $w:\bt\to\pp_*\al$ there is a unique $v:\pp^\dg\bt\to\al$ such that $(v*\pp)\ci \ee_\bt=w$: 
$$
\begin{tikzcd}
\bt\ar[r,"\ee_\bt"]\ar[dr,"w"']&\pp_*\pp^\dg\bt\ar[d,"v*\pp"]&\pp^\dg\bt\ar[d,dashed,"v"]\\ 
&\pp_*\al&\al.
\end{tikzcd}
$$

\nn(b) We say that ``$\pp^\ddg\bt$ exists'' if there is a unit $\eta_\bt:\pp_*\pp^\ddg\bt\to\bt$, that is, for all $\al:I\to\C$ and all $w:\pp_*\al\to\bt$ there is a unique $v:\al\to\pp^\ddg\bt$ such that $\eta_\bt\ci(v*\pp)=w$: 
$$
\begin{tikzcd}
\al\ar[d,dashed,"v"']&\pp_*\al\ar[d,"v*\pp"']\ar[r,"w"]&\bt\\ 
\pp^\ddg\bt&\pp_*\pp^\ddg\bt.\ar[ru,"\eta_\bt"']
\end{tikzcd}
$$

\nn(c) Let $\gamma:I\to\C$ and let $u$ be an endomorphism of the functor $\pp_*\gamma:J\to\C$. The phrase ``$\pp^\ddg\pp_*\gamma$ exists and is isomorphic to $\gamma$ via $u$'' shall mean that for all $\al:I\to\C$ and all $w:\pp_*\al\to\pp_*\gamma$ there is a unique $v:\al\to\gamma$ such that $u\ci(v*\pp)=w$: 
$$
\begin{tikzcd}
\al\ar[d,dashed,"v"']&\pp_*\al\ar[d,"v*\pp"']\ar[r,"w"]&\pp_*\gamma\\ 
\gamma&\pp_*\gamma.\ar[ru,"u"']
\end{tikzcd}
$$ 
...
\begin{prop}
If $I$ is a small category, if $S$ is in $\Set$ and $\DT S:T\to\Set$ is the corresponding constant functor, then there is a canonical bijection
$$
...
\begin{lem} 
If $I$ is a small category, $i_0$ is in $I$, and $\al:I\to\Set$ is the functor satisfying $\Hom_I(i_0,\ )$, then $\col\al$ is a terminal object of $\Set$. 
\end{lem}
...
We shall spell out Isomorphisms \qr{263a} and \qr{263b} (resp. Isomorphism \qr{263c}) in terms of Diagram \qr{cue} p.~\pr{cue} (resp. Diagram \qr{yfy} p.~\pr{yfy}). 

%Warning: We let $h:\C\to\C^\wg$ denote the Yoneda embedding. From here to the end of this section we write $X$ for $h(X)$, with the single exception of the second occurrence of $X$ in \qr{aca} below.

Warning: From here to the end of this section the symbols $X$ and $Y$ will designate either two objects of $\C$ or the image of these objects in $\C^\wg$. The context only will tell which interpretation is the good one. (It seems to me the choice of the correct interpretation will always be obvious.)

\nn$\bu$ Isomorphism \qr{263a} can be decoded as follows: Consider the functor 
$$%\begin{equation}\lb{aca}
\al:\C_A\to\C^\wg,\quad(X\to A)\mt X,
$$%\end{equation} 
and let $p:\al\to\DT A$ %(see Notation~\ref{diag} p.~\pr{diag}) 
be the tautological morphism in $(\C^\wg)^{\C_A}$ defined by 
...
\end{tikzcd}
$$ 
where $f$ is given by $f_X(X\to A):=\theta_{X\to A}$. 
...
\end{tikzcd}
$$ 
where $f$ is given by $f(Y\to A):=\theta_{Y\to A}(\id_Y)$. 
...
$$
\begin{tikzcd}
S\ar[d,dashed,"f"']&\DT S\ar[d,"\DT f"']\ar[r,"\theta"]&\gamma\\ 
B(A)&\DT B(A),\ar[ru,"p"']
\end{tikzcd}
$$ 
where $f$ is given by $f(s)_X(X\to A):=\theta_{X\to A}(s)$.
...
\begin{lem} 
Let $\al:I\to\A$ be a functor, let $A$ be in $\A$, let $p:\al\to\DT A$ (see Notation~\ref{diag} p.~\pr{diag}) be a morphism of functors, assume that $(A,p)$ is an inductive limit of $\al$ (see Definition~\ref{ilim} p.~\pr{ilim}), let $\al':I\to\A_A$ be the obvious functor, let $a$ in $\A_A$ be given by the identity of $A$, and let $q:\al'\to\DT a$ be the obvious morphism of functors. Then $q$ is a coprojection in the sense of Definition~\ref{ilim} p.~\pr{ilim}.%n inductive limit of $\al'$. 
\end{lem} 

\begin{proof} 
We know that for all $B$ in $\A$ and all $\pp:\al\to\DT B$ there is a unique $f:A\to B$ such that $\DT f\ci p=\pp$: 
\begin{equation}\lb{apa}
\begin{tikzcd}
\al\ar[r,"p"]\ar[dr,"\pp"']&\DT A\ar[d,"\DT f"]&A\ar[d,dashed,"f"]\\ 
&\DT B&B.
\end{tikzcd}
\end{equation} 
Let $a:=(A\xr{\id}A)$ and $b=(B\xr bA)$ be in $\A_A$, and let $\psi:\al'\to\DT b$, that is 
$$
\begin{tikzcd}
\al(i)\ar[rd,"p_i"']\ar[rr,"\psi_i"]&&B\ar[dl,"b"]\\ 
&A,
\end{tikzcd}
$$ 
be given. We want to prove that there is a unique $g:a\to b$, that is 
$$
\begin{tikzcd}
A\ar[rd,"\id"']\ar[rr,dashed,"g"]&&B\ar[dl,"b"]\\ 
&A,
\end{tikzcd}
$$ 
such that $\DT g\ci q=\psi$: 
$$
\begin{tikzcd}
\al'\ar[r,"q"]\ar[dr,"\psi"']&\DT a\ar[d,"\DT g"]&a\ar[d,dashed,"g"]\\ 
&\DT b&b.
\end{tikzcd}
$$ 
It is easy to see that there is a unique $\pp:\al\to\DT B$ such that $\pp_i=\psi_i$ for all $i\in I$, and that the morphism $g:=f$, where $f$ is given by \qr{apa}, does the job.
...
\begin{s} 
Proposition~\ref{b} yields the following minor variant of Theorem 2.3.3 (i) p.~52 of the book. Let $I\xl\pp J\xr\bt\C$ be functors and let $\bt$ be in $\C^J$. 
...
\begin{lem}\lb{cc}
If $\al:I\to\C$ is a functor admitting an inductive limit and if $f:\col\al\to Y$ is a morphism in $\C$, then we have 
$$
\col_{i\in I}\ (\al(i)\to\col\al\to Y)\iso(\col\al\to Y).
$$
More precisely, let $X$ be in $\C$, let $p:\al\to\DT X$ be a morphism in $\C^I$ such that $(X,p)$ is an inductive limit of $\al$, define $\bt:I\to\C_Y$ by $\bt(i):=(\al(i)\xr{p_i}X\xr fY)$ and $q:\bt\to\DT(X\xr fY)$ by $q_i:=p_i$: 
$$
\begin{tikzcd}
\al(i)\ar[rd,"f\ci p_i"']\ar[rr,"q_i"]&&X\ar[dl,"f"]\\ 
&Y.
\end{tikzcd}
$$ 
Then $((X\xr fY),q)$ is an inductive limit of $\bt$. 
\end{lem}
...
\end{tikzcd}
$$ 
It is straightforward to check that the formulas $\pp_i:=\psi_i$ and $k_i:=g_i$ do the job. 
\end{proof}
...
\nn we get %the following chain of isomorphisms, where the isomorphism marked with the symbol $(\star)$ results from \qr{leq} and Proposition~\ref{271}:
...
%
%&\iso\col_{(U\to B)\in\C_B}h(\C)^\dg(F)\Big(h(\C)(U)\Big)&\text{by Prop.~\ref{271}}\\
%
%&\iso h(\C)^\dg(F)\left(\col_{(U\to B)\in\C_B}h(\C)(U)\right)&\text{by Prop.~\ref{271}}\\
%
%&\iso\Bigg(h(\C)^\dg(F)\ci j\Big((\C^\wg)_A\Big)\Bigg)\left(\left(\col_{(U\to B)\in\C_B}h(\C)(U)\right)\to A\right)\\
%
...
(b) follows from Proposition~\ref{271} and \qr{leq};  
...
$$ 
the equality $F(A'\ci F)=\col(\psi\ci\pp)$ and the isomorphism $\col\psi\iso A'$ following respectively from \qr{c} and \qr{263a}. Moreover $f$ is an isomorphism whenever $\pp$ is cofinal. Note that the condition that $f$ is an isomorphism means that, for each $X'$ in $\C'$, the natural map 
$$
...
If $\C$ is a small category, if $A$ is in $\C^\wg$, if $B$ is a terminal object of $(\C_A)^\wg$, and if $F:\C_A\to\C$ is the forgetful functor, then we have 
$$
\widehat F(B)\iso A.
$$ 
