...
\nn$*$ P.~132, Line 2: It would be slightly better to replace ``for small and filtrant categories $I$ and $J$'' with ``for small and filtrant categories $I$ and $J$, and functors $\al:I\to\C,\beta:J\to\C$''.
...
\nn$*$ P.~136, last line: ``the cokernel of $(\al(i),\beta(i))$'' should be ``the cokernel of\linebreak $(\pp(i),\psi(i))$''. 
...
If $\al:I^{\op}\to\C$ is a functor and $\Delta:\C\to\C^{I^{\op}}$ is the diagonal functor, then a {\em projective limit of}\index{projective limit} $\al$ {\em in} $\C$ is a pair 
$$
(X,p)\in\Ob(\C)\times\Hom_{\C^{I^{\op}}}(\Delta(X),\al)
$$
such that $p$ is a $(\Hom_{\C^{I^{\op}}}(\Delta(\ ),\al),X)$\--universal element (see Definition~\ref{ue} p.~\pr{ue}). For each $i$ in $I$ the morphism $p_i:X\to\al(i)$ is called the $i$-{\em projection}\index{projection} of $X$. (We almost always write $X$ for $(X,p)$, the mental picture being that $p$ is a structure $X$ is equipped with.)
\end{df}

Recall that the condition that $p$ is a $(\Hom_{\C^{I^{\op}}}(\Delta(\ ),\al),X)$\--universal element means that for each $Y$ in $\C$ the map 
$$
\Hom_\C(Y,X)\to\Hom_{\C^{I^{\op}}}(\Delta(Y),\al),\quad f\mapsto p\ci\Delta(f)
$$ 
is bijective. Here is a picture:
$$
Y\xr fX,\quad\Delta(Y)\xr{\Delta(f)}\Delta(X)\xr{p}\al.
$$
...
$$
(X,p)\in\Ob(\C)\times\Hom_{\C^I}(\al,\Delta(X))
$$
such that $p$ is a $(\Hom_{\C^I}(\al,\Delta(\ )),X)$\--co-universal element (see Definition~\ref{ue2} p.~\pr{ue2}). For each $i$ in $I$ the morphism $p_i:\al(i)\to X$ is called the $i$-{\em coprojection}\index{coprojection} of $X$. (We almost always write $X$ for $(X,p)$, the mental picture being that $p$ is a structure $X$ is equipped with.)
\end{df}

Recall that the condition that $p$ is a $(\Hom_{\C^I}(\al,\Delta(\ )),X)$\--co-universal element means that for each $Y$ in $\C$ the map 
\begin{equation}\lb{cue}
\Hom_\C(X,Y)\to\Hom_{\C^I}(\al,\Delta(Y)),\quad f\mapsto\Delta(f)\ci p
\end{equation}
is bijective. Here is a picture:
$$
X\xr fY,\quad\al\xr{p}\Delta(X)\xr{\Delta(f)}\Delta(Y).
$$
\end{s}
...
\begin{s}
Lemma 2.1.15 p.~42. Here is a complement which will be used in \S\ref{17115ps} p.~\pr{17115ps}. Let $I$ be a small category with disjoint hom-sets, let $\al,\beta:I\to\C$ be two functors, and, for each $i$ in $I$, let $U^i:I^i\to I$ be the forgetful functor. 

\begin{thm}\lb{fgd}
The natural map 
\begin{equation}\lb{hficab}
\Hom_{\Fct(I,\C)}(\al,\beta)\to\lim_{i\in I}\Hom_{\Fct(I^i,\C)}(\al\ci U^i,\beta\ci U^i)
\end{equation} 
is bijective.
\end{thm}
...
View 
$$
\lim_{i\in I}\Hom_{\Fct(I^i,\C)}(\al\ci U^i,\beta\ci U^i)
$$ 
as a subset $S$ of 
$$
P:=\prod_{i\in I}\prod_{(i\to j)\in I^i}\Hom_\C(\al(j),\beta(j)).
$$ 
Let $x=(x_{i,i\to j})$ be in $P$. This just means that each $x_{i,i\to j}$ is a morphism $\al(j)\to\beta(j)$. Then $x$ belongs to $S$ if and only if the following two conditions hold:

\nn(a) For all $i$ in $I$ the family $(x_{i,i\to j})_{i\to j}$ is in 
$$
\Hom_{\Fct(I^i,\C)}(\al\ci U^i,\beta\ci U^i),
$$ 
that is, for all diagram $i\to j\to k$ in $I$, the square 
$$
\begin{tikzcd}
\al(j)\ar{rr}{x_{i,i\to j}}\ar{d}[swap]{\al(j\to k)}&&\beta(j)\ar{d}{\beta(j\to k)}\\ 
\al(k)\ar{rr}[swap]{x_{i,i\to j\to k}}&&\beta(k)
\end{tikzcd}
$$
commutes.
...
\nn This implies that we have $x_{i,i\to j}=x_{j,\id_j:j\to j}$ for all $i\to j$, and that the $x_{j,\id_j:j\to j}$ define a morphism from $\al$ to $\beta$. We get a map 
\begin{equation}\lb{liifii}
\lim_{i\in I}\Hom_{\Fct(I^i,\C)}(\al\ci U^i,\beta\ci U^i)\to\Hom_{\Fct(I,\C)}(\al,\beta),
\end{equation} 
...
\Hom_{\C^I}(\pp^\dg(\beta),\al)\iso\Hom_{\C^J}(\beta,\pp_*(\al)) 
\end{equation} 
...
$$
\begin{tikzcd}
J\ar{rr}{\pp}\ar{dr}[swap]{\beta}&&I\ar{dl}{\al}\\ 
&\C.
\end{tikzcd}
$$ 
...
$$
\gamma:M^{\op}\to\Set,\quad\Big(j,\pp(j)\to i\to i'\Big)\mapsto\Hom_\C\big(\beta(j),\al(i')\big), 
$$ 
$$
\delta:N^{\op}\to\Set,\quad(j\to j')\mapsto\Hom_\C\big(\beta(j),\al(\pp(j'))\big). 
$$ 
The existence of a natural bijection 
\begin{equation}\lb{nb1}
\Hom_{\C^J}\big(\beta,\pp_*(\al)\big)\xr\sim\lim\delta
\end{equation} 
...
\Hom_{\C^I}\big(\pp^\dg(\beta),\al\big)\xr\sim\lim\gamma.
\end{equation} 
...
$$
\big(\beta(j)\to\al(i')\big)_{\pp(j)\to i\to i'}
$$ 
and to a morphism $j\to j'$, a morphism $\beta(j)\to\al(\pp(j'))$ by setting 
$$
i=i'=\pp(j'),\quad(i\to i')=\id_{\pp(j')},
$$ 
and by taking as $\beta(j)\to\al(\pp(j'))$ the corresponding member of our family. We leave it to the reader to check that this defines indeed a map $\lim\gamma\to\lim\delta$. To define a map $\lim\delta\to\lim\gamma$, we attach, to a family 
$$
\big(\beta(j)\to\al(\pp(j'))\big)_{j\to j'}
$$ 
and to a chain of morphisms $\pp(j)\to i\to i'$, a morphism $\beta(j)\to\al(i')$ by setting 
$$
j':=j,\quad(j\to j'):=\id_{j},
$$ 
and by taking as $\beta(j)\to\al(i')$ the composition 
$$
\beta(j)\to\al(\pp(j))\to\al(i)\to\al(i'). 
$$ 
...
%
\Psi_{\al,\beta}:
\Hom_{\C^I}\big(\pp^\dg(\al),\beta\big)\to
\Hom_{\C^J}\big(\al,\pp_*(\beta)\big),
%
\end{equation} 
%
...
$$
\al:\C_A\to\C^\wg,\quad(X\to A)\mapsto X,
$$ 
and let $p:\al\to\Delta(A)$ be the tautological morphism in $(\C^\wg)^{\C_A}$ defined by 
%
...
$$
\Hom_{\C^\wg}(A,B)\to\Hom_{\Fct(\C_A,\C^\wg)}(\al,\Delta(B)),\quad\theta\mapsto\Delta(\theta)\ci p
$$ 
is bijective. 
...
\begin{lem} 
Let $\al:I\to\A$ be a functor, let $A$ in $\A$ be the inductive limit of $\al$, let $\al':I\to\A_A$ be the obvious functor, and let $a$ in $\A_A$ be the identity of $A$. Then the inductive limit of $\al'$ is $a$. 
\end{lem} 

\begin{proof}
If $I$ and $\C$ are categories, we write $\Delta$ for the diagonal functor from $\C$ to $\C^I$. Let $p\in\Hom_{\A^I}(\al,\Delta(A))$ be the coprojection. For any $C$ in $\A$ the map 
$$
f_C:\Hom_\A(A,C)\to\Hom_{\A^I}(\al,\Delta(C)),\quad u\mapsto\Delta(u)\ci p
$$ 
is bijective. Let $b=(b:B\to A)$ be an object of $\A_A$. We have the inclusions 
$$
\Hom_{\A_A}(a,b)\subset\Hom_\A(A,B),\quad
%
\Hom_{(\A_A)^I}(\al',\Delta(b))\subset\Hom_{\A^I}(\al,\Delta(B)).
$$ 
Let $u$ be in $\Hom_\A(A,B)$. It suffices to check 
$$
f_B(u)\in\Hom_{(\A_A)^I}(\al',\Delta(b))\ssi u\in\Hom_{\A_A}(a,b),
$$ 
that is 
$$
...
Let $\al:I\to\C$ be a functor, let $X$ be an object of $\C$, and let $p:\al\to\Delta(X)$ be a morphism in $\C^I$ (where $\Delta(X)$ is constant with value $X$). For all universe $\U$ such that $\C$ is a $\U$-category, we write $h_\U$ for the Yoneda embedding from $\C$ to $\C^\wg_\U$.

\begin{df}[universal inductive limit]\lb{uil}
If the natural map $\col\Hom_\C(Y,\al)\to\Hom_\C(Y,X)$ is bijective for all $Y$ in $\C$, we say that $X$ is a {\em universal} inductive limit\index{universal inductive limit} of $\al$, and that $\col\al$ exists {\em universally}\index{universal existence of a limit} in $\C$.
\end{df}
...
\nn{\em(a)} $X$ is a universal inductive limit of $\al$,

\nn{\em(b)} $\col h_\U\ci\al\iso h_\U(X)$ for \emph{some} universe $\U$ such that $I$ is $\U$-small and $\C$ is a $\U$-category,

\nn{\em(c)} $\col h_\U\ci\al\iso h_\U(X)$ for \emph{any} universe $\U$ such that $I$ is $\U$-small and $\C$ is a $\U$-category, 

\nn{\em(d)} $\col F\ci\al\iso F(X)$ for \emph{any} functor $F:\C\to\C'$. 
\end{prop}
...
$$
$$
\iso\lim\Hom_{\C^\wg_\U}\big(h_\U\ci\al,\Hom_{\C'}(F(\ ),X')\big)\iso\lim\Hom_{\C'}(F\ci\al,X')
$$ 
$$
\iso\Hom_{\C'}(\col F\ci\al,X').
$$
\end{proof}
\end{s}
...
\begin{lem}\lb{cc}
Let $I\xr\al\C\xr F\C'$ be functors, let $X$ be in $\C$ and $X'$ in $\C'$, let $f:F(X)\to X'$ be a morphism, assume $X\iso\col\al$, let $p:\al\to\Delta X$ be the coprojection, define $\beta:I\to\C_{X'}$ by $\beta(i):=(\al(i),f\ci F(p_i))$:
$$
...
$$ 
let $h'$ in $\Hom_{\Fct(I,\C)}(\beta,\Delta Y)$ be defined by the condition $h'_i=h_i\in\Hom_\C(\al(i),Y)$ for all $i$ in $I$; and let $k$ be in $\Hom_\C(X,Y)$. Then we have 
$$
...
Let $I$ be a filtrant small category, let $J$ be a finite category, let $\al:I\times J^{\op}\to\Set$ be a functor, and put 
$$
...
%
\begin{cor}\lb{316}
Let $I$ be a (not necessarily small) filtrant $\U$-category, $J$ a finite category, and $\al:I\times J^{\op}\to\Set$ a functor such that $\col_i\al(i,j)$ exists in $\Set$ for all $j$. Then $\col_i\lim_j\al(i,j)$ exists in $\Set$, and the natural map 
$$
...
$$
A:=\col\al,\quad
\beta_J:=\col\al_J,\quad
B:=\col\beta.
$$
Let 
$$
p_i:\al_i\to A,\quad 
p_{i,J}:\al_i\to\beta_J,\quad 
p_J:\beta_J\to B
$$
...
\nn$\bu$ the morphisms $g_{i,J}:=p_i:\al_i\to A$ (with $i$ in $J$) induce a morphism $g_J:\beta_J\to A$, 
...
$$
j\mapsto\col_{(i,u)\in I_{\psi(j)}}\al(i,j,u) 
$$ 
...
$$
M:=M\left[N'\xr{\pp}\big(\C/G(a)\big)^{L/a}\xl{\ \Delta\ci\widetilde{\al_a}}I_a\right].
$$ 
...
To define $\mu$ let 
$$
\Xi:=\Big(\big((i_\ell),Q\big)\ ,\ i_a\ ,\ \pp((i_\ell),Q)\to\Delta(\widetilde{\al_a}(i_a))\Big)
$$ 
...
$$ 
N\iso M\left[N'\xr{\pp}(\C/G(a))^{L/a}\xl{\ \Delta\ci\widetilde{\al_a}}I_a\right]. 
$$
...
\end{equation} 
%
and the projection $N'\to I_\ell$ is cofinal for all $\ell$ in $L$. It follows from Proposition 2.6.3 (ii) p.~61 of the book that $\widetilde{\al_a}$ is cofinal. By assumption $\C/G(a)$ is filtrant, and Lemma~\ref{delta} below will imply that $\Delta$ is cofinal. Thus, 
%
\begin{equation}\lb{daa}
\Delta\ci\widetilde{\al_a}\text{ is cofinal.}
\end{equation} 
% 
...
\begin{proof}
It suffices to verify Conditions (a) and (b) of Proposition 3.2.2 (iii) p.~78 of the book. Condition (b) is clear. To check Condition (a), let $\al$ be in $\C^I$. We must show that there is pair $(X,\lambda)$, where $X$ is in $\C$ and $\lambda$ is a morphism of functors from $\al$ to $\Delta(X)$. Let $S$ be a set of morphisms in $I$. It is easy to prove 
$$
...
\nn Then $f$ factorizes as $X'=\al(j)\xr{\pp_j}F(\beta(j))\xr{p_j}IF(A)$, where $p_j$ is the coprojection.
...
$$
\begin{tikzcd}
\col_i\Hom_\C(\col_j\beta(j),\al(i))\ar{r}{a}\ar{d}{b}[swap]{\sim}&\Hom_\C(\col_j\beta(j),\col_i\al(i))\ar{dd}{e}[swap]{\sim}\\ 
\col_i\lim_j\Hom_\C(\beta(j),\al(i))\ar{d}{c}[swap]{\sim}\\ 
\lim_j\col_i\Hom_\C(\beta(j),\al(i))\ar{r}{d}[swap]{\sim}&\lim_j\Hom_\C(\beta(j),\col_i\al(i)).
\end{tikzcd}
$$ 
...
\begin{lem}\lb{ord}
If $\al$ is a cardinal, then the cardinal of the set of those cardinals $\beta$ such that $\beta<\al$ does not exceed $\al$.
\end{lem}
...
Let $A$ be a well-ordered set of cardinal $\al$, and, for each cardinal $\beta$ with $\beta<\al$, let $B$ be a well-ordered set of cardinal $\beta$. Then $B$ is isomorphic to $A_{<a}$ for a unique $a$ in $A$, and the map $\beta\mapsto a$ is injective.
\end{proof}
...
$$
\theta\in\Hom_{\C^{I^i}}(\Delta(\al(i)),\al\ci\pp)
$$ 
...
$$
\begin{tikzcd}
\col\Delta(\al(i))\ar{d}[swap]{\sim}\ar[tail]{rr}{\col\theta}&&\col(\al\ci\pp)\ar{d}{\sim}\\ 
...
$$
\begin{tikzcd}
X\ar[equal]{dd}\ar{r}{f}&Y\ar[equal]{d}\ar{r}{\al(f)}&TX\oplus Y\ar{r}{\beta(f)}&TX\\
&Y\ar{r}{g}&Z\ar[equal]{d}\ar{r}{\al(g)}&TY\oplus Z\ar{r}{\beta(g)}&TY\\
X\ar[swap]{rr}{g\ci f}&&Z\ar[swap]{rr}{\al(g\ci f)}&&TX\oplus Z\ar[swap]{rr}{\beta(g\ci f)}&&TX.
\end{tikzcd}
$$ 
The goal of the proof is then to construct a commutative diagram
$$
\begin{tikzcd}
X\ar{r}{f}\ar[equal]{d}&Y\ar{r}{\al(f)}\ar{d}{g}&TX\oplus Y\ar{r}{\beta(f)}\ar{d}{u}&TX\ar[equal]{d}\\ 
X\ar{r}{g\ci f}\ar{d}[swap]{f}&Z\ar{r}{\al(g\ci f)}\ar[equal]{d}&TX\oplus Z\ar{r}{\beta(g\ci f)}\ar{d}{v}&TX\ar{d}{T(f)}\\ 
Y\ar{r}{g}\ar{d}[swap]{\al(f)}&Z\ar{r}{\al(g)}\ar{d}[swap]{\al(g\ci f)}&TY\oplus Z\ar{r}{\beta(g)}\ar[equal]{d}&TY\ar{d}{T(\al(f))}\\ 
...
$$
V\xr{\al(u)}\Mc(u)\xr{\beta(u)}TU\to TV
$$ 
...
$$
\al:(\cc{LI}_U)^{\op}\to\Set,\quad(B\xr sU)\mapsto\Hom_{\C^\wg}(B,A)
$$ 
...
$$
\col\al\xr f\col\al\ci\mu_u^{\op}\ci\lambda_u^{\op}\xr g\col\al\ci\mu_u^{\op}\xr h\col\al.
$$
...
$$
\begin{tikzcd}
B'\ar{d}[swap]{\beta}&B'\times_BA\ar{d}&A'\ar{l}[swap]{(u',\al)}\ar{r}{(v',\al)}\ar{d}{\al}&C'\times_CA\ar{d}&C'\ar{d}{\gamma}\\ 
