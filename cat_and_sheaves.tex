% about "categories and sheaves" (version_0068)
...
\nn[\S\ref{gt} p.~\pageref{gt} will be inserted here.]
...
\begin{s}\label{gt}% Jan 21, 22 2014
P. 390, Definition of a {\em Grothendieck topology}. \index{Grothendieck topology} I suggest to replace GT4 in Definition 16.1.2 p.~390 of the book with 

\nn GT4': If $S$ is a sieve over $U\in\C$, and if $S\times_UV$ is a covering sieve over $V$ for all morphism $V\to U$ in $\C$, then $S$ is a covering sieve.

It seems clear to me that GT1, GT2, GT3 and GT4' are equivalent with LE1-LE4, that is, the datum of a Grothendieck topology in this nonstandard sense is equivalent to the datum of a set of morphisms satisfying LE1-LE4.

This nonstandard definition of a Grothendieck topology is more general than the standard one. 

Indeed, here is an example of a set of sieves satisfying GT1, GT2, GT3 and GT4' but not GT4:

Let $\C$ be a category with a single object, and let $M$ be the corresponding monoid of endomorphisms. For each $f$ in $M$ and each sieve $S$, we write $f^*(S)$ for the obvious sieve attached to $f$ and $S$ (sieve which would be designated by $S\times_UU$ in the notation of the book, $U$ being the unique object of $\C$). Assume that $M$ is freely generated by a single endomorphism $a$. For $i\in\bb N$ put 
$$
S_i:=\{a^i,a^{i+1},\dots\}.
$$ 
Then $S_0=M,S_1,S_2,\dots$ is a strictly decreasing sequence of sieves, and any nonempty sieve is equal to one term of this sequence. Putting $S_i:=S_0$ for $i\in\bb Z,i<0$, we get 
$$
a^{i*}(S_j)=S_{j-i}
$$ 
for all $i,j\ge0$. In particular $s'^*(S_2)\in\{S_0,S_1\}$ for all $s'\in S_1$, and it is easy to see that $\{S_0,S_1\}$ satisfies GT1, GT2, GT3 and GT4' but not GT4.
\end{s}
