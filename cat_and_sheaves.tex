...
\begin{s}
P. 330, Definition 13.3.1. The purpose of this \S\ is to given an example of a pair $(G,X)$ where $F:\C\to\C'$ is an additive functor of abelian categories and $X$ is an object of $\oo K(\C)$ such that none of the objects $RF(X),R^+F(X),R^-F(X),R^{\text b}F(X)$ exists. In particular:
...
This example is essentially due to Yonatan Harpaz: see 

\nn http://mathoverflow.net/a/265142/461

Let $k$ be a field, let $A$ be a $k$-algebra, let $\C$ be the category of finite dimensional $A$-modules, let $\C'$ be the category of finite dimensional $k$-vector spaces, let $M$ and $N$ be two objects of $\C$, let $F:\C\to\C'$ be the functor $Y\mt\Hom_A(M,Y)$, let $M[0]\in\oo K(\C)$ be equal to $M$ in degree 0 and to 0 elsewhere, let $N[n]\in\oo K(\C)$ be equal to $N$ in degree $n$ (with $n\in\bb Z$) and to 0 elsewhere, and let us assume that $RF(N[n])$ exists. We have 
$$
R^0F(N[n])\iso H^0\col_{N[n]\to Y}\Hom_A^\bu(M[0],Y),
$$ 
where $N[n]\to Y$ runs over the quasi-isomorphisms in $\oo K(\C)$, 
$$
H^0\col_{N[n]\to Y}\Hom_A^\bu(M[0],Y)\iso\col_{N[n]\to Y}H^0\Hom_A^\bu(M[0],Y)
$$
$$
\iso\col_{N[n]\to Y}\Hom_{\oo K(\C)}(M[0],Y)\iso\Hom_{\oo D(\C)}(M[0],N[n])=\oo{Ext}^n_\C(M,N),
$$ 
that is 
$$
R^0F(N[n])\iso\oo{Ext}^n_\C(M,N)\in\C'.
$$ 
This implies in particular $\dim\oo{Ext}^n_\C(M,N)<\infty$, and the same holds if we replace $RF$ with $R^+F$, $R^-F$ or $R^{\text b}F$. 

Now let $A$ be $k[x_1,x_2,\dots]$, where $x_1,x_2,\dots$ are indeterminates, let $\mathfrak m$ be the ideal generated by the $x_i$, let $M:=N:=A/\mathfrak m$ and set $n:=1$. It is easy to see that we get a contradiction, so we can set $X:=(A/\mathfrak m)[0]$ and conclude that the objects $RF(X),R^+F(X),R^-F(X)$ and $R^{\text b}F(X)$ do \emph{not} exist, as required.
\end{s}
