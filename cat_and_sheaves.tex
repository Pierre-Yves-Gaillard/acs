...
%\nn$*$ P.~85, proof of Proposition 3.3.13, proof of implication ``$\C_A$ filtrant $\then$ $A$ commutes with finite projective limits''. One can either use Proposition~\ref{316} p.~\pr{316}, or notice that $\C$ can be assumed to be small. (The argument is the same in both cases.)
...
%\nn$*$ P.~133, proof of Theorem 6.1.8 (sated below on p.~\pr{618} as Theorem~\ref{618}), Line 2: ``It is enough to show that $A$ belongs to $\Ind(\C)$''. More generally: Let $I\xr\al\C\xr F\C'$ be functors. Assume that $F$ is fully faithful, and that there is an $X$ in $\C$ such that $F(X)\iso\col F(\al)$. Then $X\iso\col\al$. The proof is obvious.
...
\begin{s}
$\frownie$\footnote{The symbol $\frownie$ (``frowny'') means that I'm not satisfied with this \S, and that I'm planning to rewrite it.} Lemma 2.1.15 p.~42. Here is a complement which will be used in \S\ref{17115ps} p.~\pr{17115ps}. 
...
\end{tikzcd}
$$ 
Let $\mu:U\ci\al\to\DT Z$ be the morphism in $\Fct(I,\C)$ induced by $\ld$. Then there is a unique morphism $f:X\to Z$ such that $\DT f\ci(U*p)=\mu$: 
$$%\begin{equation}\label{uap}
\begin{tikzcd}
U\ci\al\ar[r,"U*p"]\ar[dr,"\mu"']&\DT X\ar[d,"\DT f"]&X\ar[d,dashed,"f"]\\ 
&\DT Z&Z.
\end{tikzcd}
$$%\end{equation} 
...
As $U*p$ is a coprojection, it suffices to show 
...
$$
\begin{tikzcd}
(U(\al(i))\ar[rr,"(U*p)_i"]&&X\ar[rr,"f"]&&Z\ar[rr]&&Y)&=\\ 
(U(\al(i))\ar[rrrr,"\ld_i"]&&&&Z\ar[rr]&&Y)&=\\ 
(U(\al(i))\ar[rrrrrr]&&&&&&Y)&=\\ 
(U(\al(i))\ar[rr,"(U*p)_i"]&&X\ar[rrrr]&&&&Y).
\end{tikzcd}
$$ 
Indeed, the first and third equalities follow from the fact that $U*p$ is a coprojection, and the second equality follows from the fact that $\ld_i$ is a morphism in $\C_Y$. 
\end{proof} 
\end{s}
...
$$ 
is bijective. Let us abbreviate $(Y\to A)\in\C_A$ by $Y$, and consider the diagram 
$$
\begin{tikzcd}
...
Clearly, Claim 2 implies Claim 1. Let us prove Claim~2. Applying $\pp$ to \qr{523v1}, we get the commutative diagram 
$$
\begin{tikzcd}
...
\bb N':=\ic_n\mbf n.
$$ 
We clearly have $\bb N'\in\Ind(\Set)$. We want to show\bigskip

\centerline{(a) $\bb N'$ is isomorphic (in $\Ind(\Set)$) to no object of $\Set$.}

\nn We claim\bigskip

\centerline{(b) the coprojection $p_n:\mbf n\to\bb N'$ is a monomorphism.}

\begin{proof}[Proof of (b)] 
Let $f$ and $g$ be two morphisms from some object $A$ of $\Ind(\Set)$ to $\mbf n$ such that $p_n\ci f=p_n\ci g$: 
$$
\begin{tikzcd}
A\ar[r,yshift=0.7ex,"f"]\ar[r,yshift=-0.7ex,"g"']&\mbf n\ar[r,"p_n"]&\bb N'.
\end{tikzcd}
$$ 
It suffices to prove $f=g$. Let $X$ be in $\Set$. As the obvious maps $f_X$ and $g_X$ in the diagram
$$
\begin{tikzcd}
A(X)\ar[r,yshift=0.7ex,"f_X"]\ar[r,yshift=-0.7ex,"g_X"']&\mbf n^X\ar[r,"p_n\ci"]&\ds\col_m\mbf m^X
\end{tikzcd}
$$ 
satify $p_n\ci f_X=p_n\ci g_X$, there is an $m>n$ such that the diagram 
$$
\begin{tikzcd}
A(X)\ar[r,yshift=0.7ex,"f_X"]\ar[r,yshift=-0.7ex,"g_X"']&\mbf n^X\ar[r,"p_{m,n}"]&\mbf m^X
\end{tikzcd}
$$ 
commutes, where $p_{m,n}$ is the obvious injection. This implies $f_X=g_X$. As $X$ is arbitrary, we get $f=g$.
\end{proof}

\begin{proof}[Proof of (a)] 
Let $f:A\epi\bb N'$ be an epimorphism (in $\Ind(\Set)$) from some object $A$ of $\Set$ onto $\bb N'$, and let us derive a contradiction. By definition of $\bb N'$, our epimorphism $f$ factors through $p_n:\mbf n\to\bb N'$ for some $n$. This implies that $p_n$ is an epimorphism. As it is also a monomorphism by (b), we see that $p_n$ is an isomorphism. Using (a) again, we get an injection of $\mbf{n+1}$ into $\mbf n$, a contradiction. 
\end{proof} 
...
\begin{s} 
P.~138, proof of Proposition 6.2.1, sentence 

``Let $w\in\Hom_\C(X,\al(i))$ and let $s:i_0\to i$.'' 

This sentence is on the fourth line of the proof when the lines of the proof are counted backward from the last to the first. Here is a more detailed phrasing: 

Let $y$ be in $\col\Hom_\C(X,\al)$. Then $y$ is of the form $p_i(w)$ where 
$$
...
Here is a consequence of Proposition 6.3.4:
...
$$ 
\begin{uspb}
I don't understand why $\HOM_{\PSh(X,\A)}(F,G)(U)$ depends functorially on $U\in\C_X$.
\end{uspb}
