...
\nn(b) The proof of (b) is similar. %The proof of (c) is left to the reader.
...
Even if it is straightforward, we %sketch the proof of
state and prove formally the fact that the conditions summarized by \qr{xfl1} and \qr{xfl2} are equivalent. %For the purpose of this proof, it is better
We prefer to rewrite \qr{xfl1} as 
...
\end{tikzcd}
\end{equation} 
%It suffices then to observe the equalities $$(\eta_{X'}\ci f)_X(\id_X)=\eta_{X',X}(f)=\eta_{X',R(X')}(\id_{R(X')})\ci L(f),$$ equalities respectively justified by the definition of the vertical composition of morphisms of functors and by the naturality of $\eta$. 

%Even if it is straightforward, we state and prove the following lemma:
\begin{lem}
...
$$ 
are morphisms of functors, then %the conditions summarized by \qr{xfl2} and \qr{xfl1b} are equivalent. More precisely, 
we have 
$$
...
\begin{proof}
%It suffices then to o
The equalities 
$$%\begin{equation}\lb{ef}
(\eta_{X'}\ci f)_X(\id_X)=\eta_{X',X}(f)=\eta_{X',R(X')}(\id_{R(X')})\ci L(f)
$$%\end{equation} 
are respectively justified by the definition of the vertical composition of morphisms of functors and by the naturality of $\eta$. As the Yoneda Lemma (Theorem~\ref{yol} p.~\ref{yol}) implies 
$$
...
$$
\col_{i,j}\al(i,j)\iso\col_j\col_i\al(i,j)
$$ 
...
%An important particular case is when $\C$ is $\A$, $G$ is $\id_\A$ and $Z=R(Y)$, where $R:\B\to\A$ is right adjoint to $F$. 
...
%We have for any functor $\al:I\to\A$ and any object $Y$ in $\B$ $$ \Hom_\B\left(F\left(\col\al\right),Y\right)\iso\Hom_\C\left(G\left(\col\al\right),Z\right)\xr\sim\Hom_\C\left(\col G\ci\al,Z\right)$$ $$\xr\sim\lim\ \Hom_\C(G(\al),Z)\iso\lim\ \Hom_\B(F(\al),Y)\xleftarrow\sim\Hom_\B(\col F\ci\al,Y),$$ and the conclusion follows from Proposition~\ref{yp} p.~\pr{yp}.\end{proof}

\begin{proof}%[Second proof] 
Let $\theta$ be the isomorphism $\Hom_\B(F(\ ),Y)\xr\sim\Hom_\C(G(\ ),Z)$, let $p:\al\to\DT\col\al$ be the coprojection, and let $Y$ be in $\B$. Consider the self-explanatory 
...
\end{tikzcd}
$$ 
The last three horizontal arrows are projections. %in the sense of Definition~\ref{plim} p.~\pr{plim}. 
...
$$ 
by $k:=h\ci\Hom_\C(g,Z)\ci\theta_{\col\al}$. It is enough to check that we have $\Hom_\B(f,Y)=k$. As $\Hom_\B(q,Y)$ %$\Hom_\C(G(p),Z)$ 
is a projection, this equality follows from the commutativity of \qr{faq} and \qr{dbf}.
\end{proof}
...
\DT\Hom_{\C^\wg}(h(p),A)\ar{rrr}[swap]{\Hom_{\C^\wg}(h(p),A)}&&&\Hom_{\C^\wg}(h(p),A),
\end{tikzcd}
$$ 
...
%To prove (a) $\then$ (d), observe the isomorphisms $$\Hom_{\C'}(F(X),X')\iso\Hom_{\C^\wg}\big(h(X),\Hom_{\C'}(F(\ ),X')\big)$$ $$\iso\lim\Hom_{\C^\wg}\big(h\ci\al,\Hom_{\C'}(F(\ ),X')\big)\iso\lim\Hom_{\C'}(F\ci\al,X').$$ Here $\lim\Hom_{\C'}(F\ci\al,X')$ is \emph{the} projective limit of $\Hom_{\C'}(F\ci\al,X')$ in the sense of Convention~\ref{thep} p.~\pr{thep}. In particular we have natural projections $$\lim\Hom_{\C'}(F\ci\al,X')\to\Hom_{\C'}(F(\al(i)),X').$$ To complete the proof, it suffices to note that the morphism $$\Hom_{\C'}(F(X),X')\to\Hom_{\C'}(F(\al(i)),X')$$ obtained by composing the above isomorphisms with the above projection coincides with the natural one.
\end{proof}
...
\begin{proof}
By Convention~\ref{thep} p.~\pr{thep}  
$$
...
\begin{s}
P.~42. Here are some additional details about the last diagram on p.~42:
...
$$
\begin{tikzcd}
\al(i)\ar[d,"\id"']\ar[r,"\pp(\id)"]&\bt(i)\ar[d,"\bt(f)"]\\ 
...
\begin{s}\lb{sbbc}
P. 47, Definition 2.2.6 (stated below as Definition~\ref{dsbc} p.~\pr{dsbc}), notion of stability by base change. We make an easy but useful observation. Let $I,J$ and $\C$ be three categories. 
...
Recall that we denote horizontal composition of morphisms of functors by $*$ (see Definition~\ref{dil1} p.~\pr{dil1}).

The effect of the functor \qr{pcp} on morphisms can be described as follows: If $\theta:\al\to\bt$ is a morphism in $\C^I$, then the morphism $\pp_*\theta:\pp_*\al\to\pp_*\bt$ in $\C^J$ is defined by $\pp_*\theta:=\theta*\pp$, which is in turn defined by $(\theta*\pp)_j:=\theta_{\pp(j)}$. 
\end{s} 

%

\begin{s}\lb{s232} 
P.~51, Definition 2.3.2. Recall that we have functors $I\xl\pp J\xr\bt\C$. We spell out Definition 2.3.2 using the terminology of Section~\ref{defat} p.~\pr{defat}. Let $\bt$ be in $\C^J$, and let $\pp^\dg\bt$ and $\pp^\ddg\bt$ be in $\C^I$. 
...
%\paragraph{First proof of \qr{coco} p.~\pr{coco}}%\nn\textbf{First proof of \qr{coco}}
...
To define $g$, we form the diagram
...
\end{equation} 
and the existence of $f$ follows from the fact that \qr{fr2} implies \qr{fr1}.
...
$$
More precisely, let $X$ be in $\C$, let $p:\al\to\DT X$ be a coprojection, define $\bt:I\to\C_Y$ by $\bt(i):=(\al(i)\xr{p_i}X\xr fY)$ and $q:\bt\to\DT(X\xr fY)$ by $q_i:=p_i$: 
$$
\begin{tikzcd}
...
\begin{s} 
Proposition~\ref{b} yields the following minor variant of Theorem 2.3.3 (i) p.~52 of the book (stated above as Theorem~\ref{233i} p.~\pr{233i}). Let $I\xl\pp J\xr\bt\C$ be functors and let $\bt$ be in $\C^J$. 
...
\col\al\ci\pp\ar{r}[swap]{a}&\ds\col_{\ell}\col_{\theta(\pp(j))\to\ell}\al(\pp(j))\ar{r}[swap]{b}&\ds\col_{k}\col_{\theta(\pp(j))\to\psi(k)}
\end{tikzcd}
$$ 
...
\end{tikzcd}
$$ 
where the vertical arrows are coprojections, the squares above $a$ and $e$ are obtained by the proof of \qr{coco} p.~\pr{coco}, the squares above $b$ and $d$ result from the cofinality of $\psi$, and the squares above $c$ result from the cofinality of $\pp_k$. %Now the proposition follows from \qr{coco}.
In particular, the maps $a,b,c,d$ and $e$ are bijective. %, and $e^{-1}\ci d\ci c\ci b\ci a$ is the natural map from $\col\al\ci\pp$ to $\col\al$. 
%Set $f:=e^{-1}\ci d\ci c\ci b\ci a$. 
As the bijection $f:=e^{-1}\ci d\ci c\ci b\ci a$ satisfies $f\ci p_j=q_j$, it is the natural map from $\col\al\ci\pp$ to $\col\al$.
\end{proof}
\end{s}
...
\begin{subequations}\lb{fyx'}% other version https://docs.google.com/document/d/1cvapjvUY-qwpIWY90ncOCdZt_1pYSS07lZrnvBDp5rM/edit
\begin{align}
...
We shall also use implicitly, not only the statements of the bijections \qr{fyx'1} and \qr{fyx'2}, but also their proofs. 
...
$$ 
is bijective. Consider the commutative diagram below, in which we abbreviate 
$$
(Y\to A)\in\C_A
$$ 
by $Y$: 
...
%We also have a functor $\al:M\to\C$. Claim:
$$
%j\mt
...
\begin{proof}
Let $j\to j'$ be a morphism in $J$. It is easily checked that there is a unique dashed arrow which make all the diagrams 
$$
\begin{tikzcd}
...
If $\psi$ is cofinal, then $M[I\to K\leftarrow J]\to I$ is cofinal. 
...
Proof of (v): We add a few details. Recall the setting: the category $\C$ admits finite projective limits, small coproducts, and a generator $G$. We already know that $\pp_G:=\Hom_\C(G,\ )$ is conservative, left exact, and that it preserves monomorphisms. 
...
$$ 
obtained by composing the chain of isomorphisms in the proof, coincides with $\ci\ee_G$. 

To prove this, we shall define a map 
$$
g:\Hom_{\F^\wg}(\pp\psi(G),F)\to\Hom_{\F^\wg}(G,F),
$$ 
claim that $f=g=\ci\ee_G$, and leave it to the reader to prove the claim. 

We define $g$ as follows: Taking into account the definitions of $\pp$ and $\psi$, we must define 
$$
g:\Hom_{\F^\wg}\left(\Hom_\C\left(\ \ ,\col_{X\to G}X\right),F\right)\to\Hom_{\F^\wg}(G,F),
$$ 
where $X\to G$ stands for $(X\to G)\in\F_G$. Let 
$$
\ld:\Hom_\C\left(\ \ ,\col_{X\to G}X\right)\to F
$$ 
be given. We must define $\mu:G\to F$. Letting $Y$ be in $\C$, we must define 
$$
\mu_Y:G(Y)\to F(Y).
$$ 
Letting $Y\to G$ be given, we must define $\mu_Y(Y\to G):Y\to F$. We do that by applying 
$$
\ld_Y:\Hom_\C\left(Y\ ,\ \col_{X\to G}X\right)\to F(Y)
$$ 
to the coprojection $Y\to\col_{X\to G}X$ attached to $Y\to G$. We leave it to the reader to check that this construction does define a morphism of functors from $G$ to $F$. 
\end{s} 
...
\nn\textbf{Step 3.} Let $A$ be in $\F^\wg$, and let $(B_i\to A)_{i\in I}$ be a small filtrant inductive system in $(\F^\wg)_A$. We claim that the natural morphism
\begin{equation}\lb{colcoim}
\col_i\Coim(B_i\to A)\to\Coim\left(\col_iB_i\to A\right)
\end{equation}
is an isomorphism.

Let $X$ and $Y$ be in $\C$.

\nn\textbf{Step 4.} Let $z:Z\to X$ be in $\F_X$. Then the natural map 
...
$$%\end{equation} 
%$$\Hom_{\F^\wg}(\Coim\pp(z),\pp(Y))$$ 
%\begin{equation}\lb{fczy}
%%\Hom_{\F^\wg}(\Coim\pp(z),\pp(Y))
%\iso\Ker\Big(\Hom_{\F^\wg}\big(\pp(Z),\pp(Y)\big)\rightrightarrows\Hom_{\F^\wg}\big(\pp(Z)\tm_{\pp(X)}\pp(Z),\pp(Y)\big)\Big).
%\end{equation}
%\begin{equation}\lb{fczy}\Hom_{\F^\wg}(\Coim z',Y')\iso\Ker\Big(\Hom_{\F^\wg}\big(Z',Y'\big)\rightrightarrows\Hom_{\F^\wg}\big(Z'\tm_{X'}Z',Y'\big)\Big).\end{equation} 
$$
...
$$ 
(We have omitted some parenthesis to save space.) Moreover, for any $Z\to Y$ in $\Hom_\C(Z,Y)$, the morphism $Z\to Y$ is in 
$$
...
$$ 
%where we have written $z',Z',X',Y'$ for $\pp(z),\pp(Z),\pp(X),\pp(Y)$ to save space.
%We claim that there is, for all $z:Z\to X$ in $\F_X$ and all $Y$ in $\C$, a natural isomorphism $$\Hom_\C(\Coim z,Y)\iso\Hom_{\F^\wg}(\Coim\pp(z),\pp(Y)).$$ 
...
\nn\textbf{Step 5.} Let us denote by $I$ the set of finite subsets of $\Ob(\F_X)$, ordered by inclusion. Regarding $I$ as a category, it is small and filtrant. Let $\xi:I\to\F_X$ be the functor defined by letting $\xi(A)$ be the natural morphism $\bigsqcup_{z\in A}\zeta(z)\to X$. We claim that the natural morphism 
$$
\col_{A\in I}\pp\zeta\xi(A)\to\pp(X) 
$$ 
is an epimorphism.

\nn\textbf{Step 6.} We claim that the natural morphism %re is a natural isomorphism 
$$
\col_{A\in I}\Coim\xi(A)\to X
$$ 
is an isomorphism.

These steps imply the theorem: Indeed, we have, in the above setting, 
\begin{align*} 
\Hom_\C(X,Y)&\xr\sim\Hom_\C\left(\col_{A\in I}\Coim\xi(A),Y\right)&\text{by Step 6}\\ \\ 
&\xr\sim\lim_{A\in I}\Hom_\C(\Coim\xi(A),Y)\\ \\ 
&\xr\sim\lim_{A\in I}\Hom_{\F^\wg}\Big(\Coim\pp\xi(A),\pp(Y)\Big)&\text{by Step 4}\\ \\ 
&\xr\sim\Hom_{\F^\wg}\left(\col_{A\in I}\Coim\pp\xi(A),\pp(Y)\right)\\ \\ 
&\xr\sim\Hom_{\F^\wg}\left(\Coim\left(\col_{A\in I}\pp\zeta\xi(A)\to\pp(X)\right),\pp(Y)\right)&\text{by Step 3}\\ \\ 
&\xr\sim\Hom_{\F^\wg}(\pp(X),\pp(Y))&\text{by Step 5,}
\end{align*} 
and it is not hard to verify that the composition of these six bijections %coincides with the natural map $\Hom_\C(X,Y)\to\Hom_{\F^\wg}(\pp(X),\pp(Y))$. 
and the natural map $\Hom_\C(X,Y)\to\Hom_{\F^\wg}(\pp(X),\pp(Y))$ are isomorphic in $\Mor(\Set)$. 

It remains to prove Steps 3, 4, 5 and 6. %We refer to the book for Step 3. The proof of Step 3' is the same. 

\begin{proof}[Proof of Step 3] 
Set $B:=\col_iB_i$. 

\begin{lem}\lb{bibj}
The natural morphism $\col_iB_i\tm_AB_i\to B\tm_AB$ is an isomorphism. 
\end{lem}
...
$$\begin{tikzcd}
\ds\col_iB_i\tm_AB_i\ar[r,"a"]&\ds\col_{i,j}B_i\tm_AB_j\\ 
B_i\tm_AB_i\ar[u]\ar[r,"\id"']&B_i\tm_AB_i,\ar[u]
\end{tikzcd}$$ 

$$\begin{tikzcd}
\ds\col_{i,j}B_i\tm_AB_j\ar[r,"b"]&\ds\col_i\col_jB_i\tm_AB_j\ar[r,"c"]&\ds\col_iB_i\tm_AB\\ 
&\ds\col_jB_i\tm_AB_j\ar[u]\ar[ur]\\ 
B_i\tm_AB_j\ar[uu]\ar[r,"\id"']&B_i\tm_AB_j\ar[u]\ar[r]&B_i\tm_AB,\ar[uu]
\end{tikzcd}$$ 

$$\begin{tikzcd}
\ds\col_iB_i\tm_AB\ar[r,"d"]&B\tm_AB\\ 
B_i\tm_AB.\ar[u]\ar[ur]
\end{tikzcd}$$ 

\nn(Note that the last row of the first diagram coincides with the first row of the second diagram, and the last row of the second diagram coincides with the first row of the third diagram.) The composition $d\ci c\ci b\ci a$ equals \qr{colcoim}, and $a$ is an isomorphism by Corollary 3.2.3 (ii) p.~79 of the book, $b$ is an isomorphism for obvious reasons, $c$ and $d$ are isomorphisms because inductive limits in $\F^\wg$ indexed by $I$ are stable by base change (see \S\ref{sbbc} p.~\pr{sbbc}). This proves Lemma~\ref{bibj}.
\end{proof}

Taking the definition of $\Coim$ into account, we have 
$$
\Coim(B_i\to A)=\Coker(B_i\tm_AB_i\parar B_i),
$$ 
$$
\Coker(B\tm_AB\parar B)=\Coim(B\to A).
$$ 
%Consider the diagrams 
Moreover, there is an obvious commutative diagram
$$
\begin{tikzcd}
\ds\col_i\Coker(B_i\tm_AB_i\parar B_i)\ar[r,"e"]&\ds\Coker(\col_i(B_i\tm_AB_i)\parar B)\\ 
\Coker(B_i\tm_AB_i\parar B_i),\ar[u]\ar[r,"\id"']&\Coker(B_i\tm_AB_i\parar B_i),\ar[u]
\end{tikzcd}
$$ 
where $e$ is an isomorphism, and Lemma \ref{bibj} yields a commutative diagram 
$$
\begin{tikzcd}
\ds\Coker(\col_i(B_i\tm_AB_i)\parar B)\ar[r,"f"]&\ds\Coker(B\tm_AB\parar B)\\ 
\Coker(B_i\tm_AB_i\parar B_i)\ar[u]\ar[r,"\id"']&\Coker(B_i\tm_AB_i\parar B_i),\ar[u]
\end{tikzcd}
$$ 
where $f$ is an isomorphism. This implies that \qr{colcoim} is an isomorphism, completing the proof of Step~3. %(Note that the last row of the first diagram coincides with the first row of the second diagram.) 
\end{proof} 
...
$$ 
is injective. This implies our claims. % yields $$\Hom_\C(\Coim z,Y)$$ $$\iso\Ker\Big(\Hom_{\F^\wg}\big(\pp(Z),\pp(Y)\big)\rightrightarrows\Hom_{\F^\wg}\big(\pp(Z)\tm_{\pp(X)}\pp(Z),\pp(Y)\big)\Big)$$ $$\iso\Hom_{\F^\wg}\Big(\Coker\big(\pp(Z)\tm_{\pp(X)}\pp(Z)\rightrightarrows\pp(Z)\big),\pp(Y)\Big)$$ $$=\Hom_{\F^\wg}(\Coim\pp(z),\pp(Y)).$$ 
\end{proof}
