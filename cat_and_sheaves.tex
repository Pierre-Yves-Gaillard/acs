...
We add the assumption that\bigskip

\centerline{$(\oo K(\C),\widetilde\PP^{\op})$ is $\Hom_\C$-injective,}\bigskip

\nn and we prove the above statement as follows, using \S\ref{abar} p.~\pr{abar}:
...
$$ 
because $\widetilde\PP$ is $\oo K(G)$-projective and $(\oo K(\C),\widetilde\PP^{\op})$ is $\Hom_\C$-injective. Similarly we have 
$$
\Hom_{\C'}^\bu(\bar A(X),F(\bar B(Y)))\iso\RHom_{\C'}(X,RF(Y))
$$ 
because $(\oo K_{\text{hi}}(\C'),\oo K(\C')^{\op})$ is $\Hom_{\C'}$-injective by Corollary 14.3.2 p.~356 of the book and $\oo K_{\text{hi}}(\C')$ is $\oo K(F)$-injective. Finally we have the adjunction 
$$
...
\begin{comment}
% previous version
\centerline{$\widetilde\PP^{\op}$ is $\Hom_\C^\bu(\ \ ,F(Y))$-injective for all $Y$ in $\oo K_{\text{hi}}(\C')$,}\bigskip

and we prove the above statement as follows, using \S\ref{abar} p.~\pr{abar}:
...
\Hom_{\C'}^\bu(\bar A(X),F(\bar B(Y)))\iso\RHom_{\C'}(X,RF(Y))
$$ 
because $\widetilde\PP^{\op}$ is $\Hom_\C^\bu(\ \ ,F(\bar B(Y)))$-injective and $\oo K_{\text{hi}}(\C')$ is $\oo K(F)$-injective. Finally we have the adjunction 
$$
\Hom_\C^\bu(G(\bar A(X)),\bar B(Y))\iso\Hom_{\C'}^\bu(\bar A(X),F(\bar B(Y))),
$$ 
and it suffices to observe that the three above bijections are functorial in $X$ and $Y$.%we get a bijection $$\RHom_\C(LG(X),Y)\iso\RHom_{\C'}(X,RF(Y))$$
