...
\begin{lem}\lb{r52}
Let $I$ and $K$ be two objects of the category $\Cat$ of small categories (see Definition~\ref{myuscat} p.~\pr{myuscat}), let $\Phi:I\to\Cat$ be a functor, and let $\theta:\Phi\to\DT K$ be a morphism of functors (recall that $\DT$ is the diagonal functor, see Definition~\ref{diag} p.~\pr{diag}). Assume 
...
We want to prove that $\pp^\dg$ defined by \qr{236} p.~\pr{236} is a functor. In the setting of Lemma~\ref{r52} we set $K:=\C$, we choose $\U$ so that $I$ and $k=\C$ are in $\Cat$, we define $\Phi:I\to\Cat$ by $\Phi(i):=J_i$ and we consider the morphism of functors $\theta:\Phi\to\DT\C$ such that $\theta_i:\Phi(i)=J_i\to\C$ is the composition of $\bt$ with the forgetful functor from $J_i$ to $J$. We assume \qr{52}. Then $\col\theta$ is nothing but $\pp^\dg(\bt)$. In particular $\pp^\dg(\bt)$ is a functor by Lemma~\ref{r52}. q.e.d.
...
\end{tikzcd}
$$ 
and claim that $f$ and $g$ are inverse isomorphisms. We have coprojections 
$$
\bt(j)\xr{p_j}\col\bt,\qquad\bt(j)\xr{q_{i,j,u}}\col_{(j,u)\in J_i}\bt(j)\xr{r_i}\col_i\ \col_{(j,u)\in J_i}\bt(j).
$$ 
...
\ds\col_i\col_{(j,u)\in J_i}\bt(j)\ar{r}[swap]{g}&\col\bt.
\end{tikzcd}
\end{equation} 
...
Let us prove that $f\ci g$ is the identity of $\col_i\col_{(j,u)\in J_i}\bt(j)$. As we have 
$$
...
\end{tikzcd}
$$ 
where $u_*$ denotes the morphism induced by $u$, commutes. The top square commutes by  \qr{cts}, and the bottom square commutes for obvious reasons. 

We leave the proof of the fact that $g\ci f$ is the identity of $\col\bt$ to the reader. q.e.d.

In Section~\ref{2111} p.~\pr{2111} we shall give another proof of \qr{coco}.
...
This section is divided into two parts. In Part~1 we give a proof of Proposition 3.4.3 (i) which is slightly different from the one in the book. In Part~2 we give a proof of \qr{coco} p.~\pr{coco} slightly different from the one in \S\ref{scoco} p.~\pr{scoco}. 
...
Choose a universe $\U$ making $\C$ a small category, let $\Phi$ be the functor from $J$ to $\Cat$ defined by $\Phi(j):=I_{\psi(j)}$, recall that $\DT$ denotes the diagonal functor (see Notation~\ref{diag} p.~\pr{diag}), and let $\theta:\Phi\to\DT\C$ be the morphism of functors such that $\theta_j:I_{\psi(j)}\to\C$ is the composition of $\al$ with the natural functor from $I_{\psi(j)}$ to $M$. Then 
$$
...
$$ 
be the coprojections. There is an obvious morphism 
$$
...
In \S\ref{2111} p.~\pr{2111} we gave a first proof of \qr{coco} p.~\pr{coco}. Here is another one.
...
$$
\begin{tikzcd} 
\ds\col_i\al(i)\ar[yshift=0.7ex]{r}{f}&\ds\col_{i,j,u}\al(i),\ar[yshift=-0.7ex]{l}{g}
\end{tikzcd}
$$ 
and leave the verification that $f$ and $g$ are inverse isomorphisms to the reader. We have coprojections 
$$
\al(i)\xr{p_i}\col_i\al(i),\quad\al(i)\xr{q_{i,j,u}}\col_{i,j,u}\al(i).
$$ 
We define $f$ by the condition that  
$$
\begin{tikzcd}
\al(i)\ar{d}[swap]{p_i}\ar{r}{\id}&\al(i)\ar{d}{q_{i,\pp(i),\id_{\pp(i)}}}\\ 
\ds\col_i\al(i)\ar{r}[swap]{f}&\ds\col_{i,j,u}\al(i)
\end{tikzcd}
$$ 
commutes for all $i$. To define $g$, consider the diagram
\begin{equation}\lb{ciju}
\begin{tikzcd}
\al(i)\ar{d}[swap]{\id}\ar{r}{\id}&\al(i)\ar{d}{p_j}\\ 
\al(i)\ar{d}[swap]{q_{i,j,u}}\ar{r}{g_{iju}}&\ds\col_i\al(i)\ar{d}{\id}\\ 
\ds\col_{i,j,u}\al(i)\ar{r}[swap]{g}&\ds\col_i\al(i).
\end{tikzcd}
\end{equation} 
We let $i$ be in $I$ and define $g_{iju}$ by the condition that the top square of \qr{ciju} commutes for all $(j,u)$. Then we define $g$ by the condition that the bottom square of \qr{ciju} commutes for all $i$. q.e.d.
