...
\end{equation} 
for all $G:\C_\SSS\to\A$ and all $\mu:RF\to G$ (see Definition~\ref{dil1} p.~\pr{dil1} and \S~\ref{durl} p.~\pr{durl}). 
...
We add the assumption that $\C$ is a Grothendieck category.
...
I don't understand the argument used in the book to prove this statement. I think this is an important point because (I believe) a similar argument is used to prove Theorem 14.4.8 (iii) p.~362 and Theorem 18.6.4 (vii) p.~451.

It seems to me the best way to explain to the reader what precisely I don't understand is to prove the above statement under the additional assumption
\begin{equation}\lb{upb}
(\oo K(\C),\widetilde\PP^{\op})\text{ is }\Hom_\C\text{-injective}.
\end{equation} 
My question will then be: how can one get rid of this extra assumption?

To prove the statement assuming \qr{upb} we shall use \S\ref{abar} p.~\pr{abar}.

Let $\T$ be the set of qis in $\widetilde\PP$, let $A:\oo D(\C)\to\widetilde\PP_\T$ be a quasi-inverse to the equivalence $\widetilde\PP_\T\to\oo D(\C)$, and let $A_o:\Ob(\oo K(\C))\to\Ob(\widetilde\PP)$ be the map induced by $A$. Similarly, let $B:\oo D(\C')\to\oo K_{\text{hi}}(\C')$ be a quasi-inverse to the equivalence $\oo K_{\text{hi}}(\C')\to\oo D(\C')$, and let $B_o:\Ob(\oo K(\C'))\to\Ob(\oo K_{\text{hi}}(\C'))$ be the map induced by $B$. Let $X$ be in $\oo K(\C)$ and $Y$ be in $\oo K(\C')$. We have 
$$
\RHom_{\C'}(LG(X),Y)=\Hom_{\C'}^\bu(G(A_o(X)),B_o(Y))
$$ 
because $\widetilde\PP$ is $\oo K(G)$-projective (as proved in the book) and $(\oo K_{\text{hi}}(\C'),\oo K(\C')^{\op})$ is $\Hom_{\C'}$-injective by Corollary 14.3.2 p.~356 of the book. Similarly we have 
$$
\Hom_\C^\bu(A_o(X),F(B_o(Y)))=\RHom_\C(X,RF(Y))
$$ 
because $\oo K_{\text{hi}}(\C')$ is $\oo K(F)$-injective and $(\oo K(\C),\widetilde\PP^{\op})$ is $\Hom_\C$-injective by Assumption~\qr{upb}, and we conclude by invoking the adjunction 
$$
\Hom_{\C'}^\bu(G(A_o(X)),B_o(Y))\iso\Hom_\C^\bu(A_o(X),F(B_o(Y))),
$$ 
which is functorial in $X$ and $Y$.
...
\begin{s}
P. 361, Theorem 14.4.8. We add the assumption that $\C_1$ and $\C_2$ are Grothendieck categories. 
\end{s}
