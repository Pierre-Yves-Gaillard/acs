% about "categories and sheaves" (version-0259)
...
\nn\emph{(i)} We say that inductive limits in $\C$ indexed by $I$ are \emph{stable by base change}\index{base change} if for any morphism $Y\to Z$ in $\C$, the base change functor 
$\C_Z\to\C_Y$ given by 
$$
...
\begin{s}\label{gcsbc}
P.~186, Definition 8.3.24 (definition of a Grothendieck category\index{Grothendieck category}). By Lemma 8.3.9 p.~83 of the book, in a Grothendieck category $\U$-small filtrant inductive limits are stable by base change (Definition 2.2.6 p.~47 of the book, stated as Definition~\ref{dsbc} p.~\pageref{dsbc}).
\end{s}
...
\begin{s}
P.~413, Example 17.2.7 (i). Recall that $f:X\to Y$ is a continuous map of small topological spaces. %The book says that $f$ is a left exact morphism of sites. It seems to me that $f$ is also right exact. 
