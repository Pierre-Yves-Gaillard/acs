...
\begin{prop}\lb{copr}
Let $I$ and $\C$ be arbitrary categories, let $\al:I\to\C$ be a functor, let $X$ be an object of $\C$ and let $p:\al\to\DT X$ be a morphism in $\C^I$. For any universe $\U$ such that $\C$ is a $\U$-category, let $h_\U:\C\to\C^\wg_\U$ be the Yoneda embedding, and, for any category $\C'$ and any functor $F:\C\to\C'$, define the morphism 
$$
...
$$
in $\Fct(I^{\op},\Set_\U)$ is a projection (see Definition~\ref{plim} p.~\pr{plim}) for any $X'$ in $\C'$ and for some (and thus for any) universe $\U$ such that $I$ is $\U$-small and $\C'$ is a $\U$-category''. Note that $(1F)$ and $(2F)$ are equivalent by Propositions~\ref{aioc} and \ref{aic}. Then the conditions \emph{(a)} and \emph{(b)} below are equivalent:

\nn\emph{(a)} The equivalent conditions $(1h_\U)$ and $(2h_\U)$ hold for some (and thus for any) universe $\U$ such that $I$ is $\U$-small and $\C$ is a $\U$-category. 
...
\begin{proof}
Propositions~\ref{aioc} and \ref{aic} imply (a) $\ssi$ (b), (c) $\ssi$ (d). Moreover (c) implies trivally (a). To prove (a) $\then$ (d), let $X'$ be in $\C'$ and set $A:=\Hom_{\C'}(F(\ ),X')$. We get the commutative diagram 
$$
\begin{tikzcd}
\DT A(X)\ar[d,"\sim"']\ar[rrr,"A(p)"]&&&A\ci\al\ar[d,"\sim"]\\ 
...
If $\al:I\to\C$ is a functor, if $h:\C\to\C^\wg$ is the Yoneda embedding and if $X$ is in $\C$, we denote by $$H[I,\al,\C,X]$$ the condition 
...
\nn Note that the conjunction of $H[I,\al,\C,X]$ and $C[I,\al,\C,X]$ implies that $X$ is a \emph{universal} inductive limit of $\al$. The proof of the following proposition is obvious.
...
\begin{s}
\begin{prop}\lb{patho}
If $I$ and $J$ are big categories, if $\C$ is a $\U$-category, if $\al:I\tm J\to\C$ is a functor and if $\col_i\al(i,\ )$ exists universally in the sense of Definition~\ref{uil} p.~\pr{uil}, then $\col_i\al(i,j)$ exists universally for all $j$. 
\end{prop}
In \S\ref{c38} p.~\pr{c38} we saw that, without the adverb ``universally'', the claim is false.
\begin{proof}
To prove the proposition, we may assume that $I$ is small. Let $h:\C\to\C^\wg$ be the Yoneda embedding and $j$ a fixed object of $J$. In the notation of Proposition~\ref{stupid} p.~\pr{stupid}, we must prove 
$$
...
\begin{thm}\lb{fgd}
Let $I$ be a small category; %with disjoint hom-sets; 
...
If $I$ and $J$ are big categories, if $\C$ is a $\U$-category, if $I\xl\pp J\xr\bt\C$ are functors, and if 
...
\end{thm}
\begin{proof}
The first sentence and the ``only if'' part of the second sentence follow from Theorem~\ref{233i} p.~\pr{233i}. To prove the ``if'' part of the second sentence, we may assume that $J$ is small. Let $i$ be in $I$. In the notation of Proposition~\ref{stupid} p.~\pr{stupid}, we must prove 
$$
...
\end{cor}
\begin{proof}
This follows from Proposition~\ref{copr} p.~\pr{copr} and Theorem~\ref{stupidkan}.
\end{proof}
% previous version:
% https://docs.google.com/document/d/1K004uKK3ECssS-Y1etYqzLcEilskE8Hlb0wVrElSqM0/edit
%\begin{comment}
...
\begin{s}\lb{742}
P. 162, Definition 7.4.2. If $F:\C\to\A$ is a functor and $X$ an object of $\C$, then the condition that $F$ is right localizable at $X$ does not depend on the choice of a universe $\U$ such that $\C$ and $\A$ are $\U$-categories.
\end{s}
...
By Proposition 8.3.27 (i) and Lemma 3.3.9 p.~83 of the book, in a Grothendieck $\U$-category\index{Grothendieck category} $\U$-small filtrant inductive limits are stable by base change (see Section~\ref{sbsarsbc} p.~\pr{sbsarsbc}).
...
The second sentence in the above quote is justified by Theorem~\ref{stupidkan} p.~\pr{stupidkan}. %\S\ref{744} p.~\ref{744}. %Proposition 7.4.4 p.~163 of the book.
...
This is justified by Theorem~\ref{stupidkan} p.~\pr{stupidkan}. 
\end{s}
