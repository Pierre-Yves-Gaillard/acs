...
Set $X_i=\al(i)$ and $Y_i=\Ima(X_i\to Y)=\Ker(Y\parar Y\sqcup_{X_i}Y)$. In particular the natural morphism $Y_i\to Y$ is a monomorphism. Since small
filtrant inductive limits are exact, 
$$
\col_iY_i\iso\Ker(Y\parar Y\sqcup_{\col_iX_i}Y)\iso\Ima(\col_iX_i\to Y)\iso Y,
$$ 
where the last isomorphism follows from the hypothesis that $\col_iX_i\to Y$ is an epimorphism together with Proposition 5.1.2 (iv). It is easy to see that the above observations %chain of isomorphism implies
imply that the natural morphism $\col_iY_i\to Y$ is an isomorphism, and that the coprojections $Y_i\to Y$ are monomorphisms. In particular, the $Y_i(G)$, which can be viewed as subsets of $Y(G)$, satisfy 
$$
\bigcup_i\ Y_i(G)=\col_iY_i(G)\xr\sim Y(G),\quad Y(G)=\bigcup_i\ Y_i(G).
$$ 
...
$$ 
Hence, there exist $i_0$ and a map $Y(G)\to Y_{i_0}(G)$ such that the composition $Y(G)\to Y_{i_0}(G)\hookrightarrow Y(G)$ is the identity. Therefore $Y_{i_0}(G)=Y(G)$ and hence, $Y_{i_0}(G)\to Y_i (G)$ is bijective for any $i_0\to i$. Hence $Y_{i_0}\to Y_i$ is an isomorphism, which implies that $Y_{i_0}\to Y$ is an isomorphism. Applying Proposition 5.1.2 (iv), we find that $X_{i_0}\to Y$ is an epimorphism. 
