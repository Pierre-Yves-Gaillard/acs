...
\begin{s}\lb{27i}
P.~65, Exercise 2.7, Line 3: %on Line 1 ``Let $Z\in\Set$'' should be ``Let $Y\to Z$ be a morphism in $\Set$'', and on Line 3 
``that the functor $\cdot\tm_ZY:\Set_Z\to\Set_Z$'' should be ``that, given $Y\in\Set_Z$, the functor $\cdot\tm_ZY:\Set_Z\to\Set_Y$''. (See also \S\ref{s3cat} p.~\pr{s3cat} below.)
\end{s}
...
\begin{s}\lb{s135a}
$*$ P.~135, Corollary 6.1.15: $f=``\ilim"\pp$ should be $f\iso``\ilim"\pp$ and $g=``\ilim"\psi$ should be $g\iso``\ilim"\psi$. (See \S\ref{s135b} p.~\pr{s135b} below.)
\end{s}
...
\nn$*$ P.~138, Proposition 6.2.1. The assumption that $I$ is small is not really necessary. 
...
$$
\begin{tikzcd}
\al\ar[d,dashed,"v"']&\pp_*\al\ar[d,"v\star\pp"']\ar[r,"w"]&\bt\\ 
...
$$
\begin{tikzcd}
\al\ar[d,dashed,"v"']&\pp_*\al\ar[d,"v\star\pp"']\ar[r,"w"]&\pp_*\gamma\\ 
...
\begin{s}\lb{s135b}
P.~135, Corollary 6.1.15. Recall the statement (see \S\ref{s135a} p.~\pr{s135a} above):
...
\nn\emph{(i)} $\ic\al$ is representable by $Z$,

\nn\emph{(ii)} there exist an object $i_0\in I$ and a morphism $\tau:Z\to\al(i_0)$ satisfying the property:

\nn for any morphism $s:i_0\to i$, there exist a morphism $q_i:\al(i)\to Z$ and a morphism $t:i\to j$ satisfying 

\nn\emph{(a)} $q_i\ci\al(s)\ci\tau=\id_Z$,

\nn\emph{(b)} $\al(t)\ci\al(s)\ci\tau\ci q_i=\al(t)$.
\end{prop}

\begin{proof}
Set $A:=\ic\al$ and let $p_i:\al(i)\to A$ be the coprojections. 

\nn$\bu$ (i) implies (ii). Let $\pp:Z\to A$ be an isomorphism. By definition of $A$ the isomorphism $\pp$ factors as $Z\xr\tau\al(i_0)\xr{p_{i_0}}A$. Let $s:i_0\to i$. We define $q_i:\al(i)\to Z$ as being the composition $\al(i)\xr{p_i}A\begin{smallmatrix}\pp\\ \longleftarrow\\ \sim\end{smallmatrix}Z$. As the three small triangles in the diagram 
$$
\begin{tikzcd}
\al(i)\ar[rr,"q_i"]\ar[rd,"p_i"]&&Z\ar[ld,"\pp","\sim"']\ar[ldd,bend left,"\tau"]\\ 
&C\\ 
&D\ar[uul,bend left,"\al(s)"]\ar[u,"p_{i_0}"']
\end{tikzcd}
$$ 
commute, we get 
$$
\pp\ci q_i\ci\al(s)\ci\tau=p_i\ci\al(s)\ci\tau=\pp,
$$ 
which implies (a). Let us prove (b). The coprojection 
$$
\Hom_\C(\al(i),\al(i))\to\col\Hom_\C(\al(i),\al)=\Hom_\C(\al(i),A)
$$ 
being the map $p_i\ci$, and $I$ being filtrant, the equalities 
$$
p_i\ci\al(s)\ci\tau\ci q_i=p_{i_0}\ci\tau\ci q_i=\pp\ci q_i=p_i=p_i\ci\id_{\al(i)}
$$ 
imply the existence of a morphism $t:i\to j$ satisfying (b). 

\nn$\bu$ (ii) implies (i). Let $\pp:Z\to A$ be the composition $Z\xr\tau\al(i_0)\xr{p_{i_0}}A$. It suffices to show that $\pp$ is an isomorphism. Let $X$ be an object of $\C$. It suffices to show that the map 
$$ 
\pp_X:\Hom_\C(X,Z)\to\Hom_\C(X,A)=\col\Hom_\C(X,\al),\quad u\mt\pp\ci u
$$ 
is bijective. 

\nn$\star\ \pp_X$ is injective. Let $u,v\in\Hom_\C(X,Z)$ satisfy $\pp\ci u=\pp\ci v$, that is $$p_{i_0}\ci\tau\ci u=p_{i_0}\ci\tau\ci v.$$ As $I$ is filtrant and as $\Hom_\C(X,A)=\col\Hom_\C(X,\al)$, there is a morphism $s:i_0\to i$ such that $\al(s)\ci\tau\ci u=\al(s)\ci\tau\ci v$. We have $p_i\ci\al(s)\ci\tau\ci u=p_{i_0}\ci\tau\ci u=\pp\ci u$, and, similarly, $p_i\ci\al(s)\ci\tau\ci v=\pp\ci v$, yielding 
$$
\pp\ci u=p_i\ci\al(s)\ci\tau\ci u=p_i\ci\al(s)\ci\tau\ci v=\pp\ci v,
$$ 
and thus $u=v$. 

\nn$\star\ \pp_X$ is surjective. Let $w:X\to\al(i)$ be a morphism. It suffices to show that there is a morphism $u:X\to Z$ such that $\pp\ci u=w$. We may assume that there is a morphism $i_0\to i$. If $q_i:\al(i)\to Z$ and $t:i\to j$ are as in (ii), we get
$$
p_i\ci w=p_j\ci\al(t)\ci w=p_j\ci\al(t)\ci\al(s)\ci\tau\ci q_i\ci w=p_{i_0}\ci\tau\ci q_i\ci w=\pp\ci q_i\ci w.
$$ 
\end{proof}
...
% Warning: the following is very bad!!!
...
$$ 
be the coprojection. To prove the existence of $\al$ as above, we let $X'\to X''$ be a morphism on $\C$ and leave it to the reader to check that there is a unique map 
$$
...
$$ 
and we leave it to the reader to check that there is a (unique) map $f$ as in \qr{cfg} such that we have, for all $X\to X'$ in $\Mor(\C)$ and all $Y\to Y'$ in $\SSS$,
$$
(p[X\to X']\ci f\ci b[Y\to Y'])(X\to Y')=a[X',Y\to Y'\to Y''](X'\to Y'')
$$ 
with $X'\to Y''\leftarrow Y'$ as above. 

Let us define $g$ in \qr{cfg}. 

Let $r$ be in the right-hand side of \qr{cfg}. There is %To any $X\to X'$ in $\SSS$ is attached 
some $(Y\to Y')\in\SSS$ and some $X'\to Y'$ in $\Mor(\C)$ such that %$$p[X\to X'](r)=a[X',Y\to Y'](X'\to Y').$$ We get in particular 
\begin{equation}\lb{pxid}
p[X\xr{\id}X](r)=a[X,Y\to Y'](X\to Y').
\end{equation}
%where $(Y\to Y')\in\SSS$ and $X\to Y'$ are attached to the identity of $X$. 
We leave it to the reader to check that there is a (unique) map $g$ as in \qr{cfg} such that 
$$
g(r)=b[Y\to Y'](X\to Y')
$$ 
...
$$ 
because we can set $Y'':=Y'$ %take $Y''$ as $Y'$ 
in \qr{y''}, yielding 
$$
...
Let $r$ be in the right-hand side of \qr{cfg}, and let $(Y\to Y')$ and $X'\to Y'$ be as in \qr{pxid}. We get $g(r)=b[Y\to Y'](X\to Y')$ and 
$$
p[X\to X'](f(g(r)))=p[X\to X'](f(b[Y\to Y'](X\to Y')))
$$ 
$$
=a[X',Y\to Y'\to Y''](X'\to Y'')
$$ 
with $Y'\to Y''$ and $X'\to Y''$ is in \qr{y''}. We also get 
$$
\al(X\to X')(a[X',Y\to Y'\to Y''](X'\to Y''))
$$ 
$$
=a[X,Y\to Y'\to Y''](X\to X'\to Y'')=a[X,Y\to Y'](X\to X').
$$ 
...
%Let us denote the object $X$ above by $X_1\oplus X_2$. 
...
\begin{conv}\lb{oplus}
In the above situation, we set $X_1\oplus X_2:=X_1\sqcup X_2$, we define $p'_j:X_1\oplus X_2\to X_j$ by $p'_j:=p_j\ci r$, and we observe that $X_1\oplus X_2$ is a product of $X_1$ and $X_2$ by $p'_1$ and $p'_2$. 
\end{conv}
\end{s}

%

The following lemma is implicit in the book. 
...
$$ 
Here $\delta_X:X\to X\tm X\iso X\oplus X$ is the diagonal morphism and $\sigma_Y:Y \oplus Y\iso Y\sqcup Y\to Y$ is the codiagonal morphism.
\end{cor}
...
$$
\begin{tikzcd}
X\tm X&X\sqcup X\ar[l,"r","\sim"']\ar[r,"\sigma_X"]&X.
\end{tikzcd}
$$ 
...
\end{tikzcd}
$$ 
is an exact commutative diagram in an category, then $Y\oplus Y'\xr{(c,a)}X$ is an epimorphism. 
\end{lem}
...
\end{tikzcd}
$$ 
quasi-commutes. The functors $\ld$ and $\ld'$ are fully faithful by Section~\ref{gcsbc} p.~\pr{gcsbc} and Theorem~\ref{536} p.~\pr{536} above. %Theorem 5.3.6 p.~124 (stated above as Theorem~\ref{536} p.~\pr{536}). 
