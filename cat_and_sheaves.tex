...
Even if it is straightforward, we state and prove formally the fact that the above condition is equivalent to the condition in Lemma~\ref{pda1}. %conditions summarized by \qr{xfl1} and \qr{xfl2} are equivalent. 
For the purpose of this proof, we prefer to rewrite \qr{xfl1} as 
...
$$ 
and we define \emph{the} projection $p:\DT\lim\al\to\al$ by $p_i(x):=x_i$. Then $p$ is \emph{a} projection in the sense of Definition~\ref{plim}. [Indeed, let $\theta$ in \qr{yfy} be given. %As often it is better to start by verifying the \emph{uniqueness} of $f$. 
If such an $f$ exists, it must satisfy %Let us verify the uniqueness of $f$. We must have 
...
Let $\al:I\to\C$ be an arbitrary functor. For each universe $\U$ such that $I$ is $\U$-small and $\C$ is a $\U$-category (Definitions \ref{myuscat} p.~\pr{myuscat} and \ref{myucat} p.~\pr{myucat}), let $h_\U:\C\to\C^\wg_\U$ be the Yoneda embedding and define $A_\U\in\C^\wg_\U$ by $A_\U(X):=\col\Hom_\C(X,\al)$, where $\col\Hom_\C(X,\al)$ is defined as in Convention~\ref{thei} p.~\pr{thei}. %, so that we have $\col h_\U\ci\al\iso X$ in $\C^\wg_\U$. 
Let $\U$ and $\V$ be two such universes. %satisfying the same conditions as $\U$. 
Then $A_\U$ is representable if and only if $A_\V$ is. Let $X$ be in $\C$. Then $X$ represents $A_\U$ if and only if $X$ represents $A_\V$. Assume that such is the case. Then $X$ is a colimit of $\al$ in $\C$. %$\col\al$ exists in $\C$ and is isomorphic to $X$. 
Moreover, for any functor $F:\C\to\C'$, the natural morphism $\col F\ci\al\to F(X)$ is an isomorphism.
\end{prop}
\begin{proof}
Let us prove the last sentence, %show that the natural morphism $\col F\ci\al\to F(X)$ is an isomorphism, 
...
\begin{thm}\lb{fgd}
Let $I$ be a small (Definition~\ref{myuscat} p.~\pr{myuscat}) category; %with disjoint hom-sets; 
...
\nn Let $\F$ be an essentially small (Definition~\ref{myuscat} p.~\pr{myuscat}) full subcategory of $\C$ such that 
...
%localization $\C_\SSS$ of $\C$ at $\SSS$ exists and is unique up to unique isomorphism. %If $\C$ is a category with disjoint hom-sets and $\SSS$ is a set of morphisms in $\C$, then the localization $\C_\SSS$ of $\C$ at $\SSS$ exists.
\end{prop}
%\begin{lem}\lb{p711}If $\C$ is a category and $\SSS$ is a set of morphisms in $\C$, then there is a category $\C'$ and a functor $Q:\C\to\C'$ such that any functor $\C\to\A$ turning the morphisms of $\SSS$ into isomorphisms factors uniquely through $Q$. Moreover this category is unique up to unique isomorphism.
%If $\C$ is a category with disjoint hom-sets and $\SSS$ is a set of morphisms in $\C$, then there is a category $\C'$ and a functor $Q:\C\to\C'$ such that any functor $\C\to\A$ turning the morphisms of $\SSS$ into isomorphisms factors uniquely through $Q$. %%functor $G:\C'\to\A$ such that $G\ci Q=F$.\end{lem} \begin{proof}[Proof of Lemma \ref{p711}] 
\begin{proof}
...
\nn(b) %We claim that the functor $Q:\C\to\C'$ in Lemma \ref{p711} defines a localization $\C_\SSS$ of $\C$ at $\SSS$ in the sense of Definition 7.1.1. %(The key point is to verify Condition~(c) of Definition 7.1.1 p.~149; see \S\ref{7116} p.~\pr{7116} below.) 
%We sketch the proof that $Q$ satisfies Condition~(c) of Definition 7.1.1 p.~149 of the book and leave the rest of the proof to the reader. 
...
\begin{s}
P. 330, Definition 13.3.1. The purpose of this \S\ is to given an example of a pair $(F,X)$ where $F:\C\to\C'$ is an additive functor of abelian categories and $X$ is an object of $\oo K(\C)$ such that none of the objects $RF(X),R^+F(X),R^-F(X),R^{\text b}F(X)$ exists. In particular:

\begin{center}
\fbox{Not all additive functors of abelian categories are derivable.\index{non derivable}}
\end{center}
%\emph{Not all additive functors of abelian categories are derivable.}

This example is essentially due to Yonatan Harpaz\index{Harpaz}: see 

\nn http://mathoverflow.net/a/265142/461

Let $k$ be a commutative ring, let $\wt\C$ be an abelian $k$-category, let $\C$ be a full abelian $k$-subcategory of $\wt\C$, let $\C'$ be a full abelian $k$-subcategory of $\Mod(k)$, let $U$ be in $\wt\C$, assume that $\Hom_{\wt\C}(U,\ ):\wt\C\to\Mod(k)$ induces a functor $F:\C\to\C'$, let $V$ be in $\wt\C$, let $V[n]\in\oo K(\C)$ be equal to $V$ in degree $n$ (with $n\in\bb Z$) and to 0 elsewhere, and let us assume that $RF(V[n])$ exists.

We claim that $\oo{Ext}^n_\C(U,V)$ is in $\C'$.

Let $U[0]\in\oo K(\C)$ be equal to $U$ in degree 0 and to 0 elsewhere. We have %$A$ be a $k$-algebra, let $\C$ be the category of finite dimensional $A$-modules, let $\C'$ be the category of finite dimensional $k$-vector spaces, let $M$ and $N$ be two objects of $\C$, let $F:\C\to\C'$ be the functor $Y\mt\Hom_A(M,Y)$, let $M[0]\in\oo K(\C)$ be equal to $M$ in degree 0 and to 0 elsewhere, let $N[n]\in\oo K(\C)$ be equal to $N$ in degree $n$ (with $n\in\bb Z$) and to 0 elsewhere, and let us assume that $RF(N[n])$ exists. We have 
$$
R^0F(V[n])\iso H^0\col_{V[n]\to Y}\Hom_\C^\bu(U[0],Y),
$$ 
where $V[n]\to Y$ runs over the quasi-isomorphisms in $\oo K(\C)$, 
$$
H^0\col_{V[n]\to Y}\Hom_\C^\bu(U[0],Y)\iso\col_{V[n]\to Y}H^0\Hom_\C^\bu(U[0],Y)
$$
$$
\iso\col_{V[n]\to Y}\Hom_{\oo K(\C)}(U[0],Y)\iso\Hom_{\oo D(\C)}(U[0],V[n])=\oo{Ext}^n_\C(U,V),
$$ 
that is 
$$
R^0F(V[n])\iso\oo{Ext}^n_\C(U,V)\in\C'.
$$ 
This implies that $\oo{Ext}^n_\C(U,V)$ is in $\C'$, as desired, %in particular $\dim\oo{Ext}^n_\C(M,N)<\infty$, 
and the same holds if we replace $RF$ with $R^+F$, $R^-F$ or $R^{\text b}F$. 

Now let $A$ be $k[x_1,x_2,\dots]$, where $x_1,x_2,\dots$ are indeterminates, set $\wt\C:=\Mod(A)$, let $\C$ be the category of finite dimensional $A$-modules, let $\C'$ be the category of finite dimensional $k$-vector spaces, let $\mathfrak m$ be the ideal generated by the $x_i$, and set $U:=V:=A/\mathfrak m$ and $n:=1$. It is easy to see that we get a contradiction, so we can define $X$ as $(A/\mathfrak m)[0]$, and conclude that the objects $RF(X),R^+F(X),R^-F(X)$ and $R^{\text b}F(X)$ do \emph{not} exist, as required.
\end{s}

%

\begin{s}% The second sentence in the above quote is justified by Proposition 7.4.4 p.~163 of the book.
...
It is implicitly assumed that the underlying universe $\U$ has been chosen so that $\C$ is $\U$-small (Definition~\ref{myuscat} p.~\pr{myuscat}). This is justified by Theorem~\ref{stupidkan} p.~\pr{stupidkan} (the ``Universal Kan Extension Theorem''). %Theorem~\ref{stupidkan} p.~\pr{stupidkan}.
\end{s}

%

\begin{uspb}
P.\index{unsolved problem} 330, phrase ``$R^*F$ is a triangulated functor from $\D^*(\C)$ to $\D^*(\C')$ if it exists'' just before Notation 13.3.2. I'm unable to prove this.
\end{uspb}
...
\begin{s}
P.~331, Part ``(a) $\then$ (b) (2)'' of the proof of Proposition 13.3.5. %The exactness of $F(X)\to F(Z^0)\to F(Z^1)$ follows from the comment made in the book just before the statement of Proposition 13.3.5.
We have 
$$
...
\begin{s}
P.~331, Remark 13.3.6 (i). See \S\ref{AB} p.~\pr{AB}. (I think that, in view of the context, the implicit assumptions are that $F$ is an additive functor and that there is an $F$-injective subcategory $\J$ of $\C$.) %It is written: 

%``Note that for $X\in\C$, $R^kF(X)\iso0$ for $k<0$ and assuming that $F$ is left exact, $R^0F(X)\iso F(X)$.''

%I think that, in view of the context, the implicit assumptions are that $F$ is an additive functor and that there is an $F$-injective subcategory $\J$ of $\C$. It seems clear to me that, if $F$ is left exact, we have $R^kF(X)\iso0$ for $k<0$ and $R^0F(X)\iso F(X)$, but I'm unable to show that, as seems to be asserted, if $F$ is only assumed to be additive, we still have $R^kF(X)\iso0$ for $k<0$.
\end{s}
...
\begin{s}
%P.~332, proof of Lemma 13.3.10. See Lemma~\ref{cart1} p.~\pr{cart1}. %below.
...
\begin{s}
P.~334, proof of Lemma 13.3.12. %Assuming $RF$ is triangulated (see \S\ref{rf} p.~\pr{rf}), t
The existence of an exact sequence $R^jF(X)$ $\to R^jF(X'')\to R^{j+1}F(X')$ attached to a given exact sequence $0\to X'\to X\to X''\to0$ in $\C$ follows from Proposition~\ref{sl4} p.~\pr{sl4}.
\end{s}
...
\begin{s}
P.~337, Theorem 13.4.1. The natural morphism 
$$
\col_{(X'\to X),(Y\to Y')\in\oo{Qis}}\Hom_{\oo K(\C)}(X',Y')\to\Hom_{\oo D(\C)}(X,Y)
$$  
an isomorphism by Remark 7.1.18 (ii) p.~156 of the book.%Theorem 13.4.1 and Corollary~\ref{1432} p.~\pr{1432} imply that the answer is yes if $\C$ is a Grothendieck category. 
...
\nn\emph{By Proposition 11.6.3, the triangulated functor: 
$$
\oo K^+F:\oo K^+(\C)\times\oo K^+(\C')\to\oo K^+(\C'')
$$ 
is naturally defined by setting:} 
$$
\oo K^+F(X,X')=\oo{tot}(F(X,X')).
$$ 

One should assume that $\C''$ admits countable direct sums, and write $\oo{tot}_\oplus$ instead of tot. 
\end{s}
\end{comment}
...
%\begin{s}% Thursday, February 16, 2017
%\begin{muspb}
%P.\index{unsolved problem}\index{major unsolved problem} 360, Theorem 14.4.5 (c). The statement is 
...
%The existence of the functor $\RHom_\C$ is not clear to me, but, even if I take this existence for granted, I'm unable to prove the functoriality in $X$.
%The proof in the book shows the existence of such an isomorphism, but, it seems to me, the functoriality statement is left unproved, and I'm unable to prove the functoriality in $X$.
%\end{muspb}
%\end{s}
