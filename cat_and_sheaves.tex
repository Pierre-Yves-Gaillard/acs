...
A presheaf $F\in\PSh(X,A)$ is a sheaf if for any local isomorphism $A\to U$ with $U\in\C_X$ and $A\in(\C_X)^\wedge$, the morphism $F(U)\to F(A)$ is an isomorphism.
\end{df}

And here is Definition II.2.1 in SGA4:

\begin{df}\label{faisceau}
Soit $C$ un site dont la catégorie sous-jacente est une $\U$-catégorie. Un préfaisceau $F$ à valeurs dans $\U$-Ens est dit séparé (resp. est un faisceau) si pour tout crible $R$ couvrant $X$, objet de $C$, l'application:
$$
\Hom_{C^\wedge}(X,F)\to\Hom_{C^\wedge}(R,F)
$$
est une injection (resp. une bijection). 
\end{df} 

It is not obvious to me that these definitions are equivalent, and that the first one is invariant under change of universes.
\end{s}
