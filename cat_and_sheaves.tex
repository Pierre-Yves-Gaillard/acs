% about "categories and sheaves" (version-0256)
...
\nn$*$ P.~227. The second sentence uses Exercise~3.4 (i) p. 90 (see Statement~6 in \S\ref{coll} p.~\pageref{coll}).
...
By Proposition 2.1.10 p. 40 of the book (see \S\ref{coll} p.~\pageref{coll} below), it suffices to check that the functor $G:\A\to\C^\wedge$ defined by 
$$
G(X')(X):=\Hom_{\A}(F(X),X').
$$ 
...
\eqref{prepa4c} follows from the fact that small inductive limits in $\Set$ are stable by base change (see Section~\ref{coll} p.~\pageref{coll} below). 
\end{proof}
...
\subsection{Six Closely Related Statements}\label{coll}
...
(See also Proposition~\ref{2.1.10} p.~\pageref{2.1.10}.)

\subsubsection{Statement 2}
Definition 2.2.6 p.~47 of the book: stability by base change\index{base change} of certain inductive limits. 
...
\begin{s}
P.~118, proof of Prop. 5.2.3: the proof of (ii) uses Exercise~3.4 (i) p.~90 of the book (see \S\ref{coll} p.~\pageref{coll}) and Proposition 3.3.7 (i) p.~83 of the book. 
...
It remains to prove Steps 3, 3', 4, 5, 6. We refer to the book for Step 3. The proof of Step 3' is the same. (It is easy to see that small inductive limits in $\F^\wedge$ are stable by base change --- see Definition 2.2.6 p.~47 of the book and Section~\ref{coll} p.~\pageref{coll}.) 
...
$$ 
As $\theta$ is a monomorphism, Statement~6 in \S\ref{coll} p.~\pageref{coll} implies that $\colim\theta$ is also a monomorphism. Then the conclusion follows from the commutativity of the diagram 
$$
\begin{tikzcd}
