...
Finally, the bijections \qr{axp1} and \qr{axp1} are functorial in $A$ and $X$.
\end{thm}
...
\Hom_{\C^I}(\al,\DT(\ ))\to X
$$ 
in $\C^\vee$ %(see the Yoneda Lemma, stated on p.~\pr{yol} as Theorem~\ref{yol}) 
is an isomorphism.
\end{df}
...
\begin{s} 
We shall spell out two versions of a certain statement about the following setting: $\al:I\to\C$ is a functor and $Z$ is an object of $\C$. The first version will be short, but somewhat incomplete, whereas the second version will be much longer, but more precise.

\nn Short version: If $\col$ exists in $\C$, then we have 
$$
\Hom_\C(\col\al,Z)\xr\sim\lim\Hom_\C(\al,Z).
$$

\nn Long version: Let $X$ be an object of $\C$ and $p:\al\to\DT X$ a coprojection in the sense of Definition~\ref{ilim} p.~\pr{ilim}: 
...
$$ 
makes sense, and prove that it holds.%Here are more details. Recall the setting: 
...
\nn(a) We say that ``$\pp^\dg\bt$ exists'' if there is a co-unit $\ee_\bt:\bt\to\pp_*\pp^\dg\bt$, that is, for all $\al:I\to\C$ and all $w:\bt\to\pp_*\al$ there is a unique $v:\pp^\dg\bt\to\al$ such that $(v*\pp)\ci \ee_\bt=w$: 
$$
\begin{tikzcd}
...
\begin{s} 
Let $I\xl\pp J\xr\bt\C\xr F\C'$ be functors, let $\bt$ be in $\C^J$ and assume that $\pp^\dg(\bt)$ and $\pp^\dg(F\ci\bt)$:
$$
\begin{tikzcd}
...
As $F\ci\pp^\dg(\bt)=\pp_*(F\ci\pp^\dg(\bt))$, it suffices to define a morphism $F\ci\bt\to F\ci\pp^\dg(\bt)\ci\pp$: 
$$
\begin{tikzcd}
...
\end{tikzcd}
$$ 
As we have $\ee_\bt:\bt\to\pp^\dg(\bt)\ci\pp$, we can take $F*\ee_\bt:F\ci\bt\to F\ci\pp^\dg(\bt)\ci\pp$. 
