% about "categories and sheaves" (version aww)
...
\nn{\em(b)} The following conditions hold:
...
\nn{\em(c)} For any small category $J$ such that $\card(\Mor(J))<\pi$ and any functor $\pp:J\to I$ there is an $i$ in $I$ such that $\lim_{j\in J}\Hom_I(\pp(j),i)\neq\varnothing$. 
\end{prop}

\begin{proof}
Implications (b) $\ssi$ (c) $\then$ (a) are proved in Proposition 9.2.1 p.~217 and Proposition 9.2.9 p.~219 of the book. The proof of Implication (a) $\then$ (c) is the same as the proof of Implication (b) $\then$ (a) in Theorem 3.1.6 p.~74 of the book.
\end{proof}
...
\nn Then we have {\em(a)} $\then$ {\em(b)} $\ssi$ {\em(c)}. Moreover, if {\em(c)} holds, then $\alpha(s):\alpha(i)\to\alpha(j)$ is an isomorphism for all morphism $s:i\to j$ in $I$.
\end{lem}
...
\end{prop}
%
The exactness of $\co_{s\in I^{i_0}}$, invoked in the penultimate line of p.~224, and the isomorphism 
$$
\co_{s\in I^{i_0}}X_i\xr\sim X_\infty, 
$$ 
used to justify the second isomorphism of the first display on p.~225, follow both from Corollary 3.2.3 (i) p.~79 of the book. 

As mentioned in \S\ref{225} p.~\pageref{225}, line~4 of p.~225 should be ``there exists $s:i_0\to i$ such that $N_s\to A$ is an epimorphism'' instead of ``there exists $i_0\to i_1$ such that $N_{i_1}\to A$ is an epimorphism''.

Inequality $\card(G^{\coprod K}(G))<\pi$ on the first line after the last display of p.~225 of the book, follows from Statement  
$$
S\subset A(G)\then\card(G^{\coprod S}(G))<\pi
$$
in the second line of the proof. 

The sentence 

\nn``Since the two compositions $B\times_AB\parar B\to X_{i_0}\to X_\infty$ are equal, there exists an arrow $i_0\to i$ such that the two compositions $B\times_AB\parar B\to X_{i_0}\to X_i$ are equal''

\nn after the diagram on p.~226, is justified by the fact that the map in the first display in p.~226 of the book is injective.

We spell out the last sentence of the proof: The notation in the diagram 
$$
\begin{tikzcd}
B\times_AB\ar[yshift=0.7ex]{r}{q_1}\ar[yshift=-0.7ex]{r}[swap]{q_2}&B\ar{d}[swap]{b}\ar[two heads]{rr}{r}&&A\ar{dl}[swap]{a}\ar{d}{f}\\ 
{}&X_{i_0}\ar{r}[swap]{X_s}\ar[equal]{d}&X_i\ar{r}[swap]{p_i}&X_\infty\ar[equal]{d}\\ 
{}&X_{i_0}\ar{rr}[swap]{p_{i_0}}&&X_\infty
\end{tikzcd}
$$ 
is, we hope, self-explanatory. Then we have 
$$
f\circ r=p_{i_0}\circ b=p_i\circ X_s\circ b=p_i\circ a\circ r,
$$ 
the first equality following from the penultimate display of p.~225 of the book, the second equality begin obvious, and the third equality following from the definition of $a$. As $r$ is an epimorphism, this forces $f=p_i\circ a$. 
...
To prove this, we argue as in the proof of Lemma 9.3.3 p. 226 of the book (which is Lemma~\ref{933} above). (Conditions (b), (c), and (e) referred to below are stated in Section~\ref{tirg} p.~\pageref{tirg}.)
...
$$ 
By Lemma \ref{cardipi} we have $\card(I)<\pi$. Since $\card(G^{\coprod B}(G))<\pi$ for all $B$ in $I$ by (e), this implies \eqref{934b}, and Proposition~\ref{932} p.~\pageref{932} entails $X\in\C_\pi$.
\end{proof}
...
\begin{s}\label{225}
P. 225, line~4: ``there exists $i_0\to i_1$ such that $N_{i_1}\to A$ is an epimorphism'' should be ``there exists $s:i_0\to i$ such that $N_s\to A$ is an epimorphism''.
\end{s}
