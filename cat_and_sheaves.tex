% about "categories and sheaves" (version aum)
...
%
Here is a slightly different wording: Let $\C$ be a category satisfying  
...
\end{prop}
%
We make a few comments about the proof of Proposition~\ref{932}: 
...
\end{tikzcd}
$$ 
is, we hope, self-explanatory. Then we have $f\circ r=p_i\circ X_s\circ b=p_i\circ a\circ r$. As $r$ is an epimorphism, this forces $f=p_i\circ a$. This ends the comments about the proof of Proposition~\ref{932}

Let $\pi_0$ be an infinite regular cardinal such that 
$$
...
\nn(Since the set of quotients of $G^{\coprod A}$ is small if $\card(A)<\pi_0$, such a cardinal $\pi_1$ exists.) In the sequel of this subsection we assume 
%
...
\end{equation}
%
in the first display of the proof can be justified as follows:
%
...
To prove this, we argue as in the proof of Lemma 9.3.3 p. 226 of the book (which is Lemma~\ref{933} above): 
...
%
\begin{s} 
P. 228. Corollary 9.3.6. As already pointed out, $\ilim$ in the statement should be $\sigma_\pi$. As for the proof, Conditions (i), (ii), and (iii) of Proposition 9.2.19 p.~223 of the book follow respectively from (9.3.1) (i) (see (i) at the beginning of Subsection~\ref{s934} p.~\pageref{s934}), (9.3.4) (b) (see (b) right after Condition~\ref{ass} p.~\pageref{ass}), and Corollary 9.3.5 (i) p.~227.
\end{s}
%
%%
%
...
%
\begin{s}
P. 230, Theorem 9.4.2. Here is a diagrammatic illustration of the computation at the end of the proof:
$$
\begin{tikzcd}
X_{i_0}\ar[equal]{d}\ar{rr}{a_{i_0}}&&X\ar[equal]{d}\ar{r}{u}&Y\ar[equal]{d}\ar{rr}{v}&&X\ar[equal]{d}\\ 
X_{i_0}\ar[equal]{d}\ar{r}[swap]{u_{i_0+1,i_0}}&X_{i_0+1}\ar{r}[swap]{a_{i_0+1}}&X\ar{r}[swap]{u}&Y\ar{r}[swap]{w}&X_{i_0+1}\ar[equal]{d}\ar{r}[swap]{a_{i_0+1}}&X\ar[equal]{d}\\ 
X_{i_0}\ar[equal]{d}\ar{rrrr}[swap]{u_{i_0+1,i_0}}&&&&X_{i_0+1}\ar{r}[swap]{a_{i_0+1}}&X\ar[equal]{d}\\ 
X_{i_0}\ar{rrrrr}[swap]{a_{i_0}}&&&&&X.
\end{tikzcd}
$$ 
\end{s}
%
...
%
\begin{proof}
We can assume that $I$ is well-ordered. We start with the case when $I$ has at least two elements, and we write $0<1$ for the smallest two elements of $I$. 

For all $i\in I$ we call $(S_i)$ the following statement:

\nn[Beginning of Statement $(S_i)$] There are families 

$$
(u_{jk}:Y'_j\to Y'_i)_{k<j<i},\ (v_j:X'\to Y'_j)_{j<i},
$$

$$
(p_{jk}:Y'_k\to Y'_{<j})_{k<j\le i},\ (w_j:Y'_{<j}\to Y'_j)_{j<i}
$$

\nn in $\Mor(\C_0)$, and there is a family 
$$
(h_j:Y_j\to Y'_j)_{j<i}
$$ 
in $\Mor(\C)$, such that the conditions below are satisfied for all $\ell<k<j<i$:
$$
u_{jk}\circ u_{k\ell}=u_{j\ell},\ u_{jk}\circ v_k=v_j,\ Y'_{<j}=\co_{k<j}Y'_k,
$$ 
the inductive limit being taken over the $u_{jk}$, and $p_{jk}$ being the coprojection (note that $Y'_{<j}$ exists in $\C_0$ thanks to (9.5.2) (i) --- see the beginning of this subsection) 
$$
u_{jk}=w_j\circ p_{jk},\ v_j=w_j\circ p_{j0}\circ v_0,\ p_{jk}\circ w_k\circ p_{k\ell}=p_{j\ell},\ h_j\circ u_j=v_j\circ f_j.
$$
\nn[End of Statement $(S_i)$]

We prove $(S_i)$ by transfinite induction over $i\in I$. Statement $(S_0)$ is vacuously true. To prove $(S_1)$, we define $v_0$ and $h_0$ by applying (9.2.5) (ii) (see \eqref{952ii} p.~\pageref{952ii}) to 
$$
X'\xleftarrow{\ f_0}X_0\xr{u_0}Y_0,
$$ 
and by putting $Y'_{<0}:=X',w_0:=v_0$. Then $(S_1)$ is clear. 

Let $i$ be $>1$ and let us prove $(S_i)$. We may (and do) assume that $(S_j)$ holds for all $j<i$. We put $Y'_{<i}=\co_{j<i}Y'_j$, we let $p_{ij}$ be the $j$ coprojection, we define $w_i$ and $h_i$ by applying (9.2.5) (ii) (see \eqref{952ii} p.~\pageref{952ii}) to 
$$
Y'_{<i}\xleftarrow{\ p_{i0}}Y'_0\xleftarrow{v_0}X'\xleftarrow{\ f_i}X_i\xr{u_i}Y_i,
$$ 
and we put $u_{ij}:=w_i\circ p_{ij}$, $v_i:=w_i\circ p_{i0}\circ v_0$. From this point, the verification of $(S_i)$ is straightforward. 

We have proved that $(S_i)$ holds for all $i$. 

To prove the lemma, we set $Y':=\co_iY_i'$ (the inductive limit being again taken over the $u_{ij}$), we let $p_i:Y_i'\to Y'$ be the coprojection, we put 
$$
g_i:=p_i\circ h_i,\quad u':=p_1\circ w_1\circ p_{10}\circ v_0,
$$ 
and we observe:
$$
g_i\circ u_i=p_i\circ h_i\circ u_i=p_i\circ w_i\circ p_{i0}\circ v_0\circ f_i
$$
$$
=p_i\circ u_{i0}\circ v_0\circ f_i=p_i\circ u_{i1}\circ u_{10}\circ v_0\circ f_i
$$
$$
=p_1\circ u_{10}\circ v_0\circ f_i=p_1\circ w_1\circ p_{10}\circ v_0\circ f_i=u'\circ f_i.
$$

Remember that we have assumed $\card(I)\ge2$. We leave the case $\card(I)\le1$ to the reader. 
\end{proof}
%
%%%
%
...
Line 6: Condition (9.5.2) (i) (see Subsection~\ref{954955} p.~\pageref{954955}) follows from 
%
...
\nn Exactness at $U$: Let $U\to T$ be a morphism whose composition with $V\to U$ is zero. By Lemma~\ref{8312}, (a)$\then$(c), it suffices to show that the commutative the diagram of solid arrows 
$$
\begin{tikzcd}
...
\end{tikzcd}
$$ 
can be completed as indicated. This follows from the fact that, by Lemma~\ref{8312}, (c)$\then$(a), the commutative diagram of solid arrows 
$$
\begin{tikzcd}
...
%\begin{equation}\label{et=0}\sum_{i=1}^n\ a_i\otimes b_i=0\end{equation}
%
...
On applying $-\otimes_RB$ to \eqref{dt=0} we get the commutative diagram of complexes
%
\begin{equation*}
\begin{tikzcd}
{}&B^m\ar{d}{\lambda}\\ 
...
%
\begin{proof}
Let $M'\to M$ be a monomorphism in $\Mod(\cc R^{\op})$. Let us show that $M'\otimes_{\cc R}P\to M\otimes_{\cc R}P$ is a monomorphism. For $U\in\C_X$ set 
$$
K(U):=\Ker\left(M'(U)\otimes_{\cc R(U)}P(U)\to M(U)\otimes_{\cc R(U)}P(U)\right).
$$ 
Then $K$ is a presheaf and $K^a\simeq\Ker(M'\otimes_{\cc R}P\to M\otimes_{\cc R}P)$. Hence it is enough to check that the morphism $K(U)\to(M'\otimes_{\cc R}P)(U)$ vanishes for any $U\in\C_X$. Let $s\in K(U)\subset M'(U)\otimes_{\cc R(U)}P(U)$. (This first part of the proof is taken almost word for word from the book.) So, we have an element $s$ in the kernel of 
$$
\varphi_1:M'(U)\otimes_{\cc R(U)}P(U)\to M(U)\otimes_{\cc R(U)}P(U),
$$ 
and it suffices to prove $s=0$. Write $s=\sum_{i=1}^na'_i\otimes b_i$ with $a'_i\in M'(U),b_i\in P(U)$, and put $a_i:=\varphi(a_i')$, so that we have $\sum_{i=1}^n\,a_i\otimes b_i=0$, and the commutative diagram of complexes \eqref{dt=0} becomes 
$$
\begin{tikzcd}
{}&\cc R^{\op}(U)^m\ar{d}{g}\\ 
...
\end{tikzcd}
$$ 
By Lemma~\ref{techlem} there is a $(b_i)$ in $P(U)^n$ such that $f_1((b_i))=s$ and a $(b'_j)$ in $P(U)^m$ such that $g_1((b'_j))=h_1((b_i))$ (see \eqref{eltwise}). The commutative diagram of complexes \eqref{d447} also induces the commutative diagram 
$$
\Delta:=\left\{
\begin{tikzcd}
