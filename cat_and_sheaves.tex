...
(\colim\theta)(i):=\colim(\theta_i)\in K\quad\forall\ i\in I. 
\end{equation} 
%
...
$$ 
Then $\colim\theta$ is a functor from $I$ to $K$. 
%
\end{lem}
%
Here is a picture:
...
\Hom_{\C^I}\big(\pp^\dagger(\beta),\alpha\big)\xr\sim\lim\gamma.
\end{equation} 
...
$$
and we must check that $f(\id_{\pp^\dagger(\alpha)})$ is an isomorphism. To this end, we define 
$$
...
$$
that is, we must attach, to each morphism $\pp(j')\to\pp(j)$, a morphism $\alpha(j')\to\alpha(j)$. As $\pp$ is fully faithful by assumption, there is an obvious way to do it. We leave it to the reader to verify that $f(\id_{\pp_*(\pp^\dagger(\alpha))})$ and $u$ are mutually inverse isomorphisms.
...
$$ 
and let $p_x:R\to M'$ be the coprojections. As Theorem 2.3.3 (i) p.~52 of the book implies $M'\simeq\pp^\ddagger(\beta)(M)$, it suffices to prove $M'\simeq M$. We define a family of linear maps $\Phi_x:R\to M$ by setting $\Phi_x:=x$, and leave it to the reader to check that the $\Phi_x$ induce a linear map $\Phi:M'\to M$. We define the set theoretic map $\Psi:M\to M'$ by putting $\Psi(x):=p_x(1)$, and leave it to the reader to verify that $\Phi$ and $\Psi$ are mutually inverse bijections. This proves \eqref{pdb1}. 
...
$$ 
and let $p_x:M'\to R^{\op}$ be the projections. As Theorem 2.3.3 (ii) p.~52 of the book implies $M'\simeq\pp^\ddagger(\beta)(M)$, it suffices to prove $M'\simeq M^*$. We define a family of linear maps $\Phi_x:M^*\to R^{\op}$ by setting $\Phi_x(f):=f(x)$, and leave it to the reader to check that the $\Phi_x$ induce a linear map $\Phi:M^*\to M'$. We define the linear map $\Psi:M'\to M^*$ by putting $\Psi((\lambda_x))(y):=\lambda_y$, and leave it to the reader to verify that $\Phi$ and $\Psi$ are mutually inverse linear bijections. This proves \eqref{pddb2}. 
...
\end{equation}
% 
for all $X\to A$ in $\C_A$. Let $B$ be in $\C^\wedge$. The claim is that the map 
$$
...
Isomorphism \eqref{263c} can be decoded as follows: Let $B$ be in $\C^\wedge$, consider the functor 
$$
...
$$ 
$$
\simeq\Hom_{\C'}(\colim F\circ\alpha,X').
$$
\end{proof}
\end{s}
...
$$
\widetilde F(A)\simeq\colim_{(X\to A)\in\C_A}F(X)
$$ 
for all $A$ in $\C^\wedge$. Let us rewrite the proof of the fact that the natural morphism $\colim\widetilde F(\alpha)\to\widetilde F\left(\colim\alpha\right)$ is an isomorphism. 
...
In this section we shall use the following notation: The Yoneda embedding $\C\to\C^\wedge$ will be denoted by $h(\C)$, and the forgetful functor $\C_A\to\C$ by $j(\C_A)$: 
$$
...
Let $\C$ be a category and $A$ an object of $\C^\wedge$. Recall that there is a unique functor 
$$
...
\end{equation} 
%
for all $(B\to A)$ in $(\C^\wedge)_A$ and all $(U\to A)$ in $\C_A$. Moreover 
\begin{equation}\label{leq}
\text{$\lambda$ is an equivalence,}
\end{equation} 
and we have 
\begin{equation}\label{lcom}
\lambda\circ h(\C)_A\simeq h(\C_A),
\end{equation}
that is, the diagram
$$
\begin{tikzcd}
\C_A\ar{r}{h(\C)_A}\ar{rd}[swap]{h(\C)_A}&(\C^\wedge)_A\ar{d}{\lambda}\\ 
&(\C_A)^\wedge
\end{tikzcd}
$$ 
...
Let $F:\C\to\A$ be a functor, assume that $\C$ is small and that $\A$ admits small inductive limits. Then the functor $h(\C)^\dagger(F):\C^\wedge\to\A$ exists, commutes with small inductive limits and satisfies $h(\C)^\dagger(F)\circ h(\C)\simeq F$.
\end{prop}

%For the reader's convenience we paste Lemma 2.1.13 (i) p.~41 of the book:

%\begin{lem}[Lemma 2.1.13 (i)]\label{2113}
%Let $I$ be a category. Assume that $\C$ admits inductive limits indexed by $I$, and that $A:\C^{\op}\to\Set$ commutes with projective limits indexed by $I$ (i.e. $A(\colim_{i\in I}X_i)\simeq\lim_{i\in I} A(X_i)$ for any inductive system $(X_i)_{i\in I}$ in $\C$). Then $\C_A$ admits inductive limits indexed by $I$ and $j(\C_A):\C_A\to\C$ commutes with such limits.
%\end{lem}

The following lemma will be useful:

\begin{lem}\label{cc}
Let $I\xrightarrow\alpha\C\xrightarrow F\C'$ be functors, let $X$ be in $\C$ and $X'$ in $\C'$, let $f:F(X)\to X'$ be a morphism, assume $X\simeq\colim\alpha$, let $p:\alpha\to\Delta X$ be the coprojection, define $\beta:I\to\C_{X'}$ by $\beta(i):=(\alpha(i),f\circ F(p_i))$:
$$
F(\alpha(i))\xrightarrow{F(p_i)}F(X)\xrightarrow fX'.
$$ 
Then $(X,f)\simeq\colim\beta$, the coprojection $q:\beta\to\Delta(X,f)$ being given by $q_i:=p_i$ for all $i$ in $I$.
\end{lem}
\begin{proof}
...
Let $F:\C\to\A$ be a functor and $A$ an object of $\C^\wedge$, and assume that $\C$ is small and that $\A$ admits small inductive and projective limits.
...
\end{tikzcd}
\end{equation}
%\begin{align*}h(\C_A)^\ddagger(F\circ j(\C_A))(\lambda(B\to A))&\simeq\lim_{((B\to A)\to(U\to A))\in((\C^\wedge)_A)^{B\to A}}F(U)\\ \\ &\simeq\lim_{(B\to U)\in\C^B}F(U)\\ \\ &\simeq h(\C)^\ddagger(F)(B)\\ \\ &\simeq h(\C)^\ddagger(F)\Big(j\big((\C^\wedge)_A\big)(B\to A)\Big).\end{align*} 

For $U$ in $\C$ and $U\to A$ in $\C_A$, we have 

$$h(\C_A)^\ddagger(F\circ j(\C_A))(\lambda(U\to A))\simeq F(U)$$

\nn by Proposition~\ref{271} and \eqref{lcom}. We also have 

$$h(\C)^\ddagger(j((\C^\wedge)_A))(U\to A)\simeq F(U)$$

\nn by Proposition~\ref{271}. For $B$ in $\C^\wedge$ and $B\to A$ in $(\C^\wedge)_A$, we have 

$$h(\C_A)^\ddagger(F\circ j(\C_A))(\lambda(B\to A))$$

$$\overset{(\text a)}\simeq h(\C_A)^\ddagger(F\circ j(\C_A))\left(\lambda\left(\left(\icolim_{(U\to B)\in\C_B}U\right)\to A\right)\right)$$

$$\overset{(\text b)}\simeq h(\C_A)^\ddagger(F\circ j(\C_A))\left(\lambda\left(\colim_{(U\to B)\in\C_B}(U\to A)\right)\right)$$

$$\overset{(\text c)}\simeq\colim_{(U\to B)\in\C_B}h(\C_A)^\ddagger(F\circ j(\C_A))(U\to A)$$

$$\overset{(\text d)}\simeq\colim_{(U\to B)\in\C_B}F(U),$$
...
$$
h(\C)^\ddagger(F)\circ j((\C^\wedge)_A)\simeq\colim_{(U\to B)\in\C_B}F(U).
$$ 
...
$$h^\dagger(j^\dagger(G))(B)\simeq\colim_{(U\to B)\in\C_B}j^\dagger(G)(U)\overset{(\text a)}\simeq\colim_{(U\to B)\in\C_B}(h_A)^\dagger(G)(\lambda(U\times A\to A))$$

$$\overset{(\text b)}\simeq(h_A)^\dagger(G)\left(\lambda\left(\colim_{(U\to B)\in\C_B}(U\times A\to A)\right)\right)$$

$$\overset{(\text c)}\simeq(h_A)^\dagger(G)\left(\lambda\left(\left(\colim_{(U\to B)\in\C_B}(U\times A)\right)\to A\right)\right)$$

$$\overset{(\text d)}\simeq(h_A)^\dagger(G)\left(\lambda\left(\left(\icolim_{(U\to B)\in\C_B}U\right)\times A\to A\right)\right)\overset{(\text e)}\simeq(h_A)^\dagger(G)(\lambda(B\times A\to A)),$$ 
...
$$
\boxed{f_*:=(f^\tau)_*}\quad,\quad\boxed{f^\dagger:=(f^\tau)^\dagger}\quad,\quad\boxed{f^\ddagger:=(f^\tau)^\ddagger}\quad,
$$ 
yielding
$$
...
\begin{proof}
Consider a monomorphism $M'\mono M$ of right $\cc R$-modules. It suffices to prove that the sheaf 
$$ 
