...
\begin{lem}\lb{pda1}
In the above setting, a morphism $\eta_{X'}:L(R(X'))\to X'$ in $\C'$ is a representation of $\Hom_{\C'}(L(\ ),X')$ if and only if for all $X$ in $\C$ and all $g:L(X)\to X'$ there is a unique $f:X\to R(X')$ such that $\eta_{X'}\ci L(f)=g$: 
...
We call $\eta_{X'}$ the \emph{unit}\index{unit} of the adjunction.%\end{s}

%\begin{s} In the above setting c
...
\begin{lem}\lb{u1}
Let $\U$ and $\V$ be universes, let $\al_\U:I\to\Set_\U$ be a functor such that $\al_\U(i)\in\V$ for all $i$ in $I$, let $\al_\V$ be the functor $I\to\Set_\V$ induced by $\al_\U$, let $X$ be in $\U\cap\V$, let $p_\U:\al_\U\to\DT X$ be a morphism in $(\Set_\U)^I$ and $p_\V:\al_\V\to\DT X$ the induced morphism in $(\Set_\V)^I$. If $p_\U$ is a coprojection (see Definition~\ref{ilim} p.~\pr{ilim}), so is $p_\V$.
\end{lem}
\begin{proof}
We can assume that %$\U\subset\V$ and that 
$I$ is $\V$-small (Definition~\ref{myuscat} p.~\pr{myuscat}). Define $Y\in\V$ as being $\col\al_\V$ in the notation of Convention~\ref{thei} p.~\pr{thei}. Using the conservative functor from $(\Set_\V)^{\op}$ to $\Set_\V$ which sends $Z$ to $\{0,1\}^Z$, we see that the natural map $Y\to X$ is bijective.
\end{proof}

\begin{lem}\lb{u2}
Let $\U$ and $\V$ be universes, let $\al_\U:I^{\op}\to\Set_\U$ be a functor such that $\al_\U(i)\in\V$ for all $i$ in $I$, let $\al_\V$ be the functor $I^{\op}\to\Set_\V$ induced by $\al_\U$, let $X$ be in $\U\cap\V$, let $p_\U:\DT X\to\al_\U$ be a morphism in $\Fct(I^{\op},\Set_\U)$ and $p_\V:\DT X\to\al_\V$ the induced morphism in $\Fct(I^{\op},\Set_\V)$. If $p_\U$ is a projection (see Definition~\ref{plim} p.~\pr{plim}), so is $p_\V$.
\end{lem}
\begin{proof}
The proof is similar to (and slightly easier than) that of Lemma~\ref{u1} p.~\pr{u1}, and is left to the reader.
\end{proof}
...
$$
in $\Fct(I^{\op},\Set_\U)$ is a projection (see Definition~\ref{plim} p.~\pr{plim}) for any $X'$ in $\C'$ and for some (and thus, by Proposition~\ref{aic} p.~\pr{aic}, for any) %\footnote{See Lemma~\ref{u2}.} 
universe $\U$ such that $I$ is $\U$-small and $\C'$ is a $\U$-category (Definitions \ref{myucat} p.~\pr{myucat} and \ref{myuscat} p.~\pr{myuscat})''. Note that $(1F)$ and $(2F)$ are equivalent by Proposition~\ref{aic} p.~\pr{aic}. Then we have
...
$$ 
but this holds by Proposition~\ref{216p} p.~\pr{216p}.
