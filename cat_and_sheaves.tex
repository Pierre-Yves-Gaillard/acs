% about "categories and sheaves" (version_0096)
...
(\C^{X'})^{\op}\simeq(\C^{\op})_{X'},\quad(\C_{X'})^{\op}\simeq(\C^{\op})^{X'}.
\end{equation}
\end{s}
...
\begin{equation*}
\begin{split}
(f^\tau)\ \widehat{}\ (B)(U)&\simeq\colim_{(V\to B)\in((\C_Y)^{\op})_B}\Hom_{(\C_X)^{\op}}(U,f^\tau(V))
\\ \\ 
{}&\simeq\colim_{(V\to B)\in((\C_Y)^{\op})_B}\Hom_{\C_X}(f^t(V),U)\\ \\ 
{}&\simeq\colim_{(U\to f^\tau(V))\in((\C_Y)^{\op})^U}B(V).
\end{split}
\end{equation*}
%
...
\begin{s}
P. 409, proof of Proposition 17.1.9. Here more details: Part (i): We have 
$$
\oo j_{A\to X*}(F)(\lambda(B\to A))\simeq
\lim_{((U\to A)\to\lambda(B\to A))\in(\C_A)_{\lambda(B\to A)}}\oo j_{A\to X*}(F)(U\to A)
$$
$$
\simeq\lim_{(U\to B)\in\C_B}\oo j_{A\to X*}(F)(U\to A)\simeq
\lim_{(U\to B)\in\C_B}F(U)\simeq F(B),
$$ 
the first isomorphism being justified by \eqref{408}, the second by the definition of $\lambda$ in the last display of p.~26 of the book, the third by the definition of $\oo j_{A\to X*}(F)$, and the fourth by \eqref{408}. 

Part (ii): We use \eqref{opslice} p.~\pageref{opslice}.

Part (iii), case $B\in\C_X$: 
$$
(\oo j^\ddagger_{A\to X} G)(B)\simeq\lim_{(\oo j_A(V\to A)\to B)\in(\C_A)_B}G(V\to A)
$$
$$
\simeq\lim_{((V\to A)\to(B\times A\to A))\in(\C_A)_{B\times A\to A}}G(V\to A)
$$
$$
...
%\begin{s} P. 424, proof of Theorem 17.5.2 (iv). Firstly, as already mentioned, there is a typo: ``The functor $f^\dagger$ is left exact'' should be ``The functor $f^\dagger$ is exact''. Secondly, the exactness of $f^\dagger$ follows from Proposition~\ref{407} p.~\pageref{407}.\end{s}
