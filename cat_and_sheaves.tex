% about "categories and sheaves" (version_0086)
...
Let $(\Gamma_U)_{U\in\C}$ be a subfamily of the family $(\Sigma_U)_{U\in\C}$ satisfying GT1-GT4, and let $\cc E$ be the set of those morphisms $A\to B$ in $\C^\wedge$ such that, for any morphism $U\to B$ in $\C^\wedge$ with $U$ in $\C$, the sieve $S_{A\times_BU}$ is in $\Gamma_U$. 
...
\nn Proof of the claim: Let $s:V\to U$ be in $S$ and let $\pp:A_S(V)\to\Hom_{\C^\wedge}(V,A_S)$ be the bijection given by the Yoneda Lemma (see Theorem~\ref{yol} p.~\pageref{yol}). One easily checks that $\pp(s)$ is an epimorphism [???] and that the diagram 
$$
\begin{tikzcd}
...
If $S=\varnothing$, then $S'$ is in $\Gamma_U$ by GT2. Otherwise, let $V\to U$ be in $S$, let $B$ be the subobject $A_{S'\times_UV}=A_{S'}\times_UV$ (see \eqref{asuv}) of $V$ in $\C^\wedge$, and consider the cartesian square 
$$
\begin{tikzcd}
B\ar{r}\ar{d}&V\ar{d}\\ 
A_{S'}\ar{r}&U.
