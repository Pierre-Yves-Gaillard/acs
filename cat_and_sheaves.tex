% about "categories and sheaves" (version auc)
...
u_{ij}\circ u_{jk}=u_{ik}\quad\forall\quad i>j>k,
\end{equation}

\nn$\bu$ a family $(v_i:X'\to Y'_i)_i$ of morphisms in $\C_0$ such that 
%
\begin{equation}\label{vi}
u_{ij}\circ v_j=v_i\quad\forall\quad i>j,
\end{equation}

\nn$\bu$ a family $(h_i:Y_i\to Y'_i)_i$ of morphisms in $\C$.

\nn If $i=0$ is the least element of $I$, we define $v_0$ and $h_0$ via Condition (9.5.2) (ii): 
$$
\begin{tikzcd}
...
Assume that $i>0$, and that we have an inductive system 
$$
...
(v_j:X'\to Y'_j)_{j<i}
$$ 
of morphisms in $\C_0$ such that $u_{jk}\circ v_k=v_j$ for all $k<j<i$. By (9.5.2) (i) (see beginning of this subsection), the object 
$$
Y'_{<i}:=\co_{j<i}Y'_i
$$ 
...
\end{equation}
%
This will imply that the families $(u_{ij})$ and $(v_i)$ satisfy \eqref{uij} and \eqref{vi}. We prove \eqref{pij} as follows:

Let $i$ be a fixed element of $I$. The statement
$$
(S_i)\qquad p_{ij}\circ w_j\circ p_{jk}=p_{ik}\quad\forall\quad k<j<i
$$ 
implies $u_{jk}\circ u_{k\ell}=u_{j\ell}$ and $u_{jk}\circ v_k=v_j$ for all $\ell<k<j<i$. Using this fact, one easily proves 
$$
