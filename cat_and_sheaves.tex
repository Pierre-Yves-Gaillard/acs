...
$$
``\ilim"(IF\circ\alpha)\xr\sim IF(``\ilim"\alpha)
$$
instead of 
$$
IF(``\ilim"\alpha)\xr\sim``\ilim"(IF\circ\alpha). 
$$ 
\end{s}
...
\nn$*$ P.~221, Lemma 9.2.15. ``Let $A\in\C$'' should be ``Let $A\in\C^\wdg$''.
...
\nn$*$ P.~406, first line of the second display: $(\C_Y)^\wdg$ should be $\C_Y$ (twice). (See \S\ref{fhat} p.~\pageref{fhat}.)

\nn$*$ P.~409, line 2: $\lambda\circ(\oo h_X^t)_A\iso\oo h_A$ should be $\lambda\circ(\oo h_X^t)_A\iso\oo h_A^t$. 
...
\nn$*$ P.~417, first sentence of the paragraph containing Display (17.4.2): $A,A'\in\C^\wdg$ should be $A,A'\in\C_X^\wdg$. 
...
\nn$*$ P.~424, proof of Theorem 17.5.2 (iv). ``The functor $f^\dg$ is left exact'' should be ``The functor $f^\dg$ is exact''. (See \S\ref{fdagger} p.~\pageref{fdagger}.) 
...
A \textbf{universe}\index{universe} is a set $\mc U$ satisfying 

(i) $\varnothing\in\mc U$,

(ii) $u\in U\in\mc U\then u\in \mc U$,

(iii) $U\in\mc U\then\{U\}\in\mc U$,

(iv) $U\in\mc U\then\mc P(U)\in\mc U$,

(v) $I\in\mc U$ and $U_i\in\mc U$ for all $i$ $\then$ $\bigcup_{i\in I}U_i\in\mc U$,

(vi) $\mathbb N\in\mc U$.

\nn We want to prove:

(vii) $U\in\mc U\then\bigcup_{u\in U}u\in\mc U$,

(viii) $U,V\in\mc U\then U\times V\in\mc U$,

(ix) $U\subset V\in\mc U\then U\in\mc U$,

(x) $I\in \mc U$ and $U_i\in\mc U$ for all $i$ $\then$ $\prod_{i\in I}U_i\in\mc U$.
...
(a) $U,V\in\mc U\then\{U,V\}\in\mc U$,

\nn and thus

(b) $U,V\in\mc U\then(U,V):=\{\{U\},\{U,V\}\}\in\mc U$.

\nn\textbf{Proof of (viii).} If $u\in U$ and $v\in V$, then $\{(u,v)\}\in\mc U$ by (ii), (b), and (iii). Now (v) yields 
$$
U\times V=\bigcup_{u\in U}\ \bigcup_{v\in V}\ \{(u,v)\}\in\mc U.\text{ q.e.d.} 
$$ 

Assume $U,V\in\mc U$, and let $V^U$ be the set of all maps from $U$ to $V$. As $V^U\in\mc P(U\times V)$, Statements (viii), (iv), and (ii) give

(c) $U,V\in\mc U\then V^U\in\mc U$.
...
Let $\C$ be a category, let $h:\C\to\C^\wdg$ be the Yoneda embedding, let $F$ be in $\C^\wdg$, let $A$ be in $\C$, and define 
$$
\begin{tikzcd} 
F(A)\ar[yshift=0.7ex]{r}{\pp}&\Hom_{\C^\wdg}(h(A),F)\ar[yshift=-0.7ex]{l}{\psi}
\end{tikzcd}
$$
by 
...
for 
$$
a\in F(A),\quad X\in\C,\quad f\in\Hom_\C(X,A),\quad\theta\in\Hom_{\C^\wdg}(h(A),F):
$$ 
$$
...
$$
\pp(a)=h(a)\in\Hom_{\C^\wdg}(h(A),h(B)).
$$
...
\subsection{Commutativity of Diagram (1.5.6) p.~28}
...
\begin{tikzcd}
\Hom_{\C'}(X',Y')\ar{r}{R}\ar{dr}[swap]{\circ\lambda_{RX',X'}(\id_{RX'})}&\Hom_\C(RX',RY'),\ar{d}{\lambda_{RX',Y'}}\\ 
&\Hom_{\C'}(LRX',Y').
\end{tikzcd}
\end{equation} 
...
$$
\begin{tikzcd}
\Hom_\C(RX',RX')\ar{r}{\lambda_{RX',X'}}\ar{d}[swap]{R(f)\circ}&\Hom_{\C'}(LRX',X'),\ar{d}{f\circ}\\ 
\Hom_\C(RX',RY')\ar{r}[swap]{\lambda_{RX',Y'}}&\Hom_{\C'}(LRX',Y')
\end{tikzcd}
$$ 
commutes for $f$ in $\Hom_{\C'}(X',Y')$, we get in particular
$$
f\circ\lambda_{RX',X'}(\id_{RX'})=\lambda_{RX',Y'}(R(f)\circ\id_{RX'})=\lambda_{RX',Y'}(R(f)).
$$ 
...
For the reader's convenience we prove \eqref{158} p.~\pageref{158} and \eqref{159} p.~\pageref{159}. It clearly suffices to prove \eqref{158}. Recall that \eqref{158} claims 
$$
(\eta*L)\circ(L*\ee)=L.
$$ 
...
\big(\lambda_{X,LX}\circ(\circ\ee_X)\big)(\id_{RLX})
$$
$$
...
$$
\begin{tikzcd}
\Hom_\C(RLX,RLX)\ar{d}[swap]{\circ\ee_X}\ar{rr}{\lambda_{RLX,LX}}&&\Hom_{\C'}(LRLX,LX)\ar{d}{\circ L(\ee_X)}\\
...
$$
such that $p$ is a $(\Hom_{\C^{I^{\op}}}(\Delta(\ ),\alpha),X)$\--universal element (see Definition~\ref{ue} p.~\pageref{ue}). For each $i$ in $I$ the morphism $p_i:X\to\alpha(i)$ is called the $i$-{\em projection}\index{projection} of $X$. (We almost always write $X$ for $(X,p)$, the mental picture being that $p$ is a structure $X$ is equipped with.)
\end{df}
...
$$
such that $p$ is a $(\Hom_{\C^I}(\alpha,\Delta(\ )),X)$\--co-universal element (see Definition~\ref{ue2} p.~\pageref{ue2}). For each $i$ in $I$ the morphism $p_i:\alpha(i)\to X$ is called the $i$-{\em coprojection}\index{coprojection} of $X$. (We almost always write $X$ for $(X,p)$, the mental picture being that $p$ is a structure $X$ is equipped with.)
\end{df}
...
\begin{s}\label{c38}
P.~38, Proposition 2.1.6. Here is an example of a functor $\alpha:I\to\C^J$ such that $\col\alpha$ exists in $\C^J$ but there is a $j$ in $J$ such that $\col\ (\rho_j\circ\alpha)$ does not exist in $\C$. (Recall that $\rho_j:\C^J\to\C$ is the evaluation at $j\in J$.) This example is taken from Section 3.3 of the book \textbf{Basic Concepts of Enriched Category Theory} of G.M. Kelly\index{Kelly}:%\medskip 
%
...
$$
in $\A^\wdg$. Then $F$ commutes with inductive limits indexed by $I$.
\end{prop}
%
\begin{proof}
...
If $\C$ is a category and $F$ an object of $\C^\wdg$, then the following conditions are equivalent:
...
\begin{s}
Lemma 2.1.15 p.~42. Here is a complement which will be used in \S\ref{17115ps} p.~\pageref{17115ps}. Let $I$ be a small category with disjoint hom-sets, let $\alpha,\beta:I\to\C$ be two functors, and, for each $i$ in $I$, let $U^i:I^i\to I$ be the forgetful functor. 

\begin{thm}\label{fgd}
The natural map 
\begin{equation}\label{hficab}
...
\begin{equation}\label{liifii}
\lim_{i\in I}\Hom_{\Fct(I^i,\C)}(\alpha\circ U^i,\beta\circ U^i)\to\Hom_{\Fct(I,\C)}(\alpha,\beta),
\end{equation} 
and we leave it to the reader to check that \eqref{hficab} and \eqref{liifii} are inverse bijections.
\end{proof}
\end{s}
...
\begin{s}
P.~50, Definition 2.3.1. The three pieces of notation $\pp_*,\pp^\dg$, and $\pp^\ddg$ are justified by Notation 17.1.5 p.~407 (see also \eqref{ttau} p.~\pageref{ttau}). 
\end{s} 
...
\begin{s} 
P.~51, Definition 2.3.2 (minor variant). We assume that no underlying universe has been given. Let $I\xleftarrow\pp J\xr\beta\C$ be functors, let $\beta$ be in $\C^J$, and let $\pp^\dg\beta$ be in $\C^I$. The following conditions are equivalent:
...
If the above equivalent conditions hold, we say that $\pp^\dg\beta$ {\em exists}. If $\pp^\dg(F\circ\beta)$ exists and is isomorphic to $F\circ\pp^\dg(\beta)$ for all functor $F:\C\to\C'$, we say that $\pp^\dg\beta$ exists {\em universally}.
\end{df}
\end{s}
...
\end{equation} 
%
for all $i$ in $I$. In particular, if $\C$ admits small inductive limits and $J$ is small, then $\pp^\dg$ exists. If moreover $\pp$ is fully faithful, then $\pp^\dg$ is fully faithful and there is an isomorphism $\id_{\C^J}\xr\sim\pp_*\circ\pp^\dg$. 
\end{thm}
...
We define $\pp^\dg(\beta)$ by \eqref{236}. The purpose of Step~(a) is to show that $\pp^\dg(\beta)$ is indeed a functor. Here is a variant of the argument of the book. The proof of the following lemma is obvious: 

\begin{lem}\label{r52}

Let $I$ and $K$ be two objects of the category $\Cat$ of small categories (see Definition~\ref{myuscat} p.~\pageref{myuscat}), let $\Phi:I\to\Cat$ be a functor, view $K$ as a constant functor from $I$ to $\Cat$, and let $\theta:\Phi\to K$ be a morphism of functors. Assume 
...
$$
(\col\theta)(i)=\col(\theta_{i'}\circ\Phi(s))\to
...
We want to prove that $\pp^\dg$ defined by \eqref{236} p.~\pageref{236} is a functor. In the setting of Lemma~\ref{r52} we set $K:=\C$, we choose $\U$ so that $I$ and $k=\C$ are in $\Cat$, we define $\Phi:I\to\Cat$ by $\Phi(i):=J_i$ and we consider the morphism of functors $\theta:\Phi\to\C$ such that $\theta_i:\Phi(i)=J_i\to\C$ is the composition of $\beta$ with the forgetful functor from $J_i$ to $J$. We assume \eqref{52}. Then $\col\theta$ is nothing but $\pp^\dg(\beta)$. In particular $\pp^\dg(\beta)$ is a functor by Lemma~\ref{r52}. q.e.d.
...
\end{equation} 
follows immediately from \eqref{2115}. Using \eqref{2115} again, it is easy to see that we also have a natural bijection 
\begin{equation}\label{nb2}
\Hom_{\C^I}\big(\pp^\dg(\beta),\alpha\big)\xr\sim\lim\gamma.
\end{equation} 
By \eqref{nb1} and \eqref{nb2}, it suffices to show 
% 
\begin{lem}
...
The purpose of Step~(c) is to prove the last two sentences of the statement of Theorem 2.3.3 (i) p.~52 of the book (see Theorem~\ref{233i} p.~\pageref{233i}). We assume that $\C$ admits small inductive limits, that $J$ is small (in particular $\pp^\dg$ exists), and that $\pp$ is fully faithful. We must show that $\pp^\dg$ is fully faithful, and that there is an isomorphism $\id_{\C^J}\xr\sim\pp_*\circ\pp^\dg$. 
...
$$
that is, we must attach, to each morphism $\pp(j')\to\pp(j)$, a morphism $\alpha(j')\to\alpha(j)$. As $\pp$ is fully faithful by assumption, there is an obvious way to do it. We leave it to the reader to verify that $f(\id_{\pp_*(\pp^\dg(\alpha))})$ and $u$ are mutually inverse isomorphisms. This proves \eqref{fip}. The fact that $\pp^\dg$ is fully faithful follows now from Proposition 1.5.6 (ii) p.~29 of the book.
...
$$
p_j(x,u'\circ\pp(s))\sim p_{j'}(\beta(s)(x),u'),
$$ 
...
\begin{s} P.~53, Corollary 2.3.4. (Another proof will be given in Section~\ref{2111} p.~\pageref{2111}.) Recall that we have functors $\C\xleftarrow\beta J\xr\pp I$, where $I$ and $J$ are small. One can prove $\col\beta\iso\col\pp^\dg\beta$, that is 
%
...
\end{equation} 
be a diagram of functors. Assume that $I,J$ and $K$ are small, and that $\C$ admits small projective limits. Then Theorem 2.3.3 (ii) p.~52 of the book implies that the functors $\pp^\ddg(\psi^\ddg(\beta))$ and $(\pp\circ\psi)^\ddg(\beta)$ from $I$ to $\C$ exist and are naturally isomorphic. 
\end{s}
...
$$ 
and let $p_x:R\to M'$ be the coprojections. As Theorem 2.3.3 (i) p.~52 of the book implies $M'\iso\pp^\ddg(\beta)(M)$, it suffices to prove $M'\iso M$. We define a family of linear maps $\Phi_x:R\to M$ by setting $\Phi_x:=x$, and leave it to the reader to check that the $\Phi_x$ induce a linear map $\Phi:M'\to M$. We define the set theoretic map $\Psi:M\to M'$ by putting $\Psi(x):=p_x(1)$, and leave it to the reader to verify that $\Phi$ and $\Psi$ are mutually inverse bijections. This proves \eqref{pdb1}. 
...
Let $\C$ be a category and let $A$ be in $\C^\wdg$. Consider the statements
%
...
\Hom_{\C^\wdg}(A,B)\xr\sim\lim_{(X\to A)\in\C_A}B(X)\text{ for all }B\in\C^\wdg. 
\end{equation}
...
$$
\alpha:\C_A\to\C^\wdg,\quad(X\to A)\mapsto X,
$$ 
and let $p:\alpha\to\Delta(A)$ be the tautological morphism in $(\C^\wdg)^{\C_A}$ defined by 
%
\begin{equation}\label{pxa}
p_{X\to A}:=(X\to A)
\end{equation}
% 
for all $X\to A$ in $\C_A$. Let $B$ be in $\C^\wdg$. The claim is that the map 
$$
\Hom_{\C^\wdg}(A,B)\to\Hom_{\Fct(\C_A,\C^\wdg)}(\alpha,\Delta(B)),\quad\theta\mapsto\Delta(\theta)\circ p
$$ 
is bijective. 

Isomorphism \eqref{263b} can be decoded as follows: Let $Y$ be in $\C$, consider the functor 
$$
...
Isomorphism \eqref{263c} can be decoded as follows: Let $B$ be in $\C^\wdg$, consider the functor 
$$
...
$$
\Hom_{\Set}(S,B(A))\to\Hom_{\Fct((\C_A)^{\op},\Set)}(\Delta(S),\gamma),\quad g\mapsto r\circ\Delta(g)
$$ 
is bijective. 
...
$$
f_C:\Hom_\A(A,C)\to\Hom_{\A^I}(\alpha,\Delta(C)),\quad u\mapsto\Delta(u)\circ p
$$ 
...
that is 
$$
\Delta(b)\circ\Delta(u)\circ p=p\ssi\Delta(b\circ u)=\id_{\Delta(A)},
$$ 
that is 
$$
\Delta(b\circ u)\circ p=p\ssi\Delta(b\circ u)=\id_{\Delta(A)},
$$ 
that is 
$$
f_A(b\circ u)=f_A(\id_A)\ssi b\circ u=\id_A.
$$ 
...
Let $\alpha:I\to\C$ be a functor, let $X$ be an object of $\C$, and let $p:\alpha\to\Delta(X)$ be a morphism in $\C^I$ (where $\Delta(X)$ is constant with value $X$). For all universe $\U$ such that $\C$ is a $\U$-category, we write $h_\U$ for the Yoneda embedding from $\C$ to $\C^\wdg_\U$.
...
\nn{\em(b)} $\col h_\U\circ\alpha\iso h_\U(X)$ for \emph{some} universe $\U$ such that $I$ is $\U$-small and $\C$ is a $\U$-category,

\nn{\em(c)} $\col h_\U\circ\alpha\iso h_\U(X)$ for \emph{any} universe $\U$ such that $I$ is $\U$-small and $\C$ is a $\U$-category, 

\nn{\em(d)} $\col F\circ\alpha\iso F(X)$ for \emph{any} functor $F:\C\to\C'$. 
\end{prop}
...
$$
\Hom_{\C'}(F(X),X')\iso\Hom_{\C^\wdg_\U}\big(h_\U(X),\Hom_{\C'}(F(\ ),X')\big)
$$
$$
\iso\lim\Hom_{\C^\wdg_\U}\big(h_\U\circ\alpha,\Hom_{\C'}(F(\ ),X')\big)\iso\lim\Hom_{\C'}(F\circ\alpha,X')
$$ 
$$
\iso\Hom_{\C'}(\col F\circ\alpha,X').
$$
\end{proof}
\end{s}
...
\end{equation}
%
exists in $\C$ for all $i$ in $I$, then $\pp^\dg\beta$ exists, and $(\pp^\dg\beta)(i)$ is isomorphic to \eqref{e233}. If, in addition, \eqref{e233} exists universally in the sense of Definition~\ref{uil} p.~\pageref{uil}, then $\pp^\dg\beta$ exists universally in the sense of Definition~\ref{232} p.~\pageref{232}. 
\end{thm}
\end{s}
...
$$
\begin{tikzcd}
\C\ar{r}{\hy_\C}\ar{dr}[swap]{F}&\C^\wdg\ar{d}{\widetilde F}&I\ar{l}[swap]{\alpha}\\
&\A,
\end{tikzcd}
$$
...
$$ 
for all $A$ in $\C^\wdg$. Let us rewrite the proof of the fact that the natural morphism $\col\widetilde F(\alpha)\to\widetilde F\left(\col\alpha\right)$ is an isomorphism. 

By Proposition 2.1.10 p. 40 of the book (stated on p.~\pageref{2.1.10} below as Proposition~\ref{2.1.10}), it suffices to check that the functor $G:\A\to\C^\wdg$ defined by 
$$
G(X')(X):=\Hom_{\A}(F(X),X').
$$ 
...
$$
$$
=\lim_{(X\to A)\in\C_A}G(X')(X)\iso\Hom_{\C^\wdg}(A,G(X')), 
$$ 
...
\C^\wdg\to\A,\quad A\mapsto(\oo h_\C^\dg F)(A)
$$ 
...
Here is a complement to Section 2.3 pp~52-54 of the book, complement which will be used in \S\ref{prepa5} p.~\pageref{prepa5} to prove Proposition 17.1.9 p.~409 of the book. 

In this section we shall use the following notation: The Yoneda embedding $\C\to\C^\wdg$ will be denoted by $h(\C)$, and the forgetful functor $\C_A\to\C$ by $j(\C_A)$: 
$$
h(\C):\C\to\C^\wdg,\qquad j(\C_A):\C_A\to\C.
$$

\subsubsection{Preliminaries}

Let $\C$ be a category and $A$ an object of $\C^\wdg$. Recall that there is a unique functor 
$$
\lambda:(\C^\wdg)_A\to(\C_A)^\wdg
$$ 
such that 
%
\begin{equation}\label{lambda}
\lambda(B\to A)(U\to A)=\Hom_{(\C^\wdg)_A}(U\to A,B\to A)
\end{equation} 
%
for all $(B\to A)$ in $(\C^\wdg)_A$ and all $(U\to A)$ in $\C_A$. Moreover 
\begin{equation}\label{leq}
\text{$\lambda$ is an equivalence,}
\end{equation} 
and we have 
\begin{equation}\label{lcom}
\lambda\circ h(\C)_A\iso h(\C_A),
\end{equation}
that is, the diagram
$$
\begin{tikzcd}
\C_A\ar{r}{h(\C)_A}\ar{rd}[swap]{h(\C)_A}&(\C^\wdg)_A\ar{d}{\lambda}\\ 
&(\C_A)^\wdg
\end{tikzcd}
$$ 
...
\begin{prop}\label{271}
Let $F:\C\to\A$ be a functor, assume that $\C$ is small and that $\A$ admits small inductive limits. Then the functor $h(\C)^\dg(F):\C^\wdg\to\A$ exists, commutes with small inductive limits and satisfies $h(\C)^\dg(F)\circ h(\C)\iso F$.
\end{prop}

%For the reader's convenience we paste Lemma 2.1.13 (i) p.~41 of the book:

%\begin{lem}[Lemma 2.1.13 (i)]\label{2113}
...
\begin{lem}\label{cc}
Let $I\xr\alpha\C\xr F\C'$ be functors, let $X$ be in $\C$ and $X'$ in $\C'$, let $f:F(X)\to X'$ be a morphism, assume $X\iso\col\alpha$, let $p:\alpha\to\Delta X$ be the coprojection, define $\beta:I\to\C_{X'}$ by $\beta(i):=(\alpha(i),f\circ F(p_i))$:
$$
...
$$
\Delta k\circ q=h\ssi\Delta k\circ p=h'.
$$
\end{proof}

Let $F:\C\to\A$ be a functor and $A$ an object of $\C^\wdg$, and assume that $\C$ is small and that $\A$ admits small inductive and projective limits.
...
h(\C_A)^\ddg(F\circ j(\C_A))\circ\lambda\iso h(\C)^\ddg(F)\circ j((\C^\wdg)_A)
\end{equation}

\nn(see the diagram \eqref{qcd} below).
...
{}\\ 
(\C_A)^\wdg\ar{uurr}[swap]{h(\C_A)^\ddg(F\circ j(\C_A))}&&(\C^\wdg)_A\ar{ll}{\lambda}\ar{rr}[swap]{j((\C^\wdg)_A)}&&\C^\wdg.\ar{uull}[swap]{h(\C)^\ddg(F)}
\end{tikzcd}
\end{equation}
%\begin{align*}h(\C_A)^\ddg(F\circ j(\C_A))(\lambda(B\to A))&\iso\lim_{((B\to A)\to(U\to A))\in((\C^\wdg)_A)^{B\to A}}F(U)\\ \\ &\iso\lim_{(B\to U)\in\C^B}F(U)\\ \\ &\iso h(\C)^\ddg(F)(B)\\ \\ &\iso h(\C)^\ddg(F)\Big(j\big((\C^\wdg)_A\big)(B\to A)\Big).\end{align*} 

For $U$ in $\C$ and $U\to A$ in $\C_A$, we have 

$$h(\C_A)^\ddg(F\circ j(\C_A))(\lambda(U\to A))\iso F(U)$$

\nn by Proposition~\ref{271} and \eqref{lcom}. We also have 

$$h(\C)^\ddg(j((\C^\wdg)_A))(U\to A)\iso F(U)$$

\nn by Proposition~\ref{271}. For $B$ in $\C^\wdg$ and $B\to A$ in $(\C^\wdg)_A$, we have 

$$h(\C_A)^\ddg(F\circ j(\C_A))(\lambda(B\to A))$$

$$\overset{(\text a)}\iso h(\C_A)^\ddg(F\circ j(\C_A))\left(\lambda\left(\left(\icolim_{(U\to B)\in\C_B}U\right)\to A\right)\right)$$

$$\overset{(\text b)}\iso h(\C_A)^\ddg(F\circ j(\C_A))\left(\lambda\left(\col_{(U\to B)\in\C_B}(U\to A)\right)\right)$$

$$\overset{(\text c)}\iso\col_{(U\to B)\in\C_B}h(\C_A)^\ddg(F\circ j(\C_A))(U\to A)$$

$$\overset{(\text d)}\iso\col_{(U\to B)\in\C_B}F(U),$$

\nn where (a) follows from \eqref{263a} p.~\pageref{263a}; (b) follows from Lemma~\ref{cc}; (c) follows from Proposition~\ref{271} and \eqref{leq}, \eqref{lcom}; and (d) follows from Proposition~\ref{271}. A similar computation shows 
$$
h(\C)^\ddg(F)\circ j((\C^\wdg)_A)\iso\col_{(U\to B)\in\C_B}F(U).
$$ 
This proves \eqref{prepa1}.
\end{proof}
...
$$
\begin{tikzcd}
\C^\wdg\ar{d}[swap]{h^\dg(j^\dg(G))}&
\C\ar{l}[swap]{h}\ar{d}[swap]{j^\dg(G)}&
\C_A\ar{l}[swap]{j}\ar{r}{h_A}\ar{d}{G}&
(\C_A)^\wdg\ar{d}{(h_A)^\dg(G)}&
(\C^\wdg)_A\ar{l}[swap]{\lambda}\\ 
\A\ar[equal]{r}&\A\ar[equal]{r}&\A\ar[equal]{r}&\A.
\end{tikzcd}
$$ 
(See \eqref{lambda} p.~\pageref{lambda} for the definition of $\lambda$.) Let $B$ be in $\C^\wdg$. We claim 
...
\nn where 
(a) follows from \eqref{prepa4}; 
(b) follows from Proposition~\ref{271} and \eqref{lcom};  
(c) follows from Lemma~\ref{cc}; 
(d) follows from the fact that small inductive limits in $\Set$ are stable by base change (Definition 2.2.6 p.~47 of the book, stated below as Definition~\ref{dsbc} p.~\pageref{dsbc}); and 
...
\C^\wdg\ar{r}[swap]{\widehat F}&\C'^\wdg.
\end{tikzcd}
$$ 
quasi-commutes. 

\begin{prop}\label{yf} 
Let $\U\in\V$ be universes, let $\A$ be the category of $\U$-small categories (see Definition~\ref{myuscat} p.~\pageref{myuscat}), let $\B$ be the category whose objects are the $\V$-small $\U$-categories (see Definition~\ref{myucat} p.~\pageref{myucat}) and whose morphisms are the isomorphism classes of functors. Then there is a functor $\Phi:\A\to\B$ satisfying $\Phi(\C)=\C^\wdg_\U$ and $\Phi(F)=\widehat F$, and there is a morphism of functors $\theta$ from the ``inclusion'' $\A\to\B$ to $\Phi$ satisfying $\theta_\C=\hy_\C$ for all $\C$ in $\A$. 
\end{prop}

\begin{rk}\label{cof}
Recall that $F:\C\to\C'$ is a functor of small categories. Let $A'$ be in $\cc C'^\wdg$, and let $\cc C_{A'\circ F}\xr\pp\cc C'_{A'}\xr\psi\C'^\wdg$ be the natural functors. By \eqref{263a} p.~\pageref{263a} the natural morphism $\col(\psi\circ\pp)\to\col\psi$ induces a morphism $f:\widehat F(A'\circ F)\to A'$ functorial in $A'$. Moreover, if $\pp$ is cofinal, then $f$ is an isomorphism. (This remark will be used to prove Proposition~\ref{myprop1} p.~\pageref{myprop1}.)
\end{rk}

The proof is obvious.

\begin{rk}
If $F$ is fully faithful, then there is an isomorphism $\widehat F(A)\circ F\xr\sim A$ functorial in $A\in\C^\wdg$.
\end{rk} 
...
Let $X$ be in $\C$ and $A$ be a terminal object of $\C^\wdg$. We have 
$$
...
\begin{rk}\label{272b}
Assume $A$ is a terminal object of $\C^\wdg$, and define, using the above identification, $G:\C\to\C'_{\widehat F(A)}$ by 
$$
G(X):=(F(X),p_X(\id_{F(X)})),
$$ 
...
Let $\C$ be a category and $\A$ a category admitting small projective limits, let $h:\C\to\C^\wdg$ the Yoneda embedding, and let $\Fct^{p\ell}((\C^\wdg)^{\op},\A)$ be the category of functors from $(\C^\wdg)^{\op}$ to $\A$ commuting with small projective limits. Then the functors 
$$
\begin{tikzcd}
\Fct^{pl}((\C^\wdg)^{\op},\A)\ar[yshift=0.7ex]{r}{(h^{\op})_*}&\Fct(\C^{\op},\A)\ar[yshift=-0.7ex]{l}{(h^{\op})^\ddg}
\end{tikzcd}
$$
are mutually quasi-inverse equivalences. 

Let $F$ be in $\Fct((\C^\wdg)^{\op},\A)$. Assume $(A_i)$ is a projective system in $(\C^\wdg)^{\op}$, or, equivalently, $(A_i)$ is an inductive system in $\C^\wdg$. In particular $(F(A_i))$ is a projective system in $\A$. 

Then $F$ is in $\Fct^{p\ell}((\C^\wdg)^{\op},\A)$ if and only if the following condition holds: 
...
\C^\wdg\to\A,\quad A\mapsto(h^{\op})^\ddg(F)(A)
$$ 
...
\end{equation} 
%
which is contained in the proof of Proposition 2.7.5 p.~64 of the book, and which follows from Corollary 2.4.6 p.~56 of the book (see Proposition~\ref{246} p.~\pageref{246}). Recall that $F:\C\to\C'$ is a functor of small categories, that $A$ is in $\C^\wdg$, and that $X'$ is in $\C'$. 

For the reader's convenience we reproduce the statement of Proposition 2.7.5: 

\begin{prop}[Proposition 2.7.5] 
If $F:\C\to\C'$ is a functor of small categories, then the functors $\widehat F$ and $(F^{\op})^\dg$ from $\C^\wdg$ to $\C'^\wdg$ are isomorphic. 
\end{prop} 
...
\begin{s}
P.~64, end of Chapter 2. One could add the following observation: If $\C$ is a small category, if $A$ is in $\C^\wdg$, if $B$ is a terminal object of $(\C_A)^\wdg$, and if $F:\C_A\to\C$ is the forgetful functor, then we have 
$$
\widehat F(B)\iso A.
$$ 
Indeed, we have 
...
Consider functors $I\xr\alpha\C\xr F\C'$, and assume that $I$ is finite, that $F$ is right exact, and that $\col\alpha$ exists in $\C$. Then $\col(F\circ\alpha)$ exists in $\C'$, and the natural morphism $\col(F\circ\alpha)\to\col\alpha$ is an isomorphism. 
\end{prop} 
...
$$
X\in\C\xleftarrow{\id_\C}\C\xr{F}\C'\ni X', 
$$ 
...
Let $\C$ be a category admitting finite inductive limits, and let $A$ be in $\C^\wdg$. Then $A$ is left exact if and only if $\C_A$ is filtrant.
\end{prop}
...
Recall the notation. We have functors $I\xr\pp K\xleftarrow\psi J$ between small categories, and 
$$
M:=M[I\xr\pp K\xleftarrow\psi J] 
$$ 
...
Let $\C$ be a $\U$-small category, and let us denote by $\hy:\C\to\C^\wdg$ and $\ky:\C\to\C^\vee$ the Yoneda embeddings. For any morphism $f:X\to Y$ in $\C$ define $\IM\hy(f)$ in $\C^\wdg$ and $\IM\ky(f)$ in $\C^\vee$ by
$$
...
$$ 
(\Coim f)(Z):=\Hom_{\C^\wdg}(\IM\hy(f),\hy(Z))
$$ 
for all $Z$ in $\C$, and the {\em image}\index{image} of $f$ is the object $\Ima f$ of $\C^\wdg$ defined by 
$$ 
...
$$ 
in $\C^\vee$ and $\C^\wdg$ respectively. Moreover, $\ky(X)\to\Coim f$ is an epimorphism, and $\Ima f\to\hy(Y)$ is a monomorphism. 
\end{prop} 
...
$$ 
(\Coim f)(Z)\iso\Ker\Big(\Hom_\C(X,Z)\parar\Hom_{\C^\wdg}\big(\hy(X)\times_{\hy(Y)}\hy(X),\hy(Z)\big)\Big), 
$$ 
$$ 
...
If $P:=X\times_YX$ exists in $\C$, then $\Coim f$ is naturally isomorphic to $\Coker(P\parar X)\in\C^\vee$. If $S:=Y\sqcup_XY$ exists in $\C$, then $\Ima f$ is naturally isomorphic to $\Ker(Y\parar S)\in\C^\wdg$. 
\end{prop} 
...
$f\mapsto\Ima\hy(f)$ and $\Ima$ are functors from $\Mor(\C)$ to $\C^\wdg$, 
...
Let $S$ be a set of objects of a category $\C$ and $\cc S$ the corresponding subcategory. We say that $S$ is a {\em system of generators}\index{generator}\index{system of generators} if the functor $\pp:\C\to\cc S^\wdg$, $X\mapsto\Hom_\C(\ ,X)$ is conservative.
\end{df}
\end{s}
...
$$
\begin{tikzcd}
\C\ar{r}{\gamma}\ar{dr}[swap]{\pp}&\G^\wdg\ar{d}{\rho}\\
&\F^\wdg
\end{tikzcd}
$$ 
...
$$
\begin{tikzcd}
\Hom_\C(X,Y)\ar{r}{\gamma'}\ar{dr}{\sim}[swap]{\pp'}&\Hom_{\G^\wdg}(\gamma(X),\gamma(Y))\ar{d}{\rho'}\\ 
{}&\Hom_{\F^\wdg}(\pp(X),\pp(Y)). 
\end{tikzcd}
$$ 
We want to prove that $\gamma'$ is bijective. As $\pp'$ is bijective, it suffices to show that $\gamma'$ is surjective. Let $\xi$ be in $\Hom_{\G^\wdg}(\gamma(X),\gamma(Y))$. There is a (unique) $f$ in $\Hom_\C(X,Y)$ such that  
...
If $\C$ is a category admitting small inductive limits and $\F$ is a small full subcategory of $\C$, then the functor $\pp:\C\to\F^\wdg$ admits a left adjoint $\psi:\F^\wdg\to\C$, and for $A$ in $\F^\wdg$ we have 
$$
...
\begin{proof}
We have, for $X$ in $\C$ and $A$ in $\F^\wdg$, 
$$
\Hom_\C\left(\col_{(Y\to A)\in\F_A}Y,X\right)\iso\lim_{(Y\to A)\in\F_A}\Hom_\C(Y,X)
$$ 
$$
\iso\lim_{(Y\to A)\in\F_A}\pp(X)(Y)\iso\Hom_{\F^\wdg}(A,\pp(X)),
$$  
...
\nn Step 3'. Let $A$ be in $\F^\wdg$, and let $(B_i\to A)_{i\in I}$ be a small filtrant inductive system in $(\F^\wdg)_A$. We claim 
$$
...
$$
\Hom_\C(\Coim z,Y)\iso\Hom_{\F^\wdg}(\Coim\pp(z),\pp(Y)).
$$ 
...
%
&\iso\lim_{A\in I}\Hom_{\F^\wdg}\Big(\Coim\pp\xi(A),\pp(Y)\Big)&\text{by Step 4}\\ \\ 
%
&\iso\Hom_{\F^\wdg}\left(\col_{A\in I}\Coim\pp\xi(A),\pp(Y)\right)\\ \\ 
%
&\iso\Hom_{\F^\wdg}\left(\Coim\left(\col_{A\in I}\pp\zeta\xi(A)\to\pp(X)\right),\pp(Y)\right)&\text{by Step 3'}\\ \\ 
%
&\iso\Hom_{\F^\wdg}(\pp(X),\pp(Y))&\text{by Step 5.}
%
\end{align*} 

It remains to prove Steps 3, 3', 4, 5, and 6. We refer to the book for Step 3. The proof of Step 3' is the same. (It is easy to see that small inductive limits in $\F^\wdg$ are stable by base change --- Definition 2.2.6 p.~47 of the book, stated above as Definition~\ref{dsbc} p.~\pageref{dsbc}.) 
...
and also 
$$
\Hom_\C(Z,Y)\iso\Hom_{\F^\wdg}(\pp(Z),\pp(Y))
$$ 
by the Yoneda Lemma. As $\pp$ is faithful, the natural map 
$$
\Hom_\C(Z\times_XZ,Y)\to\Hom_{\F^\wdg}\big(\pp(Z\times_XZ),\pp(Y)\big)
$$
$$
\iso\Hom_{\F^\wdg}\big(\pp(Z)\times_{\pp(X)}\pp(Z),\pp(Y)\big).
$$ 
...
$$
$$
\iso\Ker\Big(\Hom_{\F^\wdg}\big(\pp(Z),\pp(Y)\big)\rightrightarrows\Hom_{\F^\wdg}\big(\pp(Z)\times_{\pp(X)}\pp(Z),\pp(Y)\big)\Big)
$$
$$
\iso\Hom_{\F^\wdg}\Big(\Coker\big(\pp(Z)\times_{\pp(X)}\pp(Z)\rightrightarrows\pp(Z)\big),\pp(Y)\Big)
$$
$$
\iso\Hom_{\F^\wdg}(\Coim\pp(z),\pp(Y)).
$$ 
\end{proof}
...
$$
(\pp\zeta\xi(\{z\}))(Z)=\Hom_\C(Z,Z)\xr{z\circ}\Hom_\C(Z,X),
$$
which is obvious. 
\end{proof}
...
If $\C$ is a category, then the category $\Ind(\C)$ admits small filtrant inductive limits and the natural functor $\Ind(\C)\to\C^\wdg$ commutes with such limits.
\end{thm} 
...
Let $c$ be given by the morphism $X\to A$ in $\C^\wdg$, let $m$ be given by the morphism $Y\to\alpha(i)$ in $\Ind(\C)$, and let $\sigma$ and $\sigma'$ be given by the morphisms $s,s':X\parar Y$ making the diagram below commute:
$$
\begin{tikzcd}
...
As $p_i(X)(y\circ s)$ equals $p_i(X)(y\circ s')$ in $A(X)\iso\col\Hom_\C(X,\alpha)$ and $I$ is filtrant, there is a morphism $t:i\to j$ in $I$ such that $\alpha(t)\circ y\circ s=\alpha(t)\circ y\circ s'$, and we can set $Z:=\alpha(j)$ and $z:=\id_{\alpha(j)}$. The situation is depicted by the commutative diagram
$$
\begin{tikzcd}
...
\begin{prop} 
Let $\A$ be a category which admits small filtrant inductive limits, let $F:\C\to\A$ be a functor, and let $\C\xr i\Ind(\C)\xr j\C^\wdg$ be the natural embeddings. Then the functor $i^\dg(F):\Ind(\C)\to\A$ exists, commutes with small filtrant inductive limits, and satisfies $i^\dg(F)\circ i\iso F$. Conversely, any functor $\widetilde F:\Ind(\C)\to\A$ commuting with small filtrant inductive limits with values in $\C$, and satisfying $\widetilde F\circ i\iso F$, is isomorphic to $i^\dg(F)$. 
\end{prop} 

\begin{proof}
The proof is essentially the same as that of Proposition 2.7.1 on p.~62 of the book. (See also \S\ref{c271b} p.~\pageref{c271b}.) Again, we give some more details about the proof of the fact that $i^\dg(F)$ commutes with small filtrant inductive limits. Put $\widetilde F:=i^\dg(F)$. 

Let us attach the functor $B:=\Hom_\A(F(\ ),Y)\in\C^\wdg$ to the object $Y$ of $\A$. To apply Proposition~\ref{2.1.10b} p.~\pageref{2.1.10b} to the diagram 
$$
\begin{tikzcd}
...
\Hom_{\C^\wdg}(\ \ ,B)
$$ 
in $\Ind(\C)^\wdg_\V$, where $\V$ is a universe containing $\U$ such that $\C^\wdg$ is a $\V$-category. We have 
$$
...
$$
\iso
\lim_{(X\to A)\in\C_A}\Hom_{\C^\wdg}((j\circ i)(X),B)
\iso
\Hom_{\C^\wdg}\left(\icolim_{(X\to A)\in\C_A}X,B\right)\iso
\Hom_{\C^\wdg}(j(A),B).
$$
\end{proof}
...
\theta\circ\mu\iso\id_{\Ind(\C_1)\times\Ind(\C_2)}.
\end{equation}
%
...
\mu\circ\theta\iso\id_{\Ind(\C_1\times\C_2)}.
\end{equation}
...
\[
\begin{tikzcd}
{}&\C^\wdg_\V\ar[-]{ld}\ar[-]{rd}\\
\C^\wdg_\U\ar[-]{rd}&&\Ind^\V(\C)\ar[-]{ld}{i}\\
&\Ind^\U(\C)\ar[-]{d}\\
&\C,
\end{tikzcd}
\]
...
$$
\kappa(X)=I\rho(\kappa'(\col(\rho\circ\xi)))\iso I\rho(\ic\xi)
$$

$$
\iso\ic(I\rho\circ\iota_\C\circ\xi)\iso\ic(\rho\circ\xi), 
$$ 
...
$$
M:=M\left[N'\xr{\pp}\big(\C/G(a)\big)^{L/a}\xleftarrow{\ \Delta\circ\widetilde{\alpha_a}}I_a\right].
$$ 
...
$$ 
N\iso M\left[N'\xr{\pp}(\C/G(a))^{L/a}\xleftarrow{\ \Delta\circ\widetilde{\alpha_a}}I_a\right]. 
$$
...
\nn(a)$\then$(b). Let $f:X'\to IF(A)$ be a morphism in $\Ind(\cc C')$ with $X'$ in $\cc C'$ and $A$ in $\Ind(\cc C)$, let $\beta_0:I\to\cc C$ be a functor with $I$ small and filtrant and $\ic\beta_0\iso A$; in particular $\ic(F\circ\beta_0)\iso IF(A)$. By Proposition 6.1.13 p.~134 of the book there are functors $\alpha:J\to\cc C'$ and $\beta:J\to\cc C$, and there is a morphism of functors $\pp:\alpha\to F\circ\beta$ such that 

$J$ is small and filtrant, 

$\alpha$ is constant equal to $X'$, 

$\ic(F\circ\beta)\iso IF(A)$, 
...
$$ 
be functors such that $Q\circ R\iso\id_{\C'}$, let $\cc S$ be a subset of $\Ob(\Mor(\C))$ such that $Q(s)$ is an isomorphism for all $s$ in $\cc S$, let 
$$
...
\begin{s} 
P.~154. Below the statement of Lemma 7.1.13 it is written: ``The verification is left to the reader. // Hence, we get a big category ...''. One might add between the two sentences something like: We also leave it to the reader to define the identity $\id_X$ of $X$ viewed as an object of $\C^r_{\mc S}$, and to check the equalities $f\circ\id_X=f$, $\id_X\circ g=g$ for $f$ in $\Hom_{\C^r_{\mc S}}(X,Y)$ and $g$ in $\Hom_{\C^r_{\mc S}}(Y,X)$.
\end{s}
...
$$
R_{\cc S}F\iso F\circ R_{\cc S}\id_\C.
$$
...
Let $\C$ be a category, let $\SSS$ a right multiplicative system, and let $F:\C\to\A$ be a functor such that $F(s)$ is an isomorphism for all $s$ in $\SSS$. Then $F$ is universally right localizable (see \S\ref{durl} p.~\pageref{durl}), $R_{\SSS}F\iso F_{\SSS}$, and for any functor $K:\A\to\A'$ the diagram below commutes up to isomorphism
$$
\begin{tikzcd}
...
$$
R_\SSS(\iota_\A\circ F)(Q(X))\iso IF(\alpha_\SSS(Q(X)))
$$ 
...
$$
B:=R_\SSS(\iota_\A\circ F)(Q(X))=\col_{(X',x')\in\SSS^X}\iota_\A(F(X'))\in\Ind(\A).
$$
...
In the above setting we denote $\A$ by $\C^\wdg$ and $h$ by $\hy_\C$. (This abuse is justified by Proposition~\ref{payp}.) We also use Definitions~\ref{ue} and \ref{ue2} p.~\pageref{ue} in this context. 
\end{conv} 
...
\end{tikzcd}
$$ 
We can assume that $\C_0$ and $\M_0$ are small (in the sense of Definition~\ref{myuscat} p.~\pageref{myuscat}). If $\lambda:\C\to(\C_0)^\wdg$ and $\lambda':\M\to(\M_0)^\wdg$ are the obvious functors, then the diagram 
$$
\begin{tikzcd}
\C\ar{r}{\pp}\ar{d}[swap]{\lambda}&\M\ar{d}{\lambda'}\\
(\C_0)^\wdg\ar{r}[swap]{\widehat\pp_{{}_0}}&(\M_0)^\wdg
\end{tikzcd}
$$ 
...
$$
\lim\Hom_\C(\pp,\col\pp)\xleftarrow\sim\Hom_\C(\col\pp,\col\pp)\neq\varnothing.
$$
\end{proof}
\end{s}
...
$$ 
In particular the family $(y_j)$ belongs to $\lim A(\pp)$. Our assumption about $\C$ implies that $\col\pp$ exists in $\C$. Let $p_j:\pp(j)\to\col\pp$ be the coprojection. By surjectivity of the map $A(\col\pp)\to\lim A(\pp)$ in (ii), there is an $x:\col\pp\to A$ such that $x\circ p_j=y_j$ for all $j$. Setting 
$$
...
N_s:=\Ker(A\parar X_i).
$$ 
By Corollary 3.2.3 (i) p.~79 of the book, $I^{i_0}$ is filtrant and the forgetful functor $I^{i_0}\to I$ is cofinal. One of our assumption, namely Condition (9.3.1) (iii) in Section~\ref{931} p.~\pageref{931}, says that small filtrant inductive limits are exact in $\C$. In particular, $\col_{s\in I^{i_0}}$ is exact in $\C$, and we get  
$$
...
\nn$\bu$ Step 4: (a) holds. For each $i$ in $I$ define $Y_i:=A\times_XX_i$. As $\col_i$ is exact in $\C$, we have $\col_iY_i\iso A$. As $\card(A(G))<\pi$, Lemma 9.3.1 p.~224 of the book (stated above as Lemma~\ref{l331} p.~\pageref{l331}) implies that there is an $i_0$ in $I$ such that $B:=Y_{i_0}\to A$ is an epimorphism.
...
$$ 
As $\theta$ is a monomorphism, Proposition~\ref{34i} p.~\pageref{34i} implies that $\col\theta$ is also a monomorphism. Then the conclusion follows from the commutativity of the diagram 
$$
\begin{tikzcd}
\col\Delta(\alpha(i))\ar{d}[swap]{\sim}\ar[tail]{rr}{\col\theta}&&\col(\alpha\circ\pp)\ar{d}{\sim}\\ 
...
$$
\Hom_{\cc S^{\wdg,\text{prod}}}(\pp(X),H_0)\iso H(X)\iso\Hom_{\cc D^\wdg}(X,H)
$$ 
for $X$ in $\cc S$.
...
\end{equation} 
be an inductive system in $\A_d$. Assume that $X:=\col_iX_i$ exists in $\A$. Then the natural morphism $\col_id_{X,i}:X\to TX$ is an inductive limit of \eqref{iliad} in $\A_d$. There are analogous statements with ``projective'' instead of ``inductive'' and $\A_c$ instead of $\A_d$.
\end{s}
...
\nn(iii) the functor $Q:\oo K_c(\A)\to\oo D_c(\A)$ admits a right adjoint $R_q:\oo D_c(\A)\to\oo K_c(\A)$, this right adjoint is triangulated, satisfies $Q\circ R_q\iso\id_{\oo D_c(\A)}$, and is isomorphic to the composition of $\iota:\oo K_{c,\oo{hi}}(\A)\to\oo K_c(\A)$ and a quasi-inverse of $Q\circ\iota$,
...
To a sieve $S$ over $U$ we attach the subobject $A_S$ of $U$ in $\C^\wdg$ by the formula
$$
A_S(V):=S\cap\Hom_\C(V,U).
$$ 
Conversely, to an object $A\to U$ of $(\C^\wdg)_U$ we attach the sieve $S_A$ over $U$ by putting 
$$
S_{A\to U}:=\{V\to A\to U\}.
$$ 
...
Consider the following conditions on a set $\cc E$ of morphisms in $\C^\wdg$:

\nn LE1: $\id_U$ is in $\cc E$ for all $U$ in $\C$,

\nn LE2: if the composition of two elements of $\cc E$ exists, it belongs to $\cc E$,

\nn LE3: if the composition $v\circ u$ of two morphisms of $\C^\wdg$ exists and is in $\cc E$, then $v$ is in $\cc E$,

\nn LE4: a morphism $A\to B$ in $\C^\wdg$ is in $\cc E$ if and only if, for any morphism $U\to B$ in $\C^\wdg$ with $U$ in $\C$, the projection $A\times_BU\to U$ is in $\cc E$.
...
\end{equation}
%
be the set of those morphisms $A\to B$ in $\C^\wdg$ such that, for any morphism $U\to B$ in $\C^\wdg$ with $U$ in $\C$, the sieve $S_{A\times_BU\to U}$ is in $\Gamma_U$. 

\begin{rk}\label{1613i}
A morphism $A\to U$ in $\C^\wdg$ is in $\cc E$ if and only if $S_{A\to U}$ is in $\Gamma_U$. 
\end{rk}
...
\nn LE2: Let $A\to B\to C$ be a diagram in $\C^\wdg$, and assume that the two arrows are in $\cc E$. Consider the diagram of solid arrows with cartesian squares 
$$
\begin{tikzcd}
...
$$
S\cap\Hom_\C(V,U)\xr\pp\Hom_{\C^\wdg}(V,A_S)
$$
...
Conversely, assuming that $v$ is in $\Hom_{\C^\wdg}(V,A_S)$, it suffices to prove that $i\circ v$ is in $S$. We have 
$$
...
\end{equation}
%
Let $V\to A_S$ be a morphism in $\C^\wdg$ with $V$ in $\C$, and let 
$$
\begin{tikzcd}
C\ar{r}\ar{d}&V\ar{d}\\ 
B\ar{r}&A_S
...
\begin{thm}
If $\C$ is a $\U$-small category, if $\Gamma$ is a subfamily of $(\Sigma_U)_{U\in\C}$ satisfying GT1-GT4, and if $\cc E$ is a set of morphisms in $\C^\wdg_\U$ satisfying LE1-LE4, then the equalities $\cc E=\cc E(\Gamma,\U)$ and $\Gamma=\Gamma(\cc E)$ are equivalent.
\end{thm}

\begin{cor}\label{leu}
Let $\U\subset\U'$ be universes, let $\C$ be a $\U$-small category, let $\Gamma$ be a subfamily of $(\Sigma_U)_{U\in\C}$ satisfying GT1-GT4 (see Conditions~\ref{gt} p.~\pageref{gt}), let $u:A\to B$ be a morphism in $\C^\wdg_\U$, and let $u':A'\to B'$ be the corresponding morphism in $\C^\wdg_{\U'}$. Then $u$ is in $\cc E(\Gamma,\U)$ (see \eqref{cce} p.~\pageref{cce}) if and only if $u'$ is in $\cc E(\Gamma,\U')$.
\end{cor}
...
Let $\U\subset\U'$ be universes, let $\C$ be a $\U$-small category, let $\Gamma$ be a subfamily of $(\Sigma_U)_{U\in\C}$ satisfying GT1-GT4, let $u:A\to B$ be a morphism in $\C^\wdg_\U$, and let $u':A'\to B'$ be the corresponding morphism in $\C^\wdg_{\U'}$. Then $u$ is in $\cc M(\Gamma,\U)$ (resp. in $\cc I(\Gamma,\U)$) if and only if $u'$ is in $\cc M(\Gamma,\U')$ (resp. in $\cc I(\Gamma,\U')$).
\end{s}
...
\nn(c) for any diagram $C\parar A\to B$ such that $C$ is in $\C^\wdg$ and the two compositions coincide, there exists a local epimorphism $D\to C$ such that the two compositions $D\to C\parar A$ coincide,
...
\nn(e) for any diagram $C\parar A\to B$ such that $C$ is in $\C^\wdg$ and the two compositions coincide, there exists a local isomorphism $D\to C$ such that the two compositions $D\to C\parar A$ coincide.
...
$$
A^a(U)\iso\Hom_{({\C^\wdg})_{\cc{LI}}}(Q(U),Q(A)),
$$ 
...
$$
A(U)\iso\Hom_{\C^\wdg}(U,A)\to\Hom_{({\C^\wdg})_{\cc{LI}}}(Q(U),Q(A))\iso A^a(U).
$$ 
...
$$ 
is a diagram in $\C^\wdg$ with $U$ in $\C$ and $s$ a local isomorphism, and if 
$$
v=Q(u)\circ Q(s)^{-1}\in A^a(U)\iso\Hom_{({\C^\wdg})_{\cc{LI}}}(Q(U),Q(A)), 
$$ 
then 
...
$$
\alpha:(\cc{LI}_U)^{\op}\to\Set,\quad(B\xr sU)\mapsto\Hom_{\C^\wdg}(B,A)
$$ 
...
\begin{s}\label{fhat}
P.~406. Recall that, in the first line of the second display, $(\C_Y)^\wdg$ should be $\C_Y$ (twice). In notation \eqref{ttau}, Formula \eqref{275} p.~\pageref{275} gives, for $B$ in $\C_Y^\wdg$ and $U$ in $\C_X$, 
%
...
The functor $\fthat$ commutes with small inductive limits (Proposition 2.7.1 p.~62 of the book, Remark~\ref{272} p.~\pageref{272} above). Moreover, if $f$ is left exact, then $\fthat$ is exact (Corollary 3.3.19 p.~87 of the book).
\end{prop}

If $f:X\to Y$ is a continuous map of small topological spaces, if $B$ is in $(\oo{Op}_Y)^\wdg$ and $U$ in $\oo{Op}_X$, then \eqref{efhat1} gives 
%
...
P.~407. Let $f:X\to Y$ be a morphism of presites and let $\A$ be a category admitting small inductive and projective limits. In the notation of \eqref{ttau} p.~\pageref{ttau}, we set 
$$
...
We extend presheaves over $X$ to presheaves over $\widehat X$ using the functor $\oo h_X^\ddg$ associated with the Yoneda embedding $\oo h_X^t=\oo h_{\C_X}$. Hence, for $F$ in $\PSh(X,\A)$ and $A$ in $\C_X^\wdg$, we have 
$$
...
\end{equation}
% 
(Note that the same comment is made at the beginning of Section 17.3 p.~414.) This convention of extending each presheaf $F$ over $X$ to a presheaf, still denoted by $F$, over $\widehat X$ which commutes with small projective limits implies that we have, for $A,B$ in $\C^\wdg$, 
$$
B(A)\iso\Hom_{\C_X^\wdg}(A,B).
$$ 

In the notation of \S\ref{opddagg} p.~\pageref{opddagg}, Convention 17.1.6 can be described as follows:

If $X$ is a site, if $\C$ is the corresponding category, if $h:\C\to\C^\wdg$ is the Yoneda embedding, if $F$ is an $\A$ valued sheaf over $X$, and if $A$ is an object of $\C^\wdg$, then Convention 17.1.6 consists in putting 
$$
F(A):=(h^{\op})^\ddg(F)(A).
$$ 
\end{s}
...
\Hom_{\PSh(X,\A)}(F,G)\iso\lim_{U\in\C_X}\cc Hom_{\PSh(X,\A)}(F,G)(U).
$$
This follows from Theorem~\ref{fgd} p.~\pageref{fgd}. %\eqref{17115mod} p.~\pageref{17115mod}. 
\end{s} 
...
Claim: if a local isomorphism $u:A\to B$ in $\C_Y^\wdg$ is either a monomorphism or an epimorphism, then $\fthat(u)$ is a local isomorphism in $\C_X^\wdg$. 

Proof of the claim: Let $V\to B$ be a morphism in $\C_Y^\wdg$ with $V$ in $\C_Y$. Then $u_V: A\times_BV\to V$ is either a monomorphism or an epimorphism by Proposition~\ref{sbcs} p.~\pageref{sbcs} and Proposition~\ref{34i} p.~\pageref{34i}. Let us show that $\fthat(u_V)$ is a local isomorphism. 
...
$$
\fthat(u_V)\circ\fthat(s)\iso\id_{f^t(V)}
$$ 
...
\begin{s}
P.~413. Lemma 17.2.5 (ii) and Exercise 2.12 (ii) p.~66 of the book imply: If $f:X\to Y$ is weakly left exact, then $\fthat:\C_Y^\wdg\to\C_X^\wdg$ commutes with projective limits indexed by small connected categories.
\end{s} 
...
\begin{s}
P.~413, Example 17.2.7 (i). Recall that $f:X\to Y$ is a continuous map of small topological spaces. As explained in the book, to see that $f$ is a morphism of sites, it suffices to check that, if $u:B\to V$ is a local epimorphism in $(\oo{Op}_Y)^\wdg$ with $V$ in $\oo{Op}_Y$, then $\fthat(B)\to f^{-1}(V)$ is a local epimorphism in $(\oo{Op}_X)^\wdg$. This follows immediately from \S\ref{390} p.~\pageref{390} and \eqref{efhat2} p.~\pageref{efhat2}. 
\end{s}
...
\end{tikzcd} 
$$ 
as follows: If $p:M\to\oo j_{U\to X*}F$ is a morphism in $\PSh(U,\A)$, given by morphisms $p(V\to U):M\to F(V)$ in $\A$, then we put $\pp(p):=p(U\xr{\id_U}U)$; if $a:M\to F(U)$ is a morphism in $\A$, then we put $\psi(a)(V\xr cU):=F(c)\circ a$; and we check that $\pp$ and $\psi$ are mutually inverse bijections. 
\end{s}
...
$$
\col\alpha\xr f\col\alpha\circ\mu_u^{\op}\circ\lambda_u^{\op}\xr g\col\alpha\circ\mu_u^{\op}\xr h\col\alpha.
$$
We must show that $g\circ f$ is an isomorphism. The equality $h\circ g\circ f=\id_{\col\alpha}$ is easily checked. Being a right adjoint, $\mu_u^{\op}$ is left exact, hence cofinal by Lemma 3.3.10 p.~84 of the book, and $h$ is an isomorphism. q.e.d.
\end{s}  
...
\overset{(\text b)}{\iso}\col_{x\in\cc E}F(q_D(x))
$$
$$
\overset{(\text c)}{\iso}\col_{x\in\cc E}F\left(q_B(x)``\!\!\!\bigsqcup_{q_A(x)}\!\!\!"q_C(x)\right)
\overset{(\text d)}{\iso}\col_{x\in\cc E}(F(q_B(x))\times_{F(q_A(x))}F(q_C(x)))
$$
$$
\overset{(\text e)}{\iso}\left(\col_{x\in\cc E}F(q_B(x))\right)\times_{\col_{x\in\cc E}F(q_A(x))}\left(\col_{x\in\cc E}F(q_C(x))\right)
$$ 
$$
...
\begin{s} 
P.~422. The first sentence of the proof of Theorem 17.4.7 (iv) follows from Corollary~\ref{bre} p.~\pageref{bre}. One could add:
...
\end{tikzcd}
$$ 
in $\C_A^\wdg$ is a local epimorphism if and only if $C\to B$ is a local epimorphism in $\C_X^\wdg$.
\end{s}
...
In the setting of Definition 17.6.1, let $B\to A$ be a morphism in $\C_X^\wdg$, let $u:C\to B$ be a morphism in $\C_X^\wdg$, and let $v:(C\to A)\to(B\to A)$ be the corresponding morphism in $\C_A^\wdg$. Then $u$ is a local epimorphism if and only if $v$ is a local epimorphism, $u$ is a local monomorphism if and only if $v$ is a local monomorphism, and $u$ is a local isomorphism if and only if $v$ is a local isomorphism. 
\end{s} 
...
Step~(ii): ``$f$ factors as $X\xr{\oo j_{A\to X}}A\xr gY$'': see Definition 17.2.4 (ii) p.~413 of the book and Remark~\ref{272b} p.~\pageref{272b}. The isomorphism $f^{-1}\iso\oo j_{A\to X}^{-1}\circ g^{-1}$ follows from Proposition 17.5.3 p.~424 of the book.
\end{s} 
...
%
&\iso G\big(f^t(V)\times A\to A\big)&\text{by \eqref{e406}}, 
% 
\end{align*} 
as well as 
\begin{align*} 
...
\begin{s}
P.~427, proof of Proposition 17.6.8, Step~(ii). Let $v:V\to A$ be a morphism in $\C_X^\wdg$. Here is a proof of the fact that 
...
\end{tikzcd}
\end{equation} 
is a local isomorphism in $\C_A^\wdg$. 

As $V\times A\to V$ is a local isomorphism in $\C_X^\wdg$ by \S\ref{1768}, and $V\to V\times A\to V$ is the identity of $V$, Lemma 16.2.4 (vii) p.~396 of the book implies that $V\to V\times A$ is a local isomorphism in $\C_X^\wdg$, and thus, by \S\ref{1761b} p.~\pageref{1761b}, that \eqref{1768ii} is a local isomorphism in $\C_A^\wdg$. 
\end{s} 
...
Let $X$ be a site. In this \S, for any $A$ in $\C_X^\wdg$, we denote the corresponding site by $A'$ instead of $A$. We also identify $\C_{A'}^\wdg$ to $(\C_X^\wdg)_A$ (see Lemma 17.1.8 p.~409 of the book). In particular, we get $\oo{pt}_{A'}\iso(A\xr{\id_A}A)\in\C_{A'}^\wdg$. 

Let $A\to B$ be a local isomorphism in $\C_X^\wdg$, and let us write $\omega$ for ``the'' terminal object $\oo{pt}_{B'}\iso(B\xr{\id_B}B)$ of $\C_{B'}^\wdg$. We claim that 
\begin{equation}\label{1771i1}
(A\to B)\to\omega 
\end{equation} 
is a local isomorphism in $\C_{B'}^\wdg$.
...
$$
a_X\circ f^\dg\circ \iota_Y\circ a_Y\iso a_X\circ f^\dg. 
$$ 
...
$$
a_Y\circ \iota_Y\circ a_Y\iso a_Y,
$$ 
...
By Lemma~\ref{rff'} p.~\pageref{rff'}, the commutative diagram \eqref{d447} also induces the commutative diagram 
$$
\Delta:=\left\{
\begin{tikzcd}
...
$$  
The sequence $\Sigma\otimes_{\R|_U}P|_U$ being exact thanks to the assumption in Lemma~\ref{l1853} p.~\pageref{l1853}, we see that the commutative diagram of complexes $\Delta\otimes_{\R|_U}P|_U$ has a cartesian top square, and that, by left exactness of $\Gamma(U;-)$ (see \S\ref{a438} p.~\pageref{a438}), the commutative diagram of complexes $\Gamma(U;\Delta\otimes_{\R|_U}P|_U)$, that is (see Notation 17.6.13 p.~428 of the book), 
$$
\begin{tikzcd}
...
$$ 
If $y$ is the element of $(N\otimes_{\R|_U}P|_U)(U)$ corresponding to $x$ under Isomorphism~\eqref{nrpu}, then we get $k_1(y)=u$, and thus $\overline s=g_3(u)=g_3(k_1(y))=0$. By \eqref{s=0il}, this completes the proof. 
\end{proof}
