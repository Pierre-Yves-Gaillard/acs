% about "categories and sheaves" (version auj)
...
\end{equation}
%
%\begin{conv}\label{ct=0}Say that the commutative diagram \eqref{dt=0} is attached to Equality \eqref{et=0} by Lemma \ref{techlem}.\end{conv}
...
$$ 
and it suffices to prove $s=0$. Write $s=\sum_{i=1}^na'_i\otimes b_i$ with $a'_i\in M'(U),b_i\in P(U)$, and put $a_i:=\varphi(a_i')$, so that \eqref{et=0} holds, and \eqref{dt=0} becomes 
...
\end{tikzcd}
$$ 
%attached to Equality~\eqref{et=0} by Lemma~\ref{techlem} in the sense of Convention~\ref{ct=0} 
The above commutative diagram gives rise to the commutative diagram 
%
