...
\nn$*$ P.~188. In the second diagram $Y'\overset{l'}{\rightarrowtail}Z$ should be $Y'\overset{l'}{\rightarrowtail}X$. After the second diagram: ``the set of isomorphism classes of $\DT$'' should be ``the set of isomorphism classes of objects of $\DT$''.
...
\begin{s} 
P.~61, Proposition 2.6.4 (minor variant). Let $\al:I\to\C$ be a functor defined on a small category, let $X$ be an object of $\C$, and let $p:\al\to\DT X$ be a morphism in $\C^I$ (see Notation~\ref{diag} p.~\pr{diag}). For any $Y$ in $\C$ we write 
$$
...
Let $\al:I\to\C$ be a functor defined on a small category, let $X$ be an object of $\C$, and let $p:\al\to\DT X$ be a morphism in $\C^I$ (see Notation~\ref{diag} p.~\pr{diag}). If the the equivalent condition in Proposition~\ref{b} hold, we say that $X$ is a {\em universal} inductive limit\index{universal inductive limit} of $\al$, and that $\col\al$ exists {\em universally}\index{universal existence of a limit} in $\C$.
\end{df}
\end{s}
...
\begin{lem}
Let $I$ be a small ordered set, $\al:I\to\C$ a functor. Let $\J$ denote the set of finite subsets of $I$, ordered by inclusion. To each $J$ in $\J$, associate the restriction $\al_J:\J\to\C$ of $\al$ to $\J$. Then $\J$ is small and filtrant and moreover
$$
...
\end{equation}
%
To prove \qr{l}, one can apply Lemma~\ref{711} p.~\pr{711} with $R:\C\to\DD'$ defined by $R(X):=(0\to X)$. 
...
$$
\prod_i\ F^b(A_i)
\overset{(\text a)}{\iso}\prod_i\ \col_{(B_i\to A_i)\in\mc{LI}_{A_i}}F(B_i)
...
\overset{(\text d)}{\iso}\col_{(B\to A)\in\mc{LI}_A}F(B)
...
$$
F^b(D)
\overset{(\text a)}{\iso}\col_{y\in\mc{LI}_D}F(j_D(y))
...
$$ 
$$
\overset{(\text f)}{\iso}\left(\col_{y\in\mc{LI}_B}F(j_B(y))\right)\tm_{\col_{y\in\mc{LI}_A}F(j_A(y))}\left(\col_{y\in\mc{LI}_C}F(j_C(y))\right)
$$ 
$$
