% about "categories and sheaves" (version-0177)
...
\nn(c) $\ssi$ (e): Write $P$ for $X\times_YX$. Recall that (c) says that $P\parar X\to Y$ is exact. As (e) is the condition in Lemma~\ref{strepi} p.~\pageref{strepi}, it suffices to show that $P\parar X\to Y$ is exact if and only if, for all $Z$ in $\C$, the map $u\mapsto u\circ f$ is a bijection $\Hom_\C(Y,Z)\xr\sim(\Coim f)(Z)$. Recall also that the set $(\Coim f)(Z)$ has been ``computed'' in Proposition~\ref{epimono} p.~\pageref{epimono}.
