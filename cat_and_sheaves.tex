...
\bb N':=\ic_n\mbf n. 
$$ 
We clearly have $\bb N'\in\Ind(\Set)$. Let $X$ be in $\Set$ and let $f:X\bb N'$ be an epimorphism. To prove that $\bb N'$ is isomorphic to no object of $\Set$, it suffices to show that $f$ is \emph{not} an epimorphism. As $f$ factors through some coprojection $p_n:\mbf n\to\bb N'$, if $f$ were an epimorphism, so would be $p_n$. But it is easy to see that $p_n$ is \emph{not} an epimorphism. 
