...
\end{lem}
\begin{proof} Part (a) is obvious, and (b) follows from Lemma~\ref{8311} (a) (ii) p.~\pr{8311}. To prove (c), let $g:U\to Y$ be a morphism satisfying $bg=0$ (in this proof we write $uv$ for $u\ci v$). By Lemma~\ref{8312}, (a) $\ssi$ (b), p.~\pr{8312}, it suffices to complete the commutative diagram
$$
\begin{tikzcd}
...
\end{tikzcd}
$$ 
Invoking again Lemma~\ref{8312}, (a) $\ssi$ (b), p.~\pr{8312}, we find an epimorphism $h:V\epi U$ and a morphism $i:V\to Z$ such that $cgh=ei$, and it suffices to prove $gh=ai$. But the equality $gh=ai$ is equivalent to the conjonction of the equalities $cgh=cai$ and $bgh=bai$, equalities whose proof is straightforward.
\end{proof}\newpage
...
\begin{s}
P. 319, Definition 13.1.1. In view of (10.2.1) p.~249 of the book, it is easy to see that the derived category $\oo D(\A_c)$ attached to an abelian category with translation $(\A,T)$ is obtained by localizing $\oo K(\A_c)$ with respect to the (left and right) multiplicative system formed by the quasi-isomorphisms.
\end{s}
...
$$
\begin{tikzcd}
Z\ar[d,equal]\ar[r,"a"]&Y\ar[d,"c"]\ar[r,"b"]&X\ar[d,"d"]\\ 
Z\ar[r,"e"]&W\ar[r,"f"]&V
\end{tikzcd}
$$ 
is a commutative diagram in $\C$, if the top row is exact and the right square cocartesian, then the bottom row is exact.
\end{lem} 
\begin{proof}
It suffices to complete the commutative diagram 
$$
\begin{tikzcd}
Z\ar[dr,"0"']\ar[r,"e"]&W\ar[d,"c"]\ar[r,"g"]&V\ar[d,dashed,"h"]\\ 
&U\ar[r,dashed,rightarrowtail,"i"']&T.
\end{tikzcd}
$$ 
We have the commutative diagram 
$$
\begin{tikzcd}
Z\ar[dr,"0"']\ar[r,"a"]&Y\ar[d,"gc"]\ar[r,"b"]&X\ar[d,"j"]\\ 
&U\ar[r,rightarrowtail,"i"']&T.
\end{tikzcd}
$$ 
The right square being cocartesian, there is a unique morphism $h:V\to T$ such that $hd=j$ and $hf=ig$. 
\end{proof}
\end{comment}

\nn$\bu$ Exactness at $Z^p$: This is an immediate consequence of Lemma~\ref{cart1} p.~\pr{cart1} (c).
...
\end{tikzcd}
$$ 
is a commutative diagram in $\C$, if the top row is exact and the left square cocartesian, then the bottom row is exact.
\end{lem} 
\begin{proof}
It suffices to complete the commutative diagram 
$$
\begin{tikzcd}
W\ar[dr,"0"']\ar[r,"e"]&V\ar[d,"g"]\ar[r,"f"]&V\ar[d,dashed,"h"]\\ 
&U\ar[r,dashed,rightarrowtail,"i"']&T.
\end{tikzcd}
$$ 
We have the commutative diagram 
$$
\begin{tikzcd}
Z\ar[dr,"0"']\ar[r,"a"]&Y\ar[d,"gd"]\ar[r,"b"]&X\ar[d,"h"]\\ 
&U\ar[r,rightarrowtail,"i"']&T.
\end{tikzcd}
$$ 
It suffices to prove $hf=ig$. The left square being cocartesian, the equality $hf=ig$ is equivalent to the conjonction of the equalities $hfd=igd$ and $hfe=ige$, which are straightforward.  
\end{proof}
\end{comment}
...
In fact, the existence of the indicated limit is a sufficient condition for the existence of the right derived functor, but it is not obvious \emph{a priori} (at least to me) that this condition is necessary. (See \S\ref{253} p.~\pr{253}.)
\end{s}
...
\begin{s}\lb{rf}
\begin{uspb}
The\index{unsolved problem} sentence just before Notation 13.3.2 p.~330 starts with the claim that $R^*F$ is triangulated. I don't understand why.
\end{uspb}
...
\begin{s}
P. 330, proof Corollary 13.3.3. In view of \S\ref{330} above, it seems %is not clear 
to me that the proof should be phrased slightly differently.% the argument given in the book is complete.
\end{s}
...
\begin{comment}
Let $\C$ be an abelian category. We shall need the following statement:

\begin{lem}\lb{738a}
Let
$$
\begin{tikzcd}
0\ar{r}&Z\ar{d}{c}\ar{r}{a}&Y\ar{d}{d}\ar{r}{b}&X\ar[equal]{d}\ar{r}&0\\ 
0\ar{r}&W\ar{r}[swap]{e}&V\ar{r}[swap]{f}&X\ar{r}&0
\end{tikzcd}
$$ 
be a commutative diagram in $\C$. If the square $(Z,Y,V,W)$ is cocartesian and the top row is an exact complex, then the bottom row is an exact complex.
\end{lem}

We prefer to prove the  statement:

\begin{lem}\lb{738b}
Let
$$
\begin{tikzcd}
0\ar{r}&Z\ar[equal]{d}\ar{r}{a}&Y\ar{d}{d}\ar{r}{b}&X\ar[equal]{d}\ar{r}&0\\ 
0\ar{r}&Z\ar{r}[swap]{e}&W\ar{r}[swap]{f}&V\ar{r}&0
\end{tikzcd}
$$ 
be a commutative diagram in $\C$. If the square $(Y,X,V,W)$ is cartesian and the bottom row is an exact complex, then the top row is an exact complex.
\end{lem}

\begin{proof} The fact that the top row is a complex is clear.

\nn Exactness at $Z$: Let $g:U\to Z$ be a morphism satisfying $ag=0$ (in this proof we write $uv$ for $u\ci v$). We get $eg=cag=0$ and thus $g=0$.

\nn Exactness at $Y$: Let $g:U\to Y$ be a morphism satisfying $bg=0$. By Lemma~\ref{738a} p.~\pr{738a} it suffices to complete the commutative diagram
$$
\begin{tikzcd}
V\ar[dashed]{d}[swap]{h}\ar[dashed,two heads]{r}{i}&U\ar{dr}{0}\ar{d}[swap]{g}\\ 
Z\ar{r}[swap]{a}&Y\ar{r}[swap]{b}&X.
\end{tikzcd}
$$ 
Consider the larger commutative diagram 
$$
\begin{tikzcd}
V\ar[dashed]{d}[swap]{h}\ar[dashed,two heads]{r}{i}&U\ar{dr}{0}\ar{d}[swap]{g}\\ 
Z\ar[equal]{d}\ar{r}{a}&Y\ar{d}{c}\ar{r}{b}&X\ar{d}{d}\\ 
...
\end{tikzcd}
$$ 
We can find $h$ and $i$ such that $eh=cgh$, and we must prove $gi=ah$. But the equality $gi=ah$ is equivalent to the conjonction of the equalities $cgi=cah$ and $bgi=bah$, equalities whose proof is straightforward.

\nn Exactness at $X$: It follows from Lemma~\ref{8311} (a) (ii) p.~\pr{8311}.
\end{proof}
\end{comment}
