...
\begin{prop}\lb{aioc}
If $\al:I^{\op}\to\C$ is a functor defined on a small category (Definition~\ref{myuscat} p.~\pr{myuscat}), if $X$ is in $\C$, if $p:\DT X\to\al$ is a morphism in $\C^{I^{\op}}$, and if $h:\C\to\C^\wg$ is the Yoneda embedding, then the following conditions are equivalent:
...
$$
in $\Fct(I^{\op},\Set_\U)$ is a projection (see Definition~\ref{plim} p.~\pr{plim}) for any $X'$ in $\C'$ and for some (and thus for any)\footnote{See Lemma~\ref{u2}.} universe $\U$ such that %$I$ is $\U$-small and 
$\C'$ is a $\U$-category (Definition~\ref{myucat} p.~\pr{myucat})''. Note that $(1F)$ and $(2F)$ are equivalent by Propositions~\ref{aioc} and \ref{aic}. Then the conditions \emph{(a)} and \emph{(b)} below are equivalent:

\nn\emph{(a)} The equivalent conditions $(1h_\U)$ and $(2h_\U)$ hold for some (and thus for any) %\footnote{See Lemma~\ref{u1}.} 
universe $\U$ such that $I$ is $\U$-small and $\C$ is a $\U$-category. %and $\col\Hom_\C(Y,\al)\in\U$ for all $Y\in\C$. 

\nn\emph{(b)} The equivalent conditions $(1F)$ and $(2F)$ hold for all for all category $\C'$ and all functor $F:\C\to\C'$.
\end{prop}

\begin{proof}
Condition (b) implies trivially Condition (a). Let us prove that (a) implies (b). Let $F:\C\to\C'$ be as in (b) and let $\U$ be a universe such that $I$ is $\U$-small and $\C$ and $\C'$ are $\U$-categories. It suffices to prove $(2F)$. Let $X'$ be in $\C'$ and define $A\in\C^\wg_\U$ by $A:=\Hom_{\C'}(F(\ ),X')$. We get the commutative diagram 
$$
\begin{tikzcd}
...
%[To show that $(1h_\U)$ implies $(1h_\V)$ if $\C$ is both a $\U$-category and a $\V$-category, let $Y$ be in $\C$ and let $p_\U:\Hom_\C(Y,\al)\to\DT\Hom_\C(Y,X)$ and $p_\V:\Hom_\C(Y,\al)\to\DT\Hom_\C(Y,X)$ be the obvious morphisms in $\Fct(I,\Set_\U)$ and $\Fct(I,\Set_\V)$. It suffices to show that $p_\V$ is a coprojection, and this follows immediately from the assumption that $p_\U$ is a coprojection.]
\end{proof}
% previous version:
...
The second sentence in the above quote is justified by Proposition~\ref{insist2} p.~\pr{insist2}. %Theorem~\ref{stupidkan} p.~\pr{stupidkan}. %\S\ref{744} p.~\ref{744}. %Proposition 7.4.4 p.~163 of the book.
...
This is justified by Proposition~\ref{insist2} p.~\pr{insist2}. %Theorem~\ref{stupidkan} p.~\pr{stupidkan}. 
\end{s}
