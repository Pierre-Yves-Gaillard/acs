...
\begin{s} P.~53, Corollary 2.3.4. Recall that we have functors $\C\xl\bt J\xr\pp I$, where $I$ and $J$ are small, and that the statement is the existence of a natural isomorphism $\col\bt\iso\col\pp^\dg\bt$, that is 
\begin{equation}\lb{coco}
\col_j\bt(j)\iso\col_i\ \col_{j,u}\bt(j),
\end{equation} 
where $(j,u)$ runs over $J_i$, with $u:\pp(j)\to i$. In Section~\ref{2111} p.~\pr{2111} we shall give a proof of this fact which differs slightly from the one in the book.
