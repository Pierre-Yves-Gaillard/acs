% about "categories and sheaves" (version_0074)
...
Let $\Gamma$ be a subset of $M$ satisfying GT1-GT4, and let $\cc E=\cc E(\Gamma)$ be the set of those morphisms $A\to B$ in $\C^\wedge$ such that, for any morphism $U\to B$ in $\C^\wedge$ with $U$ in $\C$, the sieve $S_{A\times_BU}$ is in $\Gamma_U$. 

As observed in the book, $A\to U$ is in $\cc E$ if and only if $S_A$ is in $\Gamma_U$, and $A\to B$ is in $\cc E$ if and only if $A\times_BV\to V$ is in $\cc E$ for all $V\to B$ with $V$ in $\C$. 
...
\nn LE4 results easily form the following observation: In the setting $A\to U\leftarrow V$ (obvious notation), we have 
%
...
$$
Let us check that $\Gamma:=\cup\Gamma_U=\Gamma(\cc E)$ satisfies GT1-GT4.

GT1 follows from LE1, GT2 follows from LE3, and GT3 follows from LE4 and \eqref{mf}. Let us prove GT4. 
...
\nn Claim: $S\subset\cc E$. 

\nn Proof of the claim: Let $s$ be in $S$, let $V$ be in $\C$, and let $\pp:A_S(V)\to\Hom_{\C^\wedge}(V,A_S)$ be the bijection given by the Yoneda Lemma (see Theorem~\ref{yol} p.~\pageref{yol}). One easily checks that $\pp(s)$ is an epimorphism and that the diagram 
$$
\begin{tikzcd}
V\ar[two heads]{r}{\pp(s)}\ar{rd}[swap]{s}&A_S\ar[hook]{d}\\ 
{}&U
\end{tikzcd}
$$ 
commutes. Moreover, $\pp(s)$ is in $\cc E$, because, as recalled above, $\cc E$ contains all epimorphisms, and $A_S\incl U$ is in $\cc E$ by assumption. Now LE2 implies that $s$ is in $\cc E$, and the claim is proved. 

If $S=\varnothing$, then $S'$ is in $\Gamma_U$ by GT2. Otherwise, let $V\to U$ be in $S$, let $B$ be the subobject $A_{S'\times_UV}=A_{S'}\times_UV$ of $V$ in $\C^\wedge$, and consider the commutative diagram 
$$
\begin{tikzcd}
B\ar{r}\ar{d}&V\ar{d}\\ 
A_{S'}\ar{r}&U.
\end{tikzcd}
$$ 
Then $B\to V$ is in $\cc E$ because $S'\times_UV$ is in $\Gamma_V$ by assumption, $V\to U$ is in $\cc E$ by the claim, and $A_{S'}\to U$ is in $\cc E$ by LE2 and LE3. 

We have proved that $\Gamma$ satisfies GT1-GT4. 

It is easy to check that the equalities $\cc E=\cc E(\Gamma)$ and $\Gamma=\Gamma(\cc E)$ are equivalent. 
\end{s}
...
[Section~\ref{s1853} p.~\pageref{s1853}, \S\ref{1811} p.~\pageref{1811}, \S\ref{1816} p.~\pageref{1816}, and \S\ref{1867} p.~\pageref{1867} will be inserted here.]
