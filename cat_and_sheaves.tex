...
$$
$$
\xr\sim\Mc(0\to P_n)\oplus\Mc(P_{n+1}\to P_{n+1})\xr\sim P_n\oplus0\to P_n.
$$

Here is a rewriting of the last three lines:

Hence, $\pp_i$ and $\pp_{i+1}$ are monomorphisms by Exercise 8.37. Note that $\id-\text{sh}$ in Exercise 8.37 corresponds to $\pp_i$ and $X_0\to X_1\to\cdots$ corresponds to $H^i(X)\to H^i(X)\to\cdots$. Therefore we have the exact commutative diagram 
$$
\begin{tikzcd}
0\ar[r]&H^i(P)\ar[d,"\sim"]\ar[r]&H^i(Q)\ar[d,"\sim"]\ar[r]&H^i(R)\ar[d]\ar[r]&0\\ 
0\ar[r]&\ds\bigoplus_{n\ge i}H^i(X)\ar[r]&\ds\bigoplus_{n\ge i}H^i(X)\ar[r]&H^i(X)\ar[r]&0,
\end{tikzcd}
$$ 
and $H^i(R)\to H^i(X)$ is an isomorphism.
\end{s}
...
By Theorem 14.3.1 (vi) p.~355 implies that $RF$ exists. This fact is implicit in the statement and the proof of Theorem 14.4.5.
...
\nn$\bu$ The second sentence of Step~(v) of the proof is ``Hence $P$ is $\oo K(G)$-projective and $LG$ exists''. Thus, (a) and (b1) have been proved at this point. 
