...
\begin{s}
Proof of Lemma 10.5.5 p. 261. We adhere to the notation of the lemma. Let $F$ be in $\SSS^{\wg,prod}$. Define $X\in\SSS$ by $X:=\bigoplus_{C\in\SSS_0}\bigoplus_{i\in\ F(C)}C$, and for $C$ in $\SSS_0$ and $i$ in $F(C)$ let $p_{Ci}:C\to X$ be the coprojection. Define $u:X\to F$, that is $u\in F(X)\iso\prod_C\prod_iF(C)$, by setting $u_{Ci}:=i$. If $D$ is in $\SSS_0$ and $j$ in $F(D)$, then the map $u(D):X(D)\to F(D)$ satisfies $u(D)(p_{Dj})=j$. In particular $u(D)$ is surjective, and 
$u$ is an epimorphism.
\end{s}
