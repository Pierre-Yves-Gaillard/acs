% about "categories and sheaves" (version_0049)
...
If $\C$ admits small $\pi$-filtrant inductive limits, if $J$ is a category satisfying $\card(\Mor(\C))<\pi$, if $\beta:J\to\C_\pi$ is a functor, and if $\co\beta$ exists in $\C$, then it belongs to $\C_\pi$. 
\end{prop}
...
$$
\begin{tikzcd}
\co_i\Hom_\C(\co_j\beta(j),\alpha(i))\ar{r}\ar{d}[swap]{\sim}&\Hom_\C(\co_j\beta(j),\co_i\alpha(i))\ar{dd}{\sim}\\ 
\co_i\lim_j\Hom_\C(\beta(j),\alpha(i))\ar{d}[swap]{\sim}\\ 
\lim_j\co_i\Hom_\C(\beta(j),\alpha(i))\ar{r}[swap]{\sim}&\lim_j\Hom_\C(\beta(j),\co_i\alpha(i)).
\end{tikzcd}
$$
\end{proof}
\end{s}
...
\begin{lem}\label{ppi}
Let $\pi$ be an infinite cardinal, let $\alpha:I\to\C$ be a functor admitting an inductive limit $X$ in $\C$, and let $i_0$ be in $I$. Assume that the coprojections $p_i:\alpha(i)\to X$ are monomorphisms, and consider the conditions below:

\nn{\em(a)} $I$ is $\pi$-filtrant and $X$ is $\pi$-accessible,

\nn{\em(b)} the identity of $X$ factors through the coprojection $p_i$ for some $i$,

\nn{\em(c)} the coprojection $p_{i_0}$ is an isomorphism for some $i_0$,

\nn{\em(d)} $\alpha(s):\alpha(i_0)\to\alpha(i)$ is an isomorphism for all morphism $s:i_0\to i$ in $I$.
...
We give a slightly more detailed writing of the second sentence in Step (a) of the proof of Lemma 9.3.1. This second sentence is ``Set $S:=\co_iY_i(G)\subset Y(G)$.'' Here is the rewriting: The coprojection $Y_i\to Y$ being a monomorphism, so is $Y_i(G)\to Y(G)$. As small filtrant inductive limits are exact in $\Set$ (Proposition 3.3.7 (iv) p. 83 of the book), $S:=\co_iY_i(G)\to Y(G)$ is also a monomorphism.
