...
Let $\al:I\to\C$ be a functor defined on a small category (Definition~\ref{myuscat} p.~\pr{myuscat}), let $X$ be an object of $\C$, and let $p:\al\to\DT X$ be a morphism in $\C^I$ (see Notation~\ref{diag} p.~\pr{diag}). If the the equivalent condition in Proposition~\ref{copr} hold, we say that $(X,p)$ is a {\em universal} inductive limit\index{universal inductive limit} of $\al$, and that $\col\al$ exists {\em universally}\index{universal existence of a limit} in $\C$.
\end{df}
...
\end{prop}
\begin{proof}
This follows from Propositions \ref{copr} and \ref{stupid}.
\end{proof}
...
\begin{proof}
To prove the proposition, we may assume that $I$ is small (Definition~\ref{myuscat} p.~\pr{myuscat}). Let $h:\C\to\C^\wg$ be the Yoneda embedding and $j$ a fixed object of $J$. In the notation of Proposition~\ref{stupid} p.~\pr{stupid}, we must prove 
$$
C\left[I,\al(\ ,j),\C,\left(\col_i\al(i,\ )\right)(j)\right],
$$ 
so that it suffices to prove 
$$
H\left[I,\al(\ ,j),\C,\left(\col_i\al(i,\ )\right)(j)\right],
$$ 
but this holds by Proposition~\ref{evcol=colev} p.~\pr{evcol=colev}.
...
Note that, as observed by Kelly (see Section~4.2 of the book quoted above in \S\ref{c38} p.~\pr{c38}), the above sufficient condition for $\pp^\dg(\bt)$ to exist is not necessary. This non-necessity results from \S\ref{c38} and the following remark. If $\pi:I\times J\to I$ is the projection, and $\al:I\times J\to \C$ is a functor, then $\pi^\dg(\al)\iso\col_{j\in J}\al(\ ,j)$ (in the strong sense that one exists if and only if the other exists); in contrast we have $\col_{(\pp(i',j)\to i)\in(I\times J)_i}\al(j)\iso\col_{j\in J}\al(i,j)$ (in the same strong sense); and we saw that the existence of $\col_{j\in J}\al(i,j)$ for all $i$ implies that of $\col_{j\in J}\al(\ ,j)$, but the converse is not true. We have however %In Proposition~\ref{patho} p.~\ref{patho} we shall see a way to prevent this kind of pathology.

%\begin{s} 
\begin{thm}\lb{stupidkan}
If $I$ and $J$ are big categories, if $\C$ is a $\U$-category (Definition~\ref{myucat} p.~\pr{myucat}), if $I\xl\pp J\xr\bt\C$ are functors, and if 
\begin{equation}\lb{e233}
\col_{(\pp(j)\to i)\in J_i}\bt(j)
\end{equation}
exists in $\C$ for all $i$ in $I$, then $\pp^\dg\bt$ exists, and $(\pp^\dg\bt)(i)$ is isomorphic to \qr{e233}. Moreover, \qr{e233} exists universally for all $i$ in the sense of Definition~\ref{uil} p.~\pr{uil} if and only if $\pp^\dg\bt$ exists universally in the sense of Definition~\ref{232} p.~\pr{232}. 
\end{thm}
\begin{proof}
The first sentence and the ``only if'' part of the second sentence follow from Theorem~\ref{233i} p.~\pr{233i}. To prove the ``if'' part of the second sentence, we may assume that $J$ is small (Definition~\ref{myuscat} p.~\pr{myuscat}). Let $i$ be in $I$. In the notation of Proposition~\ref{stupid} p.~\pr{stupid}, we must prove 
$$
C\left[J_i,\ (\pp(j)\to i)\mt\bt(j),\ \C,\ \pp^\dg(\bt)(i)\right],
$$ 
so that it suffices to prove 
$$
H\left[J_i,\ (\pp(j)\to i)\mt\bt(j),\ \C,\ \pp^\dg(\bt)(i)\right],
$$ 
that is 
$$
\pp^\dg(\bt)(i)\iso\col_{(\pp(j)\to i)\in J_i}h(\bt(j)),
$$ 
but we have 
$$
\col_{(\pp(j)\to i)\in J_i}h(\bt(j))\iso\pp^\dg(h\ci\bt)(i)\iso\pp^\dg(\bt)(i),
$$ 
the isomorphisms following respectively from Theorem~\ref{233i} p.~\pr{233i} and from the  assumption that $\pp^\dg\bt$ exists universally in the sense of Definition~\ref{232} p.~\pr{232}.
% previous version:
% https://docs.google.com/document/d/1K004uKK3ECssS-Y1etYqzLcEilskE8Hlb0wVrElSqM0/edit
\end{proof} 
\begin{cor}
Let $I,J$ and $\C$ be arbitrary categories; let $I\xl\pp J\xr\bt\C$ be functors; let $\U$ be a universe such that $J$ is $\U$-small, $\C$ is a $\U$-category (Definitions \ref{myucat} p.~\pr{myucat} and \ref{myuscat} p.~\pr{myuscat}), the functor $\pp^\dg(h\ci\bt)$ (where $h:\C\to\C^\wg$ is the Yoneda embedding) exists and the natural morphism $\pp^\dg(h\ci\bt)\to h\ci\pp^\dg(\bt)$ is an isomorphism. Then $\pp^\dg\bt$ exists universally in the sense of Definition~\ref{232} p.~\pr{232}.
%Let $I,J$ and $\C$ be arbitrary categories; let $I\xl\pp J\xr\bt\C$ be functors; for all universe $\U$ such that $\C$ is a $\U$-category let $h_\U:\C\to\C^\wg_\U$ be the Yoneda embedding; assume that, for all such $\U$ the functor $\pp^\dg(h_\U\ci\bt)$ exists and the natural morphism $\pp^\dg(h_\U\ci\bt)\to h_\U\ci\pp^\dg(\bt)$ is an isomorphism. Then $\pp^\dg\bt$ exists universally in the sense of Definition~\ref{232} p.~\pr{232}. 
%If $I$ and $J$ are big categories, if $\C$ is a $\U$-category, if $I\xl\pp J\xr\bt\C$ are functors, if $h:\C\to\C^\wg$ is the Yoneda embedding, if $\pp^\dg(h\ci\bt)$ exists and if the natural morphism $\pp^\dg(h\ci\bt)\to h\ci\pp^\dg(\bt)$ is an isomorphism, then $\pp^\dg\bt$ exists universally in the sense of Definition~\ref{232} p.~\pr{232}.
\end{cor}
\begin{proof}
This follows from Proposition~\ref{insist1} p.~\pr{insist1} and Theorem~\ref{stupidkan}.
\end{proof}
% previous version:
% https://docs.google.com/document/d/1K004uKK3ECssS-Y1etYqzLcEilskE8Hlb0wVrElSqM0/edit
% comment removed:
...
Let us insist on the following point:
\begin{prop}\lb{insist2}
Let $I,J$ and $\C$ be arbitrary categories, and let $I\xl\pp J\xr\bt\C$ be functors. If there is a universe $\U$ such that $J$ is $\U$-small (Definition~\ref{myuscat} p.~\pr{myuscat}) and, for all $i$ in $I$, the colimit 
$$
\col_{(\pp(j)\to i)\in J_i}h_\U(\bt(j))
$$ 
%exists in $\C^\wg_\U$ and 
is representable (here $h_\U:\C\to\C^\wg_\U$ is the Yoneda embedding), then $\pp^\dg\bt$ exists universally (Definition~\ref{232} p.~\pr{232}).
\end{prop}
\begin{proof}
This follows from Propositions~\ref{insist1} and Theorem~\ref{stupidkan}.
\end{proof}
...
It is implicitly assumed that the underlying universe $\U$ has been chosen so that $\DD$ is $\U$-small (Definition~\ref{myuscat} p.~\pr{myuscat}). The second sentence in the above quote is justified by Proposition~\ref{insist2} p.~\pr{insist2}. %Theorem~\ref{stupidkan} p.~\pr{stupidkan}. %\S\ref{744} p.~\ref{744}. %Proposition 7.4.4 p.~163 of the book.
...
It is implicitly assumed that the underlying universe $\U$ has been chosen so that $\C$ is $\U$-small (Definition~\ref{myuscat} p.~\pr{myuscat}). This is justified by Proposition~\ref{insist2} p.~\pr{insist2}. %Theorem~\ref{stupidkan} p.~\pr{stupidkan}.
\end{s}
