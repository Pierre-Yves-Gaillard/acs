% about "categories and sheaves" (version aul)
...
$$ 
and it suffices to prove $s=0$. Write $s=\sum_{i=1}^na'_i\otimes b_i$ with $a'_i\in M'(U),b_i\in P(U)$, and put $a_i:=\varphi(a_i')$, so that \eqref{et=0} holds, and the commutative diagram of complexes \eqref{dt=0} becomes 
$$
\begin{tikzcd}
{}&\cc R^{\op}(U)^m\ar{d}{g}\\ 
