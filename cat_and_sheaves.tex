% about "categories and sheaves" (version-0306)
% old version:
...
\nn\emph{(ii)} small filtrant inductive limits are stable by base change (Definition 2.2.6 p.~47 of the book, stated as Definition~\ref{dsbc} p.~\pageref{dsbc}), 
...
It remains to prove Steps 3, 3', 4, 5, and 6. We refer to the book for Step 3. The proof of Step 3' is the same. (It is easy to see that small inductive limits in $\F^\wedge$ are stable by base change --- Definition 2.2.6 p.~47 of the book, stated as Definition~\ref{dsbc} p.~\pageref{dsbc}.) 
...
$$
\pp\big((i_\ell),Q\big)\left(b\xr f a\right):=\left(Q(b)\xr{u'(b)} G(b)\xr{G(f)}G(a)\right)\in\C/G(a).
$$
...
To define $\mu$ let 
$$
\xi:=\Big(\big((i_b),Q\big)\ ,\ i_a\ ,\ \pp((i_b),Q)\to\Delta(\widetilde{\alpha_a}(i_a))\Big)
$$ 
...
\nn{\em Notation.} If $\alpha:I\to\B$ is a functor whose inductive limit is $X\in\B$, then we write $p(B,i):\alpha(i)\to X$ for the coprojection. (Of course this morphism depends on $\alpha$.) 
...
\end{equation} 
%
are inverse isomorphisms, let $p_j:X_1\oplus X_2\to X_j$ and $i_j:X_j\to X_1\oplus X_2$ be the projections and coprojections, and apply Lemma 8.2.3 p.~169 of the book to the morphisms $p_j,i_j,F(p_j),F(i_j)$. q.e.d.
\end{s}
