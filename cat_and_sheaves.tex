% about "categories and sheaves" (version-0283)
...
For the sake of completeness let us prove \eqref{263b}. Let $S$ be the left-hand side of \eqref{263b}, and let 
$$
p(X\to A):\Hom_\C(Y,X)\to S
$$ 
be the coprojections. The implicit map $f:S\to A(Y)$ is induced by the maps 
$$
f(X\to A):\Hom_\C(Y,X)\to S
$$
$$
{}\kern52mm(Y\to X)\mapsto(Y\to X\to A).
$$ 
We define $g:A(Y)\to S$ by 
$$
g(Y\to A)=p(Y\to A)(\id_Y),
$$ 
and leave it to the reader to check that $f$ and $g$ are mutually inverse bijections.
\end{s}
...
Let $X$ be in $\C$ and $A$ be in $\C^\wedge$. We have 
$$
\widehat F(A)(F(X))\simeq\col_{Y\in\C}\Hom_{\C'}(F(X),F(Y)).
$$ 
Let 
$$
p_Y:\Hom_{\C'}(F(X),F(Y))\to\widehat F(A)(F(X))
$$ 
be the coprojection. 

\begin{rk}\label{272b}
There is a unique functor $G:\C\to\C'_{\widehat F(A)}$ such that we have, in the above notation, 
$$
G(X):=\big(p_X(\id_{F(X)}):F(X)\to\widehat F(A)\big).
$$ 
...
$$
\colim_{(X\to X')\in\C_{X'}}\Hom_\C(Y,X)\simeq\colim_{(Y\to X)\in\C^Y}\Hom_{\C'}(F(X),X').
$$ 
The identity of $Y$ being an initial object of $\C^Y$, we have 
$$
\colim_{(Y\to X)\in\C^Y}\Hom_{\C'}(F(X),X')\simeq\Hom_{\C'}(F(Y),X'),
$$ 
and thus
%
