...
\end{tikzcd}
$$ 
where $u_*$ denotes the morphism induced by $u$, commutes. The top square commutes by  \qr{cts} p.~\pr{cts}, and the bottom square commutes for obvious reasons. 
...
\end{equation} 
If $i_0$ is in $I$, then an object of $M^{i_0}$ is a pair of morphisms $(i_0\to i,\pp(i)\to j)$. It is easy to see that $(i_0\xr\id i_0,\pp(i_0)\xr\id\pp(i_0))$ is an initial object of $M^{i_0}$, and \qr{coco4} follows.%\begin{comment}
...
$$ 
and $\qr{qippi}$ follows from the existence of an obvious morphism from $(i,\pp(i),\id_{\pp(i)})$ to $(i,j,u)$ in $M$. q.e.d.%\end{comment}
...
\nn(c)$\ssi$(e): Write $P$ for $X\tm_YX$. Recall that (c) says that $P\parar X\to Y$ is exact. Proposition~\ref{epimono} p.~\pr{epimono} implies that Condition~(e) is equivalent to the condition in Lemma~\ref{strepi} p.~\pr{strepi}. Thus, it suffices to show that, letting $Z$ be an object of $\C$, we have in the notation of Convention~\ref{bra} p.~\pr{bra} 
$$
...
$$ 
we must check $[W\parar X\xr xZ]$. As the morphisms $W\parar X$ factor through $P\parar X$ by definition of $P$, the statement is obvious. 

(Note that the equivalence (c)$\ssi$(e) also follows from Proposition~\ref{epimono} p.~\pr{epimono}, Proposition~\ref{coimim} p.~\pr{coimim} and Proposition~\ref{strepi} p.~\pr{strepi}.)
\end{s}
...
$$
$$
\os{\text{(a)}}{\iso}\col_{y_1}\Hom_{\C_1}(X_1,Y_1)\tm\col_{y_2}\Hom_{\C_2}(X_2,Y_2)
\os{\text{(b)}}{\iso}A_1(X_1)\tm A_2(X_2),
$$ 
...
$$ 
B_1
\os{\text{(a)}}{\iso}\ic_x\ X_1\iso\ic_{x_1,x_2}\ X_1
\os{\text{(b)}}{\iso}\ic_{x_1}\ X_1
\os{\text{(c)}}{\iso}A_1.
$$ 
