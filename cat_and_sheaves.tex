...
%\begin{muspb}
%\footnote{Here is problem which, I hope, will turn out to be very easy, even if I'm unable to solve it. I'm using the overdramatic expression of ``major unsolved problem'' mainly as a note to myself, to make sure I don't forget it.}. About\index{Major Unsolved Problem} Definition 17.3.1(ii): Let $X$ be a site, let $\U\subset\V$ be universes, let $F\in\PSh(X,\U\textbf{-set})$ be a $\U\textbf{-set}$-valued presheaf and let $F'\in\PSh(X,\V\textbf{-set})$ be the corresponding $\V\textbf{-set}$-valued presheaf. Is the implication ``$F$ is a sheaf $\implies F'$ is a sheaf'' true?
%\end{muspb}
